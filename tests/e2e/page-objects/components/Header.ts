...
export class Header extends BasePage {
  private selectors = {
    container: '.northhing-nav-panel, .northhing-scene-bar, .northhing-nav-bar, [data-testid="header-container"], .northhing-header, header',
    homeBtn: '[data-testid="header-home-btn"], .northhing-nav-bar__logo-button, .northhing-header__home',
    minimizeBtn: '[data-testid="header-minimize-btn"], .northhing-title-bar__minimize, .window-controls__btn--minimize, button[aria-label*="Minimize"], button[aria-label*="最小化"]',
    maximizeBtn: '[data-testid="header-maximize-btn"], .northhing-title-bar__maximize, .window-controls__btn--maximize, button[aria-label*="Maximize"], button[aria-label*="最大化"], button[aria-label*="Restore"], button[aria-label*="还原"]',
    closeBtn: '[data-testid="header-close-btn"], .northhing-title-bar__close, .window-controls__btn--close, button[aria-label*="Close"], button[aria-label*="关闭"]',
    leftPanelToggle: '[data-testid="header-left-panel-toggle"], .northhing-nav-bar__panel-toggle',
    rightPanelToggle: '[data-testid="header-right-panel-toggle"]',
    newSessionBtn: '[data-testid="header-new-session-btn"]',
    title: '[data-testid="header-title"], .northhing-nav-bar__menu-item-main, .northhing-header__title',
    configBtn: '[data-testid="header-config-btn"], .northhing-header-right button',
  };
  ...
  async isVisible(): Promise<boolean> {
    const selectors = ['.northhing-nav-panel', '.northhing-scene-bar', '.northhing-nav-bar', '[data-testid="header-container"]', '.northhing-header', 'header'];
    for (const selector of selectors) {
      try {
        const element = await $(selector);
  ...
  async areWindowControlsVisible(): Promise<boolean> {
    const controlsContainer = await this.findExistingElement([
      '.window-controls',
      '.northhing-header-right .window-controls',
    ]);
