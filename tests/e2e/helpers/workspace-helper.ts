...
export async function getWorkspaceState(): Promise<WorkspaceState> {
  return browser.execute(async () => {
    ...
    if (typeof invoke === 'function') {
      ...
      const workspaceLabels = Array.from(document.querySelectorAll('.northhing-nav-panel__workspace-item-label'))
...
export async function ensureCodeSessionOpen(): Promise<void> {
  ...
  const selectors = [
    '.northhing-nav-panel__workspace-create-main--split-left',
