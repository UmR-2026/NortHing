...
function defaultnorthhingHome(): string {
  if (process.env.northhing_E2E_HOME) {
    return process.env.northhing_E2E_HOME;
  }
  if (process.env.northhing_E2E_USE_REAL_PROFILE === '1') {
    return process.env.northhing_HOME || path.join(os.homedir(), '.northhing');
  }
  return path.resolve(__dirname, '..', '..', '.northhing', 'runtime', 'home');
}
...
async function findSessionItem(sessionId: string): Promise<ReturnType<typeof $> | null> {
  ...
  if (process.env.northhing_E2E_PERF_VERBOSE_REPORT === '1') {
...
async function findLongSessionMetadataPath(sessionId: string): Promise<string | null> {
  const northhingHome = defaultnorthhingHome();
  ...
  try {
    const projectsDir = path.join(northhingHome, 'projects');
...
async function readLongSessionViewportState(expectedLatestTurnId?: string | null): Promise<LongSessionViewportState> {
  return browser.execute((targetTurnId) => {
    ...
    const inputOverlay = document.querySelector<HTMLElement>('.northhing-chat-input-drop-zone');
...
async function startLongSessionViewportTimelineRecorder(
  ...
  await browser.execute((targetTurnId, clickTime, shouldEnableRenderProfile) => {
    const globalWindow = window as typeof window & {
      __northhingLongSessionViewportTimeline?: LongSessionViewportTimelineSample[];
      __northhingLongSessionMainThreadTasks?: LongSessionMainThreadTask[];
      __northhingLongSessionMutationEvents?: LongSessionDomMutationEvent[];
      __northhingLongSessionVisualStateEvents?: LongSessionVisualStateEvent[];
      __northhingLongSessionLayoutShiftEvents?: LongSessionLayoutShiftEvent[];
      __northhingLongSessionViewportTimelineTimer?: number;
      __northhingLongSessionVisualFrameRequest?: number;
      __northhingLongSessionVisualProbeTimers?: number[];
      __northhingLongSessionLongTaskObserver?: PerformanceObserver;
      __northhingLongSessionLayoutShiftObserver?: PerformanceObserver;
      __northhingLongSessionMutationObserver?: MutationObserver;
      __northhingLongSessionOpenIntentAt?: number;
      __northhingLongSessionOpenIntentSessionId?: string | null;
      __northhingLongSessionOpenIntentHandler?: EventListener;
      __northhingLongSessionUserInteractionHandler?: EventListener;
      __northhing_RENDER_PROFILE_ENABLED__?: boolean;
    };
    globalWindow.__northhing_RENDER_PROFILE_ENABLED__ = shouldEnableRenderProfile;
    if (globalWindow.__northhingLongSessionViewportTimelineTimer !== undefined) {
      window.clearInterval(globalWindow.__northhingLongSessionViewportTimelineTimer);
    }
    if (globalWindow.__northhingLongSessionVisualFrameRequest !== undefined) {
      window.cancelAnimationFrame(globalWindow.__northhingLongSessionVisualFrameRequest);
    }
    for (const timerId of globalWindow.__northhingLongSessionVisualProbeTimers ?? []) {
      window.clearTimeout(timerId);
    }
    globalWindow.__northhingLongSessionVisualProbeTimers = [];
    globalWindow.__northhingLongSessionLongTaskObserver?.disconnect();
    globalWindow.__northhingLongSessionLayoutShiftObserver?.disconnect();
    globalWindow.__northhingLongSessionMutationObserver?.disconnect();
    if (globalWindow.__northhingLongSessionOpenIntentHandler) {
      window.removeEventListener(
        'flowchat:history-session-open-intent',
        globalWindow.__northhingLongSessionOpenIntentHandler,
      );
    }
    if (globalWindow.__northhingLongSessionUserInteractionHandler) {
      window.removeEventListener(
        'northhing:e2e-long-session-user-interaction',
        globalWindow.__northhingLongSessionUserInteractionHandler,
      );
    }
    globalWindow.__northhingLongSessionOpenIntentAt = undefined;
    globalWindow.__northhingLongSessionOpenIntentSessionId = undefined;
    const handleOpenIntent: EventListener = event => {
      const detail =
        event instanceof CustomEvent && typeof event.detail?.sessionId === 'string'
          ? event.detail
          : null;
      globalWindow.__northhingLongSessionOpenIntentAt = performance.now();
      globalWindow.__northhingLongSessionOpenIntentSessionId = detail?.sessionId ?? null;
      ...
      for (const delayMs of [100, 250, 500, 1_000]) {
        ...
        globalWindow.__northhingLongSessionVisualProbeTimers?.push(timerId);
      }
    };
    globalWindow.__northhingLongSessionOpenIntentHandler = handleOpenIntent;
    ...
    globalWindow.__northhingLongSessionUserInteractionHandler = handleUserInteraction;
    window.addEventListener('northhing:e2e-long-session-user-interaction', handleUserInteraction);
    ...
    const LOADING_SURFACE_SELECTORS = [
      ...
      '.northhing-scene-viewport__lazy-fallback',
      '.northhing-assistant-scene__loading',
      '.northhing-app-acp-session-loading',
    ...
    const isLoadingSurfaceElement = (element: HTMLElement): boolean =>
      Boolean(
        ...
        element.closest('.northhing-scene-viewport__lazy-fallback') ||
        element.closest('.northhing-assistant-scene__loading') ||
        element.closest('.northhing-app-acp-session-loading') ||
    ...
    const classifySurfaceElement = (
      ...
      if (element.closest('.northhing-chat-input-drop-zone')) {
        return {
    ...
    const readVisualStateEvent = (reason: string): LongSessionVisualStateEvent => {
      ...
      const signature = [
        ...
        globalWindow.__northhingLongSessionOpenIntentSessionId ?? 'no-open-intent-session',
      ...
      return {
        atMs,
        sinceClickMs: atMs - clickTime,
        sinceHistoryOpenIntentMs: typeof globalWindow.__northhingLongSessionOpenIntentAt === 'number'
          ? atMs - globalWindow.__northhingLongSessionOpenIntentAt
          : null,
        historyOpenIntentSessionId: globalWindow.__northhingLongSessionOpenIntentSessionId ?? null,
        frame: visualFrame,
        reason,
        signature,
        activeSessionId: messages?.dataset.activeSessionId || null,
    ...
    const recordAnimationFrame = () => {
      ...
      globalWindow.__northhingLongSessionVisualFrameRequest =
    ...
    globalWindow.__northhingLongSessionMutationObserver = mutationObserver;
    try {
      if (PerformanceObserver.supportedEntryTypes.includes('longtask')) {
        ...
        globalWindow.__northhingLongSessionLongTaskObserver = observer;
      }
    } catch {
      globalWindow.__northhingLongSessionLongTaskObserver = undefined;
    }
    try {
      if (PerformanceObserver.supportedEntryTypes.includes('layout-shift')) {
        ...
        globalWindow.__northhingLongSessionLayoutShiftObserver = observer;
      }
    } catch {
      globalWindow.__northhingLongSessionLayoutShiftObserver = undefined;
    }
    const readSample = (): LongSessionViewportTimelineSample => {
      ...
      const effectiveViewportHeight = scrollerRect && effectiveScrollerBottom !== null
        ...
        : null;
      const atMs = performance.now();
      const historyOpenIntentAtMs = globalWindow.__northhingLongSessionOpenIntentAt ?? null;
      const historyOpenIntentSessionId = globalWindow.__northhingLongSessionOpenIntentSessionId ?? null;
    ...
    globalWindow.__northhingLongSessionViewportTimeline = samples;
    globalWindow.__northhingLongSessionMainThreadTasks = mainThreadTasks;
    globalWindow.__northhingLongSessionMutationEvents = mutationEvents;
    globalWindow.__northhingLongSessionVisualStateEvents = visualStateEvents;
    globalWindow.__northhingLongSessionLayoutShiftEvents = layoutShiftEvents;
    record();
    globalWindow.__northhingLongSessionViewportTimelineTimer = window.setInterval(record, 50);
...
async function stopLongSessionViewportTimelineRecorder(): Promise<LongSessionViewportTimeline> {
  return browser.execute(() => {
    ...
    if (globalWindow.__northhingLongSessionViewportTimelineTimer !== undefined) {
      window.clearInterval(globalWindow.__northhingLongSessionViewportTimelineTimer);
      globalWindow.__northhingLongSessionViewportTimelineTimer = undefined;
    }
    if (globalWindow.__northhingLongSessionVisualFrameRequest !== undefined) {
      window.cancelAnimationFrame(globalWindow.__northhingLongSessionVisualFrameRequest);
      globalWindow.__northhingLongSessionVisualFrameRequest = undefined;
    }
    for (const timerId of globalWindow.__northhingLongSessionVisualProbeTimers ?? []) {
      window.clearTimeout(timerId);
    }
    globalWindow.__northhingLongSessionVisualProbeTimers = undefined;
    globalWindow.__northhingLongSessionLongTaskObserver?.disconnect();
    globalWindow.__northhingLongSessionLongTaskObserver = undefined;
    globalWindow.__northhingLongSessionLayoutShiftObserver?.disconnect();
    globalWindow.__northhingLongSessionLayoutShiftObserver = undefined;
    globalWindow.__northhingLongSessionMutationObserver?.disconnect();
    globalWindow.__northhingLongSessionMutationObserver = undefined;
    if (globalWindow.__northhingLongSessionOpenIntentHandler) {
      window.removeEventListener(
        'flowchat:history-session-open-intent',
        globalWindow.__northhingLongSessionOpenIntentHandler,
      );
      globalWindow.__northhingLongSessionOpenIntentHandler = undefined;
    }
    if (globalWindow.__northhingLongSessionUserInteractionHandler) {
      window.removeEventListener(
        'northhing:e2e-long-session-user-interaction',
      ...
      globalWindow.__northhingLongSessionUserInteractionHandler = undefined;
    }
    globalWindow.__northhingLongSessionOpenIntentAt = undefined;
    const samples = globalWindow.__northhingLongSessionViewportTimeline ?? [];
    const mainThreadTasks = globalWindow.__northhingLongSessionMainThreadTasks ?? [];
    const mutationEvents = globalWindow.__northhingLongSessionMutationEvents ?? [];
    const visualStateEvents = globalWindow.__northhingLongSessionVisualStateEvents ?? [];
    const layoutShiftEvents = globalWindow.__northhingLongSessionLayoutShiftEvents ?? [];
    globalWindow.__northhingLongSessionViewportTimeline = undefined;
    globalWindow.__northhingLongSessionMainThreadTasks = undefined;
    globalWindow.__northhingLongSessionMutationEvents = undefined;
    globalWindow.__northhingLongSessionVisualStateEvents = undefined;
    globalWindow.__northhingLongSessionLayoutShiftEvents = undefined;
    globalWindow.__northhing_RENDER_PROFILE_ENABLED__ = false;
...
async function maybeSavePerfScreenshot(name: string): Promise<string | null> {
  if (process.env.northhing_E2E_PERF_SCREENSHOTS !== '1') {
    return null;
  }
...
function readPostVisibleInteractionOption(
  ...
  const envValue = process.env.northhing_E2E_PERF_POST_VISIBLE_INTERACTION;
  if (
...
function webDriverEndpoint(pathname: string): string {
  const port = Number(process.env.northhing_E2E_WEBDRIVER_PORT || 4445);
...
async function recordLongSessionPostVisibleInteraction(
  ...
  await browser.execute((eventDetail) => {
    window.dispatchEvent(new CustomEvent('northhing:e2e-long-session-user-interaction', {
      detail: eventDetail,
    }));
  }, detail);
}
...
function readRapidSwitchSessionIds(): string[] {
  const raw = process.env.northhing_E2E_PERF_RAPID_SWITCH_SESSION_IDS;
...
async function readActiveSessionNavId(): Promise<string | null> {
  return browser.execute(() => {
    const active = document.querySelector<HTMLElement>(
      ...
      '.northhing-nav-panel__inline-item.is-active[data-session-id]',
    );
...
async function collectRapidLongSessionSwitchMeasurement(
  ...
  await startLongSessionViewportTimelineRecorder(
    ...
    clickedAtMs,
    process.env.northhing_E2E_RENDER_PROFILE === '1',
  );
  ...
  for (let index = 0; index < sessionIds.length; index += 1) {
    ...
    if (index < sessionIds.length - 1) {
      const delayMs = numericEnv('northhing_E2E_PERF_RAPID_SWITCH_DELAY_MS') ?? 75;
  ...
  const postVisibleObserveMs =
    numericEnv('northhing_E2E_PERF_RAPID_SWITCH_OBSERVE_MS') ??
    numericEnv('northhing_E2E_PERF_POST_VISIBLE_OBSERVE_MS') ??
  ...
  const verboseTimelineReport = process.env.northhing_E2E_PERF_VERBOSE_REPORT === '1';
...
async function collectLongSessionOpenMeasurement(
  ...
  latestUsable ??= await latestUsablePromise;
  const postVisibleObserveMs =
    numericEnv('northhing_E2E_PERF_POST_VISIBLE_OBSERVE_MS') ?? DEFAULT_POST_VISIBLE_OBSERVE_MS;
  const observeRemainingMs = latestUsable.usableAtMs + postVisibleObserveMs - await readPerformanceNow();
  if (observeRemainingMs > 0) {
    await browser.pause(Math.ceil(observeRemainingMs));
  }
...
describe('Performance telemetry', () => {
  ...
  it('collects startup timing from the current build', async () => {
    ...
    const maxInteractiveMs = numericEnv('northhing_E2E_PERF_MAX_INTERACTIVE_MS');
  ...
  it('collects first-open timing for a generated long session', async function () {
    ...
    const sessionId = process.env.northhing_E2E_PERF_SESSION_ID || DEFAULT_PERF_SESSION_ID;
    ...
    const maxLatestFrameMs = numericEnv('northhing_E2E_PERF_MAX_SESSION_FRAME_MS');
