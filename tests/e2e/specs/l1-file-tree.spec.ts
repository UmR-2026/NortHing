...
describe('L1 File Tree', () => {
  ...
  before(async () => {
    ...
    if (hasWorkspace) {
      ...
      const fileNavSelectors = [
        '//button[contains(@class, "northhing-nav-panel__item")]//span[contains(text(), "Files")]/..',
        '//button[contains(@class, "northhing-nav-panel__item")]//span[contains(text(), "文件")]/..',
        '.northhing-nav-panel__item[aria-label*="Files"]',
        '.northhing-nav-panel__item[aria-label*="文件"]',
        'button.northhing-nav-panel__item:first-child', // Files is usually first
      ];
  ...
  describe('File tree existence', () => {
    it('file tree container should be visible', async function () {
      ...
      const selectors = [
        '.northhing-file-explorer__tree',
        '[data-file-tree]',
        '.file-tree',
        '[class*="file-tree"]',
        '[class*="FileTree"]',
        '.northhing-file-explorer',
        '[class*="file-explorer"]',
      ];
      ...
      if (!treeFound) {
        ...
        const fileExplorer = await $('.northhing-file-explorer, .northhing-explorer-scene, [class*="Explorer"]');
    ...
    it('file tree should display workspace files', async function () {
      ...
      const fileNodes = await browser.$$('.northhing-file-explorer__node');
  ...
  describe('Folder expand/collapse', () => {
    ...
    it('clicking directory should toggle expand state', async function () {
      ...
      const dirContent = await browser.$$('.northhing-file-explorer__node-content');
  ...
  describe('File selection', () => {
    it('clicking file should select it', async function () {
      ...
      const content = await firstFile.$('.northhing-file-explorer__node-content');
    ...
    it('selected file should have selected class', async function () {
      ...
      const selectedNodes = await browser.$$('.northhing-file-explorer__node-content--selected');
