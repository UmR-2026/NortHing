...
describe('L0 Smoke Tests', () => {
  ...
  describe('Core UI components', () => {
    it('Header should be visible', async () => {
      ...
      const header = await $('.northhing-nav-panel, .northhing-scene-bar, .northhing-nav-bar, [data-testid="header-container"]');
    ...
    it('should have either startup page or workspace UI', async () => {
      ...
      const welcomeSelectors = [
        ...
        '.welcome-scene',
        '.northhing-scene-viewport--welcome',
      ];
      ...
      if (!welcomeExists) {
        ...
        const sceneViewport = await $('.northhing-scene-viewport');
