...
describe('L1 Git Panel', () => {
  ...
  describe('Git panel existence', () => {
    ...
    it('git panel should detect repository status', async function () {
      ...
      const notRepo = await $('.northhing-git-scene--not-repository');
      const isLoading = await $('.northhing-git-scene--loading');
      const isRepo = await $('.northhing-git-scene-working-copy');
  ...
  describe('Branch display', () => {
    it('current branch should be displayed if in git repo', async function () {
      ...
      const branchElement = await $('.northhing-git-scene-working-copy__branch');
