...
function defaultnorthhingUserRoot() {
  if (process.env.northhing_E2E_USER_ROOT) {
    return process.env.northhing_E2E_USER_ROOT;
  }
  if (process.env.northhing_E2E_USE_REAL_PROFILE === '1') {
    return null;
  }
  return path.resolve(__dirname, '..', '.northhing', 'runtime', 'user-root');
}

function parseArgs(argv) {
  const options = {
    workspace: undefined,
    northhingHome: defaultnorthhingHome(),
    northhingUserRoot: defaultnorthhingUserRoot(),
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    switch (arg) {
      case '--workspace':
        ...
        break;
      case '--northhing-home':
        options.northhingHome = next();
        break;
      case '--northhing-user-root':
        options.northhingUserRoot = next();
        break;
      case '--session-prefix':
...
function printHelp() {
  console.log(`Generate northhing long-session performance fixtures.
...
Options:
  ...
  --northhing-home <path>      northhing home root. Default: northhing_E2E_HOME or isolated tests/e2e/.northhing/runtime/home; northhing_HOME is used only with northhing_E2E_USE_REAL_PROFILE=1
  --northhing-user-root <path> northhing user config root for seeding active workspace. Default: northhing_E2E_USER_ROOT or isolated tests/e2e/.northhing/runtime/user-root; skipped for real profile unless provided
...
async function seedWorkspaceState(options, workspacePath) {
  if (!options.northhingUserRoot) {
    return null;
  }

  const userRoot = path.resolve(options.northhingUserRoot);
...
async function generate(options) {
  ...
  const sessionsRoot = path.join(options.northhingHome, 'projects', slug, 'sessions');
  ...
  return {
    action: 'generate',
    workspacePath,
    northhingHome: options.northhingHome,
    northhingUserRoot: options.northhingUserRoot,
    sessionsRoot,
    sessionPrefix: options.sessionPrefix,
