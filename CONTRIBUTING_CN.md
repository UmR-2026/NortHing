...
```bash
# 桌面（推荐日常开发使用）
pnpm run desktop:dev                # 构建并运行 Slint 桌面应用（冷启动）
pnpm run desktop:check              # 仅编译检查
...
> **v0.1.0**：桌面使用 Slint（非 Tauri）。`desktop:dev` 即 `cargo run -p northhing`——
> 无 HMR、无 Vite、无自动重建。`desktop:preview:debug` 为同一命令的别名。
...
| 模式贡献 | `src/crates/assembly/core/src/agentic/agents/*_mode.rs`、`src/crates/assembly/core/src/agentic/agents/prompts/*_mode.md` [缺失：src/web-ui 路径在 v0.1.0 中不存在] | 添加/改进 agent 模式（如 Plan/Debug/Agentic 或自定义模式），并保持 prompts 与 UI 文案同步 |
