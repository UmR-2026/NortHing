...
## 数据流全— ```
AI 对话— GenerateMiniApp Tool— MiniAppManager::create()— storage.rs 持久— source + meta.json— compiler.rs 生成 compiled_html（注— Bridge— emit miniapp-created 事件— 前端 useMiniAppCatalogSync 监听— 刷新 MiniAppGalleryView— 用户点击「打开」→ MiniAppScene— MiniAppRunner— <iframe srcDoc={compiled_html}>— Bridge Script 拦截 require/fetch— postMessage— useMiniAppBridge 路由— Tauri Commands— Rust 服务
```
...
// Agent 工具中通过此函数获— try_get_global_miniapp_manager() -> Option<Arc<MiniAppManager>>
```

Workspace path— `commands.rs`— `open_workspace`/`close_workspace` 同步更新:
```rust
...
└── {app_id}/
 ├── meta.json # MiniAppMeta（不— source/compiled— ├── source/— ├── index.html— ├── style.css— └── script.js
 ├── compiled.html # 完整可运— HTML
 └── versions/
 └── v{N}.json # 历史版本快照
```
...
3. `build_scroll_boundary_script()`— 阻止外层滚动穿— 4. 组装 HTML:
 - `<meta http-equiv="Content-Security-Policy">`
 - CSS 变量（`--bg`, `--fg` 等主题变量）
 - 用户 CSS（`<style>` 内联— - CDN 依赖（`<script>/<link>` 标签— - Bridge Script（在用户 JS 之前— - 用户 JS（`<script>` 内联— ## Bridge Builder 详解

`bridge_builder.rs` 生成— Bridge Script 包含:

### require() shim
- `fs/promises`— 所有方法映射为 `_rpc('fs.{method}', params)`
- `path`— JS 实现（join/resolve/dirname/basename/extname/parse— - `child_process`— `exec` 映射— `_rpc('shell.exec', params)`
- `os`— JS 实现（platform/homedir/tmpdir/cpus/hostname— - `crypto`— 映射 `window.crypto`
...
2. 将路径变— `{appdata}`, `{workspace}` 解析为绝对路— 3. 检查请求路径是否在某个已声— scope— 4. `{appdata}` 自动允许，`{workspace}` 首次弹窗确认

### Shell 权限
检查命令是否在 `permissions.shell.allow` 白名单中（按命令名前缀匹配）— ### 网络权限
检查请— URL 域名是否— `permissions.net.allow` 中（`*` 表示全部允许）— ## Tauri Command 参数约定
...
返回统一 `Result<Value, String>`，前端通过 `invoke()` 调用— ## 前端状态管— ### miniAppStore (Zustand)
```typescript
{
 apps: MiniAppMeta[], // 画廊列表
 loading: boolean,
 openedAppIds: string[], // 当前已打开— MiniApp scene
...
- 事件触发时统一刷新 store，导航入口与画廊共享同一份状— ## 工具卡片集成

`MiniAppToolDisplay.tsx` 注册— `flow_chat/tool-cards/index.ts`:
