...
export function runManifestParserSelfTest({
  ...
  const positiveCases = [
    ...
    '[dependencies.northhing-core]',
    '[dev-dependencies."northhing-core"]',
    "[target.'cfg(windows)'.dependencies.northhing-core]",
    "[target.'cfg(unix)'.build-dependencies.\"northhing-core\"]",
  ];
  const negativeCases = [
    '# northhing-core = { path = "../core" }',
    '[dependencies]',
    '[workspace.dependencies.northhing-core]',
    '[dependencies.northhing-core-extra]',
  ];

  for (const line of positiveCases) {
    if (!isManifestDependencyDeclaration(line, 'northhing-core')) {
  ...
  const parsedDeps = parseManifestDependencies([
    ...
    '    "auth",',
    '], optional = true }',
    'northhing-core = { path = "../core", default-features = false, features = ["product-full"] }',
    '[dependencies.git2]',
    'workspace = true',
    'optional = true',
    '[target.\'cfg(windows)\'.dependencies."northhing-cli"]',
    'path = "../../apps/cli"',
    '[features]',
    'image = []',
  ]);
  ...
  if (parsedByName.get('northhing-cli')?.optional !== false) {
    throw new Error('dependency profile parser must detect non-optional target dependency tables');
  }
  const parsedCoreDep = parsedByName.get('northhing-core');
  ...
  const parsedCoreTableDep = parsedCoreTableDeps.find((dep) => dep.name === 'northhing-core');
  ...
  const acceptsGitFacadeLine = createFacadeLineChecker('northhing_services_integrations::git');
  const facadePositiveCases = [
    '',
    '//! Compatibility facade.',
    'pub use northhing_services_integrations::git::GitService;',
    'pub use northhing_services_integrations::git::types::*;',
    'pub use northhing_services_integrations::git::{',
    '    build_git_graph, build_git_graph_for_branch,',
    '};',
    'pub use northhing_services_integrations::git::{build_git_graph, build_git_graph_for_branch};',
  ];
  ...
  const rejectsGitImplementationLine = createFacadeLineChecker('northhing_services_integrations::git');
  const facadeNegativeCases = [
    'pub mod service;',
    'use northhing_services_integrations::git::GitService;',
    'fn parse_git_status() {}',
  ];
  ...
  const cliBoundaryDeps = ['northhing-cli', 'ratatui', 'crossterm', 'arboard', 'syntect-tui'];
  ...
  if (!agentToolsRule?.forbiddenDeps.includes('northhing-ai-adapters')) {
    throw new Error('agent-tools lightweight boundary must forbid northhing-ai-adapters');
  }
  ...
  if (!runtimePortsProfile?.forbiddenNonOptionalDeps.includes('northhing-services-core')) {
  ...
  if (!runtimeServicesRule?.forbiddenDeps.includes('northhing-core')) {
    throw new Error('runtime-services lightweight boundary must forbid northhing-core');
  }
  if (!runtimeServicesRule?.forbiddenDeps.includes('northhing-services-integrations')) {
  ...
  if (!agentRuntimeRule?.forbiddenDeps.includes('northhing-core')) {
    throw new Error('agent-runtime lightweight boundary must forbid northhing-core');
  }
  if (!agentRuntimeRule?.forbiddenDeps.includes('northhing-services-integrations')) {
  ...
  if (!productCapabilitiesRule?.forbiddenDeps.includes('northhing-core')) {
    throw new Error('product-capabilities lightweight boundary must forbid northhing-core');
  }
  if (!productCapabilitiesRule?.forbiddenDeps.includes('northhing-product-domains')) {
    throw new Error(
  ...
  if (!productCapabilitiesProfile?.forbiddenNonOptionalDeps.includes('northhing-core')) {
    throw new Error('product-capabilities dependency profile must forbid northhing-core');
  }
  ...
  const requiredContentContracts = [
    ...
    },
    {
      ...
      contracts: ['AgentEventStream'],
    },
    {
      path: 'src/crates/execution/agent-runtime/src/runtime_types.rs',
      contracts: ['SessionSelector', 'AgentRunRequest', 'AgentRunHandle'],
    },
    {
    ...
    },
    {
      ...
      contracts: ['run_for_session_workspace', 'northhing_services_integrations::deep_research'],
    },
    {
    ...
    },
    {
      ...
      contracts: ['DeepReviewProviderCapacityQueueRuntime'],
    },
    {
      path: 'src/crates/execution/agent-runtime/src/deep_review/reviewer_admission_queue.rs',
      contracts: ['DeepReviewReviewerAdmissionQueueRuntime'],
    },
    {
    ...
    },
    {
      ...
      contracts: ['owner::WorkspaceSearchService::new_with_hooks', 'CoreWorkspaceSearchRuntimeHooks', 'WorkspaceSearchRepoConfig', 'get_global_config_service', 'ensure_workspace_gitignore_ignores_northhing'],
    },
    {
