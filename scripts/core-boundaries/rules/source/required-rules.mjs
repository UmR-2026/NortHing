...
export const requiredContentRules = [
  ...
  },
  {
    ...
    reason:
      'core cron types must preserve old import and wire paths while northhing-agent-runtime owns scheduled-job runtime state',
    patterns: [
      {
  ...
  },
  {
    ...
    patterns: [
      {
        regex: /pub use northhing_agent_runtime::deep_review::/,
  ...
  },
  {
    ...
    patterns: [
      {
        regex: /pub use northhing_agent_runtime::custom_subagent::CustomSubagentKind/,
  ...
  },
  {
    ...
    patterns: [
      {
        regex: /northhing_agent_runtime::events::FinishReason/,
  ...
  },
  {
    ...
    patterns: [
      {
        regex: /northhing_agent_runtime::events::session_state_label/,
  ...
  },
  {
    ...
    patterns: [
      {
        regex: /pub use northhing_agent_runtime::prompt::\{UserContextPolicy, UserContextSection\};/,
  ...
  },
  {
    ...
    patterns: [
      {
        regex: /pub use northhing_agent_runtime::prompt_cache::\*;/,
  ...
  },
  {
    ...
    patterns: [
      {
        regex: /pub use northhing_services_core::session::\{SessionBranchRequest,\s*SessionBranchResult\};/,
  ...
  },
  {
    ...
    patterns: [
      {
        regex: /pub use northhing_agent_runtime::agents::\{[\s\S]*mode_presentation_rank[\s\S]*resolve_mode_config_profile_id[\s\S]*shared_coding_mode_user_context_policy[\s\S]*SHARED_CODING_MODE_PROMPT_TEMPLATE[\s\S]*\};/,
  ...
  },
  {
    ...
    reason:
      'core filesystem service may keep remote-workspace overlay and northhingError compatibility, but local filesystem owner must remain services-core',
    patterns: [
      {
  ...
  },
  {
    ...
    patterns: [
      ...
      {
        regex: /"dep:northhing-ai-adapters"/,
  ...
  },
  {
    ...
    reason:
      'no-default northhing-core must keep product runtime surfaces behind explicit features',
    patterns: [
      {
  ...
  },
  {
    ...
    patterns: [
      {
        regex: /pub use northhing_core_types::\{ConnectionTestMessageCode, ConnectionTestResult, RemoteModelInfo\};/s,
        message: 'stable AI DTOs must be re-exported from core-types',
      },
      {
        regex: /#\[cfg\(feature = "ai-adapter-runtime"\)\]\s*pub use northhing_ai_adapters::types::\{GeminiResponse, GeminiUsage\};/s,
  ...
  },
  {
    ...
    patterns: [
      {
        regex: /pub\(crate\) use northhing_runtime_ports::\{DelegationPolicy, SubagentContextMode\};/,
  ...
  },
  {
    ...
    patterns: [
      {
        regex:
          /pub use northhing_agent_runtime::scheduler::\{[\s\S]*DialogRoundInjectionInterrupt[\s\S]*SessionRoundInjectionBuffer[\s\S]*\};/,
        message: 'missing agent-runtime round-boundary state compatibility re-export',
      },
      {
        regex:
          /pub use northhing_runtime_ports::\{[\s\S]*DialogRoundInjectionSource[\s\S]*RoundInjection[\s\S]*RoundInjectionKind[\s\S]*RoundInjectionTarget[\s\S]*\};/,
  ...
  },
  {
    ...
    patterns: [
      {
        regex:
          /pub use northhing_runtime_ports::\{[\s\S]*SetThreadGoalResult[\s\S]*ThreadGoal[\s\S]*ThreadGoalContinuationPlan[\s\S]*ThreadGoalStatus[\s\S]*ThreadGoalToolResponse[\s\S]*GOAL_MODE_METADATA_KEY[\s\S]*MAX_CONTEXT_SUMMARY_CHARS[\s\S]*MAX_THREAD_GOAL_OBJECTIVE_CHARS[\s\S]*THREAD_GOAL_METADATA_KEY[\s\S]*\};/,
        message: 'missing thread goal compatibility re-export',
      },
      {
        regex:
          /pub use northhing_agent_runtime::thread_goal::\{[\s\S]*build_thread_goal_continuation_plan[\s\S]*goal_tool_response[\s\S]*should_skip_goal_for_turn[\s\S]*ThreadGoalRuntime[\s\S]*\};/,
  ...
  },
  {
    ...
    patterns: [
      {
        regex: /pub use northhing_runtime_ports::\{CompressionContract, CompressionContractItem\};/,
  ...
  },
  {
    ...
    reason:
      'core agent registry must adapt config and AgentEntry facts while northhing-agent-runtime owns mode-scoped subagent availability decisions',
    patterns: [
      {
  ...
  },
  {
    ...
    reason:
      'core agent registry must preserve legacy DTO fields while northhing-agent-runtime owns query scope and availability reason contracts',
    patterns: [
      {
        regex: /pub use northhing_agent_runtime::agents::\{[\s\S]*SubAgentSource[\s\S]*SubagentListScope[\s\S]*SubagentOverrideState[\s\S]*SubagentQueryContext[\s\S]*SubagentStateReason[\s\S]*\};/,
  ...
  },
  {
    ...
    patterns: [
      ...
      {
        regex: /\bnorthhing_services_integrations::deep_research::run_for_session_workspace\b/,
  ...
  },
  {
    ...
    patterns: [
      ...
      {
        regex: /\bensure_workspace_gitignore_ignores_northhing\b/,
  ...
  },
  {
    ...
    reason:
      'services-integrations must own function-agent concrete Git snapshots without depending on northhing-core',
    patterns: [
      {
