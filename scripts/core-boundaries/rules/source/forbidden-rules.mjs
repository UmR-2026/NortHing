...
export const forbiddenContentRules = [
    ...
    patterns: [
      {
        ...
        message:
          'core DeepReview policy facade must not re-own runtime budget state; use northhing-agent-runtime::deep_review',
      },
      {
        regex: /\bstatic GLOBAL_DEEP_REVIEW_QUEUE_CONTROL_TRACKER\b/,
        message:
          'core DeepReview policy facade must not re-own queue control state; use northhing-agent-runtime::deep_review',
      },
      {
        regex: /\bpub struct DeepReviewExecutionPolicy\b/,
        message:
          'core DeepReview policy facade must not redefine execution policy; use northhing-agent-runtime::deep_review',
      },
      {
        regex: /\bpub fn record_deep_review_task_budget\b/,
        message:
          'core DeepReview policy facade must not re-own task budget recording; use northhing-agent-runtime::deep_review',
      },
    ],
  },
  {
    ...
    patterns: [
      {
        ...
        message:
          'core DeepReview report must not re-own packet metadata enrichment; use northhing-agent-runtime::deep_review::report',
      },
      {
        regex: /\bfn deep_review_cache_from_completed_reviewers\b/,
        message:
          'core DeepReview report must not re-own cache update logic; use northhing-agent-runtime::deep_review::report',
      },
      {
        regex: /\bmetadata\.deep_review_cache\s*=\s*Some\s*\(/,
        message:
      ...
      },
      {
        ...
        message:
          'core DeepReview report must not re-own cache update DTO; use northhing-agent-runtime::deep_review::report',
      },
      {
        regex: /"kind": "concurrency_limited"/,
        message:
          'core DeepReview report must not re-own runtime tracker reliability signal shaping; use northhing-agent-runtime::deep_review::report',
      },
      {
        regex: /DeepReview runtime diagnostics:/,
        message:
          'core DeepReview report must not re-own runtime diagnostics log formatting; use northhing-agent-runtime::deep_review::diagnostics',
      },
    ],
  },
  {
    ...
    patterns: [
      {
        ...
        message:
          'core DeepReview task adapter must not re-own manifest key normalization; use northhing-agent-runtime::deep_review::task_execution',
      },
      {
        regex: /\bfn deep_review_packet_id_for_cache\b/,
        message:
          'core DeepReview task adapter must not re-own packet id inference; use northhing-agent-runtime::deep_review::task_execution',
      },
      {
        regex: /\bfn ensure_deep_review_retry_coverage\b/,
        message:
          'core DeepReview task adapter must not re-own retry coverage validation; use northhing-agent-runtime::deep_review::task_execution',
      },
      {
        regex: /\bfn provider_capacity_queue_wait_seconds_for_attempt\b/,
        message:
          'core DeepReview task adapter must not re-own provider capacity backoff; use northhing-agent-runtime::deep_review::task_execution',
      },
      {
        regex: /\bclassify_deep_review_capacity_error\b/,
        message:
          'core DeepReview task adapter must not directly classify provider or local capacity errors; use northhing-agent-runtime::deep_review::task_execution',
      },
      {
        regex: /\bDeepReviewCapacityFailFastReason::DeterministicProviderError\b/,
        message:
          'core DeepReview task adapter must not re-own provider capacity category fallback; use northhing-agent-runtime::deep_review::task_execution',
      },
      {
        regex: /\bfn provider_capacity_wait_can_wake_on_active_reviewer_release\b/,
        message:
          'core DeepReview task adapter must not re-own provider capacity queue wake policy; use northhing-agent-runtime::deep_review::task_execution',
      },
      {
        regex: /\bqueue_expired_without_active_reviewer\b/,
        message:
          'core DeepReview task adapter must not re-own reviewer admission queue expiry policy; use northhing-agent-runtime::deep_review::task_execution',
      },
      {
        regex: /\bstruct QueueWaitTimer\b/,
        message:
          'core DeepReview task adapter must not re-own queue wait timing; use northhing-agent-runtime::deep_review::task_execution',
      },
      {
      ...
      },
      {
        ...
        message:
          'core DeepReview task adapter must not re-own queue control decision priority; use northhing-agent-runtime::deep_review::task_execution',
      },
      {
        regex: /\bfn prompt_with_deep_review_retry_scope\b/,
        message:
          'core DeepReview task adapter must not re-own retry prompt shaping; use northhing-agent-runtime::deep_review::task_execution',
      },
      {
        regex: /<partial_result status=/,
        message:
          'core DeepReview task adapter must not re-own task completion result presentation; use northhing-agent-runtime::deep_review::task_execution',
      },
      {
      ...
      },
      {
        regex: /Retries used:/,
        message:
          'core DeepReview task adapter must not re-own retry guidance presentation; use northhing-agent-runtime::deep_review::task_execution',
      },
      {
        regex: /DeepReview automatic retry elapsed guard exceeded/,
        message:
          'core DeepReview task adapter must not re-own auto-retry admission policy; use northhing-agent-runtime::deep_review::task_execution',
      },
      {
    ...
    patterns: [
      {
        ...
        message:
          'core DeepResearch citation hook must not re-own registry parsing; use northhing-agent-runtime::deep_research',
      },
      {
        regex: /\bfn renumber_body\b/,
        message:
          'core DeepResearch citation hook must not re-own body renumbering; use northhing-agent-runtime::deep_research',
      },
      {
        regex: /\bfn renumber_index_section\b/,
        message:
          'core DeepResearch citation hook must not re-own Citation Index rewriting; use northhing-agent-runtime::deep_research',
      },
      {
        regex: /\bstatic CIT_ID_RE\b/,
        message:
          'core DeepResearch citation hook must not re-own citation regex parsing; use northhing-agent-runtime::deep_research',
      },
      {
        regex: /\btokio::fs\b/,
        message:
          'core DeepResearch citation hook must not own report filesystem IO; use northhing-services-integrations::deep_research',
      },
      {
        regex: /\bdisplay_map\.json\b/,
        message:
          'core DeepResearch citation hook must not own display-map sidecar persistence; use northhing-services-integrations::deep_research',
      },
    ],
  },
  {
    ...
    patterns: [
      {
        ...
        message:
          'core function-agent runtime services must not re-own Git concrete snapshots; use northhing-services-integrations::function_agents',
      },
      {
        regex: /\bgit_stdout_lenient\b/,
        message:
          'core function-agent runtime services must not re-own lenient Git process fallback; use northhing-services-integrations::function_agents',
      },
      {
  ...
  },
  {
    ...
    patterns: [
      ...
      },
      {
        regex: /\bstd::fs::/,
        message:
          'core filesystem service must not own sync local filesystem IO; use northhing-services-core filesystem primitives',
      },
      {
        regex: /\bignore::WalkBuilder\b/,
        message:
          'core filesystem service must not own local file walking/search implementation; use northhing-services-core filesystem primitives',
      },
      {
        regex: /\bsha2::/,
        message:
          'core filesystem service must not own editor-sync hashing implementation; use northhing-services-core filesystem primitives',
      },
      {
        regex: /\bbase64::/,
        message:
          'core filesystem service must not own binary file encoding implementation; use northhing-services-core filesystem primitives',
      },
    ],
  },
  {
    ...
    patterns: [
      {
        ...
        message:
          'core workspace-search facade must not own flashgrep daemon clients; use northhing-services-integrations::workspace_search',
      },
      {
        regex: /\bRepoSession\b/,
        message:
          'core workspace-search facade must not own flashgrep repo sessions; use northhing-services-integrations::workspace_search',
      },
      {
        regex: /\bwith_scan_fallback\b/,
        message:
          'core workspace-search facade must not own scan fallback policy; use northhing-services-integrations::workspace_search',
      },
      {
        regex: /\bconvert_hits_to_file_search_results\b/,
        message:
          'core workspace-search facade must not own hit conversion; use northhing-services-integrations::workspace_search',
      },
      {
        regex: /\bsplit_preview\b/,
        message:
          'core workspace-search facade must not own preview mapping; use northhing-services-integrations::workspace_search',
      },
      {
        regex: /\bdunce::canonicalize\b/,
        message:
          'core workspace-search facade must not own local search path normalization; use northhing-services-integrations::workspace_search',
      },
    ],
  },
  {
    ...
    patterns: [
      {
        regex: /\bnorthhing_services_integrations::workspace_search::flashgrep\b/,
        message:
  ...
  },
  {
    ...
    patterns: [
      {
        regex:
          /\b(?:owner::flashgrep|workspace_search::flashgrep|northhing_services_integrations::workspace_search::flashgrep)\b/,
        message:
    ...
    patterns: [
      {
        ...
        message:
          'core remote workspace search must not own remote flashgrep install facts; use northhing-services-integrations::remote_ssh::workspace_search',
      },
      {
        regex: /\bconst\s+REMOTE_(?:OS|ARCHITECTURE)_PROBES\b/,
        message:
          'core remote workspace search must not own remote probe facts; use northhing-services-integrations::remote_ssh::workspace_search',
      },
      {
        regex: /\bstruct\s+LocalFlashgrepBundle\b/,
        message:
          'core remote workspace search must not own local remote-search bundle DTOs; use northhing-services-integrations::remote_ssh::workspace_search',
      },
      {
        regex:
      ...
      },
      {
        ...
        message:
          'core remote workspace search must not own remote flashgrep session/context lifecycle; use northhing-services-integrations::remote_ssh::workspace_search',
      },
      {
    ...
    patterns: [
      {
        ...
        message:
          'core prompt builder must not own user-context section facts; use northhing-agent-runtime prompt contracts',
      },
      {
        regex: /\bpub\s+struct\s+UserContextPolicy\b/,
        message:
          'core prompt builder must not own user-context policy facts; use northhing-agent-runtime prompt contracts',
      },
    ],
  },
  {
    ...
    patterns: [
      {
        ...
        message:
          'core agent mode module must not own shared coding-mode prompt facts; use northhing-agent-runtime agents',
      },
      {
        regex: /\bpub const SHARED_CODING_MODE_CONFIG_PROFILE_ID\b/,
        message:
          'core agent mode module must not own shared coding-mode config profile facts; use northhing-agent-runtime agents',
      },
      {
        regex: /\bpub const SHARED_CODING_MODE_IDS\b/,
        message:
          'core agent mode module must not own shared coding-mode membership facts; use northhing-agent-runtime agents',
      },
      {
        regex: /\bpub fn resolve_mode_config_profile_id\b/,
        message:
          'core agent mode module must not own mode config profile resolution; use northhing-agent-runtime agents',
      },
      {
        regex: /\bpub fn mode_config_profile_member_mode_ids\b/,
        message:
          'core agent mode module must not own mode config profile membership; use northhing-agent-runtime agents',
      },
      {
        regex: /\bpub fn mode_config_profile_label\b/,
        message:
          'core agent mode module must not own mode config profile labels; use northhing-agent-runtime agents',
      },
      {
        regex: /\bpub fn shared_coding_mode_user_context_policy\b/,
        message:
          'core agent mode module must not own shared coding-mode context policy; use northhing-agent-runtime agents',
      },
    ],
  },
  {
    ...
    patterns: [
      {
        ...
        message:
          'core agent registry query must not own mode presentation order; use northhing-agent-runtime agents',
      },
      {
        regex: /\bfn subagent_source_rank\b/,
        message:
          'core agent registry query must not own subagent source presentation rank; use northhing-agent-runtime agents',
      },
    ],
  },
  {
    ...
    patterns: [
      {
        ...
        message:
          'core agent registry must not own subagent source DTOs; use northhing-agent-runtime agents',
      },
    ],
  },
  {
    ...
    patterns: [
      {
        ...
        message:
          'core prompt cache must not own prompt-cache schema facts; use northhing-agent-runtime prompt_cache',
      },
      {
        regex: /\bpub struct PromptCachePolicy\b/,
        message:
          'core prompt cache must not own prompt-cache policy; use northhing-agent-runtime prompt_cache',
      },
      {
        regex: /\bpub struct SessionPromptCache\b/,
        message:
          'core prompt cache must not own prompt-cache DTOs; use northhing-agent-runtime prompt_cache',
      },
      {
        regex: /\bpub enum PromptCacheScope\b/,
        message:
          'core prompt cache must not own prompt-cache invalidation scope; use northhing-agent-runtime prompt_cache',
      },
      {
        regex: /\bpub struct SessionPromptCacheStore\b/,
        message:
          'core prompt cache must not own in-memory prompt-cache store; use northhing-agent-runtime prompt_cache',
      },
      {
        regex: /\bpub enum PromptCacheLookup\b/,
        message:
          'core prompt cache must not own prompt-cache lookup outcomes; use northhing-agent-runtime prompt_cache',
      },
    ],
  },
  {
    ...
    patterns: [
      {
        ...
        message:
          'core execution types must not own finish-reason event facts; use northhing-agent-runtime events',
      },
    ],
  },
  {
    ...
    patterns: [
      {
        ...
        message: 'core tool framework must not redefine DynamicMcpToolInfo; use northhing-agent-tools',
      },
      {
        regex: /\bpub struct DynamicToolInfo\b/,
        message: 'core tool framework must not redefine DynamicToolInfo; use northhing-agent-tools',
      },
      {
        regex: /\bpub struct ToolRenderOptions\b/,
        message: 'core tool framework must not redefine ToolRenderOptions; use northhing-agent-tools',
      },
      {
        regex: /\bpub enum ToolPathBackend\b/,
        message: 'core tool framework must not redefine ToolPathBackend; use northhing-agent-tools',
      },
      {
        regex: /\bpub struct ToolPathResolution\b/,
        message: 'core tool framework must not redefine ToolPathResolution; use northhing-agent-tools',
      },
      {
        regex: /\bpub struct ToolContextFacts\b/,
        message: 'core tool framework must not redefine ToolContextFacts; use northhing-agent-tools',
      },
      {
        regex: /\bpub enum ToolWorkspaceKind\b/,
        message: 'core tool framework must not redefine ToolWorkspaceKind; use northhing-agent-tools',
      },
      {
    ...
    patterns: [
      {
        ...
        message:
          'core custom subagent facade must not own markdown front-matter IO; use northhing-agent-runtime',
      },
      {
        regex: /\bserde_yaml::Mapping\b/,
        message:
          'core custom subagent facade must not own markdown metadata serialization; use northhing-agent-runtime',
      },
    ],
  },
  {
    ...
    patterns: [
      {
        ...
        message:
          'core custom subagent registry must not restore the old loader owner; use northhing-agent-runtime discovery report',
      },
      {
        regex: /\bread_dir\b/,
        message:
          'core custom subagent registry must not own directory scanning; use northhing-agent-runtime discovery report',
      },
      {
        regex: /\.extension\(\)\.is_some_and\(\|ext\|\s*ext\s*==\s*"md"\)/,
        message:
          'core custom subagent registry must not own markdown file discovery; use northhing-agent-runtime discovery report',
      },
    ],
  },
  {
    ...
    patterns: [
      {
        ...
        message:
          'core subagent runtime must not redefine DelegationPolicy; use northhing-runtime-ports',
      },
      {
        regex: /\benum\s+SubagentContextMode\b/,
        message:
          'core subagent runtime must not redefine SubagentContextMode; use northhing-runtime-ports',
      },
    ],
  },
  {
    ...
    patterns: [
      {
        ...
        message:
          'core round-boundary runtime must not redefine RoundInjection; use northhing-runtime-ports',
      },
      {
        regex: /\btrait\s+DialogRoundInjectionSource\b/,
        message:
          'core round-boundary runtime must not redefine DialogRoundInjectionSource; use northhing-runtime-ports',
      },
      {
        regex: /\benum\s+RoundInjectionKind\b/,
        message:
          'core round-boundary runtime must not redefine RoundInjectionKind; use northhing-runtime-ports',
      },
      {
        regex: /\benum\s+RoundInjectionTarget\b/,
        message:
          'core round-boundary runtime must not redefine RoundInjectionTarget; use northhing-runtime-ports',
      },
      {
        regex: /\bpub\s+struct\s+SessionRoundInjectionBuffer\b/,
        message:
          'core round-boundary runtime must not own round injection buffer; use northhing-agent-runtime',
      },
    ],
  },
  {
    ...
    patterns: [
      {
        ...
        message: 'core goal mode types must not redefine GOAL_MODE_METADATA_KEY; use northhing-runtime-ports',
      },
      {
        regex: /\bconst\s+MAX_GOAL_CONTINUATIONS\b/,
        message: 'core goal mode types must not redefine MAX_GOAL_CONTINUATIONS; use northhing-runtime-ports',
      },
      {
        regex: /\bconst\s+MAX_CONTEXT_SUMMARY_CHARS\b/,
        message: 'core goal mode types must not redefine MAX_CONTEXT_SUMMARY_CHARS; use northhing-runtime-ports',
      },
      {
        regex: /\bstruct\s+ThreadGoal\b/,
        message: 'core goal mode types must not redefine ThreadGoal; use northhing-runtime-ports',
      },
      {
        regex: /\benum\s+ThreadGoalStatus\b/,
        message: 'core goal mode types must not redefine ThreadGoalStatus; use northhing-runtime-ports',
      },
      {
        regex: /\bstruct\s+GoalGenerationResult\b/,
        message: 'core goal mode types must not redefine GoalGenerationResult; use northhing-runtime-ports',
      },
      {
        regex: /\bstruct\s+ThreadGoalToolResponse\b/,
        message: 'core goal mode types must not redefine ThreadGoalToolResponse; use northhing-runtime-ports',
      },
      {
        regex: /\bstruct\s+GoalActivationResult\b/,
        message: 'core goal mode types must not redefine GoalActivationResult; use northhing-runtime-ports',
      },
      {
        regex: /\bstruct\s+GoalContinuationPlan\b/,
        message: 'core goal mode types must not redefine GoalContinuationPlan; use northhing-runtime-ports',
      },
      {
        regex: /\bpub\s+struct\s+ThreadGoalRuntime\b/,
        message:
          'core goal mode must not own thread goal runtime accounting; use northhing-agent-runtime',
      },
      {
        regex: /\bfn\s+build_thread_goal_continuation_plan\b/,
        message:
          'core goal mode must not own thread goal continuation planning; use northhing-agent-runtime',
      },
      {
        regex: /\bfn\s+goal_tool_response\b/,
        message:
          'core goal mode must not own thread goal tool response assembly; use northhing-agent-runtime',
      },
      {
        regex: /\bfn\s+billable_tokens_from_counts\b/,
        message:
          'core goal mode must not own thread goal token accounting policy; use northhing-agent-runtime',
      },
    ],
  },
  {
    ...
    patterns: [
      {
        ...
        message: 'core message model must not redefine CompressionContract; use northhing-runtime-ports',
      },
      {
        regex: /\bstruct\s+CompressionContractItem\b/,
        message: 'core message model must not redefine CompressionContractItem; use northhing-runtime-ports',
      },
      {
        regex: /\bfn\s+render_contract_items\b/,
        message: 'core message model must not own compression contract rendering; use northhing-runtime-ports',
      },
    ],
  },
  {
    ...
    patterns: [
      {
        ...
        message: 'core workspace manager must not redefine RelatedPath; use northhing-runtime-ports',
      },
    ],
  },
  {
    ...
    patterns: [
      {
        ...
        message:
          'workspace file-system contract must be owned by northhing-runtime-ports; keep only concrete adapters or re-exports in core',
      },
      {
        regex: /\bpub\s+trait\s+WorkspaceShell\b/,
        message:
          'workspace shell contract must be owned by northhing-runtime-ports; keep only concrete adapters or re-exports in core',
      },
      {
        regex: /\bpub\s+struct\s+WorkspaceServices\b/,
        message:
          'workspace service bundle contract must be owned by northhing-runtime-ports; keep only concrete adapters or re-exports in core',
      },
      {
        regex: /\bpub\s+struct\s+WorkspaceCommandOptions\b/,
        message:
          'workspace command contract must be owned by northhing-runtime-ports; keep only concrete adapters or re-exports in core',
      },
      {
        regex: /\bpub\s+struct\s+WorkspaceCommandResult\b/,
        message:
          'workspace command result contract must be owned by northhing-runtime-ports; keep only concrete adapters or re-exports in core',
      },
      {
        regex: /\bpub\s+struct\s+WorkspaceDirEntry\b/,
        message:
          'workspace directory entry contract must be owned by northhing-runtime-ports; keep only concrete adapters or re-exports in core',
      },
    ],
  },
  {
    ...
    patterns: [
      {
        ...
        message: 'core tool restrictions must not redefine ToolPathOperation; use northhing-agent-tools',
      },
      {
        regex: /\bpub struct ToolPathPolicy\b/,
        message: 'core tool restrictions must not redefine ToolPathPolicy; use northhing-agent-tools',
      },
      {
        regex: /\bpub struct ToolRuntimeRestrictions\b/,
        message:
          'core tool restrictions must not redefine ToolRuntimeRestrictions; use northhing-agent-tools',
      },
      {
        regex: /\bfn\s+normalize_absolute_posix_path\b/,
        message:
          'core tool restrictions must not redefine remote POSIX path normalization; use northhing-agent-tools',
      },
    ],
  },
  {
    ...
    patterns: [
      {
        regex: /\bpub const northhing_RUNTIME_URI_PREFIX\b/,
        message:
          'core workspace path facade must not redefine the runtime URI prefix; use northhing-agent-tools',
      },
      {
        regex: /\bpub struct ParsednorthhingRuntimeUri\b/,
        message:
          'core workspace path facade must not redefine ParsednorthhingRuntimeUri; use northhing-agent-tools',
      },
      {
        regex: /\bfn\s+posix_normalize_components\b/,
        message:
          'core workspace path facade must not redefine remote POSIX path normalization; use northhing-agent-tools',
      },
      {
    ...
    patterns: [
      {
        ...
        message:
          'core file read-state runtime must delegate pure freshness normalization to northhing-agent-tools',
      },
    ],
  },
  {
    ...
    patterns: [
      {
        ...
        message:
          'core tool result storage must delegate pure preview generation to northhing-agent-tools',
      },
      {
        regex: /\bfn\s+build_persisted_output_message\b/,
        message:
          'core tool result storage must delegate persisted-output rendering to northhing-agent-tools',
      },
      {
        regex: /\bfn\s+select_candidates_to_persist\b/,
        message:
          'core tool result storage must delegate round-budget selection to northhing-agent-tools',
      },
      {
        regex: /\bstruct\s+ToolResultStoragePolicy\b/,
        message:
          'core tool result storage must use the provider-neutral storage policy from northhing-agent-tools',
      },
    ],
  },
  {
...
export const forbiddenContentUnderRules = [
    ...
    patterns: [
      {
        ...
        message:
          'DelegationPolicy and SubagentContextMode must be imported from northhing-runtime-ports, not the core compatibility re-export',
      },
    ],
  },
  {
  ...
  },
  {
    ...
    reason:
      'core pipeline must delegate deterministic tool execution admission policy to northhing-agent-tools',
    patterns: [
      {
