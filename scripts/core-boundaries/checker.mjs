...
function collectProductCoreDependencyManifestPaths(manifestEntries) {
  return manifestEntries
    .filter((entry) => {
      ...
      return deps.some((dep) => dep.name === 'northhing-core');
    })
...
function checkCargoManifest(crateDir) {
  checkForbiddenManifestDeps(crateDir, ['northhing-core'], () => {
    return 'extracted crate must not depend on northhing-core';
  });
}
  ...
  for (const manifestPath of collectProductCoreDependencyManifests()) {
    if (!rulePaths.has(manifestPath)) {
      failures.push({
        ...
        line: 1,
        message:
          'product entry crate depends on northhing-core but is not covered by product-full assembly rules',
      });
    }
  }
}
...
function checkRustImports(crateDir) {
  ...
  walkFiles(srcDir, (path) => {
    ...
    lines.forEach((line, index) => {
      if (/\bnorthhing_core::/.test(line)) {
        failures.push({
          path,
          line: index + 1,
          message: 'extracted crate must not import northhing_core',
        });
      }
    });
  });
...
export function runCoreBoundaryCheck() {
  ...
  if (process.env.northhing_BOUNDARY_CHECK_SELF_TEST === '1') {
