#!/bin/bash
# Test script for northhing ACP server
# This script demonstrates basic ACP protocol interaction

echo "=== northhing ACP Server Test ==="
echo ""

BINARY="${northhing_CLI:-target/debug/northhing-cli}"
WORKSPACE="/tmp/test-acp"
PIPE_DIR="$(mktemp -d /tmp/northhing-acp-test-sh.XXXXXX)"
ACP_IN="$PIPE_DIR/in"
