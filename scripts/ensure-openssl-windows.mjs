 ...
 * - Cached under .northhing/cache/firedaemon-openssl-<version>/x64 (gitignored).
 * - Skips download if OPENSSL_DIR already points at a valid tree, or cache hit, or northhing_SKIP_OPENSSL_BOOTSTRAP=1.
...
export const CACHE_ROOT = join(ROOT, '.northhing', 'cache', `firedaemon-openssl-${OPENSSL_VERSION}`);
...
export async function ensureOpenSslWindows() {
  ...
  if (process.env.northhing_SKIP_OPENSSL_BOOTSTRAP === '1') {
    console.log(`${LOG} northhing_SKIP_OPENSSL_BOOTSTRAP=1, skipping bootstrap`);
    return;
  }
