...
function getDesktopBinaryPath() {
  ...
  if (process.platform === 'darwin') {
    return path.join(ROOT_DIR, 'target', 'debug', 'northhing.app', 'Contents', 'MacOS', 'northhing');
  }
...
async function ensureDesktopOpenSslIfNeeded() {
  ...
  printInfo('Windows: ensuring prebuilt OpenSSL (cached under .northhing/cache/)');
  try {
...
async function rebuildDesktopDebugBinary() {
  ...
  printInfo('Building northhing-desktop in dev mode with reduced debug info for faster local relink');
  printInfo(
...
async function startDesktopPreview() {
  ...
  if (!fs.existsSync(desktopBinary)) {
    ...
    printInfo('Retry with `pnpm run desktop:preview:debug -- --force-rebuild` or build it with `cargo build -p northhing-desktop`');
    process.exit(1);
  }
