/**
 * northhing Installer build script.
 *
 * Steps:
 * 1. Build northhing main app (optional).
...
const northhing_ROOT = path.resolve(ROOT, "..");
...
Options:
  ...
  --skip-app-build       Skip building main northhing app
...
// Step 1: Build main northhing app.
if (!skipAppBuild) {
  log("Step 1: Building northhing main application...");
  run(getMainAppBuildCommand(buildMode), northhing_ROOT);
} else {
...
if (!appExePath && STRICT_PAYLOAD_VALIDATION) {
  error(
    "Could not find built northhing executable for payload. Build the desktop app first or run with --dev for local debug."
  );
}
