...
/** Same rule as main app `getSystemPreferredDefaultThemeId`: dark -> northhing-dark, else northhing-light. */
export function getSystemPreferredBuiltinThemeId(): ThemeId {
  if (typeof window === 'undefined' || typeof window.matchMedia !== 'function') {
    return 'northhing-light';
  }
  return window.matchMedia('(prefers-color-scheme: dark)').matches ? 'northhing-dark' : 'northhing-light';
}
 ...
 * When preference is `system`, follows `prefers-color-scheme` like the main northhing ThemeService.
 */
