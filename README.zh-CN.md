...
- **下载、配置首个 AI 模型后即可使用**：MCP / 文件系统 / 终端 / Git / 远程 SSH 全部内置，配置好模型就能开跑，省掉自己从零搭建协议栈的时间；
- **数据在你自己的机器上**：会话存储于 `~/.northhing/projects/<你的工作区>/sessions/`，记忆存储于 `~/.northhing/personal_assistant/`，可迁移、可导出、可审计；没有强制上云，隐私与合规场景都能用；
...
- **会自我迭代**：大部分代码由 northhing 内置 Code Agent 通过 Vibe Coding 完成，天然亲和 AI 开发。

---

## 最新动态

northhing 通过将 **flashgrep** 与 **ripgrep** 联动形成增强版的检索链路，在超大代码仓库中代码搜索速度显著高于单纯使用 ripgrep，有效缩短项目探索时间。（内部在某大型开源代码库上测量，正式可复现 benchmark 待发布。）
...
以下是 northhing 的**官方 Agent 和能力清单**，及对业界最前沿 Agent 范式的复现进度。完成一次模型提供商配置后即可使用：
...
| **Research Agent** | 通过 Skills 处理研究与文档工作流（可扩展至 PDF / DOCX / XLSX / PPTX）；无原生 Office 引擎 |
...
| **工具链** | 文件 / 终端 / Git / LSP / 构建与测试 | 浏览器 / 基于 Skill 的文档处理 / Skill 市场 |
...
桌面端基于 **Slint + Material** 构建。CI 打包目前仅覆盖 **Windows x64**；macOS / Linux 构建目标已存在于 `package.json`（`desktop:build:arm64`、`desktop:build:x86_64`、`desktop:build:linux`），但对应 CI 矩阵因交叉编译未验证而处于禁用状态。远程控制支持手机浏览器、Telegram、飞书、微信。
...
从 [Releases](https://github.com/UmR-2026/northhing/releases) 页面下载最新桌面端安装包，安装后配置模型即可开始使用。
...
2. 大部分代码由 Vibe Coding 完成。代码问题也欢迎指正，鼓励通过 AI 进行重构优化。
