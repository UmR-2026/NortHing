// REFERENCE — copied from tools/plan-compliance-checker/src/path_resolver.rs
...
pub fn find_workspace_root(start: &Path) -> Option<PathBuf> {
    ...
    loop {
        ...
        if manifest.exists() && let Ok(content) = std::fs::read_to_string(&manifest)
...
pub fn detect_path_mismatch(plan_path: &Path, workspace_root: &Path) -> PathMismatch {
    ...
    if prepended_src.exists() {
        return PathMismatch { exists_relative: false, suggestion: Some(prepended_src) };
    }
