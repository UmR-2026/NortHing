// REFERENCE — copied from tools/plan-compliance-checker/src/plan.rs
...
pub fn parse_plan(input: &str) -> Result<Plan> {
    ...
    for event in MdParser::new(input) {
        match event {
            Event::Start(Tag::Heading { level: HeadingLevel::H1, .. }) if current_task.is_none() && plan.title.is_empty() => {
            ...
            Event::Text(text) => {
                ...
                if in_task_heading {
                    if let Some(ref mut t) = current_task && t.title.is_empty() {
                        let trimmed = s.trim();
                    ...
                    } else if s.starts_with("Expected:") && let Some(ref mut step) = current_step {
            ...
            Event::Code(code) => {
                if let Some(ref mut t) = current_task {
                    if pending_create {
                        ...
                        t.files.modify.push(ModifyTarget { path: PathBuf::from(&*code), range: None });
                        pending_modify = false;
