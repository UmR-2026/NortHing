// REFERENCE — copied from src/crates/assembly/core/src/agentic/coordination/state_manager.rs
...
impl SessionStateManager {
    ...
    pub async fn initialize(&self, session_id: &str) {
        self.states.insert(session_id.to_string(), SessionState::Idle);
    }
    ...
    pub async fn set_processing_phase(&self, session_id: &str, current_turn_id: String, phase: ProcessingPhase) {
        self.update_state(session_id, SessionState::Processing { current_turn_id, phase })
            .await;
    }
    ...
    pub fn can_start_new_turn(&self, session_id: &str) -> bool {
        if let Some(state) = self.get_state(session_id) {
            matches!(
                state,
                SessionState::Idle | SessionState::Error { recoverable: true, .. }
            )
        } else {
            false
        }
    ...
    pub fn all_states(&self) -> Vec<(String, SessionState)> {
        self.states
            .iter()
