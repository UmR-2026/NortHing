// REFERENCE — copied from src/crates/assembly/core/src/agentic/tools/implementations/skill_tool.rs
...
// Use skills module
use super::skills::{skill_registry, SkillLocation};
...
- Examples:
  ...
  - `command: "user::northhing-system::ppt-design"` - invoke a specific built-in skill by stable key
...
- Do not invoke a skill that is already running
    ...
    pub(crate) async fn resolved_skills_xml_for_context(context: Option<&ToolUseContext>) -> String {
        let registry = skill_registry();
        let available_skills = match context {
            Some(ctx) if ctx.is_remote() => {
                if let Some(fs) = ctx.ws_fs() {
                    let root = ctx.workspace.as_ref().map(|w| w.root_path_string()).unwrap_or_default();
                    registry
                        .get_resolved_skills_xml_for_remote_workspace(fs, &root, ctx.agent_type.as_deref())
                        .await
                } else {
                    registry
            ...
            Some(ctx) => {
                registry
                    .get_resolved_skills_xml_for_workspace(ctx.workspace_root(), ctx.agent_type.as_deref())
                    .await
            }
            None => registry.get_resolved_skills_xml_for_workspace(None, None).await,
        };

        available_skills.join("\n")
    }

    pub(crate) async fn build_available_skills_context_section(context: Option<&ToolUseContext>) -> Option<String> {
        ...
        if context.map(|c| c.is_remote()).unwrap_or(false) && context.and_then(|c| c.ws_fs()).is_none() {
            section.push_str(
                "\n\nRemote workspace note: Project-level skills on the server could not be indexed because workspace I/O is unavailable. Only user-level skills are shown; northhing will not fall back to scanning the remote path on the local filesystem.",
            );
        }
        Some(section)
    }
    ...
    async fn call_impl(&self, input: &Value, context: &ToolUseContext) -> NortHingResult<Vec<ToolResult>> {
        ...
        let skill_data = if context.is_remote() {
            if let Some(ws_fs) = context.ws_fs() {
                ...
                if use_stable_key {
                    registry
                        .find_and_load_skill_by_key_for_workspace(skill_name, None, context.agent_type.as_deref())
                        .await?
                } else {
                    registry
                        .find_and_load_skill_for_workspace(skill_name, None, context.agent_type.as_deref())
                        .await?
                }
            }
        } else {
...
mod tests {
    ...
    use crate::agentic::workspace::{
        WorkspaceCommandOptions, WorkspaceCommandResult, WorkspaceDirEntry, WorkspaceFileSystem, WorkspaceServices,
        WorkspaceShell,
    };
    ...
    impl WorkspaceFileSystem for FakeRemoteFs {
        ...
        async fn read_file_text(&self, path: &str) -> anyhow::Result<String> {
            if path == "/remote/project/.northhing/skills/remote-only/SKILL.md" {
                return Ok(r#"---
...
"#
        ...
        async fn exists(&self, path: &str) -> anyhow::Result<bool> {
            Ok(matches!(
                path,
                "/remote/project/.northhing/skills"
                    | "/remote/project/.northhing/skills/remote-only"
                    | "/remote/project/.northhing/skills/remote-only/SKILL.md"
            ))
        }

        async fn is_file(&self, path: &str) -> anyhow::Result<bool> {
            Ok(path == "/remote/project/.northhing/skills/remote-only/SKILL.md")
        }

        async fn is_dir(&self, path: &str) -> anyhow::Result<bool> {
            Ok(matches!(
                path,
                "/remote/project/.northhing/skills" | "/remote/project/.northhing/skills/remote-only"
            ))
        }

        async fn read_dir(&self, path: &str) -> anyhow::Result<Vec<WorkspaceDirEntry>> {
            if path == "/remote/project/.northhing/skills" {
                return Ok(vec![WorkspaceDirEntry {
                    name: "remote-only".to_string(),
                    path: "/remote/project/.northhing/skills/remote-only".to_string(),
                    is_dir: true,
                    is_symlink: false,
                }]);
            }
    ...
    async fn remote_description_indexes_project_skills_through_workspace_services() {
        let identity = workspace_session_identity("/remote/project", Some("conn-1"), Some("remote-host"))
        ...
        assert!(description.contains("Remote project skill visible only through workspace services."));
    }
    ...
    async fn remote_call_loads_default_hidden_builtin_team_skill_when_explicitly_invoked() {
        ...
        assert!(data["content"].as_str().unwrap_or_default().contains("# /cso"));
    }
    ...
    async fn stable_key_loads_the_exact_builtin_skill() {
        let context = crate::agentic::tools::framework::ToolUseContext {
            ...
            computer_use_host: None,
            runtime_tool_restrictions: Default::default(),
            runtime_handles: northhing_runtime_ports::ToolRuntimeHandles::new(None, None),
        };

        let results = SkillTool::new()
            .call_impl(&json!({ "command": "user::northhing-system::ppt-design" }), &context)
            .await
            .expect("stable key should load northhing's built-in ppt-design skill");

        let ToolResult::Result { data, .. } = &results[0] else {
            panic!("expected result payload");
        };
        assert_eq!(data["skill_name"], "ppt-design");
        assert_eq!(data["skill_key"], "user::northhing-system::ppt-design");
        assert_eq!(data["source_slot"], "northhing-system");
        assert!(data["content"]
            .as_str()
            .unwrap_or_default()
    ...
    impl WorkspaceFileSystem for OrderingRemoteFs {
        ...
        async fn read_file_text(&self, path: &str) -> anyhow::Result<String> {
            match path {
                "/remote/project/.northhing/skills/z-last/SKILL.md" => {
                    Ok("---\nname: z-last\ndescription: last\n---\n\nz\n".to_string())
                }
                "/remote/project/.northhing/skills/a-first/SKILL.md" => {
                    Ok("---\nname: A-First\ndescription: first\n---\n\na\n".to_string())
                }
                "/remote/project/.northhing/skills/dup-one/SKILL.md" => {
                    Ok("---\nname: dup\ndescription: dup one\n---\n\none\n".to_string())
                }
                "/remote/project/.northhing/skills/dup-two/SKILL.md" => {
                    Ok("---\nname: dup\ndescription: dup two\n---\n\ntwo\n".to_string())
                }
        ...
        async fn is_dir(&self, path: &str) -> anyhow::Result<bool> {
            Ok(matches!(
                path,
                "/remote/project/.northhing/skills"
                    | "/remote/project/.northhing/skills/z-last"
                    | "/remote/project/.northhing/skills/a-first"
                    | "/remote/project/.northhing/skills/dup-one"
                    | "/remote/project/.northhing/skills/dup-two"
            ))
        }

        async fn read_dir(&self, path: &str) -> anyhow::Result<Vec<WorkspaceDirEntry>> {
            match path {
                "/remote/project/.northhing/skills" => Ok(vec![
                    WorkspaceDirEntry {
                        name: "z-last".to_string(),
                        path: "/remote/project/.northhing/skills/z-last".to_string(),
                        is_dir: true,
                        is_symlink: false,
                    },
                    WorkspaceDirEntry {
                        name: "a-first".to_string(),
                        path: "/remote/project/.northhing/skills/a-first".to_string(),
                        is_dir: true,
                        is_symlink: false,
                    },
                    WorkspaceDirEntry {
                        name: "dup-two".to_string(),
                        path: "/remote/project/.northhing/skills/dup-two".to_string(),
                        is_dir: true,
                        is_symlink: false,
                    },
                    WorkspaceDirEntry {
                        name: "dup-one".to_string(),
                        path: "/remote/project/.northhing/skills/dup-one".to_string(),
                        is_dir: true,
                        is_symlink: false,
                    },
                ]),
