// REFERENCE — copied from src/crates/assembly/core/src/agentic/tools/implementations/skills/builtin.rs
...
//! Built-in skills shipped with northhing.
//!
//! These skills are embedded into the `northhing-core` binary and installed into a
...
async fn read_installed_manifest(root: &Path) -> NortHingResult<Option<BuiltinSkillsManifest>> {
    let path = builtin_skills_manifest_path(root);
    match fs::read_to_string(&path).await {
        Ok(content) => match serde_json::from_str::<BuiltinSkillsManifest>(&content) {
            Ok(manifest) => Ok(Some(manifest)),
            Err(error) => {
                warn!("Invalid built-in skills manifest at {}: {}", path.display(), error);
                Ok(None)
            }
        },
...
async fn write_installed_manifest(root: &Path) -> NortHingResult<()> {
    let path = builtin_skills_manifest_path(root);
...
async fn remove_existing_path(path: &Path) -> NortHingResult<()> {
...
async fn cleanup_legacy_builtin_dirs(legacy_root: &Path) -> NortHingResult<()> {
...
async fn acquire_install_lock(legacy_root: &Path) -> NortHingResult<BuiltinSkillsInstallLock> {
    ...
    // `.system` directory.
    let file = task::spawn_blocking(move || -> NortHingResult<std::fs::File> {
    ...
    .await
    .map_err(|error| {
        crate::util::errors::NortHingError::io(format!("Failed to join built-in skills install lock task: {}", error))
    })??;

    Ok(BuiltinSkillsInstallLock { file })
}

async fn install_builtin_skills_to_staging(staging_root: &Path) -> NortHingResult<(usize, usize)> {
...
pub async fn ensure_builtin_skills_installed() -> NortHingResult<()> {
...
async fn sync_dir(dir: &Dir<'_>, dest_root: &Path) -> NortHingResult<SyncStats> {
...
fn safe_join(root: &Path, relative: &Path) -> NortHingResult<PathBuf> {
    if relative.is_absolute() {
        return Err(crate::util::errors::NortHingError::validation(format!(
...
async fn desired_file_content(file: &include_dir::File<'_>, _dest_path: &Path) -> NortHingResult<Vec<u8>> {
