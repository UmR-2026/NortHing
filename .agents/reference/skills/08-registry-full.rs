// REFERENCE — copied from src/crates/assembly/core/src/agentic/tools/implementations/skills/registry.rs
...
const NORTHHING_USER_SLOT: &str = "northhing";
const NORTHHING_SYSTEM_SLOT: &str = "northhing-system";
const NORTHHING_SYSTEM_DIR_NAME: &str = ".system";
...
impl SkillCandidate {
    fn from_data(mut data: SkillData, slot: &str, key_prefix: &str, priority: usize, is_builtin: bool) -> Self {
...
impl SkillRegistry {
    ...
    async fn scan_skills_in_dir(entry: &SkillRootEntry) -> Vec<SkillCandidate> {
        ...
        while let Ok(Some(item)) = read_dir.next_entry().await {
            ...
            match fs::read_to_string(&skill_md_path).await {
                Ok(content) => {
                    match SkillData::from_markdown(path.to_string_lossy().to_string(), &content, entry.level, false) {
                        Ok(mut skill_data) => {
    ...
    async fn scan_skill_candidates_for_workspace(&self, workspace_root: Option<&Path>) -> Vec<SkillCandidate> {
    ...
    async fn scan_remote_project_skills(fs: &dyn WorkspaceFileSystem, remote_root: &str) -> Vec<SkillCandidate> {
        ...
        for entry in roots {
            ...
            for item in entries {
                ...
                match fs.read_file_text(&skill_md_path).await {
                    Ok(content) => {
                        match SkillData::from_markdown(item.path.clone(), &content, SkillLocation::Project, false) {
                            Ok(mut skill_data) => {
                                skill_data.dir_name = dir_name;
                                skills.push(SkillCandidate::from_data(
                                    skill_data,
    ...
    pub async fn get_mode_skill_infos_for_workspace(
        ...
        Self::build_mode_skill_infos(all_skills, resolved, mode_id, &user_overrides, &disabled_project)
    }
