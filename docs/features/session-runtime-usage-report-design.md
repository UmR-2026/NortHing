...
Long northhing sessions can include model streaming, tool execution, Git operations, file writes, Skills, MCP calls, context compression, subagents, retries, user confirmation waits, and file diffs. The chat transcript shows what happened, but it does not yet answer the user's operational questions:
...
northhing can reuse several existing surfaces.
...
| CLI `/usage` coverage | Done for interactive CLI | CLI `usage_*` coverage and the shared renderer | Top-level `northhing usage --session` is outside the closed product scope. |
...
- `cargo test -p northhing-core session_usage -- --nocapture`
...
- `cargo test -p northhing-core session_usage -- --nocapture` once tests exist.
- `cargo check -p northhing-core`.
...
| `/usage` conflicts with future top-level `northhing usage` | Keep interactive command implementation isolated behind a reusable service |
...
4. Keep top-level `northhing usage --session <id>` out of scope until workspace-scoped persisted session lookup is designed.
...
- CLI supports interactive chat `/usage`; top-level `northhing usage --session <id>` is outside the closed product scope.
