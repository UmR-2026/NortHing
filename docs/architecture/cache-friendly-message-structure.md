# Cache-Friendly Message Structure In northhing

This note explains the cache-friendly request shape northhing tries to preserve
...
northhing's model requests are intentionally assembled in a stable order:
...
a compatibility facade that re-exports the owner types from `northhing-agent-runtime`.
...
- when northhing rebuilds the listing baseline to the latest snapshot, it also
...
Persisted session artifacts live under `.northhing/sessions/{session_id}/`
