...
- **K2 desktop-tauri 切 facade（已关闭，宿主删除）**：新宿主先行（体量小、在建中、F1.5/F2 正要加事件面）。src-tauri 只 import `kernel-api`。验证：GUI 冒烟（发消息→流式→完成）+ `cargo check --manifest-path src/apps/desktop-tauri/src-tauri/Cargo.toml` 0 err + `cargo check -p northhing` 绿 + K0 目标值实测对比。**顺带把 F1.5 的 `chat-tool` 事件面定义进 facade**，前端计划与 kernel 迁移在此合流。
  **关闭记录（2026-07-25）**：K2b 切换（`ae15d22`，2026-07-21）与 chat-tool 事件面（B 线 `6f039ca`/`9493450`/`062959f`）曾落地；**desktop-tauri 宿主随后于 `34a2397`（2026-07-23，"superseded by Slint"）整体删除**，K2 验收对象不复存在，本条关闭。facade 保留（K1 冻结面不变），消费者空窗期由 K4a 递补——**K4a 提升为 K 线下一主单**，切分设计稿 `docs/design/2026-07-25-k4a-desktop-facade.md`（定稿 v1.0，judge-lc APPROVED + 用户五项拍板）。
...
- **K3 ROI 闸门（K2 验收后、K3 启动前的强制决策点）**：对照 K0 目标值实测——**若"改 leaf tool 后增量 check < 30s"在 K2 已达成，K3 降级为"有空再做"**（其编译收益大头已在 K1/K2 拿到，剩下的是认知解耦收益，不值得冒最高风险强推）；若未达成，用 K0 探针的 go/no-go 证据决定 K3 是否启动。**探针证据时效**：闸门决策前重跑一次 K0 探针（同一模块），结果与 K0 证据偏差 > 50% 则探针证据失效，需重新评估。**高扇出探针前置（双 judge 建议，2026-07-20 采纳）**：K1/K2 期间补做一次代表性 HIGH-fan-out 探针（DialogScheduler 相邻模块），在 facade 尚可回退时提前暴露真实 cycles/拖拽/测试成本——K0 的 fan-out=0 探针只是成本下限（floor），不构成 K3 主体证明。闸门结论写入本文档。
  **闸门输入就绪（2026-07-26）**：K4a 完工，K0 目标实测达成（leaf touch 增量 3.40s ≪ 14.93s）。按本条规则 K3 符合降级条件，待用户正式裁定（裁定结果记录于此）。
...
- **K4a desktop 切 facade（✅ 完成 2026-07-26）**：设计稿 `docs/design/2026-07-25-k4a-desktop-facade.md`（定稿 v1.0，judge-lc APPROVED + 用户五项拍板）。七单落地：T1 `85fdd35`（bootstrap 收编）、T4p `a4ccc5a`（ProviderFormDto.provider_type）、T23q `8a9b16e`（DTO 补缺 ×5）、T23 `95db64d`+`d4c3520`（turn+session 数据流）、T4 `12a5615`（settings/skills/mcp/inspector）、T5 `6eb6209`（debug-log 微 crate + 守卫）。desktop 残留 `northhing_core::` 21 行全属豁免清单（facade 手柄/shutdown/w4_repro/actor 注入）。**K0 编译收益实测达成：touch leaf（tool-contracts）后增量 check 3.40s ≪ 14.93s 目标**。验收：cargo check 0 err，desktop lib 72 绿，kernel_facade 31 绿，cargo tree 零命中；judge-m3 全程验收。**K4b（cli+acp）未启动**——启动前须按本条重新评审 facade 面完整性（cli 100+ 处、acp 61 处耦合模式差异）。
