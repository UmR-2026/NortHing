# Model Request Cache Reuse In northhing

This note summarizes the mechanisms northhing uses to improve model-side prompt
...
northhing tries to keep the model-visible request prefix as stable as possible.
...
cache writes so northhing can measure whether these strategies are actually
working.

## 1. System Prompt And User Context Are Cached Separately

northhing does not treat the whole request prefix as one opaque string.
...
- the file is stored under the session directory in `.northhing/sessions/...`
...
the system prompt cache. That lets northhing keep the global instruction template
...
When northhing branches a session from an existing turn, it copies more than turn
text:
...
When `Task` is called with `fork_context=true`, northhing:
...
northhing's four shared coding modes are:
...
In other words, northhing aligns backend prompt identities and frontend mode-switch
...
northhing explicitly avoids baking that surface into one permanently rebuilt prompt.
...
1. On the first turn, northhing saves a full skill/subagent snapshot.
...
correctness do not always want the same baseline. In those cases northhing can keep
...
northhing does allow tool customization at the agent/profile level:
...
Skill and subagent listings are prompt-visible descriptive surfaces, so northhing
can:
...
However, northhing does not currently plan to implement this.
...
northhing's cache-hit strategy.
...
northhing does not send compression as a detached, prompt-from-scratch request.
...
Only after that shared prefix is rebuilt does northhing append the final
...
When compression is applied, northhing does three important things:
...
northhing also normalizes provider-specific cache telemetry so cache reuse can be
...
northhing keeps them separate on purpose. That avoids overstating hit rate and
...
northhing improves request cache hit rate by combining prompt splitting, session
