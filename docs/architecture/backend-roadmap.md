...
> **0.3 拆分（E-07，2026-08-17）**：0.3a = T0 + T2-1 + T2-2 + T1；0.3b = T2-9 + T2-10 + PCS-1/2。**0.3a/0.3b 均已 CLOSED（2026-08-21）**。
...
| T2-9 | **功能冗余合并批次**（2026-08-17 冗余扫描）：**批 1 已完成**（2026-08-21 ROT-1，merge f5e7922：deep_research 双胞胎 re-export 收口 runtime-ports；now_unix_ms→core-types `time.rs` 双签名 helper + 4 处命名重复转换；ndjson/FILE_LOCKS/server bootstrap/CLI init 四项经预检核销[证据 task-rot1-review.md]；内联时间戳转 ratchet 条目 unix_epoch_inline=79）；批 2 已完成 2/3（2026-08-21 T2-9-B2，merge cd750fc：A7 BackendEvent 死管道整删[-754 行] + NullDispatcher 空转路径移除[desktop 回退 legacy 直连，flag 保持 true 加注]）；**批 2 已全完**（2026-08-21 T2-9-B3，merge c7a19f0：app.json↔GlobalConfig 镜像拆除段 1——providers/default_model 单源化直穿 facade，方案 C 用户拍板：core `#[serde(skip_serializing)]` api_key 不落盘 + 加载路径 scrub 老明文，desktop 推送仅内存）；**配置镜像段 2 余量**（workspaces/onboarding 迁 core，另行排单）；延期 L 级——ExecCommand↔Bash 合并（Bash/PTY 为正）、双 ToolRegistry 迁移收尾、MCP core 包装层（3,641L）收口 | 冗余扫描 | ~~批 1 S~~ / ~~批 2 M~~（段 2 余量挂账） / 延期 L |
| ~~T2-10~~ | ~~连续性自检测试~~ | **已完成**（2026-08-21，merge 9bd56f4：seed-restore-diff 形态落地 `continuity_selfcheck.rs`，三组等价断言[会话/记忆/身份] + identity 测试隔离缝；**fake AI backend 依赖经预检裁定绕过**——离线罐头不接 agent loop，seed 形态同等确定性；fake backend 真需求[驱动完整回合含 distill]留将来） | ~~S~~ |
