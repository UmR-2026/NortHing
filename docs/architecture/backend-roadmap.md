...
| T2-9 | **功能冗余合并批次**（2026-08-17 冗余扫描）：**批 1 已完成**（2026-08-21 ROT-1，merge f5e7922：deep_research 双胞胎 re-export 收口 runtime-ports；now_unix_ms→core-types `time.rs` 双签名 helper + 4 处命名重复转换；ndjson/FILE_LOCKS/server bootstrap/CLI init 四项经预检核销[证据 task-rot1-review.md]；内联时间戳转 ratchet 条目 unix_epoch_inline=79）；批 2 已完成 2/3（2026-08-21 T2-9-B2，merge cd750fc：A7 BackendEvent 死管道整删[-754 行] + NullDispatcher 空转路径移除[desktop 回退 legacy 直连，flag 保持 true 加注]）；**批 2 余 1 项**——app.json↔GlobalConfig 镜像拆除（写穿 kernel API）；延期 L 级——ExecCommand↔Bash 合并（Bash/PTY 为正）、双 ToolRegistry 迁移收尾、MCP core 包装层（3,641L）收口 | 冗余扫描 | ~~批 1 S~~ / 批 2 M（2/3 done） / 延期 L |
