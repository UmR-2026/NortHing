...
- §0 指标表**逐行用命令重新测量**（不得凭记忆）：测试数、`grep -c "ui\.on_" src/apps/desktop/src/app_state/`（当前 21，文档写 10）、const flags（`USE_LIGHTWEIGHT_ACTOR = true` 已在 flags.rs:19 确认，其余三个确认 false）、HEAD 改为当前 commit。
- 追加 Wave1 + D1 的 6 个 commit 摘要（9a1575d/c7e7218/95e29ba/af15815/ad349f9/65a1003）。
- "next session's job" 段落改为本指南的后续任务（D2 引导/设置、W3a 排队+取消、冒烟）。

### 3.5 northing-installer/README.md + AGENTS.md

- 后端已于 af15815 从零实现。逐条核对其描述的行为与 `src-tauri/src/installer/commands.rs` 是否一致（14 命令、注册表卸载项、SHA-256 校验、卸载保留用户数据）；仍不符的改掉；补上"crate-type 仅 rlib（GNU ld ordinal 限制）"和"embed-resource pin 3.0.5"两条环境注记。
...
- B-2：被纠文档中不再出现：`src/web-ui`（无现状注记时）、"Tauri"、"hot-reload"、"Vite HMR"、不存在的 pnpm 脚本名；README 非空且命令全部真实存在。
