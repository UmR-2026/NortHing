...
**Goal:** Add OS-level fullscreen support for the northhing Desktop main window without changing maximize, panel fullscreen, CLI/TUI rendering, or product runtime logic.
...
**Tech Stack:** Tauri v2 window APIs, React hooks, northhing `ShortcutManager` where appropriate, Vitest for pure shortcut helper coverage, existing locale JSON files.
...
- Windows/Linux: pressing `F11` asks the operating system to put the whole northhing Desktop window into fullscreen.
- macOS: pressing `Control+Command+F` uses the platform fullscreen convention.
- The northhing internal layout remains the same: NavBar, SceneBar, panels, chat, editor, terminal, browser, and diff surfaces continue to render.
...
- [x] Verify. Risk: Low.
  ...
  - `cargo check -p northhing-desktop`
  - `cargo test -p northhing-desktop`
