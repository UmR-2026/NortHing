...
/// Visibility of the inner / outer windows. When `false`, the
/// corresponding OS window is hidden via `window.set_visible(false)`.
/// The room's jewel triggers toggle these flags; inner/outer subscribe
/// to the `GlobalVisibility` watch channel and follow (default per
/// block-contract §3.2: three windows docked and visible by default).
...
/// Shared visibility channel for the inner / outer windows. Same
/// pattern as `GlobalTheme` (state.rs above): the room writes
/// synchronously from the jewel click handlers, the inner/outer
/// windows subscribe via `use_future` and call
/// `window().set_visible(bool)` (event-driven, zero polling — the
/// low-frequency wakeups were proven safe with the theme channel in
/// r3p4; see `task-migrate-room-report-r3p4.md` §7).
#[derive(Debug, Clone)]
pub struct GlobalVisibility {
    tx: watch::Sender<VisibilityState>,
}

impl Default for GlobalVisibility {
    fn default() -> Self {
        Self {
            tx: watch::channel(VisibilityState::new()).0, // default: all visible
        }
    }
}
...
impl GlobalVisibility {
    ...
    /// Set inner-window visibility and notify every subscriber. No-op
    /// when unchanged. `Err` (channel closed) is treated as a no-op,
    /// matching the geometry channel's `let _ =` convention.
    pub fn set_inner(&self, visible: bool) {
        let mut state = *self.tx.borrow();
        if state.inner_visible != visible {
            state.inner_visible = visible;
            let _ = self.tx.send(state);
        }
    }

    /// Set outer-window visibility and notify every subscriber.
    pub fn set_outer(&self, visible: bool) {
        let mut state = *self.tx.borrow();
        if state.outer_visible != visible {
            state.outer_visible = visible;
            let _ = self.tx.send(state);
        }
    }

    /// Subscribe to visibility changes. The receiver starts at the
    /// current value; `changed()` wakes on every later `set_inner` /
    /// `set_outer`.
    pub fn subscribe(&self) -> watch::Receiver<VisibilityState> {
        self.tx.subscribe()
    }
}