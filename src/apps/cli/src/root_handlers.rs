...
pub async fn handle_exec_command(config: CliConfig, args: ExecCommandArgs) -> Result<()> {
    ...
    let (agentic_system, original_skip_confirmation) = crate::initialize_core_services(skip_confirmation).await?;
...
pub async fn handle_session_action(action: SessionAction) -> Result<Option<String>> {
    ...
    match action {
        SessionAction::List => {
            ...
            println!("History sessions for current project (total {})", sessions.len());
        ...
        SessionAction::Show { id } => {
            ...
            let session = coordinator.restore_session(&workspace_path, &session_id).await?;
            ...
            if !messages.is_empty() {
                ...
                for msg in recent.iter().rev() {
                    ...
                    let content_preview = match &msg.content {
                        northhing_core::agentic::core::message::MessageContent::Text(text) => {
                            text.lines().next().unwrap_or("").to_string()
                        }
                        northhing_core::agentic::core::message::MessageContent::Multimodal { text, images } => {
                            if text.is_empty() {
                        ...
                        northhing_core::agentic::core::message::MessageContent::Mixed { text, tool_calls, .. } => {
                            if text.is_empty() {
                        ...
                        northhing_core::agentic::core::message::MessageContent::ToolResult { tool_name, .. } => {
        ...
        SessionAction::Fork { id, id_only } => {
            let session_id = resolve_cli_session_id(&coordinator, &workspace_path, &id).await?;
            let (_session, turns) = coordinator.restore_session_view(&workspace_path, &session_id).await?;
            ...
            let persistence_manager = northhing_core::agentic::persistence::PersistenceManager::new(path_manager)
                .map_err(|error| anyhow::anyhow!(error.to_string()))?;
            let result = persistence_manager
                .branch_session(
                    &workspace_path,
                    &northhing_core::agentic::persistence::session_branch::SessionBranchRequest {
...
async fn resolve_cli_session_id(
    coordinator: &std::sync::Arc<northhing_core::agentic::coordination::ConversationCoordinator>,
    workspace_path: &Path,
    id: &str,
) -> Result<String> {
    if id == "last" {
...
pub fn handle_health_command() -> Result<()> {
    println!("northhing CLI is running normally");
...
pub async fn serve_acp_stdio() -> Result<()> {
    ...
    crate::initialize_terminal_service()
        .await
        .context("Failed to initialize terminal service")?;
    ...
    northhing_acp::NortHingAcpRuntime::serve_stdio(agentic_system).await?;
    Ok(())
}
