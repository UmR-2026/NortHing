...
use super::Agent;
use northhing_core::agentic::coordination::{ConversationCoordinator, DialogSubmissionPolicy, DialogTriggerSource};
use northhing_core::agentic::core::SessionConfig;
use northhing_core::agentic::events::EventQueue;
...
impl CoreAgentAdapter {
    ...
    // reason: public getter reserved for upcoming CLI subcommands that need direct coordinator access (e.g. session admin tools)
    #[allow(dead_code)]
    ...
    fn is_session_not_found_error(error_msg: &str) -> bool {
        ...
        msg.contains("session not found") || msg.contains("session does not exist") || msg.contains("not found")
    }
    ...
    pub async fn create_session_with_id(&self, session_id: String, agent_type: &str) -> Result<String> {
...
impl Agent for CoreAgentAdapter {
    ...
    async fn cancel_current_turn(&self) -> Result<()> {
        ...
        if let (Some(session_id), Some(turn_id)) = (&*session_id_guard, &*turn_id_guard) {
            ...
            self.coordinator.cancel_dialog_turn(session_id, turn_id).await?;
        }
    ...
    async fn submit_user_answers(&self, tool_id: &str, answers: serde_json::Value) -> Result<()> {
        ...
        use northhing_core::agentic::tools::user_input_manager::user_input_manager;
