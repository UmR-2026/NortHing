...
pub fn render_diff<'a>(
    ...
    match effective_mode {
        ...
        DiffViewMode::Split => render_split(old_content, new_content, theme, max_lines, available_width),
    }
}

/// Backward-compatible wrapper (used by existing code paths)
#[allow(dead_code)]
pub fn render_unified_diff<'a>(old_content: &str, new_content: &str, theme: &Theme, max_lines: usize) -> Vec<Line<'a>> {
...
fn render_unified<'a>(
    ...
    for (i, entry) in entries.iter().enumerate() {
        ...
        if !last_visible {
            ...
            let h_old = entry
                .old_ln
                .unwrap_or(entries[..i].iter().rev().find_map(|e| e.old_ln).unwrap_or(1));
            let h_new = entry
                .new_ln
                .unwrap_or(entries[..i].iter().rev().find_map(|e| e.new_ln).unwrap_or(1));
            ...
            if hunk_old_start != Some(h_old) || hunk_new_start != Some(h_new) {
                ...
                lines.push(Line::from(Span::styled(
                    format!("@@ -{},{} +{},{} @@", h_old, hunk_old_count, h_new, hunk_new_count),
        ...
        let content_width = (available_width as usize).saturating_sub(content_overhead).max(1);
    ...
    while i < entries.len() {
        ...
        if !visible[i] {
            if last_visible {
                ...
                lines.push(Line::from(Span::styled(sep_line, theme.style(StyleKind::Muted))));
                shown += 1;
            }
            last_visible = false;
            i += 1;
        ...
        match entry.tag {
            ...
            ChangeTag::Delete => {
                let has_insert_pair =
                    i + 1 < entries.len() && visible[i + 1] && entries[i + 1].tag == ChangeTag::Insert;
