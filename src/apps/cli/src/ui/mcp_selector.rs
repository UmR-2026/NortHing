...
// reason: McpItem fields are reserved for the upcoming MCP management UI (today the selector consumes an inline tuple shape instead)
#[derive(Debug, Clone)]
#[allow(dead_code)]
pub struct McpItem {
    pub id: String,
            ...
            .map(|item| {
                ...
                if is_confirm_delete {
                    let line = Line::from(vec![
                        Span::styled("\u{2717} ", theme.style(StyleKind::Error).add_modifier(Modifier::BOLD)),
                        Span::styled(&item.name, theme.style(StyleKind::Error).add_modifier(Modifier::BOLD)),
                        Span::styled(
