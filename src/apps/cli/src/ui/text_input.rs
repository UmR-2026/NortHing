...
impl TextInput {
    ...
    // reason: set_cursor_buffer_start() reserved for upcoming keyboard shortcut bindings (Ctrl+Home / Cmd+Up equivalent)
    #[allow(dead_code)]
    pub fn set_cursor_buffer_start(&mut self) {
        self.cursor = 0;
    }

    // reason: set_cursor_buffer_end() reserved for upcoming keyboard shortcut bindings (Ctrl+End / Cmd+Down equivalent)
    #[allow(dead_code)]
    ...
    pub fn render(&mut self, frame: &mut Frame, inner: Rect, style: &TextInputStyle, show_cursor: bool) {
        ...
        let visible_slice: Vec<Line<'static>> = visual_lines.into_iter().skip(scroll).take(visible_lines).collect();
        ...
        if show_cursor {
            ...
            if row_in_view < visible_lines {
                frame.set_cursor_position((inner.x + cursor_vcol as u16, inner.y + row_in_view as u16));
            }
        }
    }
}
