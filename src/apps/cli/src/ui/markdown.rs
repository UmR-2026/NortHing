...
impl MarkdownRenderer {
    ...
    pub fn render(&self, markdown: &str, width: usize) -> Vec<Line<'static>> {
        ...
        let flush_with_wrap =
            |spans: &mut Vec<Span<'static>>, lines: &mut Vec<Line<'static>>, max_w: usize, do_wrap: bool| {
                if spans.is_empty() {
                    return;
                }
        ...
        for event in parser {
            match event {
                Event::Start(tag) => match tag {
                    ...
                    Tag::Image { .. } => {
                        current_line_spans.push(Span::styled("[Image]".to_string(), self.theme.style(StyleKind::Info)));
                    }
                    Tag::Table(alignment) => {
                        flush_with_wrap(&mut current_line_spans, &mut lines, wrap_width, true);
                        lines.push(Line::from(""));
                ...
                Event::Code(code) => {
                    let span = Span::styled(format!("`{}`", code), self.theme.style(StyleKind::Success));
    ...
    fn calculate_column_widths(&self, max_total_width: usize) -> Vec<usize> {
        ...
        for row in &self.rows {
            for (i, cell) in row.iter().enumerate().take(num_cols) {
                let cell_width: usize = cell.iter().map(|s| UnicodeWidthStr::width(s.content.as_ref())).sum();
        ...
        let target_content_width = max_total_width.saturating_sub(border_and_padding).max(num_cols);
    ...
    fn render_table(&self, max_total_width: usize) -> Vec<Line<'static>> {
        ...
        for (row_idx, row) in self.rows.iter().enumerate() {
            ...
            for (col_idx, col_width) in widths.iter().copied().enumerate().take(num_cols) {
                ...
                let wrapped_spans: Vec<Vec<Span<'static>>> = wrapped_lines.into_iter().map(|line| line.spans).collect();
            ...
            for visual_row in 0..row_height {
                ...
                for col_idx in 0..num_cols {
                    ...
                    let alignment = self.alignments.get(col_idx).copied().unwrap_or(Alignment::None);
                    let content_spans = wrapped_cells[col_idx].get(visual_row).cloned().unwrap_or_default();
