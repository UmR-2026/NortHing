        ...
        match self.server_type {
            ServerType::Local => {
                ...
                if parts.len() <= 1 {
                    ...
                    format!(r#"{{"type":"stdio","command":"{}","args":[{}]}}"#, cmd, args.join(","))
                }
            }
            ServerType::Remote => {
    ...
    pub fn render(&self, frame: &mut Frame, area: Rect, theme: &Theme) {
        ...
        if self.step == Step::CommandOrUrl {
            ...
            let type_line = Line::from(vec![
                ...
                Span::styled(type_label, theme.style(StyleKind::Primary).add_modifier(Modifier::BOLD)),
            ]);
        ...
        match self.step {
            ...
            Step::ServerType => {
                ...
                let prompt_line = Line::from(Span::styled("Select MCP server type:", theme.style(StyleKind::Info)));
                frame.render_widget(Paragraph::new(prompt_line), prompt_area);
                row += 1;
            ...
            Step::CommandOrUrl => {
                ...
                let (prompt_text, placeholder) = match self.server_type {
                    ...
                    ServerType::Remote => ("Enter the remote MCP server URL:", "https://example.com/mcp"),
                };
                let prompt_line = Line::from(Span::styled(prompt_text, theme.style(StyleKind::Info)));
                frame.render_widget(Paragraph::new(prompt_line), prompt_area);
                row += 1;
        ...
        if inner.y + row < inner.y + inner.height {
            ...
            let hint = Paragraph::new(Line::from(Span::styled(hint_text, theme.style(StyleKind::Muted))));
