...
pub fn render_permission_overlay(frame: &mut Frame, prompt: &PermissionPrompt, theme: &Theme, area: Rect) {
    match &prompt.stage {
...
fn render_button_bar(frame: &mut Frame, area: Rect, theme: &Theme, options: &[&str], selected: usize, hint_text: &str) {
    ...
    for (i, option) in options.iter().enumerate() {
        ...
        if i == selected {
            ...
            spans.push(Span::styled(
                format!(" {} ", option),
                Style::default().fg(theme.muted).bg(theme.background_element),
            ));
        }
    }
