...
pub fn builtin_provider_templates() -> Vec<ProviderTemplate> {
    vec![
        ...
        ProviderTemplate {
            ...
            format: "openai".into(),
            models: vec!["kimi-k2.5".into(), "kimi-k2".into(), "kimi-k2-thinking".into()],
...
impl ProviderSelectorState {
    ...
    fn confirm_selection(&self) -> Option<ProviderSelection> {
        ...
        match &self.rows[row_idx] {
            DisplayRow::Provider(tmpl_idx) => Some(ProviderSelection::Provider(self.templates[*tmpl_idx].clone())),
    ...
    fn selectable_index_at_row(&self, mouse_row: u16, popup_area: &Rect) -> Option<usize> {
        ...
        self.selectable_row_indices.iter().position(|&ri| ri == row_index)
    }
        ...
        for (vi, row_idx) in (self.scroll_offset..visible_end).enumerate() {
            ...
            match row {
                ...
                DisplayRow::Provider(tmpl_idx) => {
                    ...
                    self.render_item_row(frame, row_area, &tmpl.name, &tmpl.description, is_selected, theme);
                }
                DisplayRow::Custom => {
                    let is_selected = self
                        .selectable_row_indices
    ...
    fn render_item_row(
        ...
        let desc_style = if is_selected {
            Style::default().bg(theme.primary).fg(theme.selection_foreground())
        } else {
            theme.style(StyleKind::Muted)
        };
        ...
        if is_selected {
            ...
            frame.render_widget(Paragraph::new(Line::from(Span::styled(bg_fill, bg_style))), row_area);
        }
