...
impl ChatView {
    ...
    pub fn take_pending_command(&mut self) -> Option<String> {
        self.mouse.pending_command.take()
    }
    ...
    pub fn take_pending_theme_preview(&mut self) -> Option<ThemeItem> {
        self.mouse.pending_theme_preview.take()
    }

    pub fn take_pending_skill_action(&mut self) -> Option<SkillSelectorAction> {
        self.mouse.pending_skill_action.take()
    }

    pub fn take_pending_subagent_action(&mut self) -> Option<SubagentSelectorAction> {
        self.mouse.pending_subagent_action.take()
    }

    pub fn handle_mouse_event(&mut self, mouse: &crossterm::event::MouseEvent) -> bool {
        // Popups take priority when visible
        if self.popups.model_selector.captures_mouse(mouse) {
            self.popups.model_selector.handle_mouse_event(mouse);
            return true;
        }
        if self.popups.theme_selector.captures_mouse(mouse) {
            self.popups.theme_selector.handle_mouse_event(mouse);
            self.mouse.pending_theme_preview = self.popups.theme_selector.selected_item().cloned();
            return true;
        }
        if self.popups.agent_selector.captures_mouse(mouse) {
            self.popups.agent_selector.handle_mouse_event(mouse);
            return true;
        }
        if self.popups.session_selector.captures_mouse(mouse) {
            self.popups.session_selector.handle_mouse_event(mouse);
            return true;
        }
        if self.popups.skill_selector.captures_mouse(mouse) {
            if let Some(action) = self.popups.skill_selector.handle_mouse_event(mouse) {
                self.mouse.pending_skill_action = Some(action);
            }
            return true;
        }
        if self.popups.subagent_selector.captures_mouse(mouse) {
            if let Some(action) = self.popups.subagent_selector.handle_mouse_event(mouse) {
                self.mouse.pending_subagent_action = Some(action);
            }
            return true;
        }
        if self.popups.mcp_selector.captures_mouse(mouse) {
            let action = self.popups.mcp_selector.handle_mouse_event(mouse);
            if let McpAction::Toggle(item) = action {
                self.mouse.pending_mcp_toggle = Some(item.id.clone());
            }
            return true;
        }
        if self.command_menu.captures_mouse(mouse) {
            if let Some(cmd) = self.command_menu.handle_mouse_event(mouse) {
                ...
                self.mouse.pending_command = Some(cmd);
            }
            return true;
        }
        false
    ...
    fn selection_bounds(&self) -> Option<(TextSelectionPoint, TextSelectionPoint)> {
        let start = self.mouse.selection_anchor?;
        let end = self.mouse.selection_focus?;
    ...
    pub fn begin_mouse_selection(&mut self, column: u16, row: u16) -> bool {
        ...
        self.mouse.selection_anchor = Some(point);
        self.mouse.selection_focus = Some(point);
        self.mouse.selection_mouse_down = Some((column, row));
        self.mouse.selection_dragged = false;
        true
    }

    pub fn update_mouse_selection(&mut self, column: u16, row: u16) -> bool {
        if self.mouse.selection_mouse_down.is_none() {
            return false;
        }
        ...
        if let Some((origin_x, origin_y)) = self.mouse.selection_mouse_down {
    ...
    pub fn complete_mouse_selection_or_click(
        ...
        let Some((origin_x, origin_y)) = self.mouse.selection_mouse_down else {
            return MouseGestureOutcome::None;
        };

        let _ = self.update_mouse_selection(column, row);
        let dragged = self.mouse.selection_dragged;
        self.mouse.selection_mouse_down = None;
        self.mouse.selection_dragged = false;
    ...
    pub fn render_mouse_selection_overlay(&mut self, frame: &mut Frame, area: Rect) {
        ...
        let dragging =
            self.mouse.selection_mouse_down.is_some() && (self.mouse.selection_dragged || start != end);
        if !dragging {
            return;
        }
    ...
    pub fn handle_mouse_click(&mut self, _column: u16, row: u16) {
        ...
        for (block_id, y_start, y_end) in &self.thinking_regions {
            if absolute_row >= *y_start as usize && absolute_row <= *y_end as usize {
                ...
                if self.selection.collapsed_thinking.contains(&block_id) {
                    self.selection.collapsed_thinking.remove(&block_id);
                } else {
                    self.selection.collapsed_thinking.insert(block_id.clone());
                }
                self.selection.thinking_user_overrides.insert(block_id);
        ...
        for (tool_id, y_start, y_end) in &self.block_tool_regions {
            if absolute_row >= *y_start as usize && absolute_row <= *y_end as usize {
                ...
                if self.selection.collapsed_tools.contains(&tool_id) {
                    self.selection.collapsed_tools.remove(&tool_id);
                } else {
                    self.selection.collapsed_tools.insert(tool_id.clone());
                }
                self.selection.focused_block_tool = Some(tool_id);
                self.invalidate_render_cache();
                break;
            }
        }
