impl ChatView {
    ...
    pub fn toggle_focused_tool_expand(&mut self, chat_state: &ChatState) {
        ...
        if self.selection.focused_block_tool.is_none() {
            self.focus_last_block_tool(chat_state);
        }

        if let Some(ref tool_id) = self.selection.focused_block_tool {
            let tool_id = tool_id.clone();
            if self.selection.collapsed_tools.contains(&tool_id) {
                self.selection.collapsed_tools.remove(&tool_id);
            } else {
                self.selection.collapsed_tools.insert(tool_id);
            }
    ...
    pub fn cycle_block_tool_focus_next(&mut self, chat_state: &ChatState) {
        ...
        if block_tool_ids.is_empty() {
            self.selection.focused_block_tool = None;
            self.invalidate_render_cache();
            return;
        }

        let current_idx = self
            .selection.focused_block_tool
            .as_ref()
        ...
        self.selection.focused_block_tool = Some(block_tool_ids[next_idx].clone());
        self.invalidate_render_cache();
    }
    ...
    pub fn cycle_block_tool_focus_prev(&mut self, chat_state: &ChatState) {
        ...
        self.selection.focused_block_tool = Some(block_tool_ids[prev_idx].clone());
        self.invalidate_render_cache();
    }
    ...
    fn focus_last_block_tool(&mut self, chat_state: &ChatState) {
        let block_tool_ids = self.collect_block_tool_ids(chat_state);
        self.selection.focused_block_tool = block_tool_ids.last().cloned();
    }
