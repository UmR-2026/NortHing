...
fn terminal_supports_truecolor() -> bool {
    ...
    if term_program == "iterm.app" || term_program == "wezterm" || term_program == "vscode" || term_program == "ghostty"
    {
        return true;
    }
...
impl Theme {
    ...
    pub fn apply_opencode_theme_json(&self, json: &OpencodeThemeJson, appearance: Appearance) -> anyhow::Result<Self> {
        ...
        Ok(Theme {
            ...
            input_background: resolved
                .input_background
                .unwrap_or(resolved.background_element.unwrap_or(fallback.input_background)),
    ...
    pub fn style(&self, kind: StyleKind) -> Style {
        match kind {
            ...
            StyleKind::Title => Style::default().fg(self.primary).add_modifier(Modifier::BOLD),
...
fn readable_foreground_for(background: Color) -> Color {
    match background {
        ...
        Color::Black | Color::Red | Color::Green | Color::Blue | Color::Magenta | Color::DarkGray => Color::White,
        Color::Yellow
        | Color::Gray
        | Color::LightRed
        | Color::LightGreen
...
impl Default for OpencodeThemeJson {
    fn default() -> Self {
        Self {
            schema: None,
            defs: None,
...
static BUILTIN_OPENCODE_THEMES: Lazy<HashMap<&'static str, OpencodeThemeJson>> = Lazy::new(|| {
    fn parse(id: &'static str, raw: &'static str) -> (&'static str, OpencodeThemeJson) {
        // Invariant: built-in themes are baked into the binary via `include_str!`
        // at compile time. A parse failure here means the build artifact is corrupt,
        // which `cargo test` will catch at build time. Treat this as an
        // unrecoverable invariant violation rather than a runtime fallback path.
        let json = serde_json::from_str::<OpencodeThemeJson>(raw).unwrap_or_else(|e| {
            tracing::warn!("Failed to parse built-in theme {}: {}. Using default.", id, e);
            OpencodeThemeJson::default()
        });
        (id, json)
    }
    ...
    HashMap::from([
        parse(
            ...
            include_str!(concat!(
                ...
                "/themes/presets/northhing-cyber.json"
            )),
        ),
        parse(
            "northhing-dark",
            include_str!(concat!(
                env!("CARGO_MANIFEST_DIR"),
                "/themes/presets/northhing-dark.json"
            )),
        ),
        parse(
            "northhing-ink-night",
            include_str!(concat!(
                env!("CARGO_MANIFEST_DIR"),
                "/themes/presets/northhing-ink-night.json"
            )),
        ),
        parse(
            "northhing-light",
            include_str!(concat!(
                env!("CARGO_MANIFEST_DIR"),
                "/themes/presets/northhing-light.json"
            )),
        ),
        parse(
            "northhing-midnight",
            include_str!(concat!(
                env!("CARGO_MANIFEST_DIR"),
                "/themes/presets/northhing-midnight.json"
            )),
        ),
        parse(
            "northhing-tokyo-night",
            include_str!(concat!(
                env!("CARGO_MANIFEST_DIR"),
                "/themes/presets/northhing-tokyo-night.json"
            )),
        ),
    ])
});
...
fn resolve_opencode_theme(json: &OpencodeThemeJson, appearance: Appearance) -> anyhow::Result<ResolvedTokens> {
    ...
    tokens.block_border_active = resolve_key(json, &defs, "borderActive", mode)?.or(tokens.primary.clone());
