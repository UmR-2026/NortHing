...
impl SkillSelectorState {
    ...
    pub fn confirm_selection(&self) -> Option<SkillSelectorAction> {
        ...
        match self.screen {
            ...
            SkillSelectorScreen::List => self.items.get(idx).cloned().map(SkillSelectorAction::Execute),
            SkillSelectorScreen::Configure => self.items.get(idx).cloned().map(SkillSelectorAction::Toggle),
        }
    }
    ...
    fn render_skill_line(&self, skill: &SkillItem, theme: &Theme, include_checkbox: bool) -> ListItem<'static> {
