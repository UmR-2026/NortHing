...
// reason: SubagentItem fields are reserved for the upcoming subagent-config UI; today the selector consumes an inline tuple shape
#[derive(Debug, Clone)]
#[allow(dead_code)]
...
impl SubagentSelectorState {
    ...
    pub fn confirm_selection(&self) -> Option<SubagentSelectorAction> {
        ...
        match self.screen {
            ...
            SubagentSelectorScreen::List => self.items.get(idx).cloned().map(SubagentSelectorAction::Launch),
            SubagentSelectorScreen::Configure => self.items.get(idx).cloned().map(SubagentSelectorAction::Toggle),
        }
    }
