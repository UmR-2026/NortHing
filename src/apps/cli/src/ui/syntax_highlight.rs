...
// reason: HighlightTheme enum is kept for theme-aware syntax highlighting; default theme path currently bypasses it
...
// reason: highlight_code() reserved for upcoming code-block rendering in chat replies (today tool output bypasses it)
#[allow(dead_code)]
pub fn highlight_code<'a>(content: &str, file_ext: &str, hl_theme: HighlightTheme) -> Vec<Line<'a>> {
    ...
    for line_str in LinesWithEndings::from(content) {
        match highlighter.highlight_line(line_str, &SYNTAX_SET) {
            ...
            Err(_) => {
                // Fallback: plain text
                lines.push(Line::from(Span::raw(line_str.trim_end_matches('\n').to_string())));
            }
        }
    }
...
// reason: highlight_bash_output() reserved for upcoming bash-output syntax coloring (today shell output is rendered plain)
#[allow(dead_code)]
