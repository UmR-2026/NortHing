...
impl Default for UiConfig {
    fn default() -> Self {
        Self {
            ...
            theme_id: "northhing-dark".to_string(),
            show_tips: true,
            animation: true,
...
impl CliConfig {
    ...
    // reason: reserved for upcoming session migration tooling (currently sessions live under each session's own storage path)
    #[allow(dead_code)]
...
mod tests {
    ...
    #[test]
    fn default_cli_config_has_expected_values() {
        let config = CliConfig::default();
        assert_eq!(config.ui.theme, "dark");
        assert_eq!(config.ui.theme_id, "northhing-dark");
        ...
        assert!(config.workspace.exclude_patterns.contains(&"node_modules".to_string()));
        assert!(config.workspace.exclude_patterns.contains(&".git".to_string()));
    ...
    #[test]
    fn config_toml_round_trip_preserves_values() {
        let mut config = CliConfig::default();
        config.ui.theme = "light".to_string();
        config.behavior.default_agent = "custom".to_string();
        ...
        assert_eq!(parsed.shortcuts.send_message, "Ctrl+Enter");
    }
}
