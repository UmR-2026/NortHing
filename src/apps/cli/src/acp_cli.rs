...
use clap::ValueEnum;
use northhing_acp::client::{AcpClientConfig, AcpClientInfo, AcpClientPermissionMode, AcpClientRequirementProbe};
use northhing_acp::AcpClientService;
...
pub fn print_status(command: &str) -> Result<()> {
    ...
    let config_dir = CliConfig::config_dir().context("Failed to resolve northhing config directory")?;

    println!("northhing ACP");
...
pub async fn print_doctor(command: &str) -> Result<bool> {
    ...
    checks.push(check_result(
        "northhing config directory",
        CliConfig::config_dir()
            .map(|path| path.display().to_string())
            .map_err(|error| error.to_string()),
    ));

    let core_config = northhing_core::service::config::initialize_global_config()
        .await
        .map(|_| "initialized".to_string())
        .map_err(|error| error.to_string());
    checks.push(check_result("Core config service", core_config));

    let ai_check = match northhing_core::service::config::get_global_config_service().await {
        Ok(service) => {
            let ai_config: northhing_core::service::config::types::AIConfig =
                service.config(Some("ai")).await.unwrap_or_default();
            let enabled_models = ai_config.models.iter().filter(|model| model.enabled).count();
            if enabled_models > 0 {
                Check::ok("Enabled AI models", format!("{} enabled model(s)", enabled_models))
            } else {
                Check::warning(
                    "Enabled AI models",
...
─────────────────────────────────\n\
northhing exposes its agent runtime as an ACP server over stdio.\n\
northhing CLI can also launch external ACP agents such as opencode, Claude Code, and Codex.\n\
\n\
...
- Run the command from the project directory you want northhing to operate on.",
        command = command
    )
}
...
pub async fn list_external_clients() -> Result<()> {
    ...
    println!();
    println!("Enable a built-in client with `northhing-cli acp clients enable opencode`.");
    println!("Run a prompt with `northhing-cli acp run opencode \"your task\"`.");
    Ok(())
}
...
pub async fn enable_external_client(client: ExternalAcpClient, permission: CliAcpPermissionMode) -> Result<()> {
    let service = create_client_service().await?;
...
pub async fn run_external_client(
    ...
    if matches!(permission, CliAcpPermissionMode::Ask) {
        bail!("`--permission ask` is not available for non-interactive `acp run`; use allow-once or reject-once.");
    }
    ...
    eprintln!("Starting external ACP agent '{}' in {}", client_id, workspace_path);
    let result = service
        .prompt_agent(
            client_id,
            prompt,
...
async fn create_client_service() -> Result<Arc<AcpClientService>> {
    northhing_core::service::config::initialize_global_config()
        .await
        .map_err(|error| anyhow!(error.to_string()))?;
    let config_service = northhing_core::service::config::get_global_config_service()
        .await
        .map_err(|error| anyhow!(error.to_string()))?;
    let path_manager =
        northhing_core::infrastructure::try_get_path_manager_arc().map_err(|error| anyhow!(error.to_string()))?;
    let service = AcpClientService::new(config_service, path_manager).map_err(|error| anyhow!(error.to_string()))?;
    service
        .initialize_all()
        .await
        .map_err(|error| anyhow!(error.to_string()))?;
...
fn update_client_config_json(
    ...
    if let Some(default_config) = default_config {
        ...
        for (key, value) in default_object {
            entry_object.entry(key.clone()).or_insert_with(|| value.clone());
        }
    }
...
fn ensure_acp_clients_object(value: &mut Value) -> Result<()> {
    ...
    if !value.get("acpClients").map(Value::is_object).unwrap_or(false) {
...
fn print_requirement_item(label: &str, item: &northhing_acp::client::AcpRequirementProbeItem) {
    ...
    if details.is_empty() {
        ...
        println!("  {} {}: {} ({})", label, item.name, installed, details.join(", "));
    }
}
...
mod tests {
    ...
    #[test]
    fn external_acp_client_id_matches_expected_values() {
        assert_eq!(ExternalAcpClient::Opencode.id(), "opencode");
        assert_eq!(ExternalAcpClient::ClaudeCode.id(), "claude-code");
        assert_eq!(ExternalAcpClient::Codex.id(), "codex");
    }

    #[test]
    fn external_acp_client_display_name_matches_expected_values() {
        assert_eq!(ExternalAcpClient::Opencode.display_name(), "opencode");
        assert_eq!(ExternalAcpClient::ClaudeCode.display_name(), "Claude Code");
        assert_eq!(ExternalAcpClient::Codex.display_name(), "Codex");
    }

    #[test]
    fn external_acp_client_config_has_enabled_true() {
        for client in [
            ExternalAcpClient::Opencode,
            ExternalAcpClient::ClaudeCode,
            ExternalAcpClient::Codex,
    ...
    #[test]
    fn cli_acp_permission_mode_converts_correctly() {
        assert!(matches!(
            CliAcpPermissionMode::Ask.to_config_mode(),
            AcpClientPermissionMode::Ask
        ));
        assert!(matches!(
            CliAcpPermissionMode::AllowOnce.to_config_mode(),
            AcpClientPermissionMode::AllowOnce
        ));
        assert!(matches!(
            CliAcpPermissionMode::RejectOnce.to_config_mode(),
            AcpClientPermissionMode::RejectOnce
        ));
    }

    #[test]
    fn acp_help_text_contains_command_placeholder() {
        let text = acp_help_text("northhing-cli");
        assert!(text.contains("northhing-cli acp"));
        assert!(text.contains("northhing-cli acp status"));
        assert!(text.contains("northhing-cli acp doctor"));
    }

    #[test]
    fn shell_command_appends_acp_subcommand() {
        assert_eq!(shell_command("northhing-cli"), "northhing-cli acp");
        assert_eq!(shell_command("my-cli"), "my-cli acp");
    }
    ...
    #[test]
    fn print_zed_config_contains_agent_servers_key() {
        let mut output = Vec::new();
        let _ = print_zed_config_to(&mut output, "northhing-cli");
        let text = String::from_utf8(output).unwrap();
        assert!(text.contains("\"agent_servers\""));
        assert!(text.contains("\"northhing\""));
        assert!(text.contains("\"command\": \"northhing-cli\""));
        assert!(text.contains("\"args\""));
        assert!(text.contains("acp"));
    }

    #[test]
    fn print_generic_config_contains_transport_stdio() {
        let mut output = Vec::new();
        let _ = print_generic_config_to(&mut output, "northhing-cli");
        let text = String::from_utf8(output).unwrap();
...
// Test helpers that capture stdout into a buffer
#[allow(dead_code)]
fn print_zed_config_to(output: &mut Vec<u8>, command: &str) -> Result<()> {
    println!("Zed settings JSON");
    println!();
    let snippet = json!({
        "agent_servers": {
    ...
    writeln!(output, "{}", serde_json::to_string_pretty(&snippet)?)?;
    println!();
    println!(
        "Use an absolute command path if your editor cannot find `{}` on PATH.",
        command
...
#[allow(dead_code)]
fn print_generic_config_to(output: &mut Vec<u8>, command: &str) -> Result<()> {
    println!("Generic ACP stdio configuration");
    println!();
    println!("Command: {}", command);
    println!("Arguments: acp");
