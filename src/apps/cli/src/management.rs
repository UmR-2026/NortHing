...
use northhing_core::agentic::agent_registry;
...
async fn ensure_global_config_service() -> Result<std::sync::Arc<northhing_core::service::config::ConfigService>> {
    initialize_global_config()
        .await
        .context("Failed to initialize global config service")?;
    northhing_core::service::config::get_global_config_service()
        .await
    ...
    if subagents.is_empty() {
        ...
        for agent in subagents {
            println!(
                ...
                agent.id, agent.name, agent.tool_count, agent.effective_enabled, agent.is_readonly, agent.is_review
            );
            if !agent.description.is_empty() {
                println!("  {}", agent.description);
            }
...
pub async fn print_models() -> Result<()> {
    ...
    let global_config: northhing_core::service::config::GlobalConfig = config_service.config(None).await?;
...
pub async fn print_mcp_servers() -> Result<()> {
    let config_service = ensure_global_config_service().await?;
    let mcp_service = northhing_core::service::mcp::MCPService::new(config_service.clone())
    ...
    for config in configs {
        ...
        let endpoint = match config.server_type {
            northhing_core::service::mcp::server::MCPServerType::Local => config
                .command
                .as_ref()
                .map(|cmd| format!("{} {}", cmd, config.args.join(" ")))
                .unwrap_or_else(|| "<missing command>".to_string()),
            northhing_core::service::mcp::server::MCPServerType::Remote => {
...
pub async fn set_default_model(model_id: &str) -> Result<()> {
    ...
    config_service.set_config("ai.default_models.primary", model_id).await?;
    for mode in modes {
...
pub async fn print_usage_report(session_id: Option<&str>) -> Result<()> {
    ...
    let persistence_manager = PersistenceManager::new(path_manager).map_err(|error| anyhow!(error.to_string()))?;
