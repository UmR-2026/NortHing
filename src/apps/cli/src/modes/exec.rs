...
pub struct ExecMode {
    // reason: config held for upcoming --config flag / env override integration in single-shot exec mode
    #[allow(dead_code)]
    config: CliConfig,
    message: String,
    agent_type: String,
...
impl ExecMode {
    ...
    fn get_git_diff(&self) -> Option<String> {
        ...
        let output = northhing_core::util::process_manager::create_command("git")
            .args(["diff", "--no-color"])
    ...
    pub async fn run(&mut self) -> Result<()> {
        ...
        loop {
            ...
            for envelope in events {
                ...
                {
                    subagent_parent_sessions.insert(subagent_session_id.clone(), parent_session_id.clone());
                    continue;
                }
                ...
                if event.session_id() != Some(&session_id) {
                    ...
                    if let AgenticEvent::ToolEvent { tool_event, .. } = event {
                        let parent_session_id = event
                            .session_id()
                            .and_then(|event_session_id| subagent_parent_sessions.get(event_session_id));
                        if parent_session_id.map(String::as_str) == Some(session_id.as_str()) {
                            ...
                            match tool_event {
                                ...
                                ToolEventData::Completed {
                                    ...
                                    let summary = result_for_assistant.clone().unwrap_or_else(|| result.to_string());
                                    self.emit(json!({
                                    ...
                                    self.print_text(|| println!("   [subagent] {} completed: {}", tool_name, summary));
                                }
                    ...
                    AgenticEvent::ToolEvent { tool_event, .. } => {
                        ...
                        match tool_event {
                            ...
                            ToolEventData::Completed {
                                ...
                                self.print_text(|| println!("   [+] {} ({}ms): {}", tool_name, duration_ms, summary));
                            }
                            ToolEventData::Failed {
                                tool_name,
                                tool_id,
    ...
    async fn prepare_session(&self) -> Result<String> {
        ...
        if self.session_options.fork_session {
            let source_session_id = resolved_resume
                ...
                .ok_or_else(|| anyhow::anyhow!("--fork-session requires --continue, --resume, or --session"))?;
    ...
    fn output_patch_if_needed(&self) {
        if let Some(ref output_target) = self.output_patch {
            if let Some(patch) = self.get_git_diff() {
                let status = if patch.trim().is_empty() { "empty" } else { "generated" };
