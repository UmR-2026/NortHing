...
mod acp_cli;
mod agent;
// reason: chat_state module is reserved for upcoming stateful-refactor of the legacy single-loop chat driver; not wired yet
#[allow(dead_code)]
mod chat_state;
mod commands;
mod config;
...
static MCP_SERVICE: OnceLock<std::sync::Arc<northhing_core::service::mcp::MCPService>> = OnceLock::new();
...
pub fn get_mcp_service() -> Option<&'static std::sync::Arc<northhing_core::service::mcp::MCPService>> {
    MCP_SERVICE.get()
}

#[derive(Parser)]
#[command(name = "northhing")]
#[command(about = "northhing CLI - AI agent-driven command-line programming assistant", long_about = None)]
#[command(version)]
struct Cli {
    #[command(subcommand)]
...
enum AcpAction {
    ...
    Status {
        ...
        #[arg(long, default_value = "northhing-cli")]
        command: String,
    },
    ...
    /// Manage external ACP agents that northhing can launch
    Clients {
        #[command(subcommand)]
...
fn terminal_scripts_dir() -> std::path::PathBuf {
    CliConfig::config_dir()
        .ok()
        .unwrap_or_else(|| std::env::temp_dir().join("northhing-cli"))
        .join("temp")
        .join("scripts")
}

async fn initialize_terminal_service() -> Result<()> {
    use northhing_core::service::runtime::RuntimeManager;
    use northhing_core::service::terminal::{TerminalApi, TerminalConfig};
    ...
    if let Ok(runtime_manager) = RuntimeManager::new() {
        ...
        if let Some(merged_path) = runtime_manager.merged_path_env(current_path.as_deref()) {
            terminal_config.env.insert("PATH".to_string(), merged_path.clone());
            #[cfg(windows)]
            {
    ...
    let _terminal_api = TerminalApi::new(terminal_config)
        .await
        .context("Failed to initialize TerminalApi (SessionManager singleton)")?;
...
async fn initialize_core_services(
    ...
    if let Some(ref cfg_svc) = config_service {
        match northhing_core::service::mcp::MCPService::new(cfg_svc.clone()) {
            Ok(mcp_service) => {
...
async fn run_interactive(_config: CliConfig, default_agent: String, _workspace_str: String) -> Result<()> {
    ...
    let mut startup_page = StartupPage::new(agentic_system.coordinator.clone(), default_agent, workspace.clone());
...
async fn run_cli() -> Result<()> {
    ...
    if is_tui_mode {
        ...
        let log_file = log_dir.join("northhing-cli.log");

        if let Ok(file) = OpenOptions::new().create(true).append(true).open(log_file) {
            // reason: file.try_clone() can fail under file descriptor
            // exhaustion. If it does, fall back to the in-memory log buffer
            // (no-op Writer) so tracing::init() does not panic at startup.
            // The first OpenOptions::open() above already succeeded, so this
            // branch only fires under exotic resource exhaustion.
            struct NullWriter;
            ...
            tracing_subscriber::fmt()
                ...
                .with_writer(move || match file.try_clone() {
                    ...
                    Err(e) => {
                        eprintln!("northhing-cli: log file clone failed, falling back to null writer: {e}");
                        Box::new(NullWriter) as Box<dyn std::io::Write>
                    }
                })
                .with_ansi(false)
                .with_target(false)
...
async fn run_interactive_with_session(config: CliConfig, session_id: String) -> Result<()> {
    ...
    let mut chat_mode =
        ChatMode::new(config, session.agent_type, workspace, &agentic_system).with_restore_session(session_id);
...
fn main() -> Result<()> {
    let worker = std::thread::Builder::new()
        ...
        .context("failed to spawn northhing-cli worker thread")?;
    ...
    match worker.join() {
        ...
        Err(_) => {
            eprintln!("Error: northhing-cli worker thread panicked");
            std::process::exit(1)
        }
    }
}
