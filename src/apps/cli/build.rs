...
fn read_prompt_files(
    ...
    for entry in fs::read_dir(current_dir)? {
        ...
        if path.is_dir() {
            ...
            let relative = path.strip_prefix(base_dir)?.to_string_lossy().replace('\\', "/");
            // Key: filename without extension, e.g. "init" from "init.md"
            let key = relative.trim_end_matches(".md").trim_end_matches(".txt").to_string();
...
fn generate_prompts_code(prompts: &HashMap<String, String>) -> Result<(), Box<dyn std::error::Error>> {
