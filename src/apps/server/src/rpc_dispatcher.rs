...
use northhing_core::agentic::agents::SubAgentSource;
use northhing_core::agentic::coordination::{DialogSubmissionPolicy, DialogTriggerSource};
use northhing_core::agentic::core::SessionConfig;
use northhing_core::agentic::deep_review_policy::{apply_deep_review_queue_control, DeepReviewQueueControlAction};
use northhing_core::service::i18n::{sync_global_i18n_service_locale, LocaleId, LocaleMetadata};
...
pub async fn dispatch(
    ...
    match method {
        ...
        "open_workspace" => {
            ...
            let path: String =
                serde_json::from_value(request.get("path").cloned().ok_or_else(|| anyhow!("Missing path"))?)?;
            let info = state
                .workspace_service
        ...
        "get_current_workspace" => {
            let ws = state.workspace_service.current_workspace().await;
            Ok(serde_json::to_value(&ws).unwrap_or(serde_json::Value::Null))
        }
        "get_recent_workspaces" => {
            let list = state.workspace_service.recent_workspaces().await;
        ...
        "get_config" => {
            ...
            let config: serde_json::Value = state.config_service.config(key).await.map_err(|e| anyhow!("{}", e))?;
            Ok(config)
        }
        "set_config" => {
            let request = extract_request(&params)?;
        ...
        "list_subagents" => {
            ...
            let source = request
                ...
                .transpose()?;
            let workspace = workspace_root_from_request(request.get("workspacePath").and_then(|v| v.as_str()));
            let list = state.agent_registry.get_subagents_info(workspace.as_deref()).await;
        ...
        "update_subagent_config" => {
                ...
                .is_some()
            {
                if let Some(enabled) = enabled {
                    let parent_agent_type = parent_agent_type
                        .as_deref()
                        .ok_or_else(|| anyhow!("parentAgentType is required when updating subagent availability"))?;
                    state
                        .agent_registry
                        .update_subagent_override(parent_agent_type, &subagent_id, enabled, workspace.as_deref())
                        .await
                ...
                if model.is_some() {
                    state
                        .agent_registry
                        .update_and_save_custom_subagent_config(&subagent_id, model, workspace.as_deref())
                        .map_err(|e| anyhow!("Failed to update configuration: {}", e))?;
                }
                Ok(serde_json::Value::Null)
            } else {
                if state.agent_registry.has_project_custom_subagent(&subagent_id) {
                    if let Some(workspace) = workspace.as_deref() {
                        return Err(anyhow!(
                ...
                if let Err(e) = northhing_core::service::config::reload_global_config().await {
                    tracing::warn!(
        ...
        "create_session" => {
            ...
            let session_id = request.get("sessionId").and_then(|v| v.as_str()).map(|s| s.to_string());
        ...
        "start_dialog_turn" => {
            ...
            let workspace_path = request
                ...
                .and_then(|v| v.as_str())
                .map(|s| s.to_string());
            let turn_id = request.get("turnId").and_then(|v| v.as_str()).map(|s| s.to_string());
        ...
        "control_deep_review_queue" => {
            ...
            let action = match action_raw.as_str() {
                ...
                other => {
                    return Err(anyhow!("Invalid DeepReview queue control action: {}", other));
                }
            };
        ...
        "i18n_get_current_language" => {
            let lang: String = state
                .config_service
                .config(Some("app.language"))
                .await
                .unwrap_or_else(|_| "zh-CN".to_string());
            let lang = LocaleId::from_str(&lang).unwrap_or_default().as_str().to_string();
        ...
        "i18n_get_config" => {
            let current_language = match state.config_service.config::<String>(Some("app.language")).await {
                Ok(language) => LocaleId::from_str(&language).unwrap_or_default().as_str().to_string(),
        ...
        // ── Fallback ─────────────────────────────────────────
        _ => {
            tracing::warn!("Unknown RPC method: {}", method);
...
fn workspace_root_from_request(workspace_path: Option<&str>) -> Option<PathBuf> {
    workspace_path.filter(|path| !path.is_empty()).map(PathBuf::from)
}
