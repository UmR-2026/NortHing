...
WebSocket relay server for northhing Remote Connect. It bridges desktop (WebSocket) and mobile (HTTP) clients while forwarding end-to-end encrypted payloads.
...
### What URL should I fill in northhing Desktop?

In **Remote Connect â?Self-Hosted â?Server URL**, use one of:
...
```bash
# From project root
cargo build --release -p northhing-relay-server

# Run
RELAY_PORT=9700 ./target/release/northhing-relay-server
```
...
4. Fill the same URL into northhing Desktop custom server settings
...
| `RELAY_ROOM_WEB_DIR` | `/tmp/northhing-room-web` | Directory for per-room uploaded mobile-web files |
...
### Room Operations (Mobile HTTP â?Desktop WS bridge)
...
```
Mobile Phone ââHTTP POSTâââ?Relay Server âââWebSocketââ Desktop Client
                               â?
                          E2E Encrypted
                          (server cannot
                           read messages)
...
âââ static/                 # Mobile-web static files
âââ Cargo.toml              # Crate manifest
âââ Dockerfile              # Docker build
âââ docker-compose.yml      # Docker Compose config
âââ Caddyfile               # Optional reverse proxy config
âââ deploy.sh               # Deploy on the target server itself
âââ start.sh                # Start service if not already running
âââ stop.sh                 # Stop running service
âââ restart.sh              # Restart service, or start if stopped
âââ README.md
```
