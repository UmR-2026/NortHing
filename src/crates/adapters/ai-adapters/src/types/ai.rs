...
pub use northhing_core_types::{ConnectionTestMessageCode, ConnectionTestResult, RemoteModelInfo};
...
impl From<northhing_agent_stream::UnifiedTokenUsage> for GeminiUsage {
    fn from(usage: northhing_agent_stream::UnifiedTokenUsage) -> Self {
        Self {
...
impl From<GeminiUsage> for northhing_agent_stream::UnifiedTokenUsage {
...
mod tests {
    ...
    fn gemini_usage_converts_to_and_from_unified_token_usage() {
        ...
        let unified: northhing_agent_stream::UnifiedTokenUsage = usage.clone().into();
        ...
        assert_eq!(unified.cached_content_token_count, usage.cached_content_token_count);
        assert_eq!(unified.cache_creation_token_count, usage.cache_creation_token_count);

        let roundtrip: GeminiUsage = unified.into();
        assert_eq!(roundtrip.prompt_token_count, usage.prompt_token_count);
        assert_eq!(roundtrip.candidates_token_count, usage.candidates_token_count);
        ...
        assert_eq!(roundtrip.cached_content_token_count, usage.cached_content_token_count);
        assert_eq!(roundtrip.cache_creation_token_count, usage.cache_creation_token_count);
    }
}
