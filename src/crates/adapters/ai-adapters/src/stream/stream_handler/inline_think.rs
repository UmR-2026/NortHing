...
impl InlineThinkParser {
    ...
    pub(crate) fn normalize_response(&mut self, mut response: UnifiedResponse) -> Vec<UnifiedResponse> {
        if !self.enabled {
            return vec![response];
        }
        ...
        if response.reasoning_content.is_some() || response.tool_call.is_some() || response.thinking_signature.is_some()
        {
...
#[cfg(test)]
mod tests {
    use super::{longest_suffix_prefix_len, InlineThinkActivation, InlineThinkMode, InlineThinkParser};
    ...
    fn inline_think_parser_respects_native_reasoning_chunks() {
        ...
        assert_eq!(responses[0].reasoning_content.as_deref(), Some("native reasoning"));
    }
}
