...
fn handle_function_call_arguments_delta(
    ...
    let Some(output_index) = output_index else {
        return Err(anyhow!("Responses function_call_arguments.delta missing output_index"));
    };
...
fn handle_function_call_output_item_done(
    ...
    let full_args = item_value.get("arguments").and_then(Value::as_str).unwrap_or_default();
    ...
    cleanup_tool_call_tracking(output_index, tool_calls_by_output_index, tool_call_index_by_id);
}
...
pub async fn handle_responses_stream(
    ...
    loop {
        ...
        if let Some(ref tx) = tx_raw_sse {
            if let Err(e) = tx.send(raw.clone()) {
        ...
        match event.kind.as_str() {
                ...
                if let Some(item) = event.item.as_ref() {
                    if let Some(tc) = InProgressToolCall::from_item_value(item) {
                        let Some(output_index) = event.output_index else {
                            let error_msg = "Responses function_call output_item.added missing output_index";
            ...
            "response.output_item.done" => {
                ...
                if let Some(mut unified_response) = parse_responses_output_item(item_value, event.output_index) {
                ...
                if let Some(response_val) = event.response.as_ref() {
                    if let Some(output) = response_val.get("output").and_then(Value::as_array) {
                        for (idx, item) in output.iter().enumerate() {
                            ...
                            let full_args = item.get("arguments").and_then(Value::as_str).unwrap_or_default();
                            if tc.args_so_far.len() < full_args.len() && full_args.starts_with(&tc.args_so_far) {
...
mod tests {
    use super::{
        super::stream_stats::StreamStats, extract_api_error_message, handle_function_call_arguments_delta,
    ...
    fn extracts_api_error_message_from_response_error() {
        ...
        assert_eq!(extract_api_error_message(&event).as_deref(), Some("provider error"));
    }
