...
use crate::stream::types::anthropic::{
    AnthropicSSEError, ContentBlock, ContentBlockDelta, ContentBlockStart, MessageDelta, MessageStart, Usage,
};
    ...
    loop {
        let sse = match next_stream_item(&mut stream, idle_timeout).await {
            ...
            TimedStreamItem::End => {
                if received_finish_reason {
                    for unified_response in inline_think_parser.flush() {
                        ...
                        if let Err(e) = tx_event.send(Ok(unified_response)) {
                            warn!("Failed to emit unified response: {e}");
                        }
                    }
                ...
                error!("{}", error_msg);
                if let Err(e) = tx_event.send(Err(anyhow!(error_msg))) {
                    warn!("Failed to emit stream error to consumer: {e}");
                }
                return;
            }
            ...
            TimedStreamItem::TimedOut => {
                ...
                let error_msg = format!("SSE Timeout: idle timeout waiting for SSE after {}s", timeout_secs);
        ...
        if let Some(ref tx) = tx_raw_sse {
            if let Err(e) = tx.send(format!("[{}] {}", event_type, data)) {
                warn!("Failed to send raw SSE to consumer: {e}");
            }
        }
        ...
        match event_type.as_str() {
            ...
            "content_block_start" => {
                let content_block_start: ContentBlockStart = match serde_json::from_str(&data) {
                    ...
                    Err(e) => {
                        ...
                        error!("{}", err_str);
                        if let Err(e) = tx_event.send(Err(anyhow!(err_str))) {
                            warn!("Failed to emit stream error to consumer: {e}");
                        }
                        return;
                    }
            ...
            "content_block_delta" => {
                ...
                match UnifiedResponse::try_from(content_block_delta) {
                    Ok(unified_response) => {
                        emit_normalized_response(&mut inline_think_parser, &tx_event, &mut stats, unified_response)
                    }
                    Err(e) => {
            ...
            "message_delta" => {
                ...
                message_delta.usage = if usage.is_empty() { None } else { Some(usage.clone()) };
            ...
            "error" => {
                ...
                if let Err(e) = tx_event.send(Err(anyhow!(String::from(sse_error.error)))) {
                    warn!("Failed to emit stream error to consumer: {e}");
                }
                return;
            }
...
fn trace_unified_response_if_useful(response: &UnifiedResponse) {
    ...
    let tool_call_summary = response.tool_call.as_ref().map(|tool_call| {
        format!(
            ...
            tool_call.arguments.as_ref().map(|value| value.len()).unwrap_or(0),
...
fn emit_normalized_response(
    ...
    for normalized_response in inline_think_parser.normalize_response(unified_response) {
        ...
        if let Err(e) = tx_event.send(Ok(normalized_response)) {
            warn!("Failed to emit unified response: {e}");
        }
    }
}
...
mod tests {
    use super::{
        format_provider_error_from_sse_message, should_log_full_stream_events, should_trace_anthropic_sse_event,
        should_trace_unified_response,
    };
    ...
    fn extracts_glm_business_error_from_message_event() {
        let raw =
            r#"{"error":{"code":"1113","message":"余额不足或无可用资源包,请充值。"},"request_id":"20260425142416"}"#;
