...
pub struct ResponsesStreamEvent {
    ...
    // reason: content_index is deserialized for forward-compat with multi-part content events; today's streaming pipeline only consumes single-part events
    #[allow(dead_code)]
    #[serde(default)]
...
pub struct ResponsesCompleted {
    // reason: id is deserialized for OpenAI Responses API compat; the unified agent-event stream does not surface response ids
    #[allow(dead_code)]
    pub id: String,
    #[serde(default)]
...
pub struct ResponsesDone {
    // reason: ResponsesDone.id is deserialized for OpenAI Responses API compat; the unified pipeline reads `usage` only
    #[serde(default)]
    #[allow(dead_code)]
...
impl From<ResponsesUsage> for UnifiedTokenUsage {
    fn from(usage: ResponsesUsage) -> Self {
        Self {
            ...
            cached_content_token_count: usage.input_tokens_details.map(|details| details.cached_tokens),
            cache_creation_token_count: None,
        }
    }
}

pub fn parse_responses_output_item(item_value: Value, tool_call_index: Option<usize>) -> Option<UnifiedResponse> {
