...
pub(crate) async fn aggregate_stream_response(stream_response: StreamResponse) -> Result<GeminiResponse> {
            ...
            Ok(chunk) => {
                    ...
                    if let Some(finalized) = outcome.finalized_previous {
                        if finalized.is_error {
                            ...
                            tool_calls.push(ToolCall {
                                ...
                                raw_arguments: (!finalized.raw_arguments.is_empty()).then_some(finalized.raw_arguments),
                            });
                        }
                    }

                    if let Some(early_detected) = outcome.early_detected {
                        debug!("[send_message] Detected tool call: {}", early_detected.tool_name);
                    }
                }
                ...
                if let Some(chunk_provider_metadata) = chunk_provider_metadata {
                    match provider_metadata.as_mut() {
                        Some(existing) => {
                            crate::client::utils::merge_json_value(existing, chunk_provider_metadata);
                        }
...
pub(crate) fn unified_usage_to_gemini_usage(usage: crate::stream::UnifiedTokenUsage) -> GeminiUsage {
    GeminiUsage {
