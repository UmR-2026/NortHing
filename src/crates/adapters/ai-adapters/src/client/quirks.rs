...
pub(crate) fn parse_glm_major_minor(model_name: &str) -> Option<(u32, u32)> {
    ...
    let minor = version_parts.next().and_then(|value| value.parse().ok()).unwrap_or(0);
...
pub(crate) fn should_append_tool_stream(url: &str, model_name: &str) -> bool {
    ...
    parse_glm_major_minor(model_name).is_some_and(|(major, minor)| major > 4 || (major == 4 && minor >= 5))
}
...
pub(crate) fn apply_openai_compatible_reasoning_fields(
    ...
    if is_dashscope_url(url) || is_siliconflow_url(url) {
        if normalized_mode != ReasoningMode::Default {
            request_body["enable_thinking"] = serde_json::json!(normalized_mode == ReasoningMode::Enabled);
        }
        return;
    }
    ...
    match normalized_mode {
        ...
        ReasoningMode::Adaptive => {
            warn!(
                "adaptive mode reached apply_openai_compatible_reasoning_fields after normalization; \
                 this indicates a code path change that should be reviewed"
            );
        }
    }
