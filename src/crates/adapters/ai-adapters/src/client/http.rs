...
pub(crate) fn create_http_client(proxy_config: Option<ProxyConfig>, skip_ssl_verify: bool) -> Client {
    let mut builder = Client::builder()
        .use_rustls_tls()
        .connect_timeout(std::time::Duration::from_secs(AIClient::STREAM_CONNECT_TIMEOUT_SECS))
        .user_agent("northhing/1.0")
        .pool_idle_timeout(std::time::Duration::from_secs(AIClient::HTTP_POOL_IDLE_TIMEOUT_SECS))
        .pool_max_idle_per_host(4)
        .tcp_keepalive(Some(std::time::Duration::from_secs(AIClient::HTTP_TCP_KEEPALIVE_SECS)))
