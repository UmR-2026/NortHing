...
{
    match ttft_timeout {
        Some(timeout) => match tokio::time::timeout(timeout, build_request().json(request_body).send()).await {
...
fn retry_after_delay_ms(headers: &HeaderMap) -> Option<u64> {
    ...
    Some(retry_at.signed_duration_since(now).num_milliseconds().max(0) as u64)
...
where
    BuildRequest: Fn() -> reqwest::RequestBuilder,
    SpawnHandler:
        Fn(reqwest::Response, mpsc::UnboundedSender<Result<UnifiedResponse>>, Option<mpsc::UnboundedSender<String>>),
{
        ...
        let response = match send_outcome {
            ...
            StreamSendOutcome::Transport(e) => {
                ...
                if attempt < max_tries - 1 {
                    ...
                    debug!("Retrying {} after {}ms (attempt {})", label, delay_ms, attempt + 2);
                    tokio::time::sleep(std::time::Duration::from_millis(delay_ms)).await;
                }
                continue;
            }
...
mod tests {
    ...
    fn format_ttft_timeout_error_includes_timeout_seconds() {
        let message =
            format_ttft_timeout_error("Codex ChatGPT Responses API", Some(std::time::Duration::from_secs(30)));
    ...
    fn retry_after_seconds_is_capped() {
        ...
        assert_eq!(retry_after_delay_ms(&headers), Some(MAX_RETRY_AFTER_DELAY_MS));
    }
