...
pub(crate) fn apply_headers(client: &AIClient, builder: RequestBuilder) -> RequestBuilder {
    shared::apply_header_policy(client, builder, |mut builder| {
        ...
        if client.config.base_url.contains("openagentapp.com") {
...
pub(crate) fn resolve_request_url(base_url: &str, model_name: &str) -> String {
    ...
    format!("{}/v1beta/models/{}:streamGenerateContent?alt=sse", base, encoded_model)
}
...
fn ensure_generation_config(request_body: &mut serde_json::Value) -> &mut serde_json::Map<String, serde_json::Value> {
    if !request_body
...
fn insert_generation_field(request_body: &mut serde_json::Value, key: &str, value: serde_json::Value) {
...
fn normalize_stop_sequences(value: &serde_json::Value) -> Option<serde_json::Value> {
    match value {
        serde_json::Value::String(sequence) => Some(serde_json::Value::Array(vec![serde_json::Value::String(
            sequence.clone(),
        )])),
...
pub(crate) fn build_request_body(
    ...
    if let Some(max_tokens) = client.config.max_tokens {
        insert_generation_field(&mut request_body, "maxOutputTokens", serde_json::json!(max_tokens));
    }

    if let Some(temperature) = client.config.temperature {
        insert_generation_field(&mut request_body, "temperature", serde_json::json!(temperature));
    }
    ...
    if let Some(tools) = gemini_tools {
        ...
        if !tools.is_empty() {
            ...
            let has_function_declarations = request_body["tools"]
                .as_array()
                .map(|tools| tools.iter().any(|tool| tool.get("functionDeclarations").is_some()))
                .unwrap_or(false);
...
pub(crate) async fn send_stream(
    ...
    let (system_instruction, contents) = GeminiMessageConverter::convert_messages(messages, &client.config.model);
    let gemini_tools = GeminiMessageConverter::convert_tools(tools);
    let request_body = build_request_body(client, system_instruction, contents, gemini_tools, extra_body);
