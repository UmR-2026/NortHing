...
pub(crate) fn apply_headers(client: &AIClient, builder: RequestBuilder) -> RequestBuilder {
    shared::apply_header_policy(client, builder, |builder| {
        builder
            ...
            .header("User-Agent", "northhing-CodeAssist/1.0")
    })
}
...
pub(crate) async fn send_stream(
    ...
    let inner = gemini_request::build_request_body(client, system_instruction, contents, gemini_tools, extra_body);
...
pub(crate) async fn list_models(_client: &AIClient) -> Result<Vec<RemoteModelInfo>> {
    ...
    if let Some(gemini_home) = gemini_home_dir() {
        if let Some(model) = read_gemini_settings_model(&gemini_home).or_else(|| read_gemini_env_model(&gemini_home)) {
