...
use crate::client::quirks::{
    is_deepseek_reasoning_effort_model, is_deepseek_url, normalize_deepseek_reasoning_effort, should_append_tool_stream,
};
...
pub(crate) fn apply_headers(client: &AIClient, builder: RequestBuilder, url: &str) -> RequestBuilder {
    shared::apply_header_policy(client, builder, |mut builder| {
        ...
        if url.contains("openagentapp.com") {
            builder = builder.header("X-Verification-Code", "from_northhing");
        }
...
fn apply_anthropic_adaptive_reasoning(request_body: &mut serde_json::Value, reasoning_effort: Option<&str>) {
...
fn apply_deepseek_anthropic_reasoning(
    ...
    match mode {
        ...
        ReasoningMode::Adaptive => {
            ...
            apply_deepseek_anthropic_reasoning(request_body, ReasoningMode::Enabled, model_name, reasoning_effort);
        }
    }
}
...
fn apply_reasoning_fields(
    ...
    let is_deepseek_reasoning_target = is_deepseek_url(url) || is_deepseek_reasoning_effort_model(model_name);
    ...
    match mode {
        ...
        ReasoningMode::Enabled => {
            ...
            if let Some(budget_tokens) = thinking_budget_tokens.or_else(|| default_anthropic_budget_tokens(max_tokens))
            {
...
pub(crate) fn build_request_body(
    ...
    let protected_body = shared::protect_request_body(
        client,
        &mut request_body,
        &["model", "messages", "max_tokens", "stream", "system", "tool_stream"],
        &[],
    );
    ...
    if let Some(tools) = anthropic_tools {
        let tool_names = tools
            .iter()
            .map(|tool| shared::extract_top_level_string_field(tool, "name").unwrap_or_else(|| "unknown".to_string()))
            .collect::<Vec<_>>();
...
pub(crate) async fn send_stream(
    ...
    let (system_message, anthropic_messages) = AnthropicMessageConverter::convert_messages(messages);
