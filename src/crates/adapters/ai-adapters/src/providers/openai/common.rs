...
pub(crate) fn apply_reasoning_fields(request_body: &mut serde_json::Value, client: &AIClient, url: &str) {
...
fn codex_model_infos(model_ids: Vec<String>) -> Vec<RemoteModelInfo> {
    dedupe_remote_models(
        model_ids
            .into_iter()
            .map(|id| RemoteModelInfo { id, display_name: None })
            .collect(),
    )
}
...
async fn list_codex_chatgpt_models(client: &AIClient, base_models_url: &str) -> Result<Vec<RemoteModelInfo>> {
    ...
    let mut model_ids = match live_models {
        ...
        Ok(_) => {
            tracing::warn!("Codex backend model discovery returned no models; using local fallback catalog");
...
pub(crate) fn attach_tools(request_body: &mut serde_json::Value, tools: Option<Vec<serde_json::Value>>, target: &str) {
    match tools {
        Some(tools) if !tools.is_empty() => {
            ...
            let has_tool_choice = request_body.get("tool_choice").is_some_and(|value| !value.is_null());
            if !has_tool_choice {
        ...
        _ => {
            if request_body
                ...
                .is_some()
            {
                tracing::debug!("Removed tool_choice from OpenAI request because no tools are attached");
            }
        }
    }
}

pub(crate) fn convert_tools_flat(tools: Option<Vec<ToolDefinition>>) -> Option<Vec<serde_json::Value>> {
    tools.map(|defs| {
        defs.into_iter()
            .map(|tool| {
                serde_json::json!({
...
mod tests {
    ...
    fn attach_tools_preserves_explicit_tool_choice_with_tools() {
        ...
        assert_eq!(request_body["tools"][0]["function"]["name"], json!("example"));
    }
}
