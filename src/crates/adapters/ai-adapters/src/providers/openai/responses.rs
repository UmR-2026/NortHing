...
pub(crate) fn build_request_body(
    ...
    common::attach_tools(&mut request_body, openai_tools, "ai::responses_stream_request");
...
pub(crate) async fn send_stream(
    ...
    if super::codex_chatgpt::is_codex_chatgpt_endpoint(&client.config.request_url) {
        return super::codex_chatgpt::send_stream(client, messages, tools, extra_body, max_tries, trace).await;
    }
    ...
    let (instructions, response_input) = OpenAIMessageConverter::convert_messages_to_responses_input(messages);
    let openai_tools = common::convert_tools_flat(tools);
    let request_body = build_request_body(client, instructions, response_input, openai_tools, extra_body);
