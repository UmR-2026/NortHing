...
use northhing_agent_stream::StreamResult;
use northhing_ai_adapters::providers::{openai::OpenAIMessageConverter, AnthropicMessageConverter};
use northhing_ai_adapters::{Message as AIMessage, ToolCall as AIToolCall};
...
async fn replays_structurally_empty_anthropic_thinking_with_signature_and_tool_use() {
    ...
    assert_eq!(result.usage.as_ref().map(|usage| usage.total_token_count), Some(25));
    ...
    let content = anthropic_messages[0]["content"].as_array().expect("assistant content");
