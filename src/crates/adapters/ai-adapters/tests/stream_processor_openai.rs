...
use common::stream_test_harness::{
    run_stream_fixture, run_stream_fixture_with_options, StreamFixtureProvider, StreamFixtureRunOptions,
};
use northhing_events::{AgenticEvent, ToolEventData};
use serde_json::json;
...
async fn openai_fixture_keeps_malformed_tool_arguments_invalid() {
    ...
    assert_eq!(result.usage.as_ref().map(|usage| usage.total_token_count), Some(12));
...
async fn openai_fixture_parses_inline_think_tags_into_reasoning_content() {
    ...
    assert_eq!(result.usage.as_ref().map(|usage| usage.total_token_count), Some(10));

    let thinking_chunks: Vec<(&str, bool)> = output
        .events
        .iter()
        .filter_map(|event| match event {
            AgenticEvent::ThinkingChunk { content, is_end, .. } => Some((content.as_str(), *is_end)),
            _ => None,
        })
        .collect();
    assert_eq!(
...
async fn openai_fixture_reattaches_id_only_prelude_to_following_payload_chunk() {
    ...
    assert!(!result.tool_calls[0].is_error);
    assert_eq!(result.usage.as_ref().map(|usage| usage.total_token_count), Some(9));
...
async fn openai_fixture_filters_unseen_id_only_orphan_tool_chunk() {
    ...
    let tool_events = output.events.iter().any(|event| {
        matches!(
            event,
            AgenticEvent::ToolEvent {
                tool_event: ToolEventData::EarlyDetected { .. } | ToolEventData::ParamsPartial { .. },
                ..
            }
        )
    });
...
async fn openai_fixture_filters_orphan_id_only_block_when_it_shares_chunk_with_first_tool_tail() {
    ...
    assert_eq!(result.usage.as_ref().map(|usage| usage.total_token_count), Some(11));
    ...
    let partial_params: Vec<(&str, &str)> = output
        ...
        .filter_map(|event| match event {
            AgenticEvent::ToolEvent {
                round_id,
                tool_event: ToolEventData::ParamsPartial { tool_id, params, .. },
                ..
            } => {
                assert_eq!(round_id, "round_fixture");
    ...
    assert_eq!(
        partial_params,
        vec![("call_1", "{\"x\":"), ("call_1", "1}"), ("call_2", "{\"y\":2}"),]
    );
}
...
async fn openai_fixture_routes_interleaved_tool_args_by_index() {
    ...
    let partial_params: Vec<(&str, &str)> = output
        ...
        .collect();
    assert_eq!(partial_params, vec![("call_1", "{\"x\":1}"), ("call_2", "{\"y\":2}")]);
}
...
async fn openai_fixture_accepts_tool_call_without_type_field() {
    ...
    assert!(!result.tool_calls[0].is_error);
    assert_eq!(result.usage.as_ref().map(|usage| usage.total_token_count), Some(15));
...
async fn openai_fixture_ignores_trailing_empty_tool_args_finish_chunk() {
    ...
    assert_eq!(result.tool_calls[0].arguments, json!({ "query": "latest news on ai" }));
    assert!(!result.tool_calls[0].is_error);
    assert_eq!(result.usage.as_ref().map(|usage| usage.total_token_count), Some(246));
    ...
    let partial_params: Vec<&str> = output
        ...
        .collect();
    assert_eq!(partial_params, vec!["{\"query\":\"latest", " news", " on ai\"}"]);
}
