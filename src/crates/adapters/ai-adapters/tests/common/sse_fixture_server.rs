...
impl FixtureSseServer {
    pub async fn spawn(payload: Vec<u8>, options: FixtureSseServerOptions) -> Self {
        ...
        let listener = TcpListener::bind("127.0.0.1:0").await.expect("bind fixture SSE server");
        let addr = listener.local_addr().expect("fixture SSE server addr");
        let server_task = tokio::spawn(async move {
            axum::serve(listener, app).await.expect("fixture SSE server should run");
        });
...
async fn stream_fixture_handler(State(state): State<FixtureSseState>) -> impl IntoResponse {
    ...
    let mut response = Response::new(Body::from_stream(ReceiverStream::new(rx).map(Ok::<Bytes, Infallible>)));
