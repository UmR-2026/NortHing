...
use northhing_agent_stream::{StreamEventSink, StreamProcessError, StreamProcessor, StreamResult};
use northhing_ai_adapters::stream::{
...
// reason: StreamFixtureProvider enum is shared by parameterized SSE fixture tests; not all test crates consume every variant
#[allow(dead_code)]
...
// reason: StreamFixtureRunOutput struct is consumed by future fixture-driven integration tests; today's tests call run_stream_fixture_with_options directly
#[allow(dead_code)]
#[derive(Debug)]
...
// reason: run_stream_fixture() is a convenience wrapper around run_stream_fixture_with_options() using default options; reserved for the shorter test-call ergonomics
#[allow(dead_code)]
...
pub async fn run_stream_fixture_with_options(
    ...
    let raw_sse_rx_for_processor = if options.log_raw_sse {
        let (tx_raw_sse_for_processor, rx_raw_sse_for_processor) = mpsc::unbounded_channel::<String>();
    ...
    match provider {
        ...
        StreamFixtureProvider::Gemini => {
            tokio::spawn(handle_gemini_stream(response, tx_event, Some(tx_raw_sse), None));
        }
        StreamFixtureProvider::Responses => {
            tokio::spawn(handle_responses_stream(response, tx_event, Some(tx_raw_sse), None));
        }
    }
