...
`northhing-ai-adapters` owns provider-specific request/response mapping and stream
protocol parsing. Keep provider quirks here, then convert stream chunks into the
provider-neutral contracts owned by `northhing-agent-stream`.
...
cargo test -p northhing-ai-adapters
```
