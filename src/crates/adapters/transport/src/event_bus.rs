...
pub struct EventBus {
    ...
    // reason: enable_logging flag is reserved for the upcoming structured-logging feature; today the event bus uses the global tracing subscriber
    #[allow(dead_code)]
    enable_logging: bool,
}
...
struct EventEnvelope {
    session_id: String,
    event: AgenticEvent,
    // reason: priority is reserved for the upcoming priority-queue routing; today's bus is FIFO
    #[allow(dead_code)]
        ...
        tokio::spawn(async move {
            while let Some(envelope) = event_rx.recv().await {
                if let Some(adapter) = adapters_clone.get(&envelope.session_id) {
                    if let Err(e) = adapter.emit_event(&envelope.session_id, envelope.event).await {
                        error!("Failed to emit event for session {}: {}", envelope.session_id, e);
                    }
                } else {
