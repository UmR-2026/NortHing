...
`northhing-transport` owns cross-platform communication contracts and adapters. It
bridges product/API events to concrete delivery channels without owning product
logic.

## Guardrails

- Do not depend on `northhing-core`, API handlers, app crates, product domains,
...
```bash
cargo check -p northhing-transport
