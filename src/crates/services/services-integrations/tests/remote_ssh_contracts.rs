...
use northhing_services_integrations::remote_ssh::{
    canonicalize_local_workspace_root, local_workspace_roots_equal, normalize_local_workspace_root_for_stable_id,
    normalize_remote_workspace_path, remote_root_to_mirror_subpath, remote_workspace_runtime_root,
    remote_workspace_session_mirror_dir, remote_workspace_stable_id, sanitize_remote_mirror_path_component,
    sanitize_ssh_connection_id_for_local_dir, sanitize_ssh_hostname_for_mirror, unresolved_remote_session_storage_dir,
    unresolved_remote_session_storage_key, workspace_logical_key, RemoteWorkspace, RemoteWorkspaceRegistry,
...
fn remote_workspace_path_helpers_preserve_current_identity_contract() {
    ...
    assert_eq!(sanitize_ssh_hostname_for_mirror(" Example.COM "), "example.com");
    assert_eq!(
        remote_root_to_mirror_subpath("/home/user/repo"),
        std::path::PathBuf::from("home").join("user").join("repo")
    );
    assert_eq!(remote_root_to_mirror_subpath("/"), std::path::PathBuf::from("_root"));
    ...
    // Windows 路径规范化会产生不同的绝对路径，导致哈希值不同
    // 这里只测试远程工作空间的哈希（平台无关）
    #[cfg(not(windows))]
    {
        let local_id = local_workspace_stable_storage_id("/Users/foo/northhing");
...
fn remote_workspace_session_paths_use_supplied_mirror_root() {
    let mirror_root = std::path::PathBuf::from("/northhing/remote_ssh");

    assert_eq!(
        remote_workspace_runtime_root(&mirror_root, " Example.COM ", "/home/user/repo"),
        mirror_root.join("example.com").join("home").join("user").join("repo")
    );
    assert_eq!(
        remote_workspace_session_mirror_dir(&mirror_root, " Example.COM ", "/"),
        mirror_root.join("example.com").join("_root").join("sessions")
    );
    assert_eq!(
...
fn local_workspace_identity_helpers_preserve_canonical_root_contract() {
    let workspace_root =
        std::env::temp_dir().join(format!("northhing-services-remote-ssh-contract-{}", std::process::id()));
    ...
    assert_eq!(
        stable_root,
        normalize_local_workspace_root_for_stable_id(&workspace_root).expect("normalized local root")
    );
    assert_eq!(stable_root, canonical_path.to_string_lossy().replace('\\', "/"));
    assert!(local_workspace_roots_equal(&workspace_root, &workspace_root));
...
async fn remote_workspace_registry_preserves_ambiguous_root_resolution_contract() {
    ...
    registry.set_active_connection_hint(Some("conn-a".to_string())).await;
    ...
    let preferred = registry.lookup_connection("/tmp", Some("conn-b")).await.unwrap();
...
async fn remote_workspace_registry_preserves_legacy_state_and_clear_contract() {
    ...
    registry.unregister_remote_workspace("conn-1", "/repo").await;
    assert!(!registry.has_any().await);
    assert!(!registry.get_state().await.is_active);
}
