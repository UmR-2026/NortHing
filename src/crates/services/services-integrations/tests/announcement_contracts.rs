...
use northhing_services_integrations::announcement::{
    AnnouncementCard, AnnouncementState, AnnouncementStateStore, CardSource, CardType, CompletionAction, ModalConfig,
...
fn announcement_card_deserialization_preserves_default_contract() {
    ...
    assert!(matches!(card.trigger.condition, TriggerCondition::AppNthOpen { n: 3 }));
...
fn announcement_state_and_trigger_defaults_preserve_runtime_assumptions() {
    ...
    assert!(matches!(trigger.condition, TriggerCondition::VersionFirstOpen));
