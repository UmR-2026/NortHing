...
use northhing_services_integrations::git::{
    build_git_changed_files_args, build_git_diff_args, parse_branch_line, parse_git_log_line, parse_name_status_output,
    parse_worktree_list, GitAuthor, GitChangedFile, GitChangedFileStatus, GitChangedFilesParams, GitCommandOutput,
    GitCommitParams, GitDiffParams, GitGraph, GitService, GitWorktreeInfo, GraphNode, GraphRef,
};
use std::fs;
...
fn git_text_parsers_preserve_branch_and_log_contracts() {
    assert_eq!(
        parse_git_log_line("abc123|northhing|northhing@example.com|2026-05-12|subject|body"),
        Some((
            "abc123".to_string(),
            "northhing".to_string(),
            "northhing@example.com".to_string(),
    ...
    assert_eq!(parse_branch_line("* main"), Some(("main".to_string(), true)));
    assert_eq!(
        parse_branch_line("  feature/test"),
        Some(("feature/test".to_string(), false))
    );
    assert_eq!(parse_branch_line("detached"), Some(("detached".to_string(), false)));
...
fn git_diff_arg_builders_preserve_existing_command_contract() {
    ...
    assert_eq!(changed_args, vec!["diff", "--name-status", "--cached", "feature"]);
}

#[tokio::test]
async fn git_service_preserves_repository_status_contract() {
    let repo_dir = TempRepoDir::new("git-service-status");
    assert_eq!(GitService::is_repository(repo_dir.path()).await.unwrap(), false);

    run_git(repo_dir.path(), &["init"]);
    fs::write(repo_dir.path().join("new-file.txt"), "hello\n").unwrap();

    assert_eq!(GitService::is_repository(repo_dir.path()).await.unwrap(), true);
...
fn run_git(repo_dir: &std::path::Path, args: &[&str]) {
    let output = Command::new("git").current_dir(repo_dir).args(args).output().unwrap();
    assert!(
...
fn git_worktree_info_preserves_camel_case_contract() {
    let worktree = GitWorktreeInfo {
        path: "D:/workspace/northhing-worktree".to_string(),
...
fn git_worktree_parser_preserves_porcelain_contract() {
    let worktrees = parse_worktree_list(
        "worktree D:/workspace/northhing\nHEAD abc123\nbranch refs/heads/main\n\nworktree D:/workspace/northhing-feature\nHEAD def456\nbranch refs/heads/feature/test\nlocked\nprunable\n",
    );

    assert_eq!(worktrees.len(), 2);
    assert_eq!(worktrees[0].path, "D:/workspace/northhing");
...
fn git_commit_params_preserves_no_verify_rename_contract() {
    let params = GitCommitParams {
        ...
        author: Some(GitAuthor {
            name: "northhing".to_string(),
            email: "northhing@example.com".to_string(),
        }),
    };
...
fn git_graph_contract_preserves_camel_case_contract() {
    let graph = GitGraph {
        nodes: vec![GraphNode {
            ...
            author_name: "northhing".to_string(),
            author_email: "northhing@example.com".to_string(),
