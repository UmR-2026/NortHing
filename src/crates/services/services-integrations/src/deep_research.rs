...
//! The deterministic report rewrite lives in `northhing-runtime-ports`.

use northhing_runtime_ports::deep_research::{renumber_research_report, ResearchCitationDisplayMapEntry};
use serde_json::json;
use std::fmt;
...
pub enum DeepResearchReportIoError {
    ...
    WriteDisplayMap { path: PathBuf, source: std::io::Error },
}
...
/// `<workspace>/.northhing/sessions/<session_id>/research/`, where both the
...
async fn write_display_map_sidecar(
    ...
    let serialized = serde_json::to_string_pretty(&body).map_err(DeepResearchReportIoError::SerializeDisplayMap)?;
    ...
    impl ScratchDir {
        fn new(label: &str) -> Self {
            let unique = std::time::SystemTime::now()
                ...
                .as_nanos();
            let path = env::temp_dir().join(format!("northhing-citation-renumber-{}-{}", label, unique));
...
";
        fs::write(work_dir.join("citations.md"), citations).await.unwrap();
...
";
        ...
        let stats = try_renumber_research_report(&report_path, &work_dir).await.unwrap();
        ...
        let map: serde_json::Value = serde_json::from_str(&fs::read_to_string(sidecar).await.unwrap()).unwrap();
