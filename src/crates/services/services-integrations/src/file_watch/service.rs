...
fn lock_event_buffer(event_buffer: &StdMutex<Vec<FileWatchEvent>>) -> std::sync::MutexGuard<'_, Vec<FileWatchEvent>> {
...
impl FileWatchService {
    ...
    async fn create_watcher(&self) -> Result<(), String> {
        ...
        let mut watcher =
            RecommendedWatcher::new(tx, Config::default()).map_err(|e| format!("Failed to create watcher: {}", e))?;
...
static GLOBAL_FILE_WATCH_SERVICE: std::sync::OnceLock<Arc<FileWatchService>> = std::sync::OnceLock::new();

pub fn global_file_watch_service() -> Arc<FileWatchService> {
...
pub async fn start_file_watch(path: String, recursive: Option<bool>) -> Result<(), String> {
    let watcher = global_file_watch_service();
