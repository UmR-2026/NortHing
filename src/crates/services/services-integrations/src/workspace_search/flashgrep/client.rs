...
use super::{
    ...
    types::{GlobOutcome, GlobRequest, OpenRepoParams, RepoStatus, SearchOutcome, SearchRequest, TaskStatus},
    FLASHGREP_LOG_TARGET,
};

const CLIENT_NAME: &str = "northhing-workspace-search";
...
impl ManagedClient {
    ...
    pub async fn open_repo(&self, params: OpenRepoParams) -> Result<RepoSession> {
        match self.send_request_with_restart(Request::OpenRepo { params }).await? {
    ...
    async fn send_request_with_restart_timeout(&self, request: Request, timeout: Option<Duration>) -> Result<Response> {
        if self.is_shutting_down() {
            return Err(AppError::Protocol("flashgrep stdio backend is shutting down".into()));
        }

        let daemon = self.get_or_start_daemon().await?;
        match daemon.send_request_with_timeout(request.clone(), timeout).await {
            Ok(response) => Ok(response),
            Err(error) if !self.is_shutting_down() && should_restart_daemon(&error, daemon.as_ref()) => {
    ...
    async fn clear_daemon_if_current(&self, current: &Arc<AsyncDaemonClient>) {
        let mut state = self.state.lock().await;
        if state.daemon.as_ref().is_some_and(|daemon| Arc::ptr_eq(daemon, current)) {
            state.daemon = None;
        }
    }
...
impl RepoSession {
    ...
    async fn send_repo_request<T>(
        ...
        let response = self.client.send_request_with_restart_timeout(request, timeout).await?;
        decode(response)
    }
}
...
impl AsyncDaemonClient {
    async fn spawn(daemon_program: Option<OsString>) -> Result<Self> {
        ...
        let stdin = child
            .stdin
            .take()
            .ok_or_else(|| AppError::Protocol("flashgrep stdio backend did not provide stdin".into()))?;
        let stdout = child
            .stdout
            .take()
            .ok_or_else(|| AppError::Protocol("flashgrep stdio backend did not provide stdout".into()))?;
    ...
    async fn initialize(&self) -> Result<()> {
        match self
            ...
            .await?
        {
            Response::InitializeResult { .. } => self.protocol.send_notification(Request::Initialized).await,
            other => unexpected_response("initialize", other),
        }
    }

    async fn send_request_with_timeout(&self, request: Request, request_timeout: Option<Duration>) -> Result<Response> {
        self.protocol.send_request_with_timeout(request, request_timeout).await
    }
    ...
    async fn shutdown(&self) -> Result<()> {
        ...
        self.mark_closed();
        self.reject_pending("flashgrep stdio backend is shutting down").await;
    ...
    async fn wait_for_child_exit(&self) -> Result<()> {
        ...
        match timeout(SHUTDOWN_TIMEOUT, child.wait()).await {
            ...
            Err(_) => process_manager::terminate_child_process_tree(child, Duration::from_millis(750))
                .await
                .map_err(AppError::Io),
        }
    }
    ...
    async fn spawn_reader_task(&self, stdout: ChildStdout) {
        ...
        let handle = tokio::spawn(async move {
            ...
            match result {
                ...
                Err(error) => {
                    protocol
                        .close_with_message(format!("flashgrep stdio backend reader failed: {error}"))
                        .await;
                }
            }
        });
