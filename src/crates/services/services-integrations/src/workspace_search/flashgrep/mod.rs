...
pub fn log_flashgrep_stderr_line_with_context(context: Option<&str>, line: &str) {
    ...
    if let Some(rest) = trimmed.strip_prefix("flashgrep[") {
        if let Some((area, rest)) = rest.split_once("][") {
            if let Some((level, message)) = rest.split_once("] ") {
                ...
                match level {
                    "error" => tracing::error!(target: FLASHGREP_LOG_TARGET, "{formatted}"),
                    "warn" => tracing::warn!(target: FLASHGREP_LOG_TARGET, "{formatted}"),
                    "info" => tracing::info!(target: FLASHGREP_LOG_TARGET, "{formatted}"),
                    "debug" => tracing::debug!(target: FLASHGREP_LOG_TARGET, "{formatted}"),
                    "trace" => tracing::trace!(target: FLASHGREP_LOG_TARGET, "{formatted}"),
                    _ => tracing::debug!(target: FLASHGREP_LOG_TARGET, "{trimmed}"),
                }
                return;
            }
        }
    ...
    match context {
        Some(context) => tracing::debug!(target: FLASHGREP_LOG_TARGET, "{context} {trimmed}"),
        None => tracing::debug!(target: FLASHGREP_LOG_TARGET, "{trimmed}"),
    }
}
...
pub use types::{
    ConsistencyMode, DirtyFileStats, FileCount, GlobOutcome, GlobRequest, OpenRepoParams, PathScope, QuerySpec,
    RefreshPolicyConfig, RepoConfig, RepoPhase, RepoStatus, SearchBackend, SearchModeConfig, SearchOutcome,
    SearchRequest, SearchResults, TaskKind, TaskPhase, TaskState, TaskStatus, WorkspaceOverlayStatus,
};
