...
impl ProtocolClient {
        ...
        let response = match request_timeout {
            Some(duration) => match timeout(duration, receiver).await {
                Ok(result) => result.map_err(|_| {
                    AppError::Protocol(format!("{} closed without sending a response", self.inner.backend_name))
                })??,
                Err(_) => {
                    self.inner.pending.lock().await.remove(&request_id);
                    return Err(AppError::Protocol(format!(
    ...
    pub async fn send_notification(&self, request: Request) -> Result<()> {
        ...
        self.inner
            ...
            .send(bytes)
            .await
            .map_err(|_| AppError::Protocol(format!("{} write channel is closed", self.inner.backend_name)))
    }
    ...
    loop {
        ...
        if name.trim().eq_ignore_ascii_case("Content-Length") {
            let length = value
                .trim()
                .parse::<usize>()
                .map_err(|error| AppError::Protocol(format!("invalid Content-Length header: {error}")))?;
            content_length = Some(length);
        }
    }

    let content_length = content_length.ok_or_else(|| AppError::Protocol("missing Content-Length header".into()))?;
...
mod tests {
    ...
    fn drains_remote_stdio_content_length_messages() {
        ...
        let messages = drain_content_length_messages(&mut buffer).expect("expected content-length message to decode");
    ...
    fn drains_remote_stdio_initialize_response_with_legacy_search_modes() {
        ...
        let mut buffer = format!("Content-Length: {}\r\n\r\n{}", body.len(), body).into_bytes();
        let messages = drain_content_length_messages(&mut buffer).expect("expected initialize response to decode");
