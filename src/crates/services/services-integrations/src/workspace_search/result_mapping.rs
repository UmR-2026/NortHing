...
use northhing_services_core::filesystem::{FileSearchResult, SearchMatchType};
...
pub(crate) fn convert_search_results(
    ...
    match output_mode {
        ...
        ContentSearchOutputMode::FilesWithMatches => convert_matched_paths_to_file_only_results(search_results),
    }
}

fn convert_line_matches_to_file_search_results(search_results: &SearchResults) -> Vec<FileSearchResult> {
    search_results
        .line_matches
        .iter()
...
fn convert_file_match_counts_to_search_results(search_results: &SearchResults) -> Vec<FileSearchResult> {
    search_results
        .file_match_counts
        .iter()
        .map(|count| FileSearchResult {
...
fn convert_matched_paths_to_file_only_results(search_results: &SearchResults) -> Vec<FileSearchResult> {
    search_results
        .matched_paths
        .iter()
...
fn split_preview(snippet: &str, matched_text: &str) -> (Option<String>, Option<String>, Option<String>) {
