...
impl RelayClient {
                    ...
                    match dial(&ctx.ws_url).await {
                        Ok(new_stream) => {
                            ...
                            *state_arc.write().await = ConnectionState::Connected;
                            ...
                            let (new_cmd_tx, mut new_cmd_rx) = mpsc::unbounded_channel::<RelayMessage>();
    ...
    pub async fn send_relay_response(&self, correlation_id: &str, encrypted_data: &str, nonce: &str) -> Result<()> {
...
async fn dial(ws_url: &str) -> Result<WsStream> {
    ...
    #[cfg(windows)]
    {
        ...
        // process.
        let connector = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| build_windows_rustls_connector()))
