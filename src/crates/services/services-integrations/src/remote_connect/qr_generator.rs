...
impl QrGenerator {
    ...
    pub fn build_url(payload: &QrPayload, web_app_url: &str, language: &str) -> String {
        let relay_ws = payload.url.replace("https://", "wss://").replace("http://", "ws://");
        format!(
    ...
    pub fn generate_png_base64_from_url(url: &str) -> Result<String> {
        let code = QrCode::new(url.as_bytes()).map_err(|e| anyhow!("QR code generation failed: {e}"))?;
...
mod tests {
    ...
    fn build_url_includes_language_parameter() {
        let payload = QrPayload {
            ...
            device_name: "northhing Desktop".to_string(),
