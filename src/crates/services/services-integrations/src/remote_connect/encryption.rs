...
pub fn encrypt(shared_secret: &[u8; 32], plaintext: &[u8]) -> Result<(Vec<u8>, [u8; NONCE_SIZE])> {
    let cipher = Aes256Gcm::new_from_slice(shared_secret).map_err(|e| anyhow!("cipher init: {e}"))?;

    let mut nonce_bytes = [0u8; NONCE_SIZE];
    OsRng.fill_bytes(&mut nonce_bytes);
    let nonce = Nonce::from_slice(&nonce_bytes);

    let ciphertext = cipher.encrypt(nonce, plaintext).map_err(|e| anyhow!("encrypt: {e}"))?;

    Ok((ciphertext, nonce_bytes))
}

/// Decrypts ciphertext using AES-256-GCM.
pub fn decrypt(shared_secret: &[u8; 32], ciphertext: &[u8], nonce_bytes: &[u8; NONCE_SIZE]) -> Result<Vec<u8>> {
    let cipher = Aes256Gcm::new_from_slice(shared_secret).map_err(|e| anyhow!("cipher init: {e}"))?;
    let nonce = Nonce::from_slice(nonce_bytes);
...
pub fn decrypt_from_base64(shared_secret: &[u8; 32], ciphertext_b64: &str, nonce_b64: &str) -> Result<String> {
    let ct = BASE64
...
pub fn parse_public_key(b64: &str) -> Result<[u8; 32]> {
    ...
    if bytes.len() != 32 {
        return Err(anyhow!("invalid public key length: expected 32, got {}", bytes.len()));
    }
    let mut key = [0u8; 32];
