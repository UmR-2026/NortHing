...
use super::types::{
    InitializeResult as NortHingInitializeResult, MCPToolResult, PromptsGetResult, PromptsListResult,
...
use crate::mcp::protocol::{
    create_mcp_client_info, map_rmcp_initialize_result, map_rmcp_prompt, map_rmcp_prompt_message, map_rmcp_resource,
    map_rmcp_resource_content, map_rmcp_tool, map_rmcp_tool_result,
};
use crate::mcp::{MCPRuntimeError, MCPRuntimeResult};
use futures::StreamExt;
use reqwest::header::{HeaderMap, HeaderName, HeaderValue, ACCEPT, CONTENT_TYPE, USER_AGENT, WWW_AUTHENTICATE};
use rmcp::model::{
    CallToolRequestParams, ClientInfo, GetPromptRequestParams, JsonObject, LoggingLevel,
    LoggingMessageNotificationParam, PaginatedRequestParams, ReadResourceRequestParams, RequestNoParam,
};
...
use rmcp::transport::streamable_http_client::{
    AuthRequiredError, SseError, StreamableHttpClient, StreamableHttpError, StreamableHttpPostResponse,
};
...
#[derive(Clone)]
struct NortHingRmcpClientHandler {
    info: ClientInfo,
}

impl ClientHandler for NortHingRmcpClientHandler {
...
enum ClientState {
    Connecting {
        transport: Option<StreamableHttpClientTransport<NortHingStreamableHttpClient>>,
    },
    Ready {
        service: Arc<RunningService<RoleClient, NortHingRmcpClientHandler>>,
    },
}

#[derive(Clone)]
struct NortHingStreamableHttpClient {
    client: reqwest::Client,
    oauth_manager: Option<Arc<Mutex<AuthorizationManager>>>,
}

impl NortHingStreamableHttpClient {
...
impl StreamableHttpClient for NortHingStreamableHttpClient {
    ...
    async fn get_stream(
        ...
        auth_token: Option<String>,
        custom_headers: HashMap<HeaderName, HeaderValue>,
    ) -> Result<futures::stream::BoxStream<'static, Result<Sse, SseError>>, StreamableHttpError<Self::Error>> {
    ...
    async fn delete_session(
        ...
        let _ = response.error_for_status().map_err(StreamableHttpError::Client)?;
        Ok(())
    }
    ...
    async fn post_message(
        ...
        match content_type.as_deref() {
            ...
            _ => {
                ...
                let bytes = response.bytes().await.map_err(StreamableHttpError::Client)?;
                let trimmed = bytes
                    .iter()
                    .copied()
                ...
                if let Ok(message) = serde_json::from_slice::<rmcp::model::ServerJsonRpcMessage>(&bytes) {
...
impl RemoteMCPTransport {
    fn build_default_headers(headers: &HashMap<String, String>) -> HeaderMap {
        ...
        for (name, value) in headers {
            ...
            let Ok(header_value) = HeaderValue::from_str(&header_value_str) else {
                warn!("Invalid HTTP header value in MCP config (skipping): header={}", name);
                continue;
            };
        ...
        if !header_map.contains_key(USER_AGENT) {
            header_map.insert(USER_AGENT, HeaderValue::from_static("northhing-MCP-Client/1.0"));
        }
    ...
    pub async fn new(
        ...
        let oauth_manager = if oauth_enabled && !default_headers.contains_key(reqwest::header::AUTHORIZATION) {
            let (manager, initialized) = build_authorization_manager(data_dir, server_id, &url).await?;
            if initialized {
    ...
    async fn service(&self) -> MCPRuntimeResult<Arc<RunningService<RoleClient, NortHingRmcpClientHandler>>> {
        let guard = self.state.lock().await;
        match &*guard {
            ClientState::Ready { service } => Ok(Arc::clone(service)),
            ClientState::Connecting { .. } => {
                Err(MCPRuntimeError::mcp("Remote MCP client not initialized".to_string()))
            }
        }
    }
    ...
    pub async fn initialize(
        ...
        client_name: &str,
        client_version: &str,
    ) -> MCPRuntimeResult<NortHingInitializeResult> {
        ...
        match &mut *guard {
            ClientState::Ready { service } => {
                let info = service
                    .peer()
                    .peer_info()
                    .ok_or_else(|| MCPRuntimeError::mcp("Handshake succeeded but server info missing".to_string()))?;
            ...
            ClientState::Connecting { transport } => {
                ...
                let handler = NortHingRmcpClientHandler {
    ...
    pub async fn ping(&self) -> MCPRuntimeResult<()> {
        ...
        let fut = service.send_request(rmcp::model::ClientRequest::PingRequest(RequestNoParam::default()));
        let result = Self::await_with_optional_timeout(self.request_timeout, fut, "MCP ping timeout".to_string())
            .await?
            .map_err(|e| MCPRuntimeError::mcp(format!("MCP ping failed: {}", e)))?;

        match result {
            rmcp::model::ServerResult::EmptyResult(_) => Ok(()),
            other => Err(MCPRuntimeError::mcp(format!("Unexpected ping response: {:?}", other))),
        }
    }

    pub async fn list_resources(&self, cursor: Option<String>) -> MCPRuntimeResult<ResourcesListResult> {
        let service = self.service().await?;
        let fut = service
            .peer()
            .list_resources(Some(PaginatedRequestParams::default().with_cursor(cursor)));
        let result =
            Self::await_with_optional_timeout(self.request_timeout, fut, "MCP resources/list timeout".to_string())
                .await?
                .map_err(|e| MCPRuntimeError::mcp(format!("MCP resources/list failed: {}", e)))?;
        Ok(ResourcesListResult {
            resources: result.resources.into_iter().map(map_rmcp_resource).collect(),
    ...
    pub async fn read_resource(&self, uri: &str) -> MCPRuntimeResult<ResourcesReadResult> {
        ...
        let result =
            Self::await_with_optional_timeout(self.request_timeout, fut, "MCP resources/read timeout".to_string())
                .await?
                .map_err(|e| MCPRuntimeError::mcp(format!("MCP resources/read failed: {}", e)))?;
        Ok(ResourcesReadResult {
            contents: result.contents.into_iter().map(map_rmcp_resource_content).collect(),
        })
    }

    pub async fn list_prompts(&self, cursor: Option<String>) -> MCPRuntimeResult<PromptsListResult> {
        let service = self.service().await?;
        let fut = service
            .peer()
            .list_prompts(Some(PaginatedRequestParams::default().with_cursor(cursor)));
        let result =
            Self::await_with_optional_timeout(self.request_timeout, fut, "MCP prompts/list timeout".to_string())
                .await?
    ...
    pub async fn get_prompt(
        ...
        let result =
            Self::await_with_optional_timeout(self.request_timeout, fut, "MCP prompts/get timeout".to_string())
                .await?
                .map_err(|e| MCPRuntimeError::mcp(format!("MCP prompts/get failed: {}", e)))?;

        Ok(PromptsGetResult {
            description: result.description,
            messages: result.messages.into_iter().map(map_rmcp_prompt_message).collect(),
        })
    }
    ...
    pub async fn list_tools(&self, cursor: Option<String>) -> MCPRuntimeResult<ToolsListResult> {
        ...
        let result = Self::await_with_optional_timeout(self.request_timeout, fut, "MCP tools/list timeout".to_string())
            .await?
    ...
    pub async fn call_tool(&self, name: &str, arguments: Option<Value>) -> MCPRuntimeResult<MCPToolResult> {
        let service = self.service().await?;
        ...
        let result = Self::await_with_optional_timeout(self.request_timeout, fut, "MCP tools/call timeout".to_string())
            .await?
