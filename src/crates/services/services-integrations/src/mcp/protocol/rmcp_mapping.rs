//! Mapping helpers from `rmcp` protocol models into northhing MCP contracts.

use super::types::{
    InitializeResult, MCPAnnotations, MCPCapability, MCPPrompt, MCPPromptArgument, MCPPromptMessage,
    MCPPromptMessageContent, MCPPromptMessageContentBlock, MCPResource, MCPResourceContent, MCPResourceIcon,
    MCPServerInfo, MCPTool, MCPToolAnnotations, MCPToolResult, MCPToolResultContent, PromptsCapability,
...
pub fn map_rmcp_tool_result(result: rmcp::model::CallToolResult) -> MCPToolResult {
    let mapped: Vec<MCPToolResultContent> = result.content.into_iter().filter_map(map_rmcp_content_block).collect();
        ...
        icons
            ...
            .map(|icon| MCPResourceIcon {
                ...
                sizes: icon
                    .sizes
                    .as_ref()
                    .map(|sizes| Value::Array(sizes.iter().cloned().map(Value::String).collect::<Vec<_>>())),
            })
            .collect()
    })
}
