...
impl MCPTransport {
    ...
    pub async fn send_request(&self, method: String, params: Option<Value>) -> MCPRuntimeResult<u64> {
    ...
    pub async fn send_request_with_id(&self, id: u64, method: String, params: Option<Value>) -> MCPRuntimeResult<()> {
        let request = MCPRequest::new(Value::Number(id.into()), method, params);
        self.send_message(MCPMessage::Request(request)).await
    }

    pub async fn send_notification(&self, method: String, params: Option<Value>) -> MCPRuntimeResult<()> {
        let notification = MCPNotification::new(method, params);
        self.send_message(MCPMessage::Notification(notification)).await
    }
    ...
    async fn send_message(&self, message: MCPMessage) -> MCPRuntimeResult<()> {
        let json = serde_json::to_string(&message)
            .map_err(|e| MCPRuntimeError::serialization(format!("Failed to serialize MCP message: {}", e)))?;

        let mut stdin = self.stdin.lock().await;
        stdin
            .write_all(json.as_bytes())
            .await
            .map_err(|e| MCPRuntimeError::io(format!("Failed to write to MCP server stdin: {}", e)))?;
        stdin
            .write_all(b"\n")
            .await
            .map_err(|e| MCPRuntimeError::io(format!("Failed to write newline to MCP server stdin: {}", e)))?;
        stdin
            .flush()
            .await
