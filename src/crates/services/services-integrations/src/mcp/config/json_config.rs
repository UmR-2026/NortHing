...
fn normalize_transport(value: &str) -> Option<&'static str> {
    match value.trim() {
        "stdio" => Some("stdio"),
        "sse" => Some("sse"),
        "http" | "streamable_http" | "streamable-http" | "streamablehttp" => Some("streamable-http"),
        _ => None,
    }
}
...
fn string_field<'a>(obj: &'a serde_json::Map<String, serde_json::Value>, key: &str) -> Option<&'a str> {
    obj.get(key)
        .and_then(|v| v.as_str())
        .map(str::trim)
        .filter(|s| !s.is_empty())
...
pub fn format_mcp_json_config_value(value: Option<&serde_json::Value>) -> serde_json::Result<String> {
...
pub fn validate_mcp_json_config(config_value: &serde_json::Value) -> Result<(), MCPJsonConfigValidationError> {
    if config_value.get("mcpServers").is_none() {
        return Err(MCPJsonConfigValidationError::new("Config missing 'mcpServers' field"));
    }
    ...
    for (server_id, server_config) in servers {
        ...
        let effective_source = match (command.is_some(), url.is_some()) {
            ...
            _ => {
                // P1-A (2026-06-25): reachable when both `command` and `url`
                // are present (or both absent). Surfacing a real error
                // prevents a panic on future schema changes.
                return Err(MCPJsonConfigValidationError::new(format!(
                    "Server '{}' has conflicting command/url configuration (command={}, url={}); exactly one of 'command' or 'url' must be set",
                    server_id,
                    command.is_some(),
                    url.is_some()
                )));
        ...
        let effective_transport = match effective_source {
            ...
            _ => {
                // P1-A (2026-06-25): reachable if a future `effective_source`
                // value is added without updating this match. Return an
                // error instead of panicking.
                return Err(MCPJsonConfigValidationError::new(format!(
                    "Server '{}' has unsupported effective_source '{}'",
                    server_id, effective_source
                )));
            }
        };
