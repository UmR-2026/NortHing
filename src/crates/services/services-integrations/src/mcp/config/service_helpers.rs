...
pub fn parse_mcp_config_array(servers: &[serde_json::Value], location: super::ConfigLocation) -> Vec<MCPServerConfig> {
    servers
        .iter()
        .filter_map(|value| match serde_json::from_value::<MCPServerConfig>(value.clone()) {
            ...
            Err(e) => {
                warn!("Failed to parse MCP config item at {:?} scope: {}", location, e);
                None
            }
        })
        .collect()
