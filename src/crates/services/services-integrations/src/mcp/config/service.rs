...
use super::{
    config_to_cursor_format, get_mcp_remote_authorization_source, get_mcp_remote_authorization_value,
    has_mcp_remote_authorization, has_mcp_remote_oauth, has_mcp_remote_xaa, merge_mcp_server_config_sources,
    normalize_mcp_authorization_value, parse_cursor_format, parse_mcp_config_array, remove_mcp_authorization_keys,
    ConfigLocation,
};
...
impl MCPConfigService {
    ...
    pub async fn set_remote_authorization(
        ...
        let mut config = self
            ...
            .await?
            .ok_or_else(|| MCPRuntimeError::not_found(format!("MCP server config not found: {}", server_id)))?;
    ...
    pub async fn clear_remote_authorization(&self, server_id: &str) -> MCPRuntimeResult<MCPServerConfig> {
        let mut config = self
            .get_server_config(server_id)
            .await?
            .ok_or_else(|| MCPRuntimeError::not_found(format!("MCP server config not found: {}", server_id)))?;
    ...
    async fn save_user_config(&self, config: &MCPServerConfig) -> MCPRuntimeResult<()> {
        ...
        self.config_store
            ...
            .await?;
        info!("Saved user-level MCP server config (Cursor format): {}", config.id);
        Ok(())
    }
    ...
    async fn save_project_config(&self, config: &MCPServerConfig) -> MCPRuntimeResult<()> {
        ...
        let value = serde_json::to_value(&configs)
            .map_err(|e| MCPRuntimeError::serialization(format!("Failed to serialize MCP config: {}", e)))?;

        self.config_store.set_config_value("project.mcp_servers", value).await?;
        Ok(())
    }
