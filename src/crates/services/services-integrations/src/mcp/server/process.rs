...
impl MCPServerProcess {
    ...
    pub async fn start(
        ...
        let (final_command, final_args) = {
            ...
            let is_node_command = node_commands.iter().any(|&cmd| command.eq_ignore_ascii_case(cmd));
        ...
        let child = cmd.spawn().map_err(|e| {
            ...
            MCPRuntimeError::process(format!("Failed to start MCP server '{}': {}", final_command, e))
        });
        ...
        info!("MCP server started successfully: name={} id={}", self.name, self.id);
    ...
    pub async fn start_remote(
        ...
        let url = config.url.as_deref().ok_or_else(|| {
            MCPRuntimeError::configuration(format!("Remote MCP server '{}' is missing a URL", self.id))
        })?;
        ...
        let connection = Arc::new(
            MCPConnection::new_remote_with_data_dir(data_dir, &self.id, url.to_string(), merged_headers, true).await?,
        );
        self.connection = Some(connection.clone());
        self.start_time = Some(Instant::now());
    ...
    async fn handshake(&mut self) -> MCPRuntimeResult<()> {
        ...
        let result: InitializeResult = connection.initialize("northhing", env!("CARGO_PKG_VERSION")).await?;
    ...
    pub async fn restart(
        ...
        if self.restart_count >= self.max_restarts {
            ...
            self.set_status_with_error(
                ...
                Some(format!("Max restart attempts ({}) reached", self.max_restarts)),
            )
            .await;
    ...
    fn start_health_check(&self) {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                if !matches!(current_status, MCPServerStatus::Connected | MCPServerStatus::Healthy) {
                    debug!(
                ...
                if let Some(conn) = &connection {
                    match conn.ping().await {
                        ...
                        Err(e) => {
                            warn!("Health check failed: server_name={} error={}", server_name, e);
                            if is_mcp_auth_error_message(&e.to_string()) {
