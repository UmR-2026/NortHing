...
use crate::mcp::protocol::{
    ...
    parse_response_result, InitializeResult, MCPError, MCPMessage, MCPResponse, MCPToolResult, MCPTransport,
    PromptsGetResult, PromptsListResult, RemoteMCPTransport, ResourcesListResult, ResourcesReadResult, ToolsListResult,
};
...
impl MCPConnection {
    ...
    pub async fn new_remote(
        ...
        Self::new_remote_with_data_dir(std::env::temp_dir(), server_id, url, headers, oauth_enabled).await
    }
    ...
    pub async fn new_remote_with_data_dir(
        ...
        let transport =
            Arc::new(RemoteMCPTransport::new(data_dir, server_id, url, headers, None, oauth_enabled).await?);
        let pending_requests = Arc::new(RwLock::new(HashMap::new()));
        let (event_tx, _) = broadcast::channel(64);
    ...
    async fn send_request_and_wait(&self, method: String, params: Option<Value>) -> MCPRuntimeResult<MCPResponse> {
        self.send_request_and_wait_with_timeout(method, params, None).await
    }
    ...
    async fn send_request_and_wait_with_timeout(
        ...
        match &self.transport {
            TransportType::Local(transport) => {
                ...
                if let Err(error) = transport.send_request_with_id(request_id, method.clone(), params).await {
                    let mut pending = self.pending_requests.write().await;
            ...
            TransportType::Remote(_transport) => Err(MCPRuntimeError::not_implemented(
                "Generic JSON-RPC send_request is not supported for Streamable HTTP connections".to_string(),
            )),
        }
    }

    /// Initializes the connection.
    pub async fn initialize(&self, client_name: &str, client_version: &str) -> MCPRuntimeResult<InitializeResult> {
        match &self.transport {
            TransportType::Local(_) => {
                let request = create_initialize_request(0, client_name, client_version);
                let response = self
                    .send_request_and_wait_with_timeout(request.method.clone(), request.params, self.initialize_timeout)
                    .await?;
    ...
    pub async fn send_response(&self, request_id: Value, result: Value) -> MCPRuntimeResult<()> {
        match &self.transport {
            ...
            TransportType::Remote(_) => Err(MCPRuntimeError::not_implemented(
                "Sending server-request responses is not supported for Streamable HTTP connections".to_string(),
            )),
        }
    }
    ...
    pub async fn send_error(&self, request_id: Value, error: MCPError) -> MCPRuntimeResult<()> {
        match &self.transport {
            ...
            TransportType::Remote(_) => Err(MCPRuntimeError::not_implemented(
                "Sending server-request errors is not supported for Streamable HTTP connections".to_string(),
            )),
        }
    }
}
...
mod tests {
    ...
    async fn local_tool_calls_do_not_inherit_initialize_timeout() {
        ...
        let connection = MCPConnection::new(stdin, rx).with_initialize_timeout(Some(Duration::from_millis(10)));

        tokio::spawn(async move {
            let mut reader = BufReader::new(stdout);
            let mut line = String::new();
            while reader.read_line(&mut line).await.expect("read request line") > 0 {
        ...
        let result = tokio::time::timeout(Duration::from_millis(500), connection.call_tool("slow_tool", None))
            .await
