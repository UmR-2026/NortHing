...
pub use runtime_policy::{compute_mcp_backoff_delay, detect_mcp_list_changed_kind, MCPListChangedKind};
...
impl MCPServerConfig {
    ...
    pub fn validate(&self) -> Result<(), MCPServerConfigValidationError> {
        if self.id.is_empty() {
            return Err(MCPServerConfigValidationError::new("MCP server id cannot be empty"));
        }

        if self.name.is_empty() {
            return Err(MCPServerConfigValidationError::new("MCP server name cannot be empty"));
        }
        ...
        match self.server_type {
            ...
            MCPServerType::Remote => {
                ...
                if !matches!(transport, MCPServerTransport::StreamableHttp | MCPServerTransport::Sse) {
                    return Err(MCPServerConfigValidationError::new(format!(
