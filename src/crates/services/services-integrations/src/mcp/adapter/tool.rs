...
impl MCPDynamicToolProvider {
    ...
    pub async fn load_tool_definitions(
        ...
        Ok(client
            ...
            .into_iter()
            .map(|mcp_tool| {
                let descriptor = build_mcp_tool_descriptor(&self.server_id, &self.server_name, &mcp_tool);
                MCPDynamicToolDefinition { mcp_tool, descriptor }
            })
            .collect())
    }
}
...
pub fn build_mcp_tool_descriptor(server_id: &str, server_name: &str, tool: &MCPTool) -> McpDynamicToolDescriptor {
