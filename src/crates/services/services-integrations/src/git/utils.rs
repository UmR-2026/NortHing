pub use super::{build_git_changed_files_args, build_git_diff_args, parse_branch_line, parse_git_log_line};
/**
...
pub fn get_repository_root<P: AsRef<Path>>(path: P) -> Result<String, GitError> {
    let repo = Repository::discover(path).map_err(|e| GitError::RepositoryNotFound(e.to_string()))?;
    ...
    match repo.head() {
        ...
        Err(e) => {
            if e.code() == git2::ErrorCode::UnbornBranch {
                ...
                Err(GitError::CommandFailed(format!("Failed to get HEAD: {}", e)))
            }
        }
    }
}
...
pub async fn execute_git_command_raw(repo_path: &str, args: &[&str]) -> Result<GitCommandOutput, GitError> {
...
pub fn execute_git_command_sync_raw(repo_path: &str, args: &[&str]) -> Result<GitCommandOutput, GitError> {
