...
impl RemoteTerminalManager {
    ...
    pub async fn create_session(
        ...
        let cwd = if let Some(dir) = initial_cwd {
            ...
            match timeout(REMOTE_PWD_PROBE_TIMEOUT, manager.execute_command(connection_id, "pwd")).await {
        ...
        tokio::spawn(async move {
            tracing::info!("Remote PTY owner task started: session_id={}", task_session_id);
            ...
            if initial_cd != "/" && !initial_cd.is_empty() {
                ...
                if let Err(e) = writer.write_all(cd_cmd.as_bytes()).await {
                    tracing::warn!("Failed to cd to initial directory: {}", e);
                }
                        ...
                        match cmd {
                            Some(PtyCommand::Write(data)) => {
                                if let Err(e) = writer.write_all(&data).await {
                                    tracing::warn!("PTY write failed: session_id={}, error={}", task_session_id, e);
                                }
                            ...
                            Some(PtyCommand::Resize(cols, rows)) => {
                                if let Err(e) = channel.window_change(cols, rows, 0, 0).await {
                                    tracing::warn!("PTY resize failed: session_id={}, error={}", task_session_id, e);
                                }
                            }
                            Some(PtyCommand::Close) | None => {
                                tracing::info!("PTY close requested: session_id={}", task_session_id);
                    ...
                    msg = channel.wait() => {
                        match msg {
                            ...
                            | Some(russh::ChannelMsg::ExitStatus { .. }) => {
                                tracing::info!("Remote PTY closed: session_id={}", task_session_id);
                                break;
                            }
                            Some(_) => continue, // WindowAdjust, Success, etc.
                            None => {
                                tracing::info!("Remote PTY channel ended: session_id={}", task_session_id);
                                break;
                            }
                        }
                    }
            ...
            tracing::info!("Remote PTY owner task exited: session_id={}", task_session_id);
        });
