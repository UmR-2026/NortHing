...
use crate::workspace_search::flashgrep::{PathScope, SearchBackend, SearchModeConfig, SearchResults};
...
pub(crate) const REMOTE_ARCHITECTURE_PROBES: &[&str] =
    &["uname -m", "arch", "sh -c 'uname -m 2>/dev/null || arch 2>/dev/null'"];

const REMOTE_FLASHGREP_INSTALL_DIR: &str = ".northhing/bin";
...
fn normalize_remote_scope_path(repo_root: &str, search_path: &Path) -> Result<String, String> {
    ...
    if normalized != repo_root && !normalized.starts_with(&repo_root_with_slash) {
        return Err(format!("Remote search path is outside workspace root: {normalized}"));
    }
    Ok(normalized)
}
...
pub(crate) fn remote_workspace_search_storage_root(repo_root: &str) -> String {
    join_remote_path(repo_root, ".northhing/search/flashgrep-index")
}
...
pub(crate) fn parse_remote_architecture_output(stdout: &str, stderr: &str) -> Option<String> {
    for stream in [stdout, stderr] {
        for line in stream.lines() {
            ...
            if normalized.contains("aarch64") || normalized.contains("arm64") || normalized.contains("armv8") {
...
fn linux_flashgrep_bundle_names_for_arch(remote_arch: &str) -> Result<&'static [&'static str], String> {
    match remote_arch {
...
pub(crate) async fn local_flashgrep_bundle_for_arch(remote_arch: &str) -> Result<LocalFlashgrepBundle, String> {
    ...
    let (binary_name, path) = bundled_binary_names
        ...
        .find_map(|binary_name| {
            resolve_local_flashgrep_bundle(binary_name).map(|path| ((*binary_name).to_string(), path))
        })
        .ok_or_else(|| {
            format!(
    ...
    let bytes = tokio::fs::read(&path)
        .await
        .map_err(|error| format!("Failed to read bundled flashgrep binary {}: {error}", path.display()))?;
...
fn resolve_local_flashgrep_bundle(binary_name: &str) -> Option<PathBuf> {
    ...
    if let Ok(current_exe) = std::env::current_exe() {
        if let Some(parent) = current_exe.parent() {
            ...
            candidates.push(parent.join("../Resources/flashgrep").join(binary_name));
            candidates.push(parent.join("../share/northhing/flashgrep").join(binary_name));
            candidates.push(
                parent
...
mod tests {
    use super::*;
    use crate::workspace_search::flashgrep::{FileCount, SearchBackend, SearchHit, SearchModeConfig, SearchResults};
    use std::path::{Path, PathBuf};
    ...
    fn remote_workspace_search_paths_preserve_current_contract() {
        assert_eq!(
            ...
            "/home/wgq/workspace/bot_detection/.northhing/bin"
        );
        assert_eq!(
            remote_workspace_search_storage_root("/home/wgq/workspace/bot_detection/"),
            "/home/wgq/workspace/bot_detection/.northhing/search/flashgrep-index"
        );
        ...
        assert_eq!(shell_escape("/home/user/repo/file.txt"), "/home/user/repo/file.txt");
        assert_eq!(
    ...
    fn remote_workspace_search_probe_parsers_preserve_current_contract() {
        ...
        assert_eq!(parse_remote_os_output("Linux\n", ""), Some("Linux".to_string()));
        assert_eq!(
        ...
        assert!(looks_like_linux_workspace_root("/home/wgq/workspace/bot_detection"));
        assert!(!looks_like_linux_workspace_root("C:/Users/wgq/workspace/bot_detection"));
    }
    ...
    fn remote_scan_fallback_retry_policy_preserves_current_contract() {
        ...
        with_paths.matched_paths.push("/repo/src/lib.rs".to_string());
        assert!(!should_retry_remote_scan_fallback_as_files_with_matches(
            SearchBackend::ScanFallback,
            SearchModeConfig::LineMatches,
            &with_paths,
    ...
    fn search_results_with_counts(matched_lines: usize, matched_occurrences: usize) -> SearchResults {
        SearchResults {
            candidate_docs: 0,
