    ...
    D: Deserializer<'de>,
{
    ...
    match Helper::deserialize(deserializer)? {
        ...
        Helper::PrivateKey { key_path, passphrase } => Ok(SSHAuthMethod::PrivateKey { key_path, passphrase }),
...
pub struct RemoteWorkspace {
    ...
    #[serde(default)]
    pub connection_name: String,
    /// SSH config `host`; used for `~/.northhing/remote_ssh/{host}/...` session storage.
    #[serde(default)]
    pub ssh_host: String,
}
