...
impl RemoteFileService {
    pub fn new(manager: Arc<tokio::sync::RwLock<Option<crate::remote_ssh::manager::SSHConnectionManager>>>) -> Self {
        Self { manager }
    }

    /// Get the SSH manager
    async fn manager(&self, _connection_id: &str) -> anyhow::Result<crate::remote_ssh::manager::SSHConnectionManager> {
    ...
    pub async fn read_file(&self, connection_id: &str, path: &str) -> anyhow::Result<Vec<u8>> {
        let manager = self.manager(connection_id).await?;
    ...
    async fn build_tree_impl(
        ...
        for entry in entries {
            if entry.is_dir {
                match Box::pin(self.build_tree_impl(connection_id, &entry.path, current_depth + 1, max_depth)).await {
    ...
    pub async fn rename(&self, connection_id: &str, old_path: &str, new_path: &str) -> anyhow::Result<()> {
        let manager = self.manager(connection_id).await?;
...
fn format_permissions(mode: Option<u32>) -> String {
    ...
    let perm_str: String = perms.iter().map(|(set, c)| if *set { *c } else { '-' }).collect();
