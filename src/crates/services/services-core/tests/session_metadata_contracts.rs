use northhing_services_core::session::{
    ...
    try_refresh_session_metadata_for_saved_turn, upsert_session_index_entry, DialogTurnData, ModelRoundData,
    SessionKind, SessionMetadata, StoredSessionIndexFile, TextItemData, ToolCallData, ToolItemData, UserMessageData,
};
...
fn turn(session_id: &str, turn_index: usize, text_count: usize, tool_count: usize) -> DialogTurnData {
    ...
    turn.model_rounds.push(round(&turn_id, text_count, tool_count));
    turn
}
...
fn full_refresh_recomputes_metadata_counters_from_turns() {
    ...
    assert_eq!(metadata.workspace_path.as_deref(), Some("D:/workspace/project"));
}
...
fn index_entry_upsert_and_remove_preserve_sorting_and_counts() {
    ...
    let index = remove_session_index_entry(Some(index), "newer", -1, 100).expect("existing index should remain");
