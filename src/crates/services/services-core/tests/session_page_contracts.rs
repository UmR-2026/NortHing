...
use northhing_services_core::session::{
    build_session_metadata_page, SessionMetadata, SessionRelationship, SessionRelationshipKind, SessionStatus,
};
...
fn session_page_keeps_visible_top_level_order_and_attaches_children() {
    ...
    let page = build_session_metadata_page(vec![archived, child, parent_latest, parent_older], None, 1);
