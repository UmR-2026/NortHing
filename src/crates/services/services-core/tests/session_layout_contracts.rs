use northhing_services_core::session::SessionStorageLayout;
...
fn session_layout_preserves_legacy_file_names() {
    ...
    assert_eq!(layout.index_path(), root.path().join("sessions").join("index.json"));
    assert_eq!(
        layout.metadata_path("session-1"),
        root.path().join("sessions").join("session-1").join("metadata.json")
    );
    assert_eq!(
        layout.state_path("session-1"),
        root.path().join("sessions").join("session-1").join("state.json")
    );
    assert_eq!(
        layout.prompt_cache_path("session-1"),
        root.path().join("sessions").join("session-1").join("prompt_cache.json")
    );
    assert_eq!(
...
async fn session_layout_lists_indexed_turn_paths_in_numeric_order() {
    ...
    assert_eq!(
        indexed_paths.iter().map(|(index, _)| *index).collect::<Vec<_>>(),
        vec![2, 10]
    );
