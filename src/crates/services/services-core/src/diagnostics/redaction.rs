...
pub fn redact_diagnostic_log_text_with_report(input: &str) -> RedactedDiagnosticLog {
    ...
    RedactedDiagnosticLog { text, redaction_count }
}
...
fn redact_absolute_paths(input: &str) -> (String, usize) {
    let (input, windows_escaped_count) = replace_all_count(input, windows_escaped_path_re(), "<redacted path>");
...
fn bearer_token_re() -> &'static Regex {
    static RE: OnceLock<Regex> = OnceLock::new();
    RE.get_or_init(|| Regex::new(r#"(?i)\bBearer\s+[A-Za-z0-9._~+/=-]{8,}"#).expect("bearer token regex must compile"))
}
...
fn unix_path_re() -> &'static Regex {
    static RE: OnceLock<Regex> = OnceLock::new();
    RE.get_or_init(|| {
        Regex::new(r#"\b/(?:Users|home|workspace|tmp|var|private)/[^"'\s,}\]]+"#).expect("Unix path regex must compile")
    })
}
