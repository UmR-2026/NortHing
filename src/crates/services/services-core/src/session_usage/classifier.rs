...
pub fn classify_tool_usage(tool_name: &str, input: Option<&serde_json::Value>) -> UsageToolCategory {
    ...
    if is_shell_tool(&normalized_tool) {
        if input.and_then(extract_command).is_some_and(command_invokes_git) {
            return UsageToolCategory::Git;
        }
...
mod tests {
    ...
    fn classify_dedicated_git_tool_as_git() {
        assert_eq!(classify_tool_usage("git_status", None), UsageToolCategory::Git);
    }
