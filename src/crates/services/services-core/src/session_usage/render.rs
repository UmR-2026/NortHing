...
pub fn render_usage_report_terminal(report: &SessionUsageReport) -> String {
    ...
    out.push(format!(
        "Workspace: {}",
        report.workspace.path_label.as_deref().unwrap_or("unavailable")
    ));
    out.push(format!(
        "Scope: {} turns{}",
    ...
    out.push(format!("Coverage: {}", coverage_level_label(&report.coverage.level)));
    out.push(format!(
        "Time accounting: {} ({})",
    ...
    if !report.slowest.is_empty() {
        ...
        for span in &report.slowest {
            ...
            out.push(format!(
                "- {} [{}]: {}{}",
                if span.redacted { "redacted" } else { &span.label },
...
pub fn render_usage_report_markdown(report: &SessionUsageReport) -> String {
    ...
    out.push_str(&format!(
        "- Workspace: {}\n",
        escape_markdown(report.workspace.path_label.as_deref().unwrap_or("unavailable"))
    ));
    out.push_str(&format!(
        "- Scope: {} turns{}\n",
        report.scope.turn_count,
    ...
    if !report.models.is_empty() {
        let include_duration = report.models.iter().any(|model| model.duration_ms.is_some());
        out.push_str("## Models\n\n");
        if include_duration {
...
fn format_optional_duration(value: Option<u64>) -> String {
    value.map(format_duration).unwrap_or_else(|| "unavailable".to_string())
}
...
fn tool_category_label(category: &crate::session_usage::classifier::UsageToolCategory) -> &'static str {
    match category {
