...
impl ProcessManager {
    ...
    fn initialize_job(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let mut job_guard = self
            .job
            .lock()
            .map_err(|e| std::io::Error::other(format!("Failed to lock process manager job mutex: {}", e)))?;
        *job_guard = Some(job);
...
#[cfg(unix)]
pub async fn terminate_child_process_tree(child: &mut Child, graceful_timeout: Duration) -> io::Result<()> {
    let pid = child.id();
