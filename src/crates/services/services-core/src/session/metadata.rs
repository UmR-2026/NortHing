...
use super::types::{
    DialogTurnData, SessionMetadata, SessionRelationship, SessionRelationshipKind, StoredSessionIndexFile,
};
...
pub fn build_session_metadata(facts: SessionMetadataBuildFacts<'_>) -> SessionMetadata {
    ...
    let created_at = existing.map(|value| value.created_at).unwrap_or(facts.created_at_ms);
...
fn build_session_relationship(
    ...
    let kind = match session_kind {
        ...
        SessionKind::Standard => existing_relationship.as_ref().and_then(|value| value.kind.clone()),
    };
...
fn estimate_turn_message_count(turn: &DialogTurnData) -> usize {
    let assistant_text_count: usize = turn.model_rounds.iter().map(|round| round.text_items.len()).sum();
...
pub fn build_session_index_snapshot(
    ...
    let mut visible_sessions = metadata_list
        ...
        .collect::<Vec<_>>();
    visible_sessions.sort_by_key(|b| std::cmp::Reverse(b.last_active_at));

    let index =
        StoredSessionIndexFile::with_metadata_file_count(updated_at, visible_sessions.clone(), metadata_file_count);
...
pub fn upsert_session_index_entry(
    ...
    index.sessions.sort_by_key(|b| std::cmp::Reverse(b.last_active_at));
...
pub fn set_deep_review_run_manifest(metadata: &mut SessionMetadata, deep_review_run_manifest: Option<Value>) {
...
mod tests {
    ...
    fn relationship_and_manifest_mutations_preserve_other_metadata() {
        ...
        assert_eq!(metadata.deep_review_run_manifest, Some(json!({ "run": "manifest" })));
    ...
    fn deep_review_cache_mutation_preserves_manifest_and_relationship() {
        ...
        assert_eq!(metadata.deep_review_cache, Some(json!({ "packet": "review output" })));
        assert_eq!(metadata.deep_review_run_manifest, Some(json!({ "run": "manifest" })));
        assert_eq!(
            metadata.relationship.as_ref().and_then(|value| value.kind.clone()),
    ...
    fn build_session_metadata_preserves_existing_fields_and_legacy_relationship() {
        ...
        assert_eq!(built.deep_review_run_manifest, Some(json!({ "run": "manifest" })));
