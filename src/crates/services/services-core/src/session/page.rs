...
pub fn build_session_metadata_page(
    ...
    let visible_sessions = indexed_sessions
        .into_iter()
        .filter(|metadata| !metadata.should_hide_from_user_lists() && metadata.status != SessionStatus::Archived)
        .collect::<Vec<_>>();
    ...
    for metadata in visible_sessions {
        if let Some(parent_id) = session_parent_id(&metadata) {
            if visible_ids.contains(&parent_id) {
                children_by_parent.entry(parent_id).or_default().push(metadata);
                continue;
            }
        }
    ...
    for metadata in selected_top_level {
        ...
        if let Some(mut children) = children_by_parent.remove(&session_id) {
            children.sort_by_key(|b| std::cmp::Reverse(b.last_active_at));
...
fn session_metadata_page_offset(cursor: Option<&str>, top_level_sessions: &[SessionMetadata]) -> usize {
    ...
    if let Ok(parsed) = serde_json::from_str::<SessionMetadataPageCursor>(cursor) {
        if let Some(index) = top_level_sessions.iter().position(|metadata| {
            metadata.session_id == parsed.session_id && metadata.last_active_at == parsed.last_active_at
        }) {
            return index + 1;
        }
