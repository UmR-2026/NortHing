...
impl SessionStorageLayout {
    ...
    pub fn turn_path(&self, session_id: &str, turn_index: usize) -> PathBuf {
        self.turns_dir(session_id).join(format!("turn-{:04}.json", turn_index))
    }
    ...
    pub async fn list_indexed_turn_paths(&self, session_id: &str) -> io::Result<Vec<(usize, PathBuf)>> {
    ...
    pub async fn delete_indexed_turn_paths_from(&self, session_id: &str, start_index: usize) -> io::Result<usize> {
