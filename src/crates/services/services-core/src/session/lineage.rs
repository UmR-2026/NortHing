...
use super::types::{DialogTurnData, SessionMetadata, SessionRelationship, SessionRelationshipKind, SessionStatus};
...
fn extract_subagent_relationship(metadata: &SessionMetadata) -> Option<SubagentRelationshipFacts> {
    ...
    let kind = relationship.and_then(|value| value.kind.clone()).or_else(|| {
        custom_metadata
...
pub fn build_branched_session_metadata(facts: BranchSessionMetadataFacts<'_>) -> SessionMetadata {
    ...
    metadata.message_count = facts.branched_turns.iter().map(estimate_turn_message_count).sum();
    metadata.tool_call_count = facts.branched_turns.iter().map(DialogTurnData::count_tool_calls).sum();
...
mod tests {
    use super::*;
    use crate::session::{
        ModelRoundData, SessionMetadata, SessionRelationship, SessionRelationshipKind, TextItemData, ToolCallData,
    ...
    fn apply_session_lineage_sets_relationship_and_removes_legacy_projection() {
        ...
        assert_eq!(
            value.relationship.as_ref().and_then(|value| value.kind.clone()),
            Some(SessionRelationshipKind::DeepReview)
        );
        let custom_metadata = value.custom_metadata.expect("preserved metadata should remain");
    ...
    fn collect_hidden_subagent_cascade_returns_post_order_matches() {
        ...
        let cascade = collect_hidden_subagent_cascade(vec![root, grandchild, unrelated], "parent", &parent_turns);
    ...
    fn build_branched_session_metadata_resets_child_state_and_counts_turns() {
        ...
        source.tags = vec!["btw".to_string(), "kept".to_string(), "deep_review".to_string()];
        ...
        let custom_metadata = branched.custom_metadata.expect("fork metadata should exist");
