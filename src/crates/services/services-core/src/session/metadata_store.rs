...
use super::metadata::{build_session_index_snapshot, remove_session_index_entry, upsert_session_index_entry};
...
impl SessionMetadataStore {
    ...
    async fn scan_metadata_dirs(&self) -> Result<Vec<SessionMetadata>, SessionMetadataStoreError> {
        ...
        metadata_list.sort_by_key(|b| std::cmp::Reverse(b.last_active_at));
        Ok(metadata_list)
    }
    ...
    async fn rebuild_index_locked(&self) -> Result<Vec<SessionMetadata>, SessionMetadataStoreError> {
        let metadata_list = self.scan_metadata_dirs().await?;
        let (index, visible_sessions) = build_session_index_snapshot(metadata_list, current_unix_ms());
    ...
    async fn upsert_index_entry_locked(
        ...
        let index_path = self.index_path();
        let existing_index = self.read_json_optional::<StoredSessionIndexFile>(&index_path).await?;
    ...
    async fn remove_index_entry_locked(
        ...
        let existing_index = self.read_json_optional::<StoredSessionIndexFile>(&index_path).await?;
        let Some(index) =
            remove_session_index_entry(existing_index, session_id, metadata_file_count_delta, current_unix_ms())
        else {
            return Ok(());
        };
        self.write_json_atomic(&index_path, &index).await
    ...
    pub async fn list_metadata(&self) -> Result<Vec<SessionMetadata>, SessionMetadataStoreError> {
        ...
        let index_path = self.index_path();
        if let Some(index) = self.read_json_optional::<StoredSessionIndexFile>(&index_path).await? {
    ...
    pub async fn list_metadata_page(
        ...
        let _guard = lock.lock().await;
        let index_path = self.index_path();
        let indexed_sessions = if let Some(index) =
            self.read_json_optional::<StoredSessionIndexFile>(&index_path).await?
        {
    ...
    pub async fn list_metadata_including_internal(&self) -> Result<Vec<SessionMetadata>, SessionMetadataStoreError> {
    ...
    pub async fn save_metadata(&self, metadata: &SessionMetadata) -> Result<(), SessionMetadataStoreError> {
        ...
        if !metadata.should_hide_from_user_lists() {
            self.upsert_index_entry_locked(metadata, metadata_file_created).await
        } else {
            self.remove_index_entry_locked(&metadata.session_id, if metadata_file_created { 1 } else { 0 })
                .await
        }
    }

    pub async fn load_metadata(&self, session_id: &str) -> Result<Option<SessionMetadata>, SessionMetadataStoreError> {
    ...
    pub async fn delete_session_dir_and_index(&self, session_id: &str) -> Result<(), SessionMetadataStoreError> {
        let lock = self.get_index_lock().await;
        let _guard = lock.lock().await;
    ...
    async fn ensure_session_dir(&self, session_id: &str) -> Result<PathBuf, SessionMetadataStoreError> {
        let dir = self.session_dir(session_id);
...
mod tests {
    ...
    async fn metadata_store_rebuilds_stale_index_entries() {
        ...
        let listed = store.list_metadata().await.expect("list metadata");
        assert_eq!(
            listed.iter().map(|value| value.session_id.as_str()).collect::<Vec<_>>(),
            vec!["existing"]
        );
    }
    ...
    async fn metadata_store_rebuild_index_counts_hidden_metadata_files() {
        ...
        hidden.session_kind = northhing_core_types::SessionKind::Subagent;
        store.save_metadata(&hidden).await.expect("save hidden metadata");
    ...
    async fn metadata_store_hides_internal_sessions_from_visible_index() {
        ...
        assert!(store.list_metadata().await.expect("visible list").is_empty());
        assert_eq!(
            store
    ...
    async fn metadata_store_delete_session_updates_visible_index() {
        ...
        store
            ...
            .await
            .expect("save metadata");
        assert_eq!(store.list_metadata().await.expect("list before delete").len(), 1);
        ...
        assert!(store.load_metadata("session-a").await.expect("load").is_none());
        assert!(store.list_metadata().await.expect("list after delete").is_empty());
    }
}
