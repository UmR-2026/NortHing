...
fn resolve_login_shell_path_entries() -> Vec<PathBuf> {
    ...
    for shell in shell_candidates {
        ...
        if let Some(path_value) = read_path_from_login_shell(&shell) {
            let entries: Vec<PathBuf> = std::env::split_paths(&path_value).filter(|p| p.is_dir()).collect();
            if !entries.is_empty() {
...
/// # Example
/// ```rust
/// use northhing_services_core::system::check_command;
///
...
pub async fn run_command_simple(cmd: &str, args: &[String], cwd: Option<&str>) -> Result<CommandOutput, SystemError> {
