...
pub fn list_directory_entries(dir_path: &str, limit: usize) -> FileSystemResult<Vec<DirectoryListingEntry>> {
                    ...
                    if let Ok(entry_metadata) = fs::symlink_metadata(&entry_path) {
                        if !entry_metadata.file_type().is_symlink() {
                            queue.push_back(DirectoryListingEntry {
                                ...
                                depth: 1,
                                modified_time: entry_metadata.modified().unwrap_or(SystemTime::UNIX_EPOCH),
                            });
                        }
                    }
                }
    ...
    while !queue.is_empty() && result.len() < limit {
        ...
        for _ in 0..current_level_size {
            ...
            let folder_name = entry_path.file_name().and_then(|name| name.to_str()).unwrap_or("");
...
pub fn format_directory_listing(entries: &[DirectoryListingEntry], dir_path: &str) -> String {
            ...
            if let Some(rel_str) = rel_path.to_str() {
                ...
                let parent = if entry.is_dir {
                    if parts_for_parent.len() > 2 {
                        format!("{}/", parts_for_parent[..parts_for_parent.len() - 2].join("/"))
                    } else {
                        "/".to_string()
                    }
    ...
    fn format_tree(tree: &HashMap<String, Vec<TreeEntry>>, parent: &str, prefix: &str, result: &mut String) {
        if let Some(children) = tree.get(parent) {
            let count = children.len();
            for (i, child) in children.iter().enumerate() {
                let is_last = i == count - 1;
                let name = if child.is_dir {
                    let dir_name = child.path[..child.path.len() - 1].rsplit('/').next().unwrap_or("");
                    format!("{}/", dir_name)
                } else {
...
pub fn get_formatted_directory_listing(dir_path: &str, limit: usize) -> FileSystemResult<FormattedDirectoryListing> {
    ...
    Ok(FormattedDirectoryListing { reached_limit, text })
}
