...
use super::{
    FileContentSearchOptions, FileInfo, FileNameSearchOptions, FileOperationOptions, FileOperationService,
    FileReadResult, FileSearchOutcome, FileSearchProgressSink, FileSearchResult, FileTreeNode, FileTreeService,
    FileWriteResult,
};
...
impl FileSystemService {
    ...
    pub async fn scan_directory(&self, root_path: &str) -> FileSystemResult<DirectoryScanResult> {
        ...
        let (files, statistics) = self.file_tree_service.build_tree_with_stats(root_path).await?;
    ...
    /// Writes a file.
    pub async fn write_file(&self, file_path: &str, content: &str) -> FileSystemResult<FileWriteResult> {
    ...
    pub async fn delete_directory(&self, dir_path: &str, recursive: bool) -> FileSystemResult<()> {
        self.file_operation_service.delete_directory(dir_path, recursive).await
    }
    ...
    pub async fn find_files_by_extension(&self, root_path: &str, extension: &str) -> FileSystemResult<Vec<String>> {
    ...
    pub fn editor_sync_sha256_hex_from_raw_bytes(&self, bytes: &[u8]) -> String {
        self.file_operation_service.editor_sync_sha256_hex_from_raw_bytes(bytes)
    }
}
