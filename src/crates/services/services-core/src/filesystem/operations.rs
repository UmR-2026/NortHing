...
impl FileOperationService {
    ...
    pub async fn read_file(&self, file_path: &str) -> FileSystemResult<FileReadResult> {
        ...
        if !path.exists() {
            return Err(FileSystemError::service(format!("File does not exist: {}", file_path)));
        }

        if path.is_dir() {
            return Err(FileSystemError::service(format!("Path is a directory: {}", file_path)));
        }

        let metadata = fs::metadata(path)
            .await
            .map_err(|e| FileSystemError::service(format!("Failed to read file metadata: {}", e)))?;
    ...
    pub async fn editor_sync_content_sha256_hex(&self, file_path: &str) -> FileSystemResult<String> {
        let path = Path::new(file_path);
    ...
    pub async fn write_file(
        ...
        if let Some(parent) = path.parent() {
            fs::create_dir_all(parent)
                .await
                .map_err(|e| FileSystemError::service(format!("Failed to create parent directory: {}", e)))?;
        }
    ...
    pub async fn copy_file(&self, from: &str, to: &str) -> FileSystemResult<u64> {
        ...
        if from_path.is_dir() {
            return Err(FileSystemError::service("Cannot copy directory as file".to_string()));
        }

        if let Some(parent) = to_path.parent() {
            fs::create_dir_all(parent)
                .await
                .map_err(|e| FileSystemError::service(format!("Failed to create target directory: {}", e)))?;
        }
    ...
    pub async fn delete_file(&self, file_path: &str) -> FileSystemResult<()> {
        ...
        if path.is_dir() {
            return Err(FileSystemError::service("Cannot delete directory as file".to_string()));
        }
    ...
    pub async fn delete_directory(&self, dir_path: &str, recursive: bool) -> FileSystemResult<()> {
        ...
        if recursive {
            fs::remove_dir_all(path)
                .await
                .map_err(|e| FileSystemError::service(format!("Failed to delete directory recursively: {}", e)))?;
        } else {
            fs::remove_dir(path)
                .await
                .map_err(|e| FileSystemError::service(format!("Failed to delete directory: {}", e)))?;
        }
        ...
        if let Some(allowed_extensions) = &self.allowed_extensions {
            if let Some(ext) = path.extension().and_then(|e| e.to_str()) {
                if !allowed_extensions.contains(&ext.to_lowercase()) {
                    return Err(FileSystemError::service(format!("File extension not allowed: {}", ext)));
                }
            }
        }
            ...
            if let Some(parent) = path.parent() {
                if parent.exists() {
                    let metadata = fs::metadata(parent).await.map_err(|e| {
                        FileSystemError::service(format!("Failed to check parent directory permissions: {}", e))
                    })?;

                    if metadata.permissions().readonly() {
                        return Err(FileSystemError::service("Parent directory is read-only".to_string()));
                    }
                }
            }
        }
