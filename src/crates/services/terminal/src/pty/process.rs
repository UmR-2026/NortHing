...
impl FlowControl {
    ...
    pub fn acknowledge_data(&self, char_count: usize) {
        ...
        if self.is_paused.load(Ordering::Relaxed) && self.unacknowledged_chars.load(Ordering::Relaxed) < LOW_WATER_MARK
        {
    ...
    pub fn track_output(&self, char_count: usize) -> bool {
        let new_count = self.unacknowledged_chars.fetch_add(char_count, Ordering::Relaxed) + char_count;
...
pub fn spawn_pty(
    ...
    // Send ready event
    let _ = event_tx.try_send(PtyEvent::Ready { pid, cwd: cwd.clone() });
        ...
        while let Some(cmd) = command_rx.recv().await {
            match cmd {
                ...
                InternalCommand::Resize { cols, rows } => {
                    ...
                    #[cfg(windows)]
                    {
                        ...
                        if is_git_bash_shell && cols == 0 && rows == 0 && !delayed_resize_triggered {
                            debug!("Git Bash with 0x0 size detected, using delayed resize mechanism");
                            ...
                            tokio::spawn(async move {
                                ...
                                let _ = event_tx_delayed.try_send(PtyEvent::ResizeCompleted { cols: 80, rows: 24 });
                            });
                            continue;
                        }
                ...
                InternalCommand::Shutdown { immediate } => {
                    ...
                    if !immediate {
                        // Wait for data flush
                        tokio::time::sleep(tokio::time::Duration::from_millis(shutdown::DATA_FLUSH_TIMEOUT_MS)).await;
                    }
    ...
    let controller = PtyController { command_tx, has_exited };
...
fn apply_sanitized_environment(cmd: &mut CommandBuilder, overlay_env: &std::collections::HashMap<String, String>) {
    cmd.env_clear();
...
mod tests {
    ...
    fn strips_tauri_host_configuration_from_parent_env() {
        ...
        assert!(is_tauri_host_env("TAURI_ANDROID_PACKAGE_NAME_PREFIX".as_ref()));
    }
