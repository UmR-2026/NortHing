...
pub struct PtyService {
    ...
    // reason: config field is held for the upcoming config-driven PTY knobs (e.g. default shell, scrollback size); today's PTY service uses hardcoded defaults
            ...
            while let Some(event) = events.recv().await {
                match event {
                    ...
                    PtyEvent::Ready { pid, cwd } => {
                        let _ = event_tx.send(PtyServiceEvent::ProcessReady { id, pid, cwd }).await;
                    }
                    PtyEvent::Exit { exit_code } => {
                        info!("PTY process {} exited with code {:?}", id, exit_code);
                        let _ = event_tx.send(PtyServiceEvent::ProcessExit { id, exit_code }).await;
                        break;
                    }
                    ...
                    PtyEvent::ResizeCompleted { cols, rows } => {
                        let _ = event_tx.send(PtyServiceEvent::ResizeCompleted { id, cols, rows }).await;
                    }
                }
            }
