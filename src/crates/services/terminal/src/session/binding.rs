...
impl TerminalSessionBinding {
    ...
    pub async fn get_or_create(&self, owner_id: &str, options: TerminalBindingOptions) -> TerminalResult<String> {
        ...
        let session_manager =
            session_manager().ok_or_else(|| TerminalError::Session("SessionManager not initialized".to_string()))?;
        ...
        self.bindings.insert(owner_id.to_string(), terminal_session_id.clone());
    ...
    pub async fn remove(&self, owner_id: &str) -> TerminalResult<()> {
        ...
        if let Some(terminal_session_id) = self.unbind(owner_id) {
            if let Err(e) = session_manager.close_session(&terminal_session_id, false).await {
                warn!("Failed to close terminal session {}: {}", terminal_session_id, e);
            }
        }
        ...
        if let Some((_, bg_sessions)) = self.background_bindings.remove(owner_id) {
            for bg_session_id in bg_sessions {
                if let Err(e) = session_manager.close_session(&bg_session_id, false).await {
                    warn!("Failed to close background terminal session {}: {}", bg_session_id, e);
                }
            }
        }
