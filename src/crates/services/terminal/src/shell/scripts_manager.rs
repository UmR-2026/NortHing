...
impl ScriptsManager {
    ...
    /// - Linux: `~/.cache/northhing_terminal/scripts/`
    /// - Windows: `%LOCALAPPDATA%\northhing_terminal\scripts\`
    /// - macOS: `~/Library/Caches/northhing_terminal/scripts/`
    ...
    pub fn ensure_scripts(&self) -> io::Result<bool> {
        ...
        info!("Generating shell integration scripts at {:?}", self.scripts_dir);
    ...
    pub fn get_script_path(&self, shell_type: &ShellType) -> Option<PathBuf> {
        match shell_type {
            ...
            ShellType::PowerShell | ShellType::PowerShellCore => Some(self.scripts_dir.join("powershell.ps1")),
            _ => None,
        }
    }
