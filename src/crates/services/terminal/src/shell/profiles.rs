...
// reason: ShellProfileManager struct is reserved for the upcoming shell-profile management UI (today profiles are derived ad-hoc from detected shells)
...
// reason: ShellProfileManager impl is reserved for the upcoming shell-profile management API (today the struct and its impl are paired for future use)
impl ShellProfileManager {
    ...
    pub fn default_profile(&self) -> Option<&ShellProfile> {
        self.default_profile_id.as_ref().and_then(|id| self.profiles.get(id))
    }
