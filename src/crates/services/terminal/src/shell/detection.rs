...
impl ShellDetector {
    ...
    fn detect_pwsh() -> Option<DetectedShell> {
        ...
        let candidates = if cfg!(windows) {
            vec![PathBuf::from(std::env::var("ProgramFiles").unwrap_or_default())
                .join("PowerShell")
                .join("7")
                .join("pwsh.exe")]
        } else {
    ...
    pub fn detect_git_bash() -> Option<DetectedShell> {
                ...
                if let Some(dir_name) = parent_dir.file_name().and_then(|n| n.to_str()) {
                    if dir_name.eq_ignore_ascii_case("cmd") || dir_name.eq_ignore_ascii_case("bin") {
                        if let Some(git_install_dir) = parent_dir.parent() {
                            ...
                            let usr_bin_bash = git_install_dir.join("usr").join("bin").join("bash.exe");
                            if bin_bash.exists() {
        ...
        let base_dirs: Vec<Option<String>> = vec![
            ...
            std::env::var("LOCALAPPDATA").map(|p| format!("{}\\Programs", p)).ok(),
        ];
    ...
    // reason: get_shell_version() is reserved for the upcoming shell-version-aware feature gating (e.g. only enable POSIX on bash >= 5)
    #[allow(dead_code)]
