...
pub enum TerminalEvent {
    ...
    Ready { session_id: String, pid: u32, cwd: String },
    ...
    ShellTypeChanged { session_id: String, shell_type: ShellType },
    ...
    /// Terminal resized
    Resized { session_id: String, cols: u16, rows: u16 },
...
impl TerminalEvent {
    ...
    pub fn is_data_event(&self) -> bool {
        matches!(self, TerminalEvent::Data { .. } | TerminalEvent::BinaryData { .. })
    }
}
...
impl TerminalEventEmitter {
    ...
    /// Emit an event
    pub async fn emit(&self, event: TerminalEvent) -> Result<(), mpsc::error::SendError<TerminalEvent>> {
        self.tx.send(event).await
    }

    /// Try to emit an event without blocking
    pub fn try_emit(&self, event: TerminalEvent) -> Result<(), mpsc::error::TrySendError<TerminalEvent>> {
