...
When this skill is invoked by northhing Team Mode, treat this skill as the product-thinking methodology and use existing Task sub-agents only for independent discovery that improves the design doc.
...
4. **List existing design docs for this project:**
   ...
   ls -t $HOME/.northhing/team/projects/$SLUG/*-design-*.md 2>/dev/null
   ```
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
grep -li "<keyword1>\|<keyword2>\|<keyword3>" $HOME/.northhing/team/projects/$SLUG/*-design-*.md 2>/dev/null
```
...
5. **Cross-model synthesis:** After presenting the second opinion output, provide 3-5 bullet synthesis:
   - Where northhing agrees with the second opinion
   - Where northhing disagrees and why
   - Whether the challenged premise changes northhing's recommendation
...
Use northhing built-in image/design capability when available. Do not install, build,
or call an external northhing image/design capability. If visual generation is unavailable in the
...
```bash
SLUG=$(basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)" | tr -cd A-Za-z0-9._-)
_DESIGN_DIR=$HOME/.northhing/team/projects/$SLUG/designs/mockup-$(date +%Y%m%d)
mkdir -p "$_DESIGN_DIR"
...
```bash
northhing image/design capability variants --brief "<assembled brief>" --count 3 --output-dir "$_DESIGN_DIR/"
```
...
If `northhing image/design capability serve` is not available or fails, fall back to AskUserQuestion:
...
2. Generate new variants with `northhing image/design capability iterate` or `northhing image/design capability variants` using updated brief
3. Create new board with `northhing image/design capability compare`
...
```bash
northhing browser/computer-use goto "file://$SKETCH_FILE"
northhing browser/computer-use screenshot /tmp/gstack-sketch.png
```

If `northhing browser/computer-use` is not available (northhing browser/computer-use tooling not set up), skip the render step. Tell the
user: "Use northhing browser/computer-use tooling for the visual sketch when it is available. If unavailable, skip the render step and keep the HTML sketch artifact."
...
```bash
mkdir -p "${northhing_TEAM_HOME:-$HOME/.northhing/team}"
```
...
```bash
echo '{"date":"TIMESTAMP","mode":"MODE","project_slug":"SLUG","signal_count":N,"signals":SIGNALS_ARRAY,"design_doc":"DOC_PATH","assignment":"ASSIGNMENT_TEXT","resources_shown":[],"topics":TOPICS_ARRAY}' >> "${northhing_TEAM_HOME:-$HOME/.northhing/team}/builder-profile.jsonl"
```
...
echo '{"skill":"office-hours","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","iterations":ITERATIONS,"issues_found":FOUND,"issues_fixed":FIXED,"remaining":REMAINING,"quality_score":SCORE}' >> $HOME/.northhing/team/analytics/spec-review.jsonl 2>/dev/null || true
```
...
Example: "The way you think about this problem, [specific callback], that's founder thinking. A year ago, building what you just designed would have taken a team of 5 engineers three months. Today you can build it this weekend with northhing. The engineering barrier is gone. What remains is taste, and you just demonstrated that."
...
**Builder Journey Summary** (session 5+): Auto-generate `$HOME/.northhing/team/builder-journey.md`
...
```bash
open "${northhing_TEAM_HOME:-$HOME/.northhing/team}/builder-journey.md"
```
...
Auto-generate updated `$HOME/.northhing/team/builder-journey.md` with narrative arc. Open it.
...
```bash
echo '{"date":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","mode":"resources","project_slug":"'"${SLUG:-unknown}"'","signal_count":0,"signals":[],"design_doc":"","assignment":"","resources_shown":["URL1","URL2","URL3"],"topics":[]}' >> "${northhing_TEAM_HOME:-$HOME/.northhing/team}/builder-profile.jsonl"
```

2. Log the selection to analytics:
```bash
mkdir -p $HOME/.northhing/team/analytics
echo '{"skill":"office-hours","event":"resources_shown","count":NUM_RESOURCES,"categories":"CAT1,CAT2","ts":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'"}' >> $HOME/.northhing/team/analytics/skill-usage.jsonl 2>/dev/null || true
```
