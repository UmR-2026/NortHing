...
When this skill is invoked by northhing Team Mode, this skill supplies the pre-landing review lens. Use existing Task sub-agents for independent diff review tracks, then consolidate findings in the main Team session.
...
2. If on the base branch, output: **"Nothing to review 鈥?you're on the base branch or have no changes against it."** and stop.
...
**If no PR exists, `gh` fails, API returns an error, or there are zero Greptile comments:** Skip this step silently. Greptile integration is additive 鈥?the review works without it.

**If Greptile comments are found:** Store the classifications (VALID & ACTIONABLE, VALID BUT ALREADY FIXED, FALSE POSITIVE, SUPPRESSED) 鈥?you will need them in Step 5.
...
Follow the output format specified in the checklist. Respect the suppressions 鈥?do NOT flag items listed in the "DO NOT flag" section.
...
## Step 4.5: Review Army 鈥?Specialist Dispatch
...
**If DIFF_LINES < 50:** Skip all specialists. Print: "Small diff ($DIFF_LINES lines) 鈥?specialists skipped." Continue to Step 5.
...
**Every finding gets action 鈥?not just critical ones.**
...
- If you claim "this pattern is safe" 鈫?cite the specific line proving safety
- If you claim "this is handled elsewhere" 鈫?read and cite the handling code
- If you claim "tests cover this" 鈫?name the test file and method
- Never say "likely handled" or "probably tested" 鈥?verify or flag as unknown
...
1. **VALID & ACTIONABLE comments:** These are included in your findings 鈥?they follow the Fix-First flow (auto-fixed if mechanical, batched into ASK if not) (A: Fix it now, B: Acknowledge, C: False positive). If the user chooses A (fix), reply using the **Fix reply template** from greptile-triage.md (include inline diff + explanation). If the user chooses C (false positive), reply using the **False Positive reply template** (include evidence + suggested re-rank), save to both per-project and global greptile-history.
...
2. **FALSE POSITIVE comments:** Present each one via AskUserQuestion:
   ...
   - Options:
     ...
     - C) Ignore 鈥?don't reply, don't fix

   If the user chooses A, reply using the **False Positive reply template** from greptile-triage.md (include evidence + suggested re-rank), save to both per-project and global greptile-history.

3. **VALID BUT ALREADY FIXED comments:** Reply using the **Already Fixed reply template** from greptile-triage.md 鈥?no AskUserQuestion needed:
   - Include what was done and the fixing commit SHA
   - Save to both per-project and global greptile-history

4. **SUPPRESSED comments:** Skip silently 鈥?these are known false positives from previous triage.
...
This is informational only 鈥?never critical. The fix action is `/document-release`.
...
If A: address the findings. Re-run `northhing Task outside-voice review` to verify.
...
- **Fix-first, not read-only.** AUTO-FIX items are applied directly. ASK items are only applied after user approval. Never commit, push, or create PRs 鈥?that's /ship's job.
