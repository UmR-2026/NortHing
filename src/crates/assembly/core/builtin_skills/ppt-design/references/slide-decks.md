...
| 并行开发 | ❌ 一个文件，多 agent 改会— 突 | ✅ 多 agent 可并行做不同页，零— 突 merge |
...
把每张 slide 的任务拆给不同 agent，同时跑— HTML 文件彼此独立，merge 时没有— 突。长 deck 用这种并行方式能把制作时间压到 1/N。
...
**多文件：某页样式和别页— 突**
→ 不可能（iframe 隔离）。如果感觉— 突，那是缓存— Cmd+Shift+R 强刷。
