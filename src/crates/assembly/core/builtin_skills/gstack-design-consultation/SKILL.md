...
When this skill is invoked by northhing Team Mode, this skill supplies the design-system methodology. Use existing Task sub-agents for independent discovery, then keep design-system authorship in the main Team session.
...
ls $HOME/.northhing/team/projects/$SLUG/*office-hours* 2>/dev/null | head -5
...
If the northhing browser/computer-use tooling is available (`northhing browser/computer-use` is set), visit the top 3-5 sites in the space and capture visual evidence:

```bash
northhing browser/computer-use goto "https://example-site.com"
northhing browser/computer-use screenshot "/tmp/design-research-site-name.png"
northhing browser/computer-use snapshot
```
...
This phase generates visual previews of the proposed design system. Two paths depending on whether the northhing image/design capability is available.
...
```bash
SLUG=$(basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)" | tr -cd A-Za-z0-9._-)
_DESIGN_DIR=$HOME/.northhing/team/projects/$SLUG/designs/design-system-$(date +%Y%m%d)
mkdir -p "$_DESIGN_DIR"
...
- Use `northhing image/design capability extract --image "$_DESIGN_DIR/variant-<CHOSEN>.png"` to analyze the approved mockup and extract design tokens (colors, typography, spacing) that will populate DESIGN.md in Phase 6. This grounds the design system in what was actually approved visually, not just what was described in text.
- If the user wants to iterate further: `northhing image/design capability iterate --feedback "<user's feedback>" --output "$_DESIGN_DIR/refined.png"`
