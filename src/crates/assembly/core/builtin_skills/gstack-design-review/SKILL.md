...
When this skill is invoked by northhing Team Mode, this skill supplies the live design-audit methodology. Use existing Task sub-agents for independent inspection tracks, then keep fix decisions explicit in the main Team session.
...
REPORT_DIR=$HOME/.northhing/team/projects/$SLUG/designs/design-audit-$(date +%Y%m%d)
...
northhing browser/computer-use js "JSON.stringify([...new Set([...document.querySelectorAll('*')].slice(0,500).map(e => getComputedStyle(e).fontFamily))])"

# Color palette in use
northhing browser/computer-use js "JSON.stringify([...new Set([...document.querySelectorAll('*')].slice(0,500).flatMap(e => [getComputedStyle(e).color, getComputedStyle(e).backgroundColor]).filter(c => c !== 'rgba(0, 0, 0, 0)'))])"

# Heading hierarchy
northhing browser/computer-use js "JSON.stringify([...document.querySelectorAll('h1,h2,h3,h4,h5,h6')].map(h => ({tag:h.tagName, text:h.textContent.trim().slice(0,50), size:getComputedStyle(h).fontSize, weight:getComputedStyle(h).fontWeight})))"

# Touch target audit (find undersized interactive elements)
northhing browser/computer-use js "JSON.stringify([...document.querySelectorAll('a,button,input,[role=button]')].filter(e => {const r=e.getBoundingClientRect(); return r.width>0 && (r.width<44||r.height<44)}).map(e => ({tag:e.tagName, text:(e.textContent||'').trim().slice(0,30), w:Math.round(e.getBoundingClientRect().width), h:Math.round(e.getBoundingClientRect().height)})).slice(0,20))"

# Performance baseline
northhing browser/computer-use perf
```
...
```bash
northhing browser/computer-use goto <url>
northhing browser/computer-use snapshot -i -a -o "$REPORT_DIR/screenshots/{page}-annotated.png"
northhing browser/computer-use responsive "$REPORT_DIR/screenshots/{page}"
northhing browser/computer-use console --errors
northhing browser/computer-use perf
```
...
```bash
northhing browser/computer-use url
```
...
```bash
northhing browser/computer-use snapshot -i
northhing browser/computer-use click @e3           # perform action
northhing browser/computer-use snapshot -D          # diff to see what changed
```
...
**Local:** `.northhing/team/design-reports/design-audit-{domain}-{YYYY-MM-DD}.md`

**Project-scoped:**
```bash
SLUG=$(basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)" | tr -cd A-Za-z0-9._-) && mkdir -p $HOME/.northhing/team/projects/$SLUG
```
Write to: `$HOME/.northhing/team/projects/{slug}/{user}-{branch}-design-audit-{datetime}.md`
...
### 8a.5. Target Mockup (if northhing image/design capability is available)

If the northhing image/design capability is available and the finding involves visual layout, hierarchy, or spacing (not just a CSS value fix like wrong color or font-size), generate a target mockup showing what the corrected version should look like:

```bash
northhing image/design capability generate --brief "<description of the page/component with the finding fixed, referencing DESIGN.md constraints>" --output "$REPORT_DIR/screenshots/finding-NNN-target.png"
```
...
```bash
northhing browser/computer-use goto <affected-url>
northhing browser/computer-use screenshot "$REPORT_DIR/screenshots/finding-NNN-after.png"
northhing browser/computer-use console --errors
northhing browser/computer-use snapshot -D
```
...
SLUG=$(basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)" | tr -cd A-Za-z0-9._-) && mkdir -p $HOME/.northhing/team/projects/$SLUG
```
Write a one-line summary to `$HOME/.northhing/team/projects/{slug}/{user}-{branch}-design-audit-{datetime}.md` with a pointer to the full report in `$REPORT_DIR`.
