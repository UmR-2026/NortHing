...
fn embed_agents_prompt_data() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let shared_prompt_path = Path::new(&manifest_dir).join("src").join("agentic").join("prompts");
...
fn gen_key(relative_path: &str, prefix: &str) -> String {
    format!(
        "{}{}",
        prefix,
        relative_path.trim_end_matches(".txt").trim_end_matches(".md")
    )
}

// Read prompts and add prefix
// reason: read_prompts_with_prefix() is a build-time helper reserved for the upcoming prompt-template path-rewriting feature; not yet wired into the build pipeline
#[allow(dead_code)]
fn read_prompts_with_prefix(
    ...
    for entry in fs::read_dir(current_dir)? {
        ...
        if path.is_dir() {
            ...
            let relative_path = path.strip_prefix(base_dir)?.to_string_lossy().replace('\\', "/"); // Use forward slash
...
fn generate_embedded_prompts_code(
    ...
    // reason: emitted helper is reserved for the upcoming runtime prompt-discovery API (today's prompt loader reads by name directly)
    writeln!(file, "#[allow(dead_code)]")?;
    writeln!(file, "pub fn get_all_embedded_prompt_names() -> Vec<&'static str> {{")?;
