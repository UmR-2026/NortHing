...
pub fn builtin_agent_specs() -> Vec<BuiltinAgentSpec> {
    runtime_agents::builtin_agent_definition_specs()
        .into_iter()
        .filter_map(|spec| {
            let factory = match builtin_agent_factory(spec.id) {
                Some(f) => f,
                None => {
                    // A builtin agent ID was registered (in runtime_agents) without a matching
                    // legacy `Agent` factory here. Skip rather than panic so the rest of the
                    // registry still loads; the gap will surface in diagnostics / tests.
                    warn!("missing legacy Agent factory for builtin agent {}; skipping", spec.id);
                    return None;
                }
            };
            Some(BuiltinAgentSpec {
                factory,
...
fn builtin_agent_factory(id: &str) -> Option<fn() -> Arc<dyn Agent>> {
    let factory: fn() -> Arc<dyn Agent> = match id {
