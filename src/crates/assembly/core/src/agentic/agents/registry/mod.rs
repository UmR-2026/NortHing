...
impl AgentRegistry {
    ...
    fn read_project_subagents(&self) -> std::sync::RwLockReadGuard<'_, HashMap<PathBuf, HashMap<String, AgentEntry>>> {
    ...
    pub fn has_subagent(&self, agent_id: &str, source: SubAgentSource) -> bool {
        if self
            .read_agents()
            .get(agent_id)
            .is_some_and(|e| e.category == AgentCategory::SubAgent && e.subagent_source == Some(source))
        {
            return true;
        }
        ...
        self.read_project_subagents().values().any(|entries| {
            entries
                .get(agent_id)
                .is_some_and(|entry| entry.category == AgentCategory::SubAgent && entry.subagent_source == Some(source))
        })
    }
}
