...
use super::support::{
    get_mode_configs, get_subagent_overrides, load_project_subagent_overrides_local, merge_dynamic_mcp_tools,
};
use super::AgentRegistry;
use crate::agentic::agents::registry::types::{is_review_agent_entry, AgentEntry};
use crate::agentic::agents::{
    mode_presentation_rank, resolve_mode_config_profile_id, AgentCategory, AgentInfo, AgentToolPolicy,
...
use northhing_agent_runtime::agents::subagent_source_presentation_rank;
...
impl AgentRegistry {
    ...
    pub async fn get_agent_tool_policy(&self, agent_type: &str, workspace_root: Option<&Path>) -> AgentToolPolicy {
        ...
        match entry.category {
            AgentCategory::Mode => {
                ...
                let allowed_tool_set: HashSet<&str> = allowed_tools.iter().map(String::as_str).collect();
    ...
    pub async fn get_modes_info(&self) -> Vec<AgentInfo> {
        ...
        let mut result: Vec<AgentInfo> = map
            ...
            .collect();
        drop(map);
        result.sort_by_key(|a| mode_presentation_rank(&a.id));
        result
    }
    ...
    fn entry_is_visible_for_query(
        ...
        let availability = resolve_availability(entry, query.parent_agent_type, project_overrides, user_overrides);
        ...
        match query.list_scope {
            ...
            SubagentListScope::TaskVisible => {
                entry.visibility_policy.show_in_global_registry
                    || entry.visibility_policy.can_access_from_parent(query.parent_agent_type)
            }
        }
    }
    ...
    pub async fn get_subagents_for_query(&self, query: &SubagentQueryContext<'_>) -> Vec<AgentInfo> {
        ...
        let project_overrides = match query.workspace_root {
            Some(workspace_root) => load_project_subagent_overrides_local(workspace_root).await.ok(),
            None => None,
        };
        let map = self.read_agents();
        let mut result: Vec<AgentInfo> = map
            .values()
            .filter(|entry| Self::entry_is_visible_for_query(entry, query, project_overrides.as_ref(), &user_overrides))
            .map(|e| {
                let mut agent_info = AgentInfo::from_agent_entry(e);
                let availability =
                    resolve_availability(e, query.parent_agent_type, project_overrides.as_ref(), &user_overrides);
            ...
            if let Some(project_entries) = self.read_project_subagents().get(workspace_root) {
                result.extend(
                    project_entries
                        .values()
                        .filter(|entry| {
                            Self::entry_is_visible_for_query(entry, query, project_overrides.as_ref(), &user_overrides)
                        })
                        .map(|entry| {
    ...
    pub async fn can_parent_access_subagent(
        ...
        self.find_agent_entry(subagent_id, workspace_root).is_some_and(|entry| {
            Self::entry_is_visible_for_query(&entry, &query, project_overrides.as_ref(), &user_overrides)
        })
    }
}
