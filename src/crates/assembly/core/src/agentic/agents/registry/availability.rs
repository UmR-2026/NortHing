...
use northhing_agent_runtime::agents::{
    ...
    ResolvedSubagentAvailability, SubagentOverrideLayers as ResolvedOverrideLayers, SubagentOverrideState,
};
    ...
    match entry.subagent_source {
        Some(SubAgentSource::Project) => ResolvedOverrideLayers {
            project_override: project_overrides
                .and_then(|overrides| subagent_override_for_parent(overrides, parent_agent_type, &subagent_key))
...
pub fn resolve_availability(
    ...
    resolve_subagent_availability(subagent_source_kind(entry.subagent_source), default_enabled, layers)
}

pub fn prune_override_config(overrides: &mut AgentSubagentOverrideConfig, parent_agent_type: &str, subagent_key: &str) {
...
mod tests {
    ...
    fn builtin_and_user_subagents_only_use_global_overrides() {
        ...
        let builtin_key =
            subagent_key_for(builtin_entry.subagent_source, builtin_entry.agent.as_ref()).expect("builtin key");
        ...
        assert_eq!(builtin_layers.user_override, Some(SubagentOverrideState::Enabled));

        let user_entry = make_entry(SubAgentSource::User, "UserScout");
        let user_key = subagent_key_for(user_entry.subagent_source, user_entry.agent.as_ref()).expect("user key");
        ...
        assert_eq!(user_layers.user_override, Some(SubagentOverrideState::Enabled));
    }

    #[test]
    fn project_subagents_only_use_project_overrides() {
        let entry = make_entry(SubAgentSource::Project, "ProjectScout");
        let key = subagent_key_for(entry.subagent_source, entry.agent.as_ref()).expect("project key");
        ...
        assert_eq!(layers.project_override, Some(SubagentOverrideState::Disabled));
