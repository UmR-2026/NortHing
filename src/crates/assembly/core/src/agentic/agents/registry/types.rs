...
use crate::agentic::agents::registry::visibility::{SubagentVisibilityPolicy, SubagentVisibilitySummary};
use crate::agentic::agents::{
    mode_config_profile_label, mode_config_profile_member_mode_ids, resolve_mode_config_profile_id, Agent,
    AgentToolPolicyOverrides,
};
...
use northhing_agent_runtime::prompt_cache::prompt_cache_scope_key;
...
pub fn subagent_key_for(source: Option<SubAgentSource>, agent: &dyn Agent) -> Option<String> {
    ...
    let slot = match source {
        ...
        SubAgentSource::Project => {
            ...
            match custom.kind {
                CustomSubagentKind::Project => "northhing",
                CustomSubagentKind::User => "northhing",
            }
        }
        SubAgentSource::User => {
            let custom = agent.as_any().downcast_ref::<CustomSubagent>()?;
...
impl AgentInfo {
    pub(crate) fn from_agent_entry(entry: &AgentEntry) -> Self {
        ...
        let config_profile_id =
            (entry.category == AgentCategory::Mode).then(|| resolve_mode_config_profile_id(agent.id()).into_owned());
        ...
        let path = agent.as_any().downcast_ref::<CustomSubagent>().map(|c| c.path.clone());
        ...
        AgentInfo {
            ...
            path,
            model,
            visibility: (entry.category == AgentCategory::SubAgent).then(|| entry.visibility_policy.summary()),
        }
    }
}
