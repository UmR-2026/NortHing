...
impl AgentRegistry {
    ...
    pub async fn load_custom_subagents(&self, workspace_root: &Path) {
        ...
        let custom = load_custom_subagent_definitions(&custom_subagent_discovery_roots(workspace_root));
        for load_error in custom.errors {
            let error = NortHingError::Agent(load_error.error);
            error!(
        ...
        map.retain(|_, entry| {
            !(entry.category == AgentCategory::SubAgent && entry.subagent_source == Some(SubAgentSource::User))
        });
    ...
    async fn get_valid_model_ids() -> Vec<String> {
        let mut valid_models: Vec<String> = if let Ok(config_service) = GlobalConfigManager::service().await {
            config_service
                .get_ai_models()
                .await
                .unwrap_or_default()
    ...
    fn validate_custom_subagent(
        ...
        let valid_tools_set: std::collections::HashSet<&str> = valid_tools.iter().map(|s| s.as_str()).collect();
        let (valid, invalid): (Vec<_>, Vec<_>) = original_tools
            .into_iter()
            .partition(|t| valid_tools_set.contains(t.as_str()));
        if !invalid.is_empty() {
            warn!("[Subagent {}] Invalid tools filtered out: {:?}", agent_id, invalid);
        }
        if subagent.review {
            subagent.readonly = true;
            let readonly_tools_set: std::collections::HashSet<&str> =
                readonly_tools.iter().map(|s| s.as_str()).collect();
            let (review_tools, writable_tools): (Vec<_>, Vec<_>) =
                valid.into_iter().partition(|t| readonly_tools_set.contains(t.as_str()));
    ...
    fn ensure_review_tools_are_readonly(
        ...
        let readonly_tools_set: std::collections::HashSet<&str> = readonly_tools.iter().map(|s| s.as_str()).collect();
        ...
        Err(NortHingError::agent(format!(
    ...
    pub fn update_and_save_custom_subagent_config(
        ...
        let entry = entries
            .get_mut(agent_id)
            .ok_or_else(|| NortHingError::agent(format!("Subagent not found: {}", agent_id)))?;

        Self::update_custom_entry_config(agent_id, entry, model)
    }

    fn update_custom_entry_config(agent_id: &str, entry: &mut AgentEntry, model: Option<String>) -> NortHingResult<()> {
        if entry.category != AgentCategory::SubAgent {
            return Err(NortHingError::agent(format!("Agent '{}' is not a subagent", agent_id)));
        }

        let config = entry
            .custom_config
            .as_mut()
            .ok_or_else(|| NortHingError::agent(format!("Subagent '{}' is not a custom subagent", agent_id)))?;
        ...
        let custom_subagent = entry.agent.as_any().downcast_ref::<CustomSubagent>().ok_or_else(|| {
            NortHingError::agent(format!("Failed to downcast agent '{}' to CustomSubagent", agent_id))
        })?;
    ...
    pub async fn get_custom_subagent_detail(
        ...
        agent_id: &str,
        workspace_root: Option<&Path>,
    ) -> NortHingResult<CustomSubagentDetail> {
    ...
    fn get_custom_subagent_detail_inner(
        ...
        if entry.subagent_source == Some(SubAgentSource::Builtin) {
            return Err(NortHingError::agent(
                "Built-in subagents cannot be edited here".to_string(),
            ));
        }
        let custom =
            entry.agent.as_any().downcast_ref::<CustomSubagent>().ok_or_else(|| {
                NortHingError::agent(format!("Subagent '{}' is not a custom subagent file", agent_id))
            })?;
    ...
    pub async fn update_custom_subagent_definition(
        ...
        if entry.subagent_source == Some(SubAgentSource::Builtin) {
            return Err(NortHingError::agent("Built-in subagents cannot be edited".to_string()));
        }
        let old =
            entry.agent.as_any().downcast_ref::<CustomSubagent>().ok_or_else(|| {
                NortHingError::agent(format!("Subagent '{}' is not a custom subagent file", agent_id))
        ...
        let valid_models = Self::get_valid_model_ids().await;
        Self::validate_custom_subagent(&mut new_subagent, &valid_tools, &readonly_tools, &valid_models);
    ...
    fn replace_custom_subagent_entry(
        ...
        if map.contains_key(agent_id) {
            ...
            if old_entry.subagent_source == Some(SubAgentSource::Builtin) {
                return Err(NortHingError::agent("Cannot replace built-in subagent".to_string()));
            }
        ...
        let root = workspace_root
            .ok_or_else(|| NortHingError::agent("Workspace path is required to update project subagent".to_string()))?;
        let mut pm = self.write_project_subagents();
        let entries = pm
            .get_mut(root)
            .ok_or_else(|| NortHingError::agent("Project subagent cache not loaded for this workspace".to_string()))?;
    ...
    pub fn remove_subagent(&self, agent_id: &str) -> NortHingResult<Option<String>> {
        let mut map = self.write_agents();
        if let Some(entry) = map.get(agent_id) {
            ...
            if entry.subagent_source == Some(SubAgentSource::Builtin) {
                return Err(NortHingError::agent(format!(
        ...
        Err(NortHingError::agent(format!("Subagent not found: {}", agent_id)))
    }
    ...
    pub async fn update_subagent_override(
        ...
        let subagent_key = subagent_key_for(entry.subagent_source, entry.agent.as_ref())
            .ok_or_else(|| NortHingError::agent(format!("Failed to resolve subagent key for '{}'", agent_id)))?;
        ...
        match entry.subagent_source {
            ...
            Some(SubAgentSource::Builtin) | Some(SubAgentSource::User) => {
                ...
                let profile_id = crate::agentic::agents::resolve_mode_config_profile_id(parent_agent_type).into_owned();
            ...
            None => Err(NortHingError::agent(format!(
...
fn custom_subagent_discovery_roots(workspace_root: &Path) -> CustomSubagentDiscoveryRoots {
    CustomSubagentDiscoveryRoots {
        ...
        northhing_user_agents_dir: Some(path_manager_arc().user_agents_dir()),
        home_dir: dirs::home_dir(),
    }
}

fn custom_subagent_from_loaded_definition(loaded: LoadedCustomSubagentDefinition) -> CustomSubagent {
