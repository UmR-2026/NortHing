...
use crate::agentic::agents::registry::types::{
    subagent_source_from_custom_kind, AgentCategory, AgentEntry, CustomSubagentConfig, SubAgentSource,
    SubagentListScope, SubagentOverrideState, SubagentQueryContext,
};
use crate::agentic::agents::registry::visibility::{BuiltinSubagentExposure, SubagentVisibilityPolicy};
...
async fn computer_use_is_builtin_subagent_not_mode() {
    ...
    assert!(computer_use.default_tools.contains(&"ControlHub".to_string()));
    assert!(computer_use.default_tools.contains(&"ComputerUse".to_string()));
    assert_eq!(
...
async fn task_visible_subagents_are_filtered_by_parent_agent() {
    ...
    assert!(agentic_visible.iter().any(|agent| agent.id == "GeneralPurpose"));
    assert!(!agentic_visible.iter().any(|agent| agent.id == "ReviewSecurity"));
    assert!(!agentic_visible.iter().any(|agent| agent.id == "ResearchSpecialist"));
    ...
    let deep_review_visible = registry
        ...
        .await;
    assert!(deep_review_visible.iter().any(|agent| agent.id == "ReviewSecurity"));
    assert!(!deep_review_visible.iter().any(|agent| agent.id == "ResearchSpecialist"));
    ...
    assert!(!deep_research_visible.iter().any(|agent| agent.id == "ReviewSecurity"));
}
...
fn project_subagent_config_lookup_is_workspace_scoped() {
    ...
    assert!(
        registry.get_custom_subagent_config("SharedReviewer", None).is_none(),
...
async fn parent_subagent_overrides_follow_source_scopes() {
    ...
    let project_override_key = "project::northhing::ProjectScout".to_string();
    let user_override_key = "user::northhing::UserScout".to_string();
    ...
    user_overrides.insert(resolve_mode_config_profile_id("agentic").into_owned(), user_parent_map);
    ...
    assert_eq!(visible.0.override_state, Some(SubagentOverrideState::Disabled));
