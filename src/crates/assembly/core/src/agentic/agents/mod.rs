...
pub use northhing_agent_runtime::agents::{
    ...
    resolve_mode_config_profile_id, shared_coding_mode_user_context_policy, SHARED_CODING_MODE_CONFIG_PROFILE_ID,
    SHARED_CODING_MODE_CONFIG_PROFILE_LABEL, SHARED_CODING_MODE_IDS, SHARED_CODING_MODE_PROMPT_TEMPLATE,
};
pub use prompt_builder::{
    build_prompt_context_for_workspace, AgentPromptCacheIdentity, PartitionedLoader, PrependedPromptReminders,
    PromptBuilder, PromptBuilderContext, RemoteExecutionHints, RuntimeContextNeeds, ToolListingSections,
    UserContextPolicy, UserContextSection, USE_PARTITIONED_LOADER,
};
pub use registry::catalog::{builtin_agent_specs, BuiltinAgentSpec};
pub use registry::types::{
    subagent_source_from_custom_kind, AgentCategory, AgentInfo, AgentToolPolicy, CustomSubagentConfig, SubAgentSource,
...
pub trait Agent: Send + Sync + 'static {
    ...
    async fn build_prompt(&self, context: &PromptBuilderContext) -> NortHingResult<String> {
        ...
        let system_prompt_template = get_embedded_prompt(template_name)
            .ok_or_else(|| NortHingError::Agent(format!("{} not found in embedded files", template_name)))?;
    ...
    async fn get_system_prompt(&self, context: Option<&PromptBuilderContext>) -> NortHingResult<String> {
        if let Some(context) = context {
            self.build_prompt(context).await
        } else {
            Err(NortHingError::Agent("Prompt build context is required".to_string()))
        }
    }
    ...
    async fn get_system_reminder(
        ...
        if let Some(system_reminder_template_name) = self.system_reminder_template_name() {
            let system_reminder = get_embedded_prompt(system_reminder_template_name).ok_or_else(|| {
                NortHingError::Agent(format!("{} not found in embedded files", system_reminder_template_name))
            })?;
...
mod tests {
    use super::{
        shared_coding_mode_tool_exposure_overrides, shared_coding_mode_tools, shared_coding_mode_user_context_policy,
        Agent, AgenticMode, DebugMode, MultitaskMode, PlanMode,
    };
