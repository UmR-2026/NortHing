...
pub use northhing_agent_runtime::custom_subagent::CustomSubagentKind;
use northhing_agent_runtime::custom_subagent::{
    custom_subagent_read_markdown_file, custom_subagent_save_markdown_parts, CustomSubagentDefinition,
};
...
impl Agent for CustomSubagent {
    ...
    async fn build_prompt(&self, context: &PromptBuilderContext) -> NortHingResult<String> {
        ...
        let prompt = prompt_builder.build_prompt_from_template(&self.prompt).await?;
...
impl CustomSubagent {
    pub fn new(
        ...
        let definition = CustomSubagentDefinition::new(name, description, tools, prompt, readonly, kind);

        Self::from_definition(path, definition)
    }

    pub fn from_file(path: &str, kind: CustomSubagentKind) -> NortHingResult<Self> {
        let definition = custom_subagent_read_markdown_file(path, kind).map_err(NortHingError::Agent)?;
    ...
    pub fn save_to_file(&self, model: Option<&str>) -> NortHingResult<()> {
...
mod tests {
    ...
    fn review_metadata_round_trips_through_front_matter() {
        let dir = TestTempDir::new("northhing-subagent-test");
        let path = dir.path().join("review-agent.md");
        ...
        subagent.save_to_file(None).expect("review subagent should save");
