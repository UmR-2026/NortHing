...
impl DebugMode {
    ...
    async fn get_debug_config(&self) -> DebugModeConfig {
        if let Ok(config_service) = GlobalConfigManager::service().await {
            config_service
                .config::<DebugModeConfig>(Some("ai.debug_mode_config"))
                .await
                .unwrap_or_default()
        } else {
    ...
    fn load_reminder_template(&self, template_name: &str) -> NortHingResult<String> {
        get_embedded_prompt(template_name).map(str::to_string).ok_or_else(|| {
            crate::util::errors::NortHingError::Agent(format!("{} not found in embedded files", template_name))
        })
    }
    ...
    fn build_language_templates_prompt(config: &DebugModeConfig, detected_languages: &[String]) -> String {
        ...
        let user_other_templates: Vec<_> = config
            ...
            .filter(|(lang, template)| {
                *lang != "javascript" && template.enabled && !template.instrumentation_template.trim().is_empty()
            })
            .collect();
...
"#,
    ...
    fn build_ongoing_reminder(&self) -> NortHingResult<String> {
    ...
    fn build_first_entry_reminder(
        ...
        let reminder_template = self.load_reminder_template(DEBUG_MODE_FIRST_ENTRY_REMINDER_TEMPLATE)?;

        let language_templates = Self::build_language_templates_prompt(debug_config, &project_info.languages);

        debug!("Debug mode language templates length: {}", language_templates.len());
