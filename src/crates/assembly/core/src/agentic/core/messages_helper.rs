...
impl MessageHelper {
    ...
    pub fn estimate_request_tokens(
        ...
        let reasoning_frontier_start = match reasoning_policy {
            ...
            RequestReasoningTokenPolicy::LatestTurnOnly => Some(Self::find_reasoning_frontier_start(messages)),
        ...
        let mut total = messages
            ...
            .enumerate()
            .map(|(index, message)| {
                let include_reasoning = reasoning_frontier_start.is_some_and(|frontier_start| index >= frontier_start);
    ...
    pub fn split_messages_in_middle(mut messages: Vec<Message>) -> Option<(Vec<Message>, Vec<Message>)> {
        let messages_tokens: Vec<usize> = messages.iter_mut().map(|m| m.tokens()).collect();
...
mod tests {
    ...
    fn latest_turn_reasoning_policy_uses_turn_id_boundary() {
        let messages = vec![
            ...
            Message::assistant_with_reasoning(Some("old reasoning".to_string()), "old answer".to_string(), Vec::new())
                .with_turn_id("turn-1".to_string()),
            Message::user("new user".to_string()).with_turn_id("turn-2".to_string()),
            Message::assistant_with_reasoning(Some("new reasoning".to_string()), "new answer".to_string(), Vec::new())
                .with_turn_id("turn-2".to_string()),
        ];

        let full = MessageHelper::estimate_request_tokens(&messages, None, RequestReasoningTokenPolicy::FullHistory);
        let latest =
            MessageHelper::estimate_request_tokens(&messages, None, RequestReasoningTokenPolicy::LatestTurnOnly);
        let skip_all = MessageHelper::estimate_request_tokens(&messages, None, RequestReasoningTokenPolicy::SkipAll);

        assert_eq!(full - latest, TokenCounter::estimate_tokens("old reasoning"));
        assert_eq!(latest - skip_all, TokenCounter::estimate_tokens("new reasoning"));
    }
