...
pub(super) fn sanitize_user_text(text: &str, options: &CompressionFallbackOptions) -> Option<String> {
    let normalized = strip_prompt_markup(text);
    sanitize_text(&normalized, options.user_chars)
}

pub(super) fn sanitize_assistant_text(text: &str, options: &CompressionFallbackOptions) -> Option<String> {
...
pub(super) fn sanitize_todo_snapshot(arguments: &Value) -> Option<CompressedTodoSnapshot> {
    ...
    for todo in todos {
        ...
        let status = todo_object.get("status").and_then(Value::as_str).unwrap_or("pending");
        let id = todo_object.get("id").and_then(Value::as_str).map(str::to_string);
...
pub(super) fn sanitize_generic_value(value: &Value, options: &CompressionFallbackOptions) -> Option<Value> {
    match value {
        Value::Null => None,
...
fn insert_sanitize_text(source: &Map<String, Value>, target: &mut Map<String, Value>, key: &str, limit: usize) {
