...
use crate::agentic::core::{
    render_system_reminder, render_user_query, CompressedMessageRole, CompressionContract, CompressionContractItem,
    CompressionEntry, CompressionPayload, InternalReminderKind, Message, MessageSemanticKind, ToolCall, ToolResult,
};
use serde_json::json;
...
fn clears_tool_results_from_compressed_history() {
    ...
    let summary_artifact = build_structured_compression_summary(
        vec![vec![Message::user("inspect".to_string()), assistant, tool_result]],
        &default_options(),
    );
...
fn reuses_existing_compression_payload_atomically() {
    ...
    let summary_artifact = build_structured_compression_summary(vec![vec![reminder_message]], &default_options());
...
fn strips_user_query_markup_from_fallback_user_messages() {
    ...
    let summary_artifact = build_structured_compression_summary(vec![vec![Message::user(raw)]], &default_options());
    ...
    assert_eq!(user_message.text.as_deref(), Some("Implement manual /compact"));
...
fn groups_consecutive_assistant_messages_under_single_role_header() {
    ...
    assert!(summary_artifact.summary_text.contains("Updated the styling changes."));
}
...
fn renders_contract_facts_even_when_tool_results_are_cleared() {
    ...
    assert!(summary_artifact.summary_text.contains("Compaction contract:"));
    assert!(summary_artifact.summary_text.contains("src/main.rs"));
    assert!(summary_artifact.summary_text.contains("cargo test"));
    assert!(summary_artifact.summary_text.contains("pnpm run type-check:web"));
