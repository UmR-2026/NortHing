...
use crate::agentic::core::{
    render_system_reminder, CompressedMessage, CompressedMessageRole, CompressedTodoSnapshot, CompressionContract,
    CompressionEntry, CompressionPayload, Message, MessageContent, MessageHelper, MessageRole, MessageSemanticKind,
};
...
impl ContextCompressor {
    ...
    fn collect_conversation_turns(
    ...
    ) -> NortHingResult<Vec<TurnWithTokens>> {
        debug!(
        ...
        let turns_tokens: Vec<usize> = turns_messages
            .iter_mut()
            .map(|turn| turn.iter_mut().map(|m| m.tokens()).sum::<usize>())
            .collect();
        ...
        Ok(turns_messages.into_iter().map(TurnWithTokens::new).collect())
    }
    ...
    ) -> NortHingResult<CompressionResult> {
        self.compress_turns_with_contract(session_id, context_window, turns, mode, None, model_summary)
    }
    ...
    pub fn compress_turns_with_contract(
        ...
        if matches!(mode, CompressionMode::Auto) {
            self.append_live_boundary_context(&mut summary_artifact, last_user_message.as_ref(), last_todo.as_ref());
        }
    ...
    fn create_summary_turn(&self, summary_artifact: CompressionSummaryArtifact) -> (Message, Message) {
    ...
    fn append_live_boundary_context(
        ...
        if let Some(last_user_text) = last_user_message.and_then(Self::render_boundary_user_message_text) {
        ...
        let todo_text = todo_snapshot.map(Self::render_todo_snapshot).unwrap_or_default();
        ...
        summary_artifact.payload.entries.push(CompressionEntry::Turn {
            turn_id: None,
    ...
    fn render_boundary_marker_text(used_model_summary: bool) -> String {
        let mut msg = "The earlier conversation is summarized in the next assistant message. Use it as prior context."
            .to_string();
    ...
    fn build_fallback_summary_artifact(
        ...
        build_structured_compression_summary_with_contract(
            turns_to_compress.into_iter().map(|turn| turn.messages).collect(),
    ...
    fn build_fallback_options(&self, context_window: usize) -> CompressionFallbackOptions {
        CompressionFallbackOptions {
            max_tokens: ((context_window as f32 * self.config.fallback_max_tokens_ratio) as usize).max(256),
...
mod tests {
    ...
    use crate::agentic::core::{
        render_system_reminder, CompressionContract, CompressionContractItem, CompressionEntry, CompressionPayload,
        Message, MessageSemanticKind,
    };
    ...
    fn manual_compression_creates_closed_compression_turn() {
        ...
        let result = compressor
            .compress_turns("session", 8000, vec![todo_turn()], CompressionMode::Manual, None)
    ...
    fn synthetic_summary_turn_payload_remains_atomic_on_recompression() {
        let marker = Message::user(render_system_reminder("Earlier conversation was compressed."))
            .with_semantic_kind(MessageSemanticKind::CompressionBoundaryMarker);
        ...
        let summary_artifact = crate::agentic::session::compression::fallback::build_structured_compression_summary(
    ...
    fn model_summary_output_without_tags_keeps_plain_text() {
        let normalized = ContextCompressor::normalize_model_summary_output("Plain summary without tags");
    ...
    fn model_summary_output_with_analysis_but_no_summary_is_rejected() {
        let normalized =
            ContextCompressor::normalize_model_summary_output("<analysis>\ninternal reasoning\n</analysis>");
