use super::sanitize::{sanitize_assistant_text, sanitize_todo_snapshot, sanitize_tool_arguments, sanitize_user_text};
use super::types::CompressionFallbackOptions;
use crate::agentic::core::{
    strip_prompt_markup, CompressedMessage, CompressedMessageRole, CompressedTodoSnapshot, CompressedToolCall,
    CompressionEntry, Message, MessageContent, MessageRole, MessageSemanticKind,
};
...
fn build_entries_from_messages(
    ...
    let turn_id = messages.first().and_then(|message| message.metadata.turn_id.clone());
    let mut turn_messages = Vec::new();
    let mut latest_todo = None;

    for message in messages {
        if let Some(entries) = extract_nested_compression_entries(&message) {
            flush_turn_entry(output, turn_id.clone(), &mut turn_messages, &mut latest_todo);
...
fn extract_nested_compression_entries(message: &Message) -> Option<Vec<CompressionEntry>> {
    match message.metadata.semantic_kind {
        ...
        Some(MessageSemanticKind::CompressionSummary) | Some(MessageSemanticKind::InternalReminder) => {}
        _ => return None,
    }
