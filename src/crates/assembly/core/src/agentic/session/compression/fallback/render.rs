use crate::agentic::core::{
    CompressedMessage, CompressedMessageRole, CompressionContract, CompressionEntry, CompressionPayload,
};
use serde_json::{json, Value};

pub(super) fn render_payload_for_model(payload: &CompressionPayload) -> String {
    if payload.entries.is_empty() {
        return "No detailed historical entries fit within the remaining context budget.".to_string();
    }
    ...
    for (index, entry) in payload.entries.iter().enumerate() {
        match entry {
            ...
            CompressionEntry::ModelSummary { text } => {
                history_sections.push(format!("Earlier summarized history {}:\n{}", index + 1, text));
            }
