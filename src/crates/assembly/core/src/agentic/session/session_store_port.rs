...
use northhing_runtime_ports::{
    ...
    SessionStoragePathResolution, SessionStorePort,
};

use crate::agentic::core::SessionConfig;
use crate::service::remote_ssh::workspace_state::{
    resolve_workspace_session_identity, unresolved_remote_session_storage_dir, LOCAL_WORKSPACE_SSH_HOST,
};
...
impl CoreSessionStorePort {
    pub async fn resolve_storage_path_for_config(config: &SessionConfig) -> Option<SessionStoragePathResolution> {
        ...
        Self.resolve_session_storage_path(request).await.ok()
    }
}
...
impl SessionStorePort for CoreSessionStorePort {
    async fn resolve_session_storage_path(
        ...
        .await
        .ok_or_else(|| PortError::new(PortErrorKind::InvalidRequest, "Session workspace_path is required"))?;

        let requested_workspace_path = request.workspace_path;
        let (resolution, _remote_ssh_host) = if identity.hostname == LOCAL_WORKSPACE_SSH_HOST {
            (
                SessionStoragePathResolution::local(PathBuf::from(identity.logical_workspace_path())),
                None,
            )
        } else if identity.hostname == "_unresolved" {
            (
                SessionStoragePathResolution::unresolved_remote(
                    ...
                    identity.remote_connection_id.unwrap_or_default(),
                ),
                None,
            )
        } else {
            (
                SessionStoragePathResolution::remote(
                    requested_workspace_path,
