...
impl TurnSkillAgentSnapshotStore {
    ...
    pub fn create_session(&self, session_id: &str) {
        self.session_snapshots.entry(session_id.to_string()).or_default();
    }

    pub fn get_snapshot(&self, session_id: &str, turn_index: usize) -> Option<TurnSkillAgentSnapshot> {
        self.session_snapshots
            .get(session_id)
            .and_then(|snapshots| snapshots.get(&turn_index).cloned())
    }

    pub fn set_snapshot(&self, session_id: &str, turn_index: usize, snapshot: TurnSkillAgentSnapshot) {
        if let Some(mut snapshots) = self.session_snapshots.get_mut(session_id) {
            snapshots.insert(turn_index, snapshot);
        } else {
            let mut snapshots = BTreeMap::new();
            snapshots.insert(turn_index, snapshot);
            self.session_snapshots.insert(session_id.to_string(), snapshots);
        }
    }
    ...
    pub fn latest_snapshot_at_or_before(
        ...
        self.session_snapshots.get(session_id).and_then(|snapshots| {
            snapshots
                .range(..=turn_index)
                .next_back()
