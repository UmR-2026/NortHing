...
mod tests {
    ...
    fn sample_state(start_line: usize, end_line: usize, total_lines: usize, is_partial_view: bool) -> FileReadState {
        FileReadState {
            content: String::new(),
            timestamp_ms: 0,
            start_line,
...
impl FileReadStateStore {
    ...
    pub fn create_session(&self, session_id: &str) {
        self.session_states.entry(session_id.to_string()).or_default();
    ...
    pub fn set(&self, session_id: &str, logical_path: &str, state: FileReadState) {
        let session_states = self.session_states.entry(session_id.to_string()).or_default();
