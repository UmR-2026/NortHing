...
fn compression_contract_item_from_summary_item(item: EvidenceLedgerSummaryItem) -> CompressionContractItem {
...
mod tests {
    use super::{
        EvidenceLedgerCheckpoint, EvidenceLedgerEvent, EvidenceLedgerEventStatus, EvidenceLedgerTargetKind,
    ...
    fn ledger_reads_events_scoped_by_session_and_turn() {
        ...
        assert_eq!(ledger.events_for_turn("session-a", "turn-a"), vec![appended.clone()]);
    ...
    fn checkpoint_created_event_preserves_recovery_boundary_metadata() {
        ...
        let event =
            EvidenceLedgerEvent::checkpoint_created("session-a", "turn-a", "Edit", "src/lib.rs", checkpoint.clone());
    ...
    fn summary_projects_partial_subagent_results() {
        ...
        assert_eq!(
            summary.partial_subagent_results[0].partial_output.as_deref(),
    ...
    fn summary_projects_into_compression_contract() {
        ...
        let contract: crate::agentic::core::CompressionContract = ledger.summary_for_session("session-a", 10).into();

        assert_eq!(contract.touched_files, vec!["src/main.rs"]);
        assert_eq!(contract.verification_commands[0].target, "cargo test");
        assert_eq!(contract.blocking_failures[0].error_kind.as_deref(), Some("exit_code:1"));
