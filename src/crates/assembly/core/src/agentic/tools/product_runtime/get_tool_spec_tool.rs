...
use super::{
    product_get_tool_spec_runtime, resolve_product_get_tool_spec_results, ProductGetToolSpecRuntime,
...
use northhing_agent_tools::{
    build_get_tool_spec_catalog_description, build_get_tool_spec_description, GetToolSpecCollapsedToolSummary,
...
fn map_get_tool_spec_execution_error(error: GetToolSpecExecutionError) -> NortHingError {
    match error {
        GetToolSpecExecutionError::MissingToolName => NortHingError::tool(error.to_string()),
        GetToolSpecExecutionError::Detail(message) => NortHingError::Validation(message),
    }
}
...
mod tests {
    ...
    fn collapsed_tools_context_lists_names_without_short_descriptions() {
        ...
        let description = GetToolSpecTool::build_collapsed_tools_context_section(&[
            northhing_agent_tools::GetToolSpecCollapsedToolSummary {
    ...
    async fn reloading_already_unlocked_tool_returns_assistant_hint() {
        ...
        let results = tool.call_impl(&json!({ "tool_name": "WebFetch" }), &context).await;
