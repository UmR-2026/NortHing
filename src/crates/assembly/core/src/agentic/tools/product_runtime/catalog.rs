...
use crate::agentic::agents::{agent_registry, AgentToolPolicyOverrides};
use crate::agentic::tools::framework::{Tool, ToolExposure, ToolResult};
use crate::agentic::tools::registry::{global_tool_registry, ToolRef};
use crate::agentic::tools::tool_context_runtime::ToolUseContext;
use crate::util::errors::{NortHingError, NortHingResult};
use crate::util::types::ToolDefinition;
use northhing_agent_tools::{
    ContextualToolManifest, ContextualVisibleTools, GetToolSpecCatalogProvider, GetToolSpecCollapsedToolSummary,
    GetToolSpecExecutionError, GetToolSpecRuntime, ToolCatalogRuntime, ToolCatalogSnapshotProvider,
    ToolManifestDefinition, GET_TOOL_SPEC_TOOL_NAME,
};
...
impl ProductToolCatalogProvider {
    ...
    async fn contextual_collapsed_tools(&self, context: &ToolUseContext) -> NortHingResult<Vec<ToolRef>> {
        let agent_type = context
            .agent_type
            .as_deref()
            .ok_or_else(|| NortHingError::Validation("GetToolSpec requires agent type context".to_string()))?;
        let workspace_root = context.workspace_root();
        let agent_registry = agent_registry();
        let policy = agent_registry.get_agent_tool_policy(agent_type, workspace_root).await;
...
pub(crate) fn product_get_tool_spec_runtime(provider: &ProductToolCatalogProvider) -> ProductGetToolSpecRuntime<'_> {
    GetToolSpecRuntime::new(provider, GET_TOOL_SPEC_TOOL_NAME)
}

pub(crate) fn product_tool_catalog_runtime(provider: &ProductToolCatalogProvider) -> ProductToolCatalogRuntime<'_> {
...
pub(crate) async fn resolve_product_readonly_enabled_tools() -> Vec<ToolRef> {
    let provider = ProductToolCatalogProvider;
    product_tool_catalog_runtime(&provider).readonly_enabled_tools().await
}
...
mod tests {
    use super::{
        ...
        resolve_product_resolved_tool_manifest, resolve_product_resolved_visible_tools, resolve_product_tool_manifest,
    ...
    use northhing_agent_tools::{GetToolSpecCatalogProvider, ToolCatalogSnapshotProvider, GET_TOOL_SPEC_TOOL_NAME};
    use serde_json::json;
    ...
    async fn product_catalog_provider_reads_global_registry_snapshot() {
        ...
        let expected_builtin_names = create_tool_registry().tool_names();
        assert!(
    ...
    async fn product_catalog_provider_default_get_tool_spec_catalog_matches_registry() {
        ...
        let expected_builtin_collapsed_names = create_tool_registry().collapsed_tool_names();
        assert!(
    ...
    async fn product_get_tool_spec_rejects_expanded_webfetch_in_agentic_mode() {
        ...
        assert!(
            error.to_string().contains("not an available collapsed tool"),
    ...
    async fn product_catalog_facade_resolves_readonly_enabled_tools_from_same_provider_owner() {
        ...
        for tool in create_tool_registry().all_tools() {
    ...
    async fn product_manifest_guard_preserves_get_tool_spec_unlock_surface() {
        ...
        assert_eq!(
            manifest.collapsed_tool_names,
            vec!["GetFileDiff".to_string(), "WebFetch".to_string(), "Git".to_string()],
    ...
    async fn product_manifest_expands_tool_when_agent_override_requests_it() {
        ...
        let manifest =
            resolve_product_resolved_tool_manifest(&allowed_tools, &overrides, &tool_context(Some("test-agent"))).await;

        assert!(manifest.collapsed_tool_names.is_empty());
        assert!(manifest.tool_definitions.iter().any(|tool| tool.name == "WebFetch"));
        assert!(!manifest
            .tool_definitions
            .iter()
            .any(|tool| tool.name == GET_TOOL_SPEC_TOOL_NAME));
