...
pub fn should_retry_action(error: &NortHingError, action_type: &str) -> bool {
    ...
    if error_msg.contains("permission") || error_msg.contains("not enabled") || error_msg.contains("not available") {
        return false;
    }
