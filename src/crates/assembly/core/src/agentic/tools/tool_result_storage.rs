...
use northhing_agent_tools::{
    ...
    PersistedToolOutput, ToolResultPersistenceCandidate, ToolResultStoragePolicy, GET_TOOL_SPEC_TOOL_NAME,
};
#[cfg(test)]
use northhing_agent_tools::{DEFAULT_MAX_TOOL_RESULT_CHARS, PERSISTED_OUTPUT_TAG};
...
pub(crate) async fn maybe_persist_large_tool_result(mut result: ToolResult, context: &ToolUseContext) -> ToolResult {
...
pub(crate) async fn apply_round_tool_result_budget(
    ...
    let selected =
        select_tool_result_indices_for_persistence(&candidates, total_visible_chars, policy.per_round_limit_chars);
...
fn should_skip_tool_result(result: &ToolResult) -> bool {
    result.tool_name == GET_TOOL_SPEC_TOOL_NAME || result.image_attachments.as_ref().is_some_and(|v| !v.is_empty())
}
...
async fn persist_and_render_replacement(
    ...
    let persisted = persist_tool_result(result, context, policy.preview_chars, content_override).await?;
    Ok(build_persisted_tool_output_message(&persisted, policy.preview_chars))
}
...
async fn persist_tool_result(
    ...
    preview_chars: usize,
    content_override: Option<String>,
) -> NortHingResult<PersistedToolOutput> {
    let session_id = context
        .session_id
        .as_deref()
        .ok_or_else(|| NortHingError::tool("A session id is required to persist tool results".to_string()))?;
    ...
    let reference = context
        .build_session_runtime_artifact_reference(session_id, &format!("tool-results/{}", file_name))
...
async fn write_once(path: &Path, content: &str) -> NortHingResult<()> {
...
fn serialize_tool_result_content(result: &ToolResult) -> NortHingResult<(String, bool)> {
    ...
    serde_json::to_string_pretty(&result.result)
        ...
        .map(|text| (text, true))
        .map_err(|error| NortHingError::serialization(format!("Failed to serialize tool result: {}", error)))
}
...
fn content_override_if_oversized(result: &ToolResult, limit: usize) -> Option<String> {
    ...
    let output = result.result.get("output").and_then(|value| value.as_str())?;
...
fn result_visible_content(result: &ToolResult) -> String {
    if let Some(text) = result.result_for_assistant.as_ref().filter(|text| !text.is_empty()) {
        return text.clone();
    }
...
mod tests {
    ...
    async fn single_large_result_persists_and_replaces_assistant_text() {
        ...
        let result = tool_result("tool/one", "Bash", "x".repeat(DEFAULT_MAX_TOOL_RESULT_CHARS + 1));
    ...
    async fn read_result_stays_inline_when_under_read_limit() {
        ...
        assert_eq!(processed.result_for_assistant.as_deref(), Some(text.as_str()));
        let session_dir = context
            .current_workspace_session_tool_results_dir("session_1")
            .expect("session tool-results dir");
        assert!(!session_dir.join("read_1.txt").exists());
    ...
    async fn bash_full_output_persists_even_when_assistant_text_is_already_truncated() {
        ...
        let context = test_context(root.clone());
        let full_output = format!("{}\nfinal-error", "x".repeat(SHELL_MAX_TOOL_RESULT_CHARS + 1));
