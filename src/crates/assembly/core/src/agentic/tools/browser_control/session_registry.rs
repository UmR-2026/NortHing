...
impl BrowserSessionState {
    ...
    pub async fn query_network(
        ...
        Self::query(&self.network_events, filter, threshold, limit, |item| {
            if let Some(method) = method {
                ...
                let actual = item.get("method").and_then(|v| v.as_str()).map(str::to_uppercase);
    ...
    pub async fn query_console(&self, filter: Option<&str>, since: Option<&str>, limit: usize) -> Vec<Value> {
        let threshold = self.since_threshold(since);
        Self::query(&self.console_events, filter, threshold, limit, |_| true).await
    }

    pub async fn query_errors(&self, filter: Option<&str>, since: Option<&str>, limit: usize) -> Vec<Value> {
        let threshold = self.since_threshold(since);
...
impl BrowserSessionRegistry {
    ...
    pub async fn set_default(&self, session_id: &str) -> NortHingResult<()> {
        let mut g = self.inner.write().await;
    ...
    pub async fn get(&self, session_id: Option<&str>) -> NortHingResult<BrowserSession> {
        ...
        let resolved = {
            ...
            let id = match session_id {
                ...
                None => g.default_id.clone().ok_or_else(|| {
                    NortHingError::tool("No browser session registered. Use action 'connect' first.".to_string())
                })?,
            };
        ...
        let (id, session) = resolved.ok_or_else(|| {
            NortHingError::tool("Browser session is not connected. Use action 'connect' or 'switch_page'.".to_string())
        })?;
