...
impl CdpClient {
    ...
    pub async fn get_version(port: u16) -> NortHingResult<CdpVersionInfo> {
        let url = format!("http://127.0.0.1:{}/json/version", port);
        let resp = reqwest::get(&url)
            .await
            .map_err(|e| NortHingError::tool(format!("Cannot reach browser CDP on port {}: {}", port, e)))?;
        let info: CdpVersionInfo = resp
            .json()
            .await
            .map_err(|e| NortHingError::tool(format!("Invalid CDP version response: {}", e)))?;
        Ok(info)
    }

    /// List all pages/tabs on the given debug port.
    pub async fn list_pages(port: u16) -> NortHingResult<Vec<CdpPageInfo>> {
        let url = format!("http://127.0.0.1:{}/json", port);
        let resp = reqwest::get(&url)
            .await
            .map_err(|e| NortHingError::tool(format!("Cannot list CDP pages on port {}: {}", port, e)))?;
        let pages: Vec<CdpPageInfo> = resp
            .json()
            .await
            .map_err(|e| NortHingError::tool(format!("Invalid CDP pages response: {}", e)))?;
        Ok(pages)
    }

    /// Create a new page/tab on the given debug port.
    pub async fn create_page(port: u16, url: Option<&str>) -> NortHingResult<CdpPageInfo> {
        ...
        let resp = reqwest::Client::new()
            ...
            .send()
            .await
            .map_err(|e| NortHingError::tool(format!("Cannot create CDP page on port {}: {}", port, e)))?;
        let page: CdpPageInfo = resp
            .json()
            .await
            .map_err(|e| NortHingError::tool(format!("Invalid CDP new page response: {}", e)))?;
        Ok(page)
    }

    /// Connect to a specific page by its WebSocket debugger URL.
    pub async fn connect(ws_url: &str) -> NortHingResult<Self> {
        info!("CDP connecting to {}", ws_url);
        let (ws_stream, _) = connect_async(ws_url)
            .await
            .map_err(|e| NortHingError::tool(format!("CDP WebSocket connect failed: {}", e)))?;
    ...
    pub async fn connect_to_first_page(port: u16) -> NortHingResult<Self> {
    ...
    pub async fn send(&self, method: &str, params: Option<Value>) -> NortHingResult<Value> {
        ...
        debug!("CDP send id={} method={}", id, method);
        {
            ...
            sink.send(Message::Text(msg.to_string().into()))
                .await
                .map_err(|e| NortHingError::tool(format!("CDP send failed: {}", e)))?;
        }

        let result = tokio::time::timeout(std::time::Duration::from_secs(30), rx)
            .await
            .map_err(|_| NortHingError::tool(format!("CDP timeout for method {}", method)))?
            .map_err(|_| NortHingError::tool("CDP response channel closed".to_string()))?;

        if let Some(error) = result.get("error") {
            return Err(NortHingError::tool(format!("CDP error: {}", error)));
        }
            ...
            match msg_result {
                Ok(Message::Text(text)) => {
                    if let Ok(val) = serde_json::from_str::<Value>(&text) {
                        ...
                        } else if let Some(method) = val.get("method").and_then(|v| v.as_str()).map(str::to_string) {
