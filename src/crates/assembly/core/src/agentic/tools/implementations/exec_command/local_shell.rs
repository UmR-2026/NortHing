...
pub(crate) async fn resolve_local_exec_shell() -> ResolvedLocalExecShell {
    ...
    let detected_shells: Vec<_> = ShellDetector::detect_available_shells()
        ...
        .collect();
    let system_default = {
        let shell = ShellDetector::default_shell();
...
mod tests {
    use super::{
        parse_configured_shell_preference, select_non_windows_local_exec_shell, select_windows_local_exec_shell,
        ConfiguredShellPreference, ResolvedLocalExecShell,
    };
    use std::path::PathBuf;
    ...
    fn windows_cmd_prefers_pwsh_then_powershell() {
        let detected = vec![
            ...
            shell("Git Bash", "C:\\Program Files\\Git\\bin\\bash.exe", ShellType::Bash),
        ];
        let resolved = select_windows_local_exec_shell(Some(ConfiguredShellPreference::Cmd), &detected, &detected[0]);
    ...
    fn windows_pwsh_falls_back_to_powershell_when_pwsh_is_missing() {
        ...
        let resolved =
            select_windows_local_exec_shell(Some(ConfiguredShellPreference::PowerShellCore), &detected, &detected[0]);
    ...
    fn windows_bash_uses_detected_git_bash_path() {
        ...
        let resolved = select_windows_local_exec_shell(Some(ConfiguredShellPreference::Bash), &detected, &detected[0]);

        assert_eq!(resolved.shell_type, ShellType::Bash);
        assert_eq!(resolved.path, PathBuf::from("D:\\Tools\\Git\\bin\\bash.exe"));
    }
    ...
    fn windows_unsupported_shell_falls_back_to_powershell() {
        ...
        let resolved = select_windows_local_exec_shell(Some(ConfiguredShellPreference::Fish), &detected, &detected[0]);
    ...
    fn non_windows_uses_configured_detected_shell_when_available() {
        ...
        let resolved =
            select_non_windows_local_exec_shell(Some(ConfiguredShellPreference::Zsh), &detected, &detected[0]);
