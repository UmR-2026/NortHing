...
mod tests {
    ...
    async fn get_time_returns_current_time_facts() {
        ...
        assert!(data["local_time"].as_str().is_some_and(|value| !value.is_empty()));
        assert!(data["utc_time"].as_str().is_some_and(|value| !value.is_empty()));
        assert!(data["weekday"].as_str().is_some_and(|value| !value.is_empty()));
