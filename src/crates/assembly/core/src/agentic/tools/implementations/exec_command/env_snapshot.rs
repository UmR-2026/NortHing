use crate::service::remote_ssh::{
    global_remote_exec_process_manager, RemoteExecCommandRequest, RemoteExecControlAction, RemoteExecControlOrigin,
...
const ENV_SNAPSHOT_BEGIN: &str = "__northhing_REMOTE_ENV_SNAPSHOT_BEGIN__";
const ENV_SNAPSHOT_END: &str = "__northhing_REMOTE_ENV_SNAPSHOT_END__";
...
static REMOTE_ENV_SNAPSHOT_CACHE: OnceLock<Mutex<HashMap<RemoteEnvSnapshotKey, CachedSnapshot>>> = OnceLock::new();
...
pub(super) async fn remote_env_snapshot_for(
    ...
    let snapshot = match capture_remote_env_snapshot(ssh_manager, connection_id, shell_path, shell_type).await {
...
async fn capture_remote_env_snapshot(
    ...
    let manager = global_remote_exec_process_manager();
...
fn is_volatile_env_var(key: &str) -> bool {
    matches!(key, "_" | "PWD" | "OLDPWD" | "SHLVL" | "TERM" | "COLUMNS" | "LINES")
}
...
#[cfg(test)]
mod tests {
    use super::{parse_remote_env_snapshot_output, remote_env_snapshot_command, remote_env_snapshot_shell_args};
    use terminal_core::ShellType;

    #[test]
    fn parses_env_snapshot_between_markers() {
        let snapshot = parse_remote_env_snapshot_output(
            "noise\r\n__northhing_REMOTE_ENV_SNAPSHOT_BEGIN__\r\nPATH=/home/me/.nvm/bin:/usr/bin\r\nNVM_DIR=/home/me/.nvm\r\nPWD=/tmp\r\nBAD-NAME=value\r\n__northhing_REMOTE_ENV_SNAPSHOT_END__\r\nmore noise",
        )
        ...
        assert_eq!(snapshot.env.get("NVM_DIR").map(String::as_str), Some("/home/me/.nvm"));
    ...
    fn snapshot_command_uses_interactive_login_shell_for_bash() {
        ...
        assert!(command.contains("__northhing_REMOTE_ENV_SNAPSHOT_BEGIN__"));
        assert!(command.contains("__northhing_REMOTE_ENV_SNAPSHOT_END__"));
    }
