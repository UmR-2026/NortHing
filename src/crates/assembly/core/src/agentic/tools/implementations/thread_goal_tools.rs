...
use northhing_agent_runtime::thread_goal_tools::{
    build_goal_tool_result, parse_create_goal_args, parse_update_goal_args, parse_update_goal_status,
    CREATE_GOAL_TOOL_NAME, GET_GOAL_TOOL_NAME, UPDATE_GOAL_TOOL_NAME,
};
use northhing_runtime_ports::ThreadGoalStatus;
use serde_json::{json, Value};

fn require_coordinator() -> NortHingResult<std::sync::Arc<crate::agentic::coordination::ConversationCoordinator>> {
    global_coordinator().ok_or_else(|| NortHingError::Validation("coordinator is unavailable".to_string()))
}

fn require_session_context(context: &ToolUseContext) -> NortHingResult<(String, std::path::PathBuf)> {
    let session_id = context
        .session_id
        .clone()
        .ok_or_else(|| NortHingError::Validation("session_id is unavailable".to_string()))?;
    let workspace_path = context
        .workspace_root()
        .ok_or_else(|| NortHingError::Validation("workspace_path is unavailable".to_string()))?
        .to_path_buf();
...
impl Tool for GetGoalTool {
    ...
    async fn call_impl(&self, _input: &Value, context: &ToolUseContext) -> NortHingResult<Vec<ToolResult>> {
        ...
        let result =
            build_goal_tool_result(goal, false).map_err(|error| NortHingError::Validation(error.to_string()))?;
...
Set token_budget only when an explicit token budget is requested. Fails if a goal exists; use {UPDATE_GOAL_TOOL_NAME} only for status."
    ...
    async fn call_impl(&self, input: &Value, context: &ToolUseContext) -> NortHingResult<Vec<ToolResult>> {
        let parsed =
            parse_create_goal_args(input.clone()).map_err(|error| NortHingError::Validation(error.to_string()))?;
        let coordinator = require_coordinator()?;
        let (session_id, workspace_path) = require_session_context(context)?;
        let goal = coordinator
            .create_thread_goal(
            ...
            .map_err(user_facing_thread_goal_error)?;
        let result =
            build_goal_tool_result(Some(goal), false).map_err(|error| NortHingError::Validation(error.to_string()))?;
        Ok(vec![ToolResult::Result {
            data: result.data,
            result_for_assistant: Some(result.result_for_assistant),
            image_attachments: None,
...
You cannot use this tool to pause, resume, budget-limit, or usage-limit a goal."
    ...
    async fn call_impl(&self, input: &Value, context: &ToolUseContext) -> NortHingResult<Vec<ToolResult>> {
        let parsed =
            parse_update_goal_args(input.clone()).map_err(|error| NortHingError::Validation(error.to_string()))?;
        let status =
            parse_update_goal_status(&parsed.status).map_err(|error| NortHingError::Validation(error.to_string()))?;
        let coordinator = require_coordinator()?;
        let (session_id, workspace_path) = require_session_context(context)?;
        let goal = coordinator
        ...
        let result = build_goal_tool_result(Some(goal), include_report)
            .map_err(|error| NortHingError::Validation(error.to_string()))?;
        Ok(vec![ToolResult::Result {
            data: result.data,
            result_for_assistant: Some(result.result_for_assistant),
            image_attachments: None,
