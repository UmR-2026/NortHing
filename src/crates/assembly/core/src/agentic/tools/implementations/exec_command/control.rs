...
use crate::service::remote_ssh::{
    global_remote_exec_process_manager, RemoteExecControlAction, RemoteExecControlOrigin, RemoteExecControlRequest,
    RemoteExecError, RemoteExecSessionCompletion, RemoteExecSessionCompletionSource, RemoteExecSessionCompletionStatus,
};
use crate::util::errors::{NortHingError, NortHingResult};
use async_trait::async_trait;
use serde_json::{json, Value};
use terminal_core::{
    global_exec_process_manager, LocalExecControlAction, LocalExecControlOrigin, LocalExecControlRequest,
    LocalExecSessionCompletion, LocalExecSessionCompletionSource, LocalExecSessionCompletionStatus, TerminalError,
};
...
pub async fn control_exec_command_session(
    ...
    if request.remote {
        let response = global_remote_exec_process_manager()
            ...
            .map_err(|error| match error {
                RemoteExecError::SessionNotFound(session_id) => ExecCommandControlError::SessionNotFound(session_id),
                error => ExecCommandControlError::Tool(NortHingError::tool(format!("ExecControl failed: {error}"))),
            })?;
    ...
    let response = global_exec_process_manager()
        ...
        .await
        .map_err(|error| match error {
            TerminalError::SessionNotFound(_) => ExecCommandControlError::SessionNotFound(request.session_id),
            error => ExecCommandControlError::Tool(NortHingError::tool(format!("ExecControl failed: {error}"))),
        })?;
...
impl ExecControlTool {
    ...
    fn response_for_assistant(data: &Value, action: ExecCommandControlAction) -> String {
        ...
        match action {
            ExecCommandControlAction::Interrupt => status_lines.push("Sent interrupt to process.".to_string()),
    ...
    fn session_not_found_result(session_id: i32, action: ExecCommandControlAction, remote: bool) -> Vec<ToolResult> {
    ...
    fn local_completion(completion: LocalExecSessionCompletion) -> ExecCommandCompletion {
        ExecCommandCompletion {
            status: match completion.status {
                ...
                LocalExecSessionCompletionStatus::Interrupted => ExecCommandCompletionStatus::Interrupted,
            ...
            source: match completion.source {
                ...
                LocalExecSessionCompletionSource::OutOfBandControl => ExecCommandCompletionSource::OutOfBandControl,
            },
        }
    }
    ...
    fn remote_completion(completion: RemoteExecSessionCompletion) -> ExecCommandCompletion {
        ExecCommandCompletion {
            status: match completion.status {
                ...
                RemoteExecSessionCompletionStatus::Interrupted => ExecCommandCompletionStatus::Interrupted,
            ...
            source: match completion.source {
                ...
                RemoteExecSessionCompletionSource::OutOfBandControl => ExecCommandCompletionSource::OutOfBandControl,
            },
        }
    }

    async fn call_remote_pipe(&self, input: &Value) -> NortHingResult<Vec<ToolResult>> {
        let session_id = Self::session_id_from_input(input)
            .ok_or_else(|| NortHingError::tool("session_id is required for ExecControl".to_string()))?;
        let action = Self::action_from_input(input)
            .ok_or_else(|| NortHingError::tool("action must be either 'interrupt' or 'kill'".to_string()))?;
...
mod tests {
    use super::{
        control_exec_command_session, ExecCommandControlAction, ExecCommandControlError, ExecCommandControlOrigin,
        ExecCommandControlRequest, ExecControlTool,
    };
    use crate::agentic::tools::framework::ToolResult;

    #[test]
    fn session_not_found_result_uses_plain_assistant_message() {
        let results = ExecControlTool::session_not_found_result(456, ExecCommandControlAction::Interrupt, false);
        ...
        assert_eq!(
            data.get("requested_session_id").and_then(|value| value.as_i64()),
            Some(456)
        );
    ...
    async fn control_exec_command_session_returns_structured_session_not_found() {
        ...
        assert!(matches!(error, ExecCommandControlError::SessionNotFound(987_654)));
    }
}
