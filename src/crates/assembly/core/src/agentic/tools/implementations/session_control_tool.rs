...
use northhing_agent_runtime::session_control::{
    render_session_control_tool_use_message, resolve_session_control_cancel_route,
    session_control_agent_type_or_default, session_control_cancel_result_message, session_control_cancel_status,
    session_control_created_result_message, session_control_creator_marker, session_control_deleted_result_message,
    session_control_session_name_or_default, validate_session_control_input, validate_session_id, SessionControlAction,
    SessionControlCancelRoute, SessionControlInput, SessionControlValidationContext, SessionControlValidationResult,
};
use northhing_runtime_ports::{
    AgentSessionCreateRequest, AgentSessionDeleteRequest, AgentSessionListRequest, AgentSessionSummary,
...
impl SessionControlTool {
    ...
    fn current_workspace_session<'a>(&self, context: &'a ToolUseContext, workspace: &str) -> Option<&'a str> {
        let current_session_id = context.session_id.as_deref()?;
        let current_workspace = context.workspace_root()?;
        let normalized_current_workspace = normalize_path(current_workspace.to_string_lossy().as_ref());
    ...
    fn creator_session_marker(&self, context: &ToolUseContext) -> NortHingResult<String> {
        let creator_session_id = context
            .session_id
            .as_ref()
            .ok_or_else(|| NortHingError::tool("create requires a creator session in tool context".to_string()))?;
        ...
        match action {
            SessionControlAction::Cancel | SessionControlAction::Delete => {
                let session_id = session_id
                    .ok_or_else(|| NortHingError::tool(format!("session_id is required for {}", action.as_str())))?;
    ...
    async fn ensure_session_exists(
        ...
        let existing_sessions = runtime
            ...
            .await
            .map_err(|error| NortHingError::tool(CoreServiceAgentRuntime::runtime_error_message(error)))?;
        if existing_sessions.iter().any(|session| session.session_id == session_id) {
            Ok(())
        } else {
            Err(NortHingError::NotFound(format!(
    ...
    fn build_list_result_for_assistant(
        ...
        lines.push("| session_id | session_name | agent_type | created_at | last_active_at |".to_string());
        ...
        for session in sessions {
            lines.push(format!(
                ...
                Self::format_system_time(Self::system_time_from_epoch_ms(session.last_active_at_ms)),
            ));
        }
        lines.join("\n")
    }
...
- "session_id": Required for cancel and delete."#
    ...
    async fn call_impl(&self, input: &Value, context: &ToolUseContext) -> NortHingResult<Vec<ToolResult>> {
        let params: SessionControlInput =
        ...
        let runtime = CoreServiceAgentRuntime::agent_runtime(coordinator.clone()).map_err(NortHingError::tool)?;

        match params.action {
            SessionControlAction::Create => {
                let workspace = self
                    .resolve_effective_workspace(SessionControlAction::Create, None, context, &runtime)
                    .await?;
                let session_name = session_control_session_name_or_default(params.session_name.as_deref());
                ...
                let result_for_assistant =
                    session_control_created_result_message(&created_session_id, &workspace, &created_agent_type);
            ...
            SessionControlAction::Cancel => {
                let session_id = params
                    .session_id
                    .as_deref()
                    .ok_or_else(|| NortHingError::tool("session_id is required for cancel".to_string()))?;
                validate_session_id(session_id).map_err(NortHingError::tool)?;
                let workspace = self
                    .resolve_effective_workspace(SessionControlAction::Cancel, Some(session_id), context, &runtime)
                    .await?;
                ...
                self.ensure_session_exists(&runtime, &workspace, session_id).await?;

                let scheduler = global_scheduler();
                let cancel_route =
                    resolve_session_control_cancel_route(context.session_id.as_deref(), scheduler.is_some());
                let (runtime, requester_session_id) = match (cancel_route, scheduler) {
                    (SessionControlCancelRoute::RequesterViaScheduler { requester_session_id }, Some(scheduler)) => {
                        let runtime =
                            CoreServiceAgentRuntime::agent_runtime_with_scheduler_ports(coordinator.clone(), scheduler)
                ...
                let result_for_assistant =
                    session_control_cancel_result_message(session_id, &workspace, cancelled_turn_id.as_deref());
            ...
            SessionControlAction::Delete => {
                let session_id = params
                    .session_id
                    .as_deref()
                    .ok_or_else(|| NortHingError::tool("session_id is required for delete".to_string()))?;
                validate_session_id(session_id).map_err(NortHingError::tool)?;
                let workspace = self
                    .resolve_effective_workspace(SessionControlAction::Delete, Some(session_id), context, &runtime)
                    .await?;
                if self.current_workspace_session(context, &workspace) == Some(session_id) {
                    return Err(NortHingError::tool(
                ...
                Ok(vec![ToolResult::Result {
                    ...
                    result_for_assistant: Some(session_control_deleted_result_message(session_id, &workspace)),
                    image_attachments: None,
                }])
            }
            SessionControlAction::List => {
                let workspace = self
                    .resolve_effective_workspace(SessionControlAction::List, None, context, &runtime)
                    .await?;
...
mod tests {
    ...
    async fn validate_cancel_requires_session_id() {
        ...
        assert_eq!(validation.message.as_deref(), Some("session_id is required for cancel"));
    }
    ...
    async fn validate_list_rejects_session_id() {
        let tool = SessionControlTool::new();
        let workspace = TestTempDir::new("northhing-session-control-tool-test");
    ...
    async fn validate_list_requires_workspace() {
        ...
        assert!(!validation.result);
        assert_eq!(validation.message.as_deref(), Some("workspace is required for list"));
    }
