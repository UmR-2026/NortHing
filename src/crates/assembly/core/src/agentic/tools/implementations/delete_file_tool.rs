...
use std::path::Path;
use tool_runtime::fs::{
    build_remote_delete_command, delete_local_path, inspect_local_delete_target, DeleteLocalPathRequest,
};
...
3. **Path Requirements**:
   - You can use either relative paths (e.g., "temp/data.txt"), absolute paths inside the current workspace, or exact `northhing://runtime/...` URIs returned by another tool
 ...
 - The tool will fail gracefully if permissions are insufficient"#.to_string())
    ...
    fn input_schema(&self) -> Value {
        json!({
            ...
            "properties": {
                "path": {
                    "type": "string",
                    "description": "The file or directory to delete. Use a workspace-relative path, an absolute path inside the current workspace, or an exact northhing://runtime URI returned by another tool."
                },
                "recursive": {
                    "type": "boolean",
    ...
    async fn validate_input(&self, input: &Value, context: Option<&ToolUseContext>) -> ValidationResult {
        ...
        let resolved = match context.map(|ctx| ctx.resolve_tool_path(path_str)) {
            ...
            None => {
                if is_northhing_runtime_uri(path_str) {
                    return ValidationResult {
                        result: false,
        ...
        if !resolved.uses_remote_workspace_backend() {
            ...
            if local_path.is_dir() {
                ...
                let is_empty = tokio::task::spawn_blocking(move || inspect_local_delete_target(&local_path))
                    .await
                    .ok()
                    .and_then(Result::ok)
    ...
    fn render_result_for_assistant(&self, output: &Value) -> String {
        if let Some(path) = output.get("path").and_then(|v| v.as_str()) {
            let is_directory = output.get("is_directory").and_then(|v| v.as_bool()).unwrap_or(false);
    ...
    async fn call_impl(&self, input: &Value, context: &ToolUseContext) -> NortHingResult<Vec<ToolResult>> {
        let path_str = input
            .get("path")
            .and_then(|v| v.as_str())
            .ok_or_else(|| NortHingError::tool("path is required".to_string()))?;
        ...
        context
            .record_light_checkpoint("Delete", &resolved.logical_path, vec![resolved.logical_path.clone()])
            .await;
        ...
        if resolved.uses_remote_workspace_backend() {
            let ws_shell = context
                .ws_shell()
                .ok_or_else(|| NortHingError::tool("Workspace shell not available for remote Delete".to_string()))?;
            ...
            let (_stdout, stderr, exit_code) = ws_shell
                ...
                .await
                .map_err(|e| NortHingError::tool(format!("Failed to delete on remote: {}", e)))?;

            if exit_code != 0 && !stderr.is_empty() {
                return Err(NortHingError::tool(format!("Remote delete failed: {}", stderr)));
            }
        ...
        let outcome = tokio::task::spawn_blocking(move || delete_local_path(delete_request))
            .await
            .map_err(|error| NortHingError::tool(format!("Delete task failed: {}", error)))?
