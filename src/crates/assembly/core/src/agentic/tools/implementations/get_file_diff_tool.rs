...
impl GetFileDiffTool {
    ...
    async fn try_baseline_diff(
        ...
        if let Some(id) = baseline_id {
            ...
            let (additions, deletions) = self.calculate_diff_stats(&baseline_content, &current_content);
    ...
    async fn try_git_diff(&self, file_path: &Path) -> Option<NortHingResult<Value>> {
        ...
        let diff_output = match GitService::get_diff(file_dir, &git_diff_params).await {
            ...
            Err(e) => {
                ...
                let head_content = match GitService::get_file_content(file_dir, &relative_path_str, Some("HEAD")).await
                {
                    Ok(content) => content,
                    Err(e) => {
                        debug!(
                            "GetFileDiff tool failed to get HEAD file content: {}, file may be new or untracked",
                            e
                        );
                ...
                // Calculate statistics
                let (additions, deletions) = self.calculate_diff_stats(&head_content, &current_content);
        ...
        let original_content = match GitService::get_file_content(file_dir, &relative_path_str, Some("HEAD")).await {
            Ok(content) => content,
            Err(e) => {
    ...
    fn return_full_content(&self, file_path: &Path) -> NortHingResult<Value> {
        let content =
            fs::read_to_string(file_path).map_err(|e| NortHingError::tool(format!("Failed to read file: {}", e)))?;
...
Usage:
- The file_path parameter must be workspace-relative, an absolute path inside the current workspace, or an exact `northhing://runtime/...` URI returned by another tool.
    ...
    fn input_schema(&self) -> Value {
        json!({
            ...
            "properties": {
                "file_path": {
                    "type": "string",
                    "description": "The file to get diff for. Use a workspace-relative path, an absolute path inside the current workspace, or an exact northhing://runtime URI returned by another tool."
                }
            },
            "required": ["file_path"],
    ...
    async fn validate_input(&self, input: &Value, context: Option<&ToolUseContext>) -> ValidationResult {
        if let Some(file_path) = input.get("file_path").and_then(|v| v.as_str()) {
            ...
            let resolved = match context.map(|ctx| ctx.resolve_tool_path(file_path)) {
                ...
                None => {
                    if is_northhing_runtime_uri(file_path) {
                        return ValidationResult {
                            result: false,
                            message: Some("Tool context is required to resolve northhing runtime URIs".to_string()),
                            error_code: Some(400),
                            meta: None,
                        };
                    }
    ...
    async fn call_impl(&self, input: &Value, context: &ToolUseContext) -> NortHingResult<Vec<ToolResult>> {
        ...
        if resolved.uses_remote_workspace_backend() {
            let ws_fs = context.ws_fs().ok_or_else(|| {
                NortHingError::tool("Workspace file system not available for remote diff".to_string())
            })?;
            let content = ws_fs
                .read_file_text(&resolved.resolved_path)
                .await
                .map_err(|e| NortHingError::tool(format!("Failed to read file: {}", e)))?;
            let total_lines = content.lines().count();
            let data = json!({
        ...
        if resolved.is_runtime_artifact() {
            let content =
                fs::read_to_string(path).map_err(|e| NortHingError::tool(format!("Failed to read file: {}", e)))?;
            let total_lines = content.lines().count();
            let data = json!({
                "file_path": resolved.logical_path,
                "diff_type": "full",
        ...
        if let Some(result) = self.try_baseline_diff(path, context.workspace_root()).await {
            match result {
                ...
                Err(e) => {
                    warn!("GetFileDiff tool baseline diff failed: {}, trying git diff", e);
                    // Continue trying git
                }
            }
        }
        ...
        if let Some(result) = self.try_git_diff(path).await {
            match result {
                ...
                Err(e) => {
                    warn!("GetFileDiff tool git diff failed: {}, returning full content", e);
