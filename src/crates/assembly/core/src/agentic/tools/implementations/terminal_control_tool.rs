...
The terminal_session_id is returned inside <terminal_session_id>...</terminal_session_id> tags in BashTool results."#
    ...
    async fn validate_input(&self, input: &Value, _context: Option<&ToolUseContext>) -> ValidationResult {
        if input.get("terminal_session_id").and_then(|v| v.as_str()).is_none() {
    ...
    async fn call_impl(&self, input: &Value, _context: &ToolUseContext) -> NortHingResult<Vec<ToolResult>> {
        let terminal_session_id = input
            .get("terminal_session_id")
            .and_then(|v| v.as_str())
            .ok_or_else(|| NortHingError::tool("terminal_session_id is required".to_string()))?;
        ...
        let terminal_api = TerminalApi::from_singleton()
            .map_err(|e| NortHingError::tool(format!("Terminal not initialized: {}", e)))?;

        match action {
            "interrupt" => {
                debug!("TerminalControl: sending SIGINT to session {}", terminal_session_id);
                ...
                terminal_api
                    ...
                    .await
                    .map_err(|e| NortHingError::tool(format!("Failed to interrupt terminal session: {}", e)))?;
            ...
            "kill" => {
                ...
                if is_primary {
                    binding
                        ...
                        .await
                        .map_err(|e| NortHingError::tool(format!("Failed to close terminal session: {}", e)))?;
                } else {
                    terminal_api
                ...
                let result_for_assistant = if is_primary {
                    ...
                    format!("Background terminal session '{}' has been killed.", terminal_session_id)
                };
            ...
            _ => Err(NortHingError::tool(format!(
