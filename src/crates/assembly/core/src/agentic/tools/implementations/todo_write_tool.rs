...
"###.to_string())
    ...
    async fn call_impl(&self, input: &Value, _context: &ToolUseContext) -> NortHingResult<Vec<ToolResult>> {
        ...
        let todos = input
            ...
            .ok_or(NortHingError::validation("Missing required field: todos"))?;
        ...
        for todo in todos {
            ...
            if let Some(obj) = todo_obj.as_object_mut() {
                if !obj.contains_key("status") {
                    return Err(NortHingError::validation("Todo item missing status field"));
                }
                if !obj.contains_key("content") {
                    return Err(NortHingError::validation("Todo item missing content field"));
                }
