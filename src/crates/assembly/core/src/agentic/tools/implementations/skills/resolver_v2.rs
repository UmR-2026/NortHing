...
pub fn resolve_for_prompt_with_max(prompt: &str, skills: &[SkillInfo], max_results: usize) -> Vec<ResolvedSkill> {
...
fn tokenize(text: &str) -> std::collections::HashSet<String> {
    static STOP_WORDS: &[&str] = &[
        "the", "a", "an", "and", "or", "but", "for", "with", "to", "of", "in", "on", "at", "is", "are", "was", "were",
        "be", "been", "being", "have", "has", "had", "do", "does", "did", "will", "would", "should", "could", "may",
        "might", "must", "shall", "can", "need", "this", "that", "these", "those", "i", "you", "he", "she", "it", "we",
        "they", "me", "my", "your", "his", "her", "its", "our", "their", "what", "which", "who", "when", "where",
        "why", "how", "all", "each", "every", "both", "few", "more", "most", "other", "some", "such", "no", "not",
        "only", "own", "same", "so", "than", "too", "very", "just", "about", "into", "from", "up", "out", "if", "then",
        "there", "here", "now", "please", "help", "want", "get", "make", "use", "using", "used", "via", "per", "etc",
        "via",
    ];
...
mod tests {
    ...
    fn test_resolve_returns_relevant_skill() {
        let skills = vec![
            make_skill(
                ...
                "Systematically debug Rust code errors and crashes",
            ),
        ...
        assert!(
            resolved[0].score > 0.1,
            "expected debug skill score > 0.1, got {}",
            resolved[0].score
        );
    }
    ...
    fn test_score_skill_name_weighted_higher() {
        let prompt_keywords: std::collections::HashSet<String> = ["debug"].iter().map(|s| s.to_string()).collect();
