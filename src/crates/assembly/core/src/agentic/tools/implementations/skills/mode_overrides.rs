...
use crate::service::config::agent_profile_project_store::{
    ...
    save_project_agent_profiles_document_local, set_disabled_project_skills, set_project_skill_disabled,
...
fn normalize_user_overrides(disabled_skills: Vec<String>, enabled_skills: Vec<String>) -> UserModeSkillOverrides {
...
pub async fn load_user_mode_skill_overrides(mode_id: &str) -> NortHingResult<UserModeSkillOverrides> {
    ...
    Ok(normalize_user_overrides(
        config.map(|item| item.disabled_user_skills.clone()).unwrap_or_default(),
        config.map(|item| item.enabled_user_skills.clone()).unwrap_or_default(),
    ))
}
...
) -> NortHingResult<UserModeSkillOverrides> {
...
pub async fn clear_user_mode_skill_overrides(mode_id: &str) -> NortHingResult<UserModeSkillOverrides> {
    persist_agent_profile_from_value(
        mode_id,
        json!({
...
pub fn get_disabled_mode_skills_from_document(document: &ProjectAgentProfilesDocument, mode_id: &str) -> Vec<String> {
...
pub async fn load_project_mode_skills_document_local(
    workspace_root: &Path,
) -> NortHingResult<ProjectAgentProfilesDocument> {
...
pub async fn load_disabled_mode_skills_local(workspace_root: &Path, mode_id: &str) -> NortHingResult<Vec<String>> {
    let document = load_project_agent_profiles_document_local(workspace_root).await?;
    Ok(get_disabled_project_skills(&document, &resolve_profile_id(mode_id)))
}
...
pub async fn load_disabled_mode_skills_remote(
    ...
    let content = fs
        .read_file_text(&path)
        .await
        .map_err(|error| NortHingError::config(format!("Failed to read remote project mode profiles: {}", error)))?;
