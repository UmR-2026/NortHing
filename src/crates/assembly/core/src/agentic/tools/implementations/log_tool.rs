...
impl LogTool {
    ...
    /// Read log file
    async fn read_log(&self, log_path: &str, lines: Option<usize>) -> NortHingResult<String> {
        ...
        let mut file = fs::File::open(&path)
            .await
            .map_err(|e| NortHingError::io(format!("Failed to open log file: {}", e)))?;

        let mut content = String::new();
        file.read_to_string(&mut content)
            .await
            .map_err(|e| NortHingError::io(format!("Failed to read log file: {}", e)))?;
    ...
    /// Search log content
    async fn search_log(&self, log_path: &str, pattern: &str, level: Option<String>) -> NortHingResult<String> {
    ...
    async fn analyze_log(&self, log_path: &str) -> NortHingResult<Value> {
        let content = self.read_log(log_path, None).await?;
...
impl Tool for LogTool {
    ...
    async fn description(&self) -> NortHingResult<String> {
        Ok(
            r#"Read and analyze log files to debug issues, monitor application behavior, and understand system events.
...
The tool will return the log content or analysis results that you can use to diagnose issues."#
                .to_string(),
        )
    }

    fn short_description(&self) -> String {
        "Read and analyze log files for debugging and monitoring; mainly used in Debug mode.".to_string()
    }
    ...
    async fn call_impl(&self, input: &Value, _context: &ToolUseContext) -> NortHingResult<Vec<ToolResult>> {
        // Validate input
        let action = input
            .get("action")
            .and_then(|v| v.as_str())
            .ok_or_else(|| NortHingError::validation("Missing required field: action"))?;
        ...
        if action == "tail" && input.get("lines").is_none() {
            return Err(NortHingError::validation("tail action requires 'lines' parameter"));
        }

        // Validate search operation requires pattern parameter
        if action == "search" && input.get("pattern").is_none() {
            return Err(NortHingError::validation("search action requires 'pattern' parameter"));
        }

        // Parse input
        let log_input: LogToolInput = serde_json::from_value(input.clone())
            .map_err(|e| NortHingError::validation(format!("Invalid input: {}", e)))?;

        let log_path = log_input
            .log_path
            .as_ref()
            .ok_or_else(|| NortHingError::validation("log_path is required"))?;
        ...
        let result = match log_input.action.as_str() {
            ...
            "search" => {
                let pattern = log_input
                    .pattern
                    .as_ref()
                    .ok_or_else(|| NortHingError::validation("pattern is required for search action"))?;
                let level_filter = log_input.level.clone();
                let results = self.search_log(log_path, pattern, level_filter.clone()).await?;
                json!({
                    "action": "search",
                    "file_path": log_path,
                    "pattern": pattern,
        ...
        let result_for_assistant = serde_json::to_string_pretty(&result).unwrap_or_else(|_| result.to_string());
