use crate::agentic::tools::file_read_state_runtime::{local_file_modification_time_ms, record_file_read_state};
...
impl FileReadTool {
    ...
    pub fn with_config(default_max_lines_to_read: usize, max_line_chars: usize, max_total_chars: usize) -> Self {
        Self {
    ...
    async fn read_remote_window(
    ...
    ) -> NortHingResult<tool_runtime::fs::read_file::ReadFileResult> {
        let ws_shell = context
            .ws_shell()
            .ok_or_else(|| NortHingError::tool("Remote workspace shell is unavailable".to_string()))?;
        ...
        let (stdout, stderr, status) = ws_shell.exec(&command, None).await.map_err(|e| {
            ...
            NortHingError::tool(format!("Failed to read file: {}", e))
        })?;
        debug!(
...
"#,
    ...
    fn input_schema(&self) -> Value {
        json!({
            ...
            "properties": {
                "file_path": {
                    "type": "string",
                    "description": "The file to read. Use a workspace-relative path, an absolute path inside the current workspace, or an exact northhing://runtime URI returned by another tool."
                },
                "start_line": {
                    "type": "number",
    ...
    async fn call_impl(&self, input: &Value, context: &ToolUseContext) -> NortHingResult<Vec<ToolResult>> {
        ...
        let start_line = input.get("start_line").and_then(|v| v.as_u64()).unwrap_or(1) as usize;
        ...
        let lines_read = if read_file_result.total_lines == 0 || read_file_result.end_line < read_file_result.start_line
        {
            0
        } else {
