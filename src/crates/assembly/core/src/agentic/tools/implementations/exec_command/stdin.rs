...
use crate::service::remote_ssh::{
    global_remote_exec_process_manager, RemoteExecError, RemoteExecSessionCompletion,
...
use terminal_core::{
    global_exec_process_manager, LocalExecSessionCompletion, LocalExecSessionCompletionSource,
...
impl WriteStdinTool {
    ...
    async fn call_remote_pipe(&self, input: &Value, context: &ToolUseContext) -> NortHingResult<Vec<ToolResult>> {
        let session_id = Self::session_id_from_input(input)
            .ok_or_else(|| NortHingError::tool("session_id is required for WriteStdin".to_string()))?;
        let chars = input
            .get("chars")
            .and_then(Value::as_str)
            .unwrap_or_default()
            .to_string();
        let append_enter = input.get("append_enter").and_then(Value::as_bool).unwrap_or(false);
        ...
        let response_result = if let Some(bridge) = progress_bridge.as_ref() {
            ...
            global_remote_exec_process_manager().write_stdin(request).await
        };
        ...
        let response = match response_result {
            ...
            Err(error) => return Err(NortHingError::tool(format!("WriteStdin failed: {error}"))),
        };
...
Output is only what was produced during this tool call's wait window."#
    ...
    async fn call_impl(&self, input: &Value, context: &ToolUseContext) -> NortHingResult<Vec<ToolResult>> {
        ...
        let response_result = if let Some(bridge) = progress_bridge.as_ref() {
            ...
            global_exec_process_manager().write_stdin(request).await
        };
        if let Some(bridge) = progress_bridge {
            bridge.finish().await;
        }
