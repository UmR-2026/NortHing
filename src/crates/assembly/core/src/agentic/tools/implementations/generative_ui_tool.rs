...
use crate::agentic::tools::framework::{Tool, ToolExposure, ToolResult, ToolUseContext, ValidationResult};
...
impl GenerativeUITool {
    ...
    fn combined_reminder() -> String {
        format!(
            ...
            Self::northhing_design_system_reminder(),
            Self::northhing_widget_scaffold_reminder()
        )
    }

    fn builtin_theme_snapshot(theme_id: &str) -> Option<ThemePromptSnapshot> {
        match theme_id {
            "northhing-dark" => Some(ThemePromptSnapshot {
                id: "northhing-dark",
                theme_type: "dark",
                bg_primary: "#0e0e10",
                bg_secondary: "#1c1c1f",
            ...
            "northhing-light" => Some(ThemePromptSnapshot {
                id: "northhing-light",
                theme_type: "light",
                bg_primary: "#f3f3f5",
                bg_secondary: "#ffffff",
            ...
            "northhing-slate" => Some(ThemePromptSnapshot {
                id: "northhing-slate",
                theme_type: "dark",
                bg_primary: "#14161a",
                bg_secondary: "#22262c",
            ...
            "northhing-midnight" => Some(ThemePromptSnapshot {
                id: "northhing-midnight",
                theme_type: "dark",
                bg_primary: "#2b2d30",
                bg_secondary: "#1e1f22",
                bg_scene: "#27292c",
            ...
            "northhing-cyber" => Some(ThemePromptSnapshot {
                id: "northhing-cyber",
                theme_type: "dark",
                bg_primary: "#101010",
                bg_secondary: "#151515",
                ...
                style_notes:
                    "neon cyber tooling, black surfaces, glowing cyan accents, still compact and workbench-first",
            }),
            "northhing-tokyo-night" => Some(ThemePromptSnapshot {
                id: "northhing-tokyo-night",
                theme_type: "dark",
                bg_primary: "#1a1b26",
                bg_secondary: "#16161e",
                bg_scene: "#1a1b26",
                ...
                style_notes:
                    "Tokyo Night indigo night, soft blue accent and violet secondary highlights, calm IDE mood",
            }),
            "northhing-china-style" => Some(ThemePromptSnapshot {
                id: "northhing-china-style",
                theme_type: "light",
                bg_primary: "#faf8f0",
                bg_secondary: "#f5f3e8",
                bg_scene: "#fdfcf6",
            ...
            "northhing-china-night" => Some(ThemePromptSnapshot {
                id: "northhing-china-night",
                theme_type: "dark",
                bg_primary: "#1a1814",
                bg_secondary: "#212019",
                bg_scene: "#1e1c17",
    ...
    fn baseline_theme_context() -> String {
        let dark = Self::builtin_theme_snapshot("northhing-dark")
            .map(|snapshot| Self::format_theme_snapshot(&snapshot))
            .unwrap_or_default();
        let light = Self::builtin_theme_snapshot("northhing-light")
            .map(|snapshot| Self::format_theme_snapshot(&snapshot))
            .unwrap_or_default();
        format!(
    ...
    async fn build_theme_prompt_context(&self) -> Option<String> {
        ...
        let selected_theme_id = config_service
            .config::<String>(Some("themes.current"))
            .await
            .ok()
            .filter(|value| !value.trim().is_empty())
            .unwrap_or_else(|| "northhing-light".to_string());

        if selected_theme_id == "system" {
            return Some(format!(
                "northhing active theme selection: system. Exact runtime resolution is host-dependent, so do not assume one palette. {}",
        ...
        if let Some(snapshot) = Self::builtin_theme_snapshot(&selected_theme_id) {
            return Some(format!(
                "northhing active theme snapshot: {}. {}",
        ...
        Some(format!(
            "northhing active theme selection: {}. Backend does not have an exact built-in snapshot for this theme, so use northhing CSS variables strictly and avoid hard-coded fallback palettes. {}",
            selected_theme_id,
            Self::baseline_theme_context()
        ))
    }
...
28. If the widget is meant to match northhing's product UI, apply these reminders strictly: {} {}"#,
            Self::northhing_design_system_reminder(),
            Self::northhing_widget_scaffold_reminder()
        ))
    }
        ...
        json!({
            ...
            "properties": {
                ...
                "widget_code": {
                    "type": "string",
                    "description": format!(
                        "Raw HTML fragment or raw SVG. No Markdown code fences. For HTML: no <!DOCTYPE>, <html>, <head>, or <body>. The 260-line / 28KB guideline is a soft reliability threshold. For larger widgets, use data-driven loops, shared CSS classes, and simpler markup rather than truncating required behavior. {} If the widget should match northhing, rely on the host CSS variables instead of hard-coded colors or spacing. If the user asked for file navigation, do not finish this field until each clickable node has verified file metadata or is intentionally non-clickable.",
                        Self::combined_reminder()
                    )
                },
                "width": {
    ...
    async fn input_schema_for_model_with_context(&self, _context: Option<&ToolUseContext>) -> Value {
        ...
        if let Some(obj) = schema.as_object_mut() {
            obj.insert(
                "x-northhing-reminder".to_string(),
                Value::String(Self::combined_reminder()),
            );
            obj.insert(
                "x-northhing-design-system".to_string(),
                Value::String(Self::northhing_design_system_reminder().to_string()),
            );
            if let Some(theme_context) = theme_context {
                obj.insert(
                    "x-northhing-theme-context".to_string(),
    ...
    async fn validate_input(&self, input: &Value, _context: Option<&ToolUseContext>) -> ValidationResult {
        ...
        if widget_code.starts_with("```") {
            return ValidationResult {
                result: false,
                message: Some("widget_code must be raw HTML or SVG, not Markdown code fences".to_string()),
                error_code: Some(400),
                meta: None,
            };
        }
        ...
        if line_count > LARGE_WIDGET_CODE_SOFT_LINE_LIMIT || byte_count > LARGE_WIDGET_CODE_SOFT_BYTE_LIMIT {
            return ValidationResult {
                result: true,
                message: Some(format!(
    ...
    fn render_result_for_assistant(&self, output: &Value) -> String {
        let title = output.get("title").and_then(|v| v.as_str()).unwrap_or("widget");
    ...
    fn render_tool_use_message(
        ...
        let title = input.get("title").and_then(|v| v.as_str()).unwrap_or("widget");
        format!("Rendering widget: {}", title)
    }

    async fn call_impl(&self, input: &Value, context: &ToolUseContext) -> NortHingResult<Vec<ToolResult>> {
        let title = input.get("title").and_then(|v| v.as_str()).unwrap_or("Widget");
        let widget_code = input.get("widget_code").and_then(|v| v.as_str()).unwrap_or("");
        ...
        Ok(vec![ToolResult::Result {
            ...
            result_for_assistant: Some(format!("Rendered widget '{}' inline in the FlowChat tool card.", title)),
