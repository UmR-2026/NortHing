...
- The plan file path returned in the tool result may be an absolute runtime path (local) or a `northhing://runtime/...` URI (remote)
...
- A good plan is glanceable, not a wall of text."###
    ...
    async fn call_impl(&self, input: &Value, context: &ToolUseContext) -> NortHingResult<Vec<ToolResult>> {
        ...
        let name = input
            .get("name")
            .and_then(|v| v.as_str())
            .ok_or(NortHingError::validation("Missing required field: name"))?;

        let overview = input
            .get("overview")
            .and_then(|v| v.as_str())
            .ok_or(NortHingError::validation("Missing required field: overview"))?;

        let plan = input
            .get("plan")
            .and_then(|v| v.as_str())
            .ok_or(NortHingError::validation("Missing required field: plan"))?;
        ...
        fs::write(&plan_file_path, &file_content)
            .await
            .map_err(|e| NortHingError::tool(format!("Failed to write plan file: {}", e)))?;
