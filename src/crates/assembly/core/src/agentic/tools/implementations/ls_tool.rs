...
Usage:
- The path parameter must be relative to the current workspace, an absolute path inside the current workspace, or an exact `northhing://runtime/...` URI returned by another tool
    ...
    fn input_schema(&self) -> Value {
        json!({
            ...
            "properties": {
                "path": {
                    "type": "string",
                    "description": "Directory to list. Use a workspace-relative path, an absolute path inside the current workspace, or an exact northhing://runtime URI returned by another tool."
                },
                "limit": {
                    "type": "number",
        ...
        if let Some(path) = input.get("path").and_then(|v| v.as_str()) {
            ...
            if !resolved.uses_remote_workspace_backend() {
                ...
                if !local_path.exists() {
                    return ValidationResult {
                        result: false,
                        message: Some(format!("Directory does not exist: {}", resolved.logical_path)),
                        error_code: Some(404),
                        meta: None,
                    };
                }
                ...
                if !local_path.is_dir() {
                    return ValidationResult {
                        result: false,
                        message: Some(format!("Path is not a directory: {}", resolved.logical_path)),
                        error_code: Some(400),
                        meta: None,
                    };
                }
    ...
    async fn call_impl(&self, input: &Value, context: &ToolUseContext) -> NortHingResult<Vec<ToolResult>> {
        ...
        if resolved.uses_remote_workspace_backend() {
            let ws_shell = context
                .ws_shell()
                .ok_or_else(|| NortHingError::tool("Workspace shell not available for remote LS".to_string()))?;
            ...
            let (stdout, _stderr, _exit_code) = ws_shell
                ...
                .await
                .map_err(|e| NortHingError::tool(format!("Failed to list remote directory: {}", e)))?;
