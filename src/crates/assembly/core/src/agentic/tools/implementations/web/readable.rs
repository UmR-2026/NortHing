...
type ExtractorFn = fn(&str, &str) -> NortHingResult<ExtractedCandidate>;

pub(crate) fn normalize_requested_format(format: Option<&str>) -> NortHingResult<RequestedFormat> {
...
pub(crate) fn is_html(content_type: Option<&str>, content: &str) -> bool {
    ...
    sample_lower.contains("<!doctype html") || sample_lower.contains("<html") || sample_lower.contains("</html>")
}

pub(crate) fn extract_markdown_with_text_fallback(html: &str, base_url: &str) -> NortHingResult<ReadableOutput> {
...
fn attempt_legible(html: &str, base_url: &str) -> NortHingResult<ExtractedCandidate> {
    ...
    let article = match parse_legible(html, Some(base_url), Some(options)) {
        ...
        Err(LegibleError::NoBody) => {
            ...
            .map_err(|err| NortHingError::tool(format!("Legible extraction failed: {}", err)))?
        }
        Err(err) => return Err(NortHingError::tool(format!("Legible extraction failed: {}", err))),
    };
...
fn attempt_readability_js(html: &str, base_url: &str) -> NortHingResult<ExtractedCandidate> {
    ...
    let article = reader
        ...
        .map_err(|err| NortHingError::tool(format!("readability-js extraction failed: {}", err)))?;
...
fn convert_html_to_markdown(html: &str, base_url: &str) -> NortHingResult<String> {
    let converter = HtmlToMarkdown::builder()
        .skip_tags(vec!["script", "style", "noscript", "iframe"])
        .build();
    let markdown = converter
        .convert(html)
        .map_err(|err| NortHingError::tool(format!("Failed to convert HTML to markdown: {}", err)))?;
...
pub(crate) fn html_to_text(html: &str) -> String {
    ...
    let text = Regex::new(r"<[^>]+>").expect("valid tag regex").replace_all(&text, " ");
...
fn markdown_looks_usable(markdown: &str) -> bool {
    ...
    normalized.chars().count() >= MIN_MARKDOWN_CHARS && !looks_noisy(&normalized) && !looks_like_html(&normalized)
}

fn plain_text_looks_usable(text: &str) -> bool {
    let normalized = normalize_text(text);
    normalized.chars().count() >= MIN_PLAIN_TEXT_CHARS && !looks_noisy(&normalized) && !looks_like_html(&normalized)
}
