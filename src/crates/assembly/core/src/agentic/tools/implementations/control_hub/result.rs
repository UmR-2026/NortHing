...
pub fn ok_response(domain: &str, action: &str, data: Value, summary_for_assistant: Option<String>) -> Vec<ToolResult> {
    let body = json!({
        "ok": true,
        "domain": domain,
        "action": action,
...
/// receives the structured error JSON instead of a plain NortHingError text).
...
/// Convenience: lift a `NortHingError` into the structured envelope using the
...
pub fn lift_error(domain: &str, action: &str, default_code: ErrorCode, err: NortHingError) -> Vec<ToolResult> {
    err_response(domain, action, ControlHubError::new(default_code, err.to_string()))
}
