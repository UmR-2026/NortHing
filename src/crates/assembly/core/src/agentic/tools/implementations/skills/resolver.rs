...
mod tests {
    ...
    use crate::agentic::tools::implementations::skills::types::{ModeSkillStateReason, SkillInfo, SkillLocation};
    use std::collections::HashSet;

    fn builtin_skill(dir_name: &str) -> SkillInfo {
        SkillInfo {
            key: format!("user::northhing-system::{}", dir_name),
            name: dir_name.to_string(),
            description: String::new(),
            path: format!("/tmp/{}", dir_name),
            level: SkillLocation::User,
            source_slot: "northhing-system".to_string(),
    ...
    fn custom_user_skill(dir_name: &str) -> SkillInfo {
        SkillInfo {
            key: format!("user::northhing::{}", dir_name),
            name: dir_name.to_string(),
            description: String::new(),
            path: format!("/tmp/{}", dir_name),
            level: SkillLocation::User,
            source_slot: "northhing".to_string(),
            dir_name: dir_name.to_string(),
            is_builtin: false,
            group_key: None,
            is_shadowed: false,
    ...
    fn custom_user_skills_are_enabled_by_default() {
        ...
        let state =
            resolve_skill_state_for_mode(&custom, "agentic", &UserModeSkillOverrides::default(), &HashSet::new());
    ...
    fn overrides_apply_on_top_of_defaults() {
        ...
        let disabled_state = resolve_skill_state_for_mode(&pdf, "agentic", &overrides, &disabled_project);
        assert!(!disabled_state.effective_enabled);
        assert_eq!(disabled_state.reason, ModeSkillStateReason::BuiltinPolicyDisabled);

        overrides.enabled_skills.push(pdf.key.clone());
        let enabled_state = resolve_skill_state_for_mode(&pdf, "agentic", &overrides, &disabled_project);
        assert!(enabled_state.effective_enabled);
        assert_eq!(enabled_state.reason, ModeSkillStateReason::EnabledByUserOverride);
    }
}
