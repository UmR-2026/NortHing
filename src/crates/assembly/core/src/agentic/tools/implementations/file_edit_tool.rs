use crate::agentic::tools::file_read_state_runtime::{
    ...
    update_file_read_state_after_mutation, validate_edit_against_read_state, validate_edit_has_prior_read,
    FILE_UNEXPECTEDLY_MODIFIED_ERROR,
};
use crate::agentic::tools::file_tool_guidance::file_tool_guidance_message;
use crate::agentic::tools::framework::{Tool, ToolPathResolution, ToolResult, ToolUseContext, ValidationResult};
...
- The `file_path` parameter must be a workspace-relative path, an absolute path inside the current workspace, or an exact `northhing://runtime/...` URI returned by another tool.
...
impl FileEditTool {
    ...
    fn assert_atomic_edit_freshness(
        ...
        if let Some(error) = assert_file_not_unexpectedly_modified(read_state.as_ref(), content, current_mtime_ms).err()
        {
            return Err(NortHingError::tool(file_tool_guidance_message(
...
impl Tool for FileEditTool {
        ...
        if let Some(ctx) = context {
            ...
            let file_content = match read_current_file_content(ctx, &resolved).await {
                ...
                Err(error) => {
                    return ValidationResult {
                        result: false,
                        message: Some(format!("Failed to read file {}: {}", resolved.logical_path, error)),
                        error_code: Some(400),
                        meta: None,
                    };
                }
            ...
            if let Err(error) = apply_edit_to_content(&file_content, old_string, new_string, replace_all) {
    ...
    async fn call_impl(&self, input: &Value, context: &ToolUseContext) -> NortHingResult<Vec<ToolResult>> {
        ...
        let new_string = input
            .get("new_string")
            .and_then(|v| v.as_str())
            .ok_or_else(|| NortHingError::tool("new_string is required".to_string()))?;

        let old_string = input
            .get("old_string")
            .and_then(|v| v.as_str())
            .ok_or_else(|| NortHingError::tool("old_string is required".to_string()))?;
        ...
        context
            .record_light_checkpoint("Edit", &resolved.logical_path, vec![resolved.logical_path.clone()])
            .await;
        ...
        if resolved.uses_remote_workspace_backend() {
            ...
            let edit_result =
                apply_edit_to_content(&content, old_string, new_string, replace_all).map_err(|error| {
                    if Self::is_edit_content_guardrail_error(&error) {
                        NortHingError::tool(file_tool_guidance_message(error))
                    } else {
                        NortHingError::tool(error)
                    }
                })?;
            ...
            update_file_read_state_after_mutation(context, &resolved, &edit_result.new_content, timestamp_ms);
        ...
        let content = std::fs::read_to_string(&resolved.resolved_path)
            .map_err(|e| NortHingError::tool(format!("Failed to read file {}: {}", resolved.logical_path, e)))?;
        Self::assert_atomic_edit_freshness(context, &resolved, &content)?;
...
mod tests {
    ...
    fn edit_tool_short_description_matches_claude_summary() {
        assert_eq!(FileEditTool::new().short_description(), "A tool for editing files");
    }
