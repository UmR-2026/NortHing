...
static BACKGROUND_COMMAND_OUTPUT_CAPTURE: OnceLock<Arc<BackgroundCommandOutputCapture>> = OnceLock::new();
...
impl BackgroundCommandOutputCapture {
    ...
    pub async fn set_session_id(&self, capture_id: &str, exec_session_id: Option<i32>) {
        ...
        let old_key = record.exec_session_id.map(|id| BackgroundCommandSessionKey {
            remote: record.remote,
            exec_session_id: id,
        });
        record.exec_session_id = exec_session_id;
        let new_key = record.exec_session_id.map(|id| BackgroundCommandSessionKey {
            remote: record.remote,
            exec_session_id: id,
        });
        drop(records);
    ...
    pub async fn update_lifecycle(
        ...
        self.session_index.lock().await.insert(key, capture_id.to_string());
    ...
    pub async fn list(&self, request: ListBackgroundCommandOutputRequest) -> ListBackgroundCommandOutputResponse {
        self.cleanup_expired().await;
            ...
            .filter(|record| {
                record.exec_session_id.is_some()
                    && request
                        .agent_session_id
                        .as_ref()
                        .is_none_or(|session_id| record.agent_session_id.as_deref() == Some(session_id.as_str()))
            })
...
impl BackgroundCommandOutputRecord {
    fn snapshot(&self) -> String {
        self.chunks.iter().map(|chunk| chunk.text.as_str()).collect::<String>()
    }
...
mod tests {
    use super::{
        background_command_output_capture, BackgroundCommandOutputStatus, ReadBackgroundCommandOutputRequest,
    ...
    async fn background_command_output_reads_snapshot_then_incremental_chunks() {
        ...
        capture
            .update_lifecycle(&capture_id, 4242, BackgroundCommandOutputStatus::Running, None)
            .await
            .expect("record exists");
        tx.send("hello".to_string()).expect("capture receiver alive");
        tx.send(" world".to_string()).expect("capture receiver alive");
        ...
        assert_eq!(second.metadata.status, BackgroundCommandOutputStatus::Exited);
    }
}
