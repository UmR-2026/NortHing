...
impl SessionHistoryTool {
    ...
    fn resolve_session_id(&self, session_id: &str) -> NortHingResult<String> {
        let session_id = session_id.trim().to_string();
        Self::validate_session_id(&session_id).map_err(NortHingError::tool)?;
        Ok(session_id)
    }
}
...
5. Export a middle range: `turns=["2:5"]`"#
    ...
    async fn call_impl(&self, input: &Value, _context: &ToolUseContext) -> NortHingResult<Vec<ToolResult>> {
        let params: SessionHistoryInput =
        ...
        let workspace = coordinator
            ...
            .ok_or_else(|| {
                NortHingError::NotFound(format!("Workspace for session '{}' could not be resolved", session_id))
            })?;
