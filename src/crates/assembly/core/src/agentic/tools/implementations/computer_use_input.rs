use crate::agentic::tools::computer_use_host::{
    ComputerUseImplicitScreenshotCenter, ComputerUseNavigateQuadrant, ComputerUseScreenshotParams, ScreenshotCropCenter,
};
...
pub fn ensure_pointer_move_uses_screen_coordinates_only(input: &Value) -> NortHingResult<()> {
...
pub fn coordinate_mode(input: &Value) -> &str {
    input.get("coordinate_mode").and_then(|v| v.as_str()).unwrap_or("image")
}

#[allow(dead_code)] // kept around for the deprecation shim — no longer wired in
pub fn parse_screenshot_crop_center(input: &Value) -> NortHingResult<Option<ScreenshotCropCenter>> {
    ...
    match (x_none, y_none) {
        ...
        (false, false) => {
            let x = xv
                .and_then(|v| v.as_u64())
                .ok_or_else(|| NortHingError::tool("screenshot_crop_center_x must be a non-negative integer (full-display native pixels).".to_string()))?;
            let y = yv
                .and_then(|v| v.as_u64())
                .ok_or_else(|| NortHingError::tool("screenshot_crop_center_y must be a non-negative integer (full-display native pixels).".to_string()))?;
            Ok(Some(ScreenshotCropCenter {
                x: u32::try_from(x)
                    .map_err(|_| NortHingError::tool("screenshot_crop_center_x is too large.".to_string()))?,
                y: u32::try_from(y)
                    .map_err(|_| NortHingError::tool("screenshot_crop_center_y is too large.".to_string()))?,
            }))
        }
...
// reason: kept around for the deprecation shim — no longer wired in
#[allow(dead_code)]
pub fn parse_screenshot_crop_half_extent_native(input: &Value) -> NortHingResult<Option<u32>> {
    match input.get("screenshot_crop_half_extent_native") {
        None => Ok(None),
        Some(v) if v.is_null() => Ok(None),
        Some(v) => {
            let n = v.as_u64().ok_or_else(|| {
                NortHingError::tool("screenshot_crop_half_extent_native must be a non-negative integer.".to_string())
            })?;
            Ok(Some(u32::try_from(n).map_err(|_| {
                NortHingError::tool("screenshot_crop_half_extent_native is too large.".to_string())
            })?))
        }
    }
}
...
// reason: kept around for the deprecation shim — no longer wired in
#[allow(dead_code)]
pub fn parse_screenshot_implicit_center(input: &Value) -> NortHingResult<Option<ComputerUseImplicitScreenshotCenter>> {
    match input
        ...
        .map(str::trim)
    {
        ...
        Some(other) => Err(NortHingError::tool(format!(
...
// reason: kept around for the deprecation shim — no longer wired in
#[allow(dead_code)]
pub fn parse_screenshot_navigate_quadrant(input: &Value) -> NortHingResult<Option<ComputerUseNavigateQuadrant>> {
    let value = input
...
pub fn parse_screenshot_window_flag(input: &Value) -> bool {
    let raw = input.get("screenshot_window").or_else(|| input.get("window"));
    let Some(v) = raw else {
        return false;
    };
...
pub fn parse_screenshot_params(input: &Value) -> NortHingResult<(ComputerUseScreenshotParams, bool)> {
...
mod tests {
    ...
    fn screenshot_params_silently_ignore_legacy_quadrant_and_crop_fields() {
        ...
        let (params, ignored_crop) = parse_screenshot_params(&input).expect("parse screenshot params");
