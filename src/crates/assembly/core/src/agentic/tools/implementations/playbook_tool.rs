...
impl PlaybookTool {
    pub fn new() -> Self {
                ...
                if ext == "yaml" || ext == "yml" {
                    if let Some(contents) = entry.contents_utf8() {
                        match serde_yaml::from_str::<PlaybookDef>(contents) {
                            ...
                            Err(e) => {
                                tracing::warn!("Failed to parse playbook {}: {}", entry.path().display(), e);
                            }
                        }
                    }
                }
        ...
        info!("PlaybookTool initialized with {} builtin playbooks", playbooks.len());
        Self { playbooks }
    }
    ...
    fn resolve_templates(value: &Value, vars: &HashMap<String, String>) -> Value {
        match value {
            ...
            Value::Array(arr) => Value::Array(arr.iter().map(|v| Self::resolve_templates(v, vars)).collect()),
            other => other.clone(),
        }
    }
...
{}"#,
    ...
    async fn call_impl(&self, input: &Value, _context: &ToolUseContext) -> NortHingResult<Vec<ToolResult>> {
        let action = input
            .get("action")
            .and_then(|v| v.as_str())
            .ok_or_else(|| NortHingError::tool("Missing 'action'".to_string()))?;
        ...
        match action {
            ...
            "run" => {
                let name = input
                    .get("name")
                    .and_then(|v| v.as_str())
                    .ok_or_else(|| NortHingError::tool("'run' requires 'name'".to_string()))?;

                let pb = self.find_playbook(name).ok_or_else(|| {
                    let available: Vec<&str> = self.playbooks.iter().map(|p| p.name.as_str()).collect();
                    NortHingError::tool(format!("Playbook '{}' not found. Available: {:?}", name, available))
                })?;
