use super::readable::{
    extract_html_title, extract_markdown_with_text_fallback, is_html, normalize_requested_format, RequestedFormat,
};
...
- markdown: Readable content mode. For HTML pages, northhing extracts the main content and returns markdown when possible, automatically falling back to plain text when markdown conversion is not reliable.
...
- Get API data: {"url": "https://api.example.com/data", "format": "json"}"#
    ...
    async fn call_impl(&self, input: &Value, _context: &ToolUseContext) -> NortHingResult<Vec<ToolResult>> {
        let url = input
            .get("url")
            .and_then(|v| v.as_str())
            .ok_or_else(|| NortHingError::tool("url is required".to_string()))?;

        let requested_format = normalize_requested_format(input.get("format").and_then(|v| v.as_str()))?;

        let client = reqwest::Client::builder()
            .user_agent("northhing/1.0")
            .timeout(std::time::Duration::from_secs(30))
            .build()
            .map_err(|e| NortHingError::tool(format!("Failed to create HTTP client: {}", e)))?;

        let response = client
            .get(url)
            .send()
            .await
            .map_err(|e| NortHingError::tool(format!("Failed to fetch URL: {}", e)))?;
        ...
        let content = response
            .text()
            .await
            .map_err(|e| NortHingError::tool(format!("Failed to read response: {}", e)))?;
        ...
        let (processed_content, content_representation, extractor, title) = match requested_format {
            ...
            RequestedFormat::Json => {
                serde_json::from_str::<Value>(&content)
                    .map_err(|e| NortHingError::tool(format!("Invalid JSON response: {}", e)))?;
