...
use crate::service::search::{
    ...
    workspace_search_runtime_available, GlobSearchRequest,
};
...
use tool_runtime::search::glob_search::{
    build_remote_find_command, build_remote_rg_command, collect_remote_glob_matches, execute_local_glob,
    ...
    fn input_schema(&self) -> Value {
        json!({
            ...
            "properties": {
                ...
                "path": {
                    "type": "string",
                    "description": "The directory to search in. Omit this field to search the current workspace. If provided, use a workspace-relative path, an absolute path inside the current workspace, or an exact northhing://runtime URI. Do not enter \"undefined\", \"null\", host roots, or placeholder paths such as /workspace."
                },
                "limit": {
                    "type": "number",
    ...
    async fn call_impl(&self, input: &Value, context: &ToolUseContext) -> NortHingResult<Vec<ToolResult>> {
        ...
        let resolved = match input.get("path").and_then(|v| v.as_str()) {
            ...
            None => {
                let root = context
                    ...
                    .ok_or_else(|| {
                        NortHingError::tool("workspace_path is required when Glob path is omitted".to_string())
                    })?;
        ...
        if resolved.uses_remote_workspace_backend() {
            if workspace_search_feature_enabled().await {
                let remote_workspace_glob_result = async {
                    ...
                    let preferred_connection_id = context
                        ...
                        .map(str::to_string);
                    let search_service = remote_workspace_search_service_for_path(&repo_root, preferred_connection_id)
                        .await
                    ...
                    Ok::<Vec<ToolResult>, NortHingError>(vec![ToolResult::Result {
                        data: json!({
                            "pattern": pattern,
            ...
            let (_stdout, _stderr, exit_code) = ws_shell
                ...
                .await
                .map_err(|e| NortHingError::tool(format!("Failed to detect rg on remote: {}", e)))?;
            ...
            let (stdout, _stderr, _exit_code) = ws_shell
                ...
                .await
                .map_err(|e| NortHingError::tool(format!("Failed to glob on remote with rg: {}", e)))?;
        ...
        .await
        .map_err(|err| NortHingError::tool(format!("Glob tool task failed: {}", err)))?
        .map_err(NortHingError::tool)?;
...
mod tests {
    ...
    use tool_runtime::search::glob_search::{
        derive_walk_root, execute_local_glob, extract_glob_base_directory, normalize_path, LocalGlobRequest,
    };
    ...
    fn make_temp_dir(name: &str) -> PathBuf {
        let unique = SystemTime::now()
            ...
            .unwrap_or_default()
            .as_nanos();
        let dir = std::env::temp_dir().join(format!("northhing-glob-tool-{name}-{unique}"));
    ...
    fn extracts_static_glob_prefix() {
        ...
        assert_eq!(extract_glob_base_directory("*.rs"), (String::new(), "*.rs".to_string()));
        assert_eq!(
    ...
    fn does_not_expand_walk_root_outside_search_path() {
        let root = std::env::temp_dir().join("northhing-glob-root");
