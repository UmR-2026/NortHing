...
use northhing_agent_tools::{file_read_facts_are_fresh, file_read_facts_content_matches, FileReadFreshnessFacts};
use std::path::Path;
...
pub fn validate_write_has_prior_read(context: &ToolUseContext, resolved: &ToolPathResolution) -> Option<String> {
...
pub fn read_state_tracking_enabled(context: &ToolUseContext) -> bool {
    context.session_id.is_some() && global_coordinator().is_some()
}
...
pub fn record_file_read_state(
    ...
    coordinator
        .session_manager()
        .set_file_read_state(session_id, &resolved.logical_path, state);
}

pub fn get_stored_file_read_state(context: &ToolUseContext, resolved: &ToolPathResolution) -> Option<FileReadState> {
    let session_id = context.session_id.as_deref()?;
    let coordinator = global_coordinator()?;
    coordinator
        .session_manager()
...
pub fn content_unchanged_since_full_read(read_state: &FileReadState, current_content: &str) -> bool {
...
pub fn assert_file_not_unexpectedly_modified(
    ...
    if !file_read_facts_are_fresh(file_read_freshness_facts(read_state), current_content, current_mtime_ms) {
        return Err(FILE_UNEXPECTEDLY_MODIFIED_ERROR.to_string());
    }
...
pub fn validate_edit_has_prior_read(context: &ToolUseContext, resolved: &ToolPathResolution) -> Option<String> {
    let session_id = context.session_id.as_deref()?;
    let coordinator = global_coordinator()?;
    let Some(read_state) = coordinator
        .session_manager()
...
pub fn update_file_read_state_after_mutation(
    ...
    let (start_line, end_line) = if line_count == 0 { (0, 0) } else { (1, line_count) };
    let state = FileReadState {
...
pub async fn read_current_file_content(
    ...
    if resolved.uses_remote_workspace_backend() {
        let ws_fs = context.ws_fs().ok_or_else(|| {
            crate::util::errors::NortHingError::tool("Remote workspace file system is unavailable".to_string())
        })?;
        ws_fs
            .read_file_text(&resolved.resolved_path)
            .await
            .map_err(|error| crate::util::errors::NortHingError::tool(format!("Failed to read file: {}", error)))
    } else {
        std::fs::read_to_string(&resolved.resolved_path).map_err(|error| {
            crate::util::errors::NortHingError::tool(format!(
                "Failed to read file {}: {}",
                resolved.logical_path, error
            ))
        })
...
async fn file_modification_time_ms(_context: &ToolUseContext, resolved: &ToolPathResolution) -> Option<u64> {
    if resolved.uses_remote_workspace_backend() {
        return None;
    }
...
pub async fn file_mutation_timestamp_ms(context: &ToolUseContext, resolved: &ToolPathResolution) -> u64 {
...
mod tests {
    ...
    fn assert_file_not_unexpectedly_modified_rejects_partial_read_after_newer_mtime() {
        ...
        assert!(assert_file_not_unexpectedly_modified(Some(&state), "full file\n", Some(200)).is_err());
    }
    ...
    fn validate_content_freshness_allows_matching_remote_content_without_mtime() {
        ...
        assert!(validate_edit_content_freshness_against_read_state("src/main.rs", &state, "alpha\n", None,).is_none());
    }
    ...
    fn validate_content_freshness_rejects_changed_remote_content_without_mtime() {
        ...
        assert_eq!(
            validate_edit_content_freshness_against_read_state("src/main.rs", &state, "beta\n", None,).as_deref(),
            Some("The file src/main.rs no longer matches the last Read result. Use Read again, then retry Edit.")
        );
    }
    ...
    fn validate_content_freshness_allows_newer_mtime_when_full_read_content_matches() {
        ...
        assert!(
            validate_edit_content_freshness_against_read_state("src/main.rs", &state, "alpha\n", Some(200),).is_none()
        );
    }
