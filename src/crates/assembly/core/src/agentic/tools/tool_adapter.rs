...
//! reviewed owner migration. Generic contracts live in `northhing-agent-tools`;
...
use northhing_agent_tools::{ContextualToolManifestItem, ToolRegistryItem};
...
impl ToolRegistryItem for dyn Tool {
    ...
    async fn description(&self) -> Result<String, String> {
        Tool::description(self).await.map_err(|error| error.to_string())
    }
...
impl ContextualToolManifestItem<ToolUseContext> for dyn Tool {
    ...
    async fn input_schema_for_model_with_context(&self, context: &ToolUseContext) -> serde_json::Value {
