...
pub use northhing_agent_tools::{
    ...
    resolve_tool_path_with_context, tool_path_is_effectively_absolute, DynamicMcpToolInfo, DynamicToolInfo,
    PortableToolContextProvider, ToolContextFacts, ToolExposure, ToolPathBackend, ToolPathResolution,
    ToolRenderOptions, ToolResult, ToolWorkspaceKind, ValidationResult,
};
...
pub trait Tool: Send + Sync {
    ...
    async fn call(&self, input: &Value, context: &ToolUseContext) -> NortHingResult<Vec<ToolResult>> {
        crate::agentic::tools::tool_context_runtime::call_tool_with_runtime_hooks(self, input, context).await
    }
}
