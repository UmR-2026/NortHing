...
mod tests {
    ...
    use northhing_agent_tools::GET_TOOL_SPEC_TOOL_NAME;
    ...
    async fn manifest_resolver_facade_preserves_product_owner_output() {
        ...
        let facade = resolve_tool_manifest(&allowed_tools, &AgentToolPolicyOverrides::default(), &context).await;
        let owner =
            resolve_product_resolved_tool_manifest(&allowed_tools, &AgentToolPolicyOverrides::default(), &context)
                .await;
        ...
        assert!(facade.allowed_tool_names.contains(&GET_TOOL_SPEC_TOOL_NAME.to_string()));
    }
    ...
    async fn visible_tools_facade_preserves_product_owner_output() {
        ...
        let facade = resolve_visible_tools(&allowed_tools, &AgentToolPolicyOverrides::default(), &context).await;
        let owner =
            resolve_product_resolved_visible_tools(&allowed_tools, &AgentToolPolicyOverrides::default(), &context)
                .await;
