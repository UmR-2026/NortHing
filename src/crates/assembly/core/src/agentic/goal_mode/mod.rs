...
pub use northhing_agent_runtime::thread_goal::{
    billable_tokens_from_counts, build_objective_updated_plan, build_thread_goal_continuation_plan,
    clear_thread_goal_patch, completion_budget_report, continuation_prompt, effective_subagent_timeout_seconds,
...
pub use northhing_runtime_ports::{
    validate_thread_goal_objective, SetThreadGoalResult, ThreadGoal, ThreadGoalContinuationPlan, ThreadGoalStatus,
    ThreadGoalToolResponse, GOAL_MODE_METADATA_KEY, MAX_CONTEXT_SUMMARY_CHARS, MAX_GOAL_CONTINUATIONS,
    MAX_THREAD_GOAL_AUTO_CONTINUATIONS, MAX_THREAD_GOAL_OBJECTIVE_CHARS, THREAD_GOAL_METADATA_KEY,
};
use std::path::Path;
...
pub fn thread_goal_from_custom_metadata(custom_metadata: Option<&serde_json::Value>) -> Option<ThreadGoal> {
    northhing_agent_runtime::thread_goal::thread_goal_from_custom_metadata(
        custom_metadata,
...
pub fn is_usage_limit_error(error: &NortHingError) -> bool {
...
impl<'a> ThreadGoalStore<'a> {
    ...
    async fn load_metadata(
        ...
        session_id: &str,
        workspace_path: &Path,
    ) -> NortHingResult<Option<serde_json::Value>> {
        Ok(self
            .session_manager
    ...
    pub async fn clear_thread_goal(&self, session_id: &str, workspace_path: &Path) -> NortHingResult<()> {
        self.persist_thread_goal(session_id, workspace_path, None).await
    }
    ...
    pub async fn set_thread_goal(
    ...
    ) -> NortHingResult<SetThreadGoalResult> {
        ...
        .map_err(|error| match error {
            northhing_agent_runtime::thread_goal::ThreadGoalRuntimeError::Validation(message) => {
                NortHingError::Validation(message)
            }
            northhing_agent_runtime::thread_goal::ThreadGoalRuntimeError::NotFound(message) => {
                NortHingError::NotFound(message)
            }
        })?;
    ...
    pub async fn create_thread_goal(
        ...
        if self.get_thread_goal(session_id, workspace_path).await?.is_some() {
    ...
    if let Some(goal) = outcome.goal_to_persist.as_ref() {
        ...
        if outcome.scheduled_auto_continuation {
            info!(
                ...
                session_id, goal.auto_continuation_count, MAX_THREAD_GOAL_AUTO_CONTINUATIONS, goal.objective
            );
        }
    }
...
pub fn user_facing_thread_goal_error(error: NortHingError) -> NortHingError {
    match error {
        NortHingError::Validation(_) | NortHingError::NotFound(_) => error,
        other => {
            warn!("Thread goal operation failed: {other}");
            NortHingError::Validation("Thread goal operation failed. Check session state and try again.".to_string())
        }
    }
}
...
mod tests {
    ...
    fn resumable_statuses_match_ui_actions() {
        ...
        assert!(thread_goal_status_is_resumable(ThreadGoalStatus::UsageLimited));
        assert!(!thread_goal_status_is_resumable(ThreadGoalStatus::Active));
        assert!(!thread_goal_status_is_resumable(ThreadGoalStatus::BudgetLimited));
    ...
    fn continuation_plan_metadata_marks_completion_check() {
        ...
        assert_eq!(plan.user_message_metadata["threadGoalContinuationCheck"], true);
    ...
    fn usage_limit_error_detects_quota_messages() {
        assert!(is_usage_limit_error(&NortHingError::AIClient(
            "insufficient_quota: billing hard limit".to_string()
        )));
        assert!(!is_usage_limit_error(&NortHingError::Validation(
