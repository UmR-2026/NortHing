...
impl ImageAnalyzer {
    ...
    pub async fn analyze_images(
    ...
    ) -> NortHingResult<Vec<ImageAnalysisResult>> {
        ...
        for img_ctx in request.images {
            ...
            let task = tokio::spawn(async move {
                Self::analyze_single_image(img_ctx, &model, user_msg.as_deref(), workspace, ai_client).await
            });
        ...
        for task in tasks {
            match task.await {
                ...
                Err(e) => {
                    ...
                    return Err(NortHingError::service(format!("Image analysis task failed: {}", e)));
                }
            }
        }
    ...
    async fn analyze_single_image(
        ...
        workspace_path: Option<PathBuf>,
        ai_client: Arc<AIClient>,
    ) -> NortHingResult<ImageAnalysisResult> {
        let start = std::time::Instant::now();

        debug!("Analyzing image: {}", image_ctx.id);

        let (image_data, fallback_mime) = Self::load_image_from_context(&image_ctx, workspace_path.as_deref()).await?;
        ...
        let messages =
            build_multimodal_message(&analysis_prompt, &processed.data, &processed.mime_type, &model.provider)?;
        ...
        let ai_response = ai_client.send_message(messages, None).await.map_err(|e| {
            ...
            NortHingError::service(format!("Image analysis AI call failed: {}", e))
        })?;
    ...
    async fn load_image_from_context(
    ...
    ) -> NortHingResult<(Vec<u8>, Option<String>)> {
        ...
        Err(NortHingError::validation("Image context missing path or data"))
    }
        ...
        if let Ok(parsed) = serde_json::from_str::<serde_json::Value>(json_str) {
            return ImageAnalysisResult {
                ...
                summary: parsed["summary"]
                    ...
                    .to_string(),
                detailed_description: parsed["detailed_description"].as_str().unwrap_or(response).to_string(),
