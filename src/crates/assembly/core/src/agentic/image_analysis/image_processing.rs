...
use crate::service::config::types::{AIConfig as ServiceAIConfig, AIModelConfig, ModelCapability, ModelCategory};
...
pub fn resolve_vision_model_from_ai_config(ai_config: &ServiceAIConfig) -> NortHingResult<AIModelConfig> {
    ...
    let Some(id) = target_model_id else {
        return Err(NortHingError::service(
            "Image understanding model is not configured.\nPlease select a model in Settings.".to_string(),
        ));
    };
    ...
    let model = ai_config
        ...
        .find(|m| m.id == id)
        .cloned()
        .ok_or_else(|| NortHingError::service(format!("Model not found: {}", id)))?;

    if !model.enabled {
        return Err(NortHingError::service(format!("Model is disabled: {}", id)));
    }
...
pub async fn resolve_vision_model_from_global_config() -> NortHingResult<AIModelConfig> {
    let config_service = get_global_config_service().await?;
    let ai_config: ServiceAIConfig = config_service
        .config(Some("ai"))
        .await
        .map_err(|e| NortHingError::service(format!("Failed to get AI config: {}", e)))?;

    resolve_vision_model_from_ai_config(&ai_config)
}

pub fn resolve_image_path(path: &str, workspace_path: Option<&Path>) -> NortHingResult<PathBuf> {
...
pub async fn load_image_from_path(path: &Path, _workspace_path: Option<&Path>) -> NortHingResult<Vec<u8>> {
    fs::read(path)
        .await
        .map_err(|e| NortHingError::io(format!("Failed to read image: {}", e)))
}

pub fn decode_data_url(data_url: &str) -> NortHingResult<(Vec<u8>, Option<String>)> {
    if !data_url.starts_with("data:") {
        return Err(NortHingError::validation("Invalid data URL format"));
    }

    let parts: Vec<&str> = data_url.splitn(2, ',').collect();
    if parts.len() != 2 {
        return Err(NortHingError::validation("Data URL format error"));
    }
    ...
    let image_data = BASE64
        .decode(base64_data)
        .map_err(|e| NortHingError::parse(format!("Base64 decode failed: {}", e)))?;

    Ok((image_data, mime_type))
}

pub fn detect_mime_type_from_bytes(image_data: &[u8], fallback_mime: Option<&str>) -> NortHingResult<String> {
    ...
    Err(NortHingError::validation("Unsupported or unrecognized image format"))
}
...
pub fn optimize_image_with_size_limit(
    ...
    let dynamic = image::load_from_memory(&image_data)
        .map_err(|e| NortHingError::validation(format!("Failed to decode image data: {}", e)))?;
...
pub async fn process_image_contexts_for_provider(
...
) -> NortHingResult<Vec<ProcessedImage>> {
    let limits = ImageLimits::for_provider(provider);
    ...
    for ctx in image_contexts {
        ...
        let processed = optimize_image_for_provider(image_data, provider, fallback_mime.as_deref())?;
...
fn encode_dynamic_image(
    ...
    match target_format {
        ImageFormat::Png => {
            ...
            encoder
                .write_image(rgba.as_raw(), image.width(), image.height(), ColorType::Rgba8.into())
                .map_err(|e| NortHingError::tool(format!("PNG encode failed: {}", e)))?;
        }
        ImageFormat::Jpeg => {
            let mut encoder = JpegEncoder::new_with_quality(&mut buffer, jpeg_quality);
            encoder
                .encode_image(image)
                .map_err(|e| NortHingError::tool(format!("JPEG encode failed: {}", e)))?;
        }
        other => {
            // Defensive: a new `ImageFormat` variant was added without being mapped in
            // the `format -> target_format` normalization above. Fail the encode
            // gracefully so the caller (image pipeline) can surface a tool error
    ...
    let mime = image_format_to_mime(target_format).unwrap_or("image/png").to_string();
