...
pub(crate) async fn build_init_agents_md_user_input() -> NortHingResult<(String, Vec<Message>)> {
    let prompt = get_embedded_prompt(INIT_AGENTS_MD_PROMPT_NAME)
        .ok_or_else(|| NortHingError::Agent(format!("{} not found in embedded files", INIT_AGENTS_MD_PROMPT_NAME)))?;
