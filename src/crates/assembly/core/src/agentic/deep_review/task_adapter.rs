...
use crate::agentic::deep_review_policy::{
    ...
    deep_review_max_retries_per_role, deep_review_queue_control_snapshot, record_deep_review_capacity_skip_for_reason,
...
use crate::agentic::events::{DeepReviewQueueReason, DeepReviewQueueState, DeepReviewQueueStatus, ErrorCategory};
use crate::util::errors::{NortHingError, NortHingResult};
use northhing_agent_runtime::deep_review::task_execution as runtime_task_execution;
pub(crate) use northhing_agent_runtime::deep_review::task_execution::{
    attach_deep_review_cache, deep_review_incremental_cache_hit_for_task, deep_review_incremental_cache_hit_result,
    deep_review_launch_batch_for_task, ensure_deep_review_retry_coverage, prompt_with_deep_review_retry_scope,
    DeepReviewProviderCapacityRetryDecision, DeepReviewProviderCapacityRetryRuntime,
};
pub(crate) use northhing_agent_runtime::deep_review::{DeepReviewLaunchBatchInfo, DeepReviewQueueWaitSkipReason};
...
pub(crate) fn deep_review_cancelled_reviewer_result(
    ...
    runtime_task_execution::deep_review_cancelled_reviewer_result(subagent_type, reason, duration_ms)
}

pub(crate) fn queue_reason_to_event_reason(reason: DeepReviewCapacityQueueReason) -> DeepReviewQueueReason {
    match reason {
        DeepReviewCapacityQueueReason::ProviderRateLimit => DeepReviewQueueReason::ProviderRateLimit,
        DeepReviewCapacityQueueReason::ProviderConcurrencyLimit => DeepReviewQueueReason::ProviderConcurrencyLimit,
        DeepReviewCapacityQueueReason::RetryAfter => DeepReviewQueueReason::RetryAfter,
        DeepReviewCapacityQueueReason::LocalConcurrencyCap => DeepReviewQueueReason::LocalConcurrencyCap,
        DeepReviewCapacityQueueReason::LaunchBatchBlocked => DeepReviewQueueReason::LaunchBatchBlocked,
        DeepReviewCapacityQueueReason::TemporaryOverload => DeepReviewQueueReason::TemporaryOverload,
    }
}

pub(crate) fn capacity_decision_for_provider_error(error: &NortHingError) -> DeepReviewCapacityQueueDecision {
    let detail = error.error_detail();
    let error_message = error.to_string();
    let code = detail.provider_code.as_deref().unwrap_or_default();
    let message = detail.provider_message.as_deref().unwrap_or(error_message.as_str());
    runtime_task_execution::capacity_decision_for_provider_error_facts(
        runtime_task_execution::DeepReviewProviderCapacityErrorFacts {
            ...
            category: match detail.category {
                ErrorCategory::RateLimit => runtime_task_execution::DeepReviewProviderCapacityErrorCategory::RateLimit,
...
pub(crate) fn capacity_skip_result_for_local_queue_outcome(
    ...
    let effective_parallel_instances =
        deep_review_effective_concurrency_snapshot(dialog_turn_id, conc_policy.max_parallel_instances)
...
pub(crate) async fn wait_for_provider_capacity_retry(
    ...
    loop {
        ...
        let effective_parallel_instances =
            deep_review_effective_parallel_instances(dialog_turn_id, conc_policy.max_parallel_instances);
        let control_snapshot = deep_review_queue_control_snapshot(dialog_turn_id, tool_id);
        let queue_step = queue_runtime.step(runtime_task_execution::DeepReviewProviderCapacityQueueRuntimeInput {
            now,
...
pub(crate) fn record_provider_capacity_retry(dialog_turn_id: &str, reason: DeepReviewCapacityQueueReason) {
    record_deep_review_runtime_provider_capacity_retry(dialog_turn_id, reason);
}

pub(crate) fn record_provider_capacity_retry_success(dialog_turn_id: &str, reason: DeepReviewCapacityQueueReason) {
...
pub(crate) async fn wait_for_reviewer_admission(
    ...
    loop {
        ...
        let control_snapshot = deep_review_queue_control_snapshot(dialog_turn_id, tool_id);
        let queue_step =
            queue_runtime.begin_step(runtime_task_execution::DeepReviewReviewerAdmissionQueueRuntimeInput {
                now,
                control_snapshot,
                poll_interval: DEEP_REVIEW_QUEUE_POLL_INTERVAL,
            });
        ...
        let blocked_capacity_reason =
            match try_begin_reviewer_admission(dialog_turn_id, effective_parallel_instances, launch_batch_info) {
                Ok(Some(guard)) => {
