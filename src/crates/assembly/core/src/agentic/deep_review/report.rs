...
//! updates are owned by `northhing-agent-runtime::deep_review::report`. This file
...
use northhing_agent_runtime::deep_review::diagnostics as runtime_diagnostics;
use northhing_agent_runtime::deep_review::report as runtime_report;
pub(crate) use northhing_agent_runtime::deep_review::report::{
    deep_review_cache_from_completed_reviewers, fill_deep_review_cache_update_signals, fill_deep_review_packet_metadata,
};
use northhing_services_core::session::set_deep_review_cache;
...
pub(crate) fn compression_contract_for_context(context: &ToolUseContext) -> Option<CompressionContract> {
    let session_id = context.session_id.as_deref()?;
    let coordinator = global_coordinator()?;
    let session = coordinator.session_manager().get_session(session_id)?;
    ...
    should_report_compression_preserved(session.compression_state.compression_count, Some(&contract))
        .then_some(contract)
}

pub(crate) fn reliability_contract_limit(agent_type: Option<&str>, model_id: Option<&str>) -> usize {
...
pub(crate) fn fill_deep_review_reliability_signals(
    ...
    runtime_report::fill_deep_review_reliability_signals(input, run_manifest, compression_preserved_signal_count);
}

pub(crate) fn fill_deep_review_runtime_tracker_signals(input: &mut Value, dialog_turn_id: Option<&str>) {
    let Some(dialog_turn_id) = dialog_turn_id.map(str::trim).filter(|value| !value.is_empty()) else {
        return;
    };
    let count =
...
pub(crate) async fn persist_deep_review_cache(context: &ToolUseContext, cache_value: Value) -> NortHingResult<()> {
