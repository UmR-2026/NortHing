...
//! `GetFileDiff` calls are measured, and northhing runtime URIs are ignored. It
...
use northhing_agent_runtime::post_call_hooks::{
    resolve_deep_review_shared_context_tool_use, DeepReviewSharedContextToolUseFacts,
};
use serde_json::Value;

pub(crate) fn maybe_record_shared_context_tool_use(tool_name: &str, input: &Value, context: &ToolUseContext) {
    let Some(record) = resolve_deep_review_shared_context_tool_use(DeepReviewSharedContextToolUseFacts {
        tool_name,
        input,
