...
use northhing_services_core::session::{build_branched_session_metadata, BranchSessionMetadataFacts};
pub use northhing_services_core::session::{SessionBranchRequest, SessionBranchResult};
use std::path::Path;
...
impl PersistenceManager {
    pub async fn branch_session(
    ...
    ) -> NortHingResult<SessionBranchResult> {
        let source_session = self.load_session(workspace_path, &request.source_session_id).await?;
        ...
        let branch_result = async {
            ...
            for (new_index, source_turn) in source_turns.iter().take(source_turn_index + 1).enumerate() {
                if let Some(messages) = self
                    .load_turn_context_snapshot(workspace_path, &request.source_session_id, source_turn.turn_index)
                    .await?
                {
                    self.save_turn_context_snapshot(workspace_path, &target_session_id, new_index, &messages)
                        .await?;
                }
                if let Some(snapshot) = self
                    .load_turn_skill_agent_snapshot(workspace_path, &request.source_session_id, source_turn.turn_index)
                    .await?
                {
                    self.save_turn_skill_agent_snapshot(workspace_path, &target_session_id, new_index, &snapshot)
                        .await?;
                }
            }
            ...
            if let Some(snapshot) = self
                .load_skill_agent_baseline_override_snapshot(workspace_path, &request.source_session_id)
                .await?
            {
                self.save_skill_agent_baseline_override_snapshot(workspace_path, &target_session_id, &snapshot)
                    .await?;
            }
            ...
            self.save_session_metadata(workspace_path, &branched_metadata).await?;
        ...
        if let Err(error) = branch_result {
            let _ = self.delete_session(workspace_path, &target_session_id).await;
            return Err(error);
        }
...
mod tests {
    ...
    use crate::agentic::session::{
        CachedSystemPrompt, CachedUserContext, SessionPromptCache, SystemPromptCacheIdentity, UserContextCacheIdentity,
    };
    ...
    impl TestWorkspace {
        fn new() -> Self {
            let path = std::env::temp_dir().join(format!("northhing-session-branch-test-{}", Uuid::new_v4()));
    ...
    fn build_turn(session_id: &str, turn_id: &str, turn_index: usize, content: &str) -> DialogTurnData {
    ...
    async fn branch_session_copies_turns_snapshots_and_lineage_metadata() {
        ...
        let mut source_session = Session::new("Source Title".to_string(), "agentic".to_string(), Default::default());
        ...
        manager
            .save_prompt_cache(workspace.path(), &source_session.session_id, &source_prompt_cache)
            .await
