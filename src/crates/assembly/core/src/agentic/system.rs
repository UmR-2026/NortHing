...
pub async fn init_agentic_system() -> Result<AgenticSystem> {
    ...
    event_router.subscribe_internal("thread_goal_tokens".to_string(), Arc::new(ThreadGoalTokenSubscriber));
            ...
            match internal_event_rx.recv().await {
                Ok(envelope) => {
                    if let Err(error) = internal_event_router.route(envelope).await {
                        tracing::warn!("Internal agentic event routing failed: {}", error);
                    }
                }
                Err(tokio::sync::broadcast::error::RecvError::Lagged(skipped)) => {
                    tracing::warn!("Internal agentic event router lagged by {} events", skipped);
                }
