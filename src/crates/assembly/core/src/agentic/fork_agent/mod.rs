...
impl ForkAgentContextSnapshot {
    pub fn from_parent_session(parent_session: &Session, messages: Vec<Message>) -> NortHingResult<Self> {
        let workspace_path = parent_session.config.workspace_path.clone().ok_or_else(|| {
...
mod tests {
    ...
    fn snapshot_retains_inherited_messages() {
        ...
        let snapshot = ForkAgentContextSnapshot::from_parent_session(&parent, inherited.clone()).expect("snapshot");
    ...
    fn snapshot_builds_child_session_config_from_parent() {
        let parent = parent_session();
        let snapshot = ForkAgentContextSnapshot::from_parent_session(&parent, Vec::new()).expect("snapshot");

        let child_config = snapshot.build_child_session_config(Some(7));

        assert_eq!(child_config.workspace_path.as_deref(), Some("/workspace/project"));
        assert_eq!(child_config.remote_connection_id.as_deref(), Some("remote-1"));
