...
use crate::service::config::{GlobalConfigManager, ModelExchangeTracingConfig, ModelExchangeTracingMode};
use crate::service::workspace_runtime::{workspace_runtime_service_arc, WorkspaceRuntimeContext};
use async_trait::async_trait;
use chrono::{DateTime, Utc};
use dashmap::DashMap;
use northhing_ai_adapters::{
...
impl WorkspaceModelExchangeTraceSink {
    ...
    async fn allocate_sequence(&self) -> Result<u64, String> {
        let session_dir = self.runtime_context.request_trace_session_dir(&self.session_id);
    ...
    async fn write_record(&self, path: &Path, record: &ModelExchangeTraceRecord) -> Result<(), String> {
    ...
    async fn read_record(&self, path: &Path) -> Result<ModelExchangeTraceRecord, String> {
        ...
        serde_json::from_slice(&bytes).map_err(|error| format!("Failed to deserialize trace record: {}", error))
    }

    async fn update_response(&self, trace_id: &str, response: &ModelExchangeResponseTrace) -> Result<(), String> {
        let Some(path) = self.trace_paths.get(trace_id).map(|entry| entry.value().clone()) else {
            return Ok(());
        };
    ...
    fn sanitize_response(&self, response: &ModelExchangeResponseTrace) -> ModelExchangeResponseTrace {
        ModelExchangeResponseTrace {
            ...
            tool_calls: self
                ...
                .flatten(),
            usage: self.policy.capture_usage.then(|| response.usage.clone()).flatten(),
...
impl ModelExchangeTraceSink for WorkspaceModelExchangeTraceSink {
    async fn request_attempt_started(
        ...
        let path = self.runtime_context.request_trace_path(&self.session_id, sequence);
    ...
    async fn request_attempt_failed(&self, handle: Option<&ModelExchangeRequestTraceHandle>, error: &str) {
...
pub async fn prepare_model_exchange_trace_for_workspace(
    ...
    let runtime_context = match workspace_runtime_service_arc()
...
async fn current_model_exchange_trace_policy() -> Option<ModelExchangeTracePolicy> {
    let Ok(config_service) = GlobalConfigManager::service().await else {
        return None;
    };

    let tracing_config: ModelExchangeTracingConfig = config_service
        .config(Some("app.logging.model_exchange_tracing"))
        .await
        .unwrap_or_default();
