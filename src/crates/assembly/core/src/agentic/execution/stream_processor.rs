...
pub use northhing_agent_stream::{StreamProcessOptions, StreamProcessorError, ToolCall as StreamToolCall};

/// Stream processing result exposed through northhing-core compatibility types.
#[derive(Debug, Clone)]
...
impl From<northhing_agent_stream::StreamResult> for StreamResult {
    fn from(result: northhing_agent_stream::StreamResult) -> Self {
        Self {
...
/// Stream processing error exposed through northhing-core compatibility errors.
#[derive(Debug)]
pub struct StreamProcessError {
    pub error: NortHingError,
    pub has_effective_output: bool,
}

impl From<northhing_agent_stream::StreamProcessError> for StreamProcessError {
    fn from(error: northhing_agent_stream::StreamProcessError) -> Self {
        Self {
...
pub struct StreamProcessor {
    inner: northhing_agent_stream::StreamProcessor,
}

impl StreamProcessor {
    pub fn new(event_queue: Arc<EventQueue>) -> Self {
        Self {
            inner: northhing_agent_stream::StreamProcessor::new(event_queue),
        }
    }

    pub fn derive_watchdog_timeout(stream_idle_timeout: Option<Duration>) -> Option<Duration> {
        northhing_agent_stream::StreamProcessor::derive_watchdog_timeout(stream_idle_timeout)
    }

    #[allow(clippy::too_many_arguments)]
    pub async fn process_stream(
        &self,
        stream: BoxStream<'static, Result<northhing_ai_adapters::UnifiedResponse, anyhow::Error>>,
