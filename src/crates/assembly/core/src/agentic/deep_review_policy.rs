...
//! in `northhing-agent-runtime::deep_review`. Core only keeps product config
...
pub use northhing_agent_runtime::deep_review::{
    apply_deep_review_queue_control, classify_deep_review_capacity_error, clear_deep_review_queue_control_for_tool,
    deep_review_active_reviewer_count, deep_review_capacity_skip_count, deep_review_concurrency_cap_rejection_count,
    deep_review_effective_concurrency_snapshot, deep_review_effective_parallel_instances,
    deep_review_has_judge_been_launched, deep_review_max_retries_per_role, deep_review_queue_control_snapshot,
    deep_review_retries_used, deep_review_runtime_diagnostics_snapshot,
    deep_review_shared_context_measurement_snapshot, deep_review_turn_elapsed_seconds, default_review_team_definition,
    record_deep_review_capacity_skip, record_deep_review_capacity_skip_for_reason,
    record_deep_review_concurrency_cap_rejection, record_deep_review_effective_concurrency_capacity_error,
    ...
    record_deep_review_runtime_provider_capacity_queue, record_deep_review_runtime_provider_capacity_retry,
    record_deep_review_runtime_provider_capacity_retry_success, record_deep_review_runtime_queue_wait,
    ...
    try_begin_deep_review_active_reviewer_for_launch_batch, ChangeRiskFactors, DeepReviewActiveReviewerGuard,
    DeepReviewBudgetTracker, DeepReviewCapacityFailFastReason, DeepReviewCapacityQueueDecision,
    DeepReviewCapacityQueueReason, DeepReviewConcurrencyPolicy, DeepReviewEffectiveConcurrencySnapshot,
    DeepReviewExecutionPolicy, DeepReviewIncrementalCache, DeepReviewPolicyViolation, DeepReviewQueueControlAction,
...
pub async fn load_default_deep_review_policy() -> NortHingResult<DeepReviewExecutionPolicy> {
    let config_service = GlobalConfigManager::service().await.map_err(|error| {
    ...
    let raw_config = match config_service
        .config::<Value>(Some(DEFAULT_REVIEW_TEAM_CONFIG_PATH))
        .await
    {
    ...
    Ok(DeepReviewExecutionPolicy::from_config_value(raw_config.as_ref()))
}

pub fn is_missing_default_review_team_config_error(error: &NortHingError) -> bool {
    matches!(error, NortHingError::NotFound(message)
...
mod tests {
    use super::{
        ...
        DeepReviewExecutionPolicy, DeepReviewRunManifestGate, DeepReviewStrategyLevel, DeepReviewSubagentRole,
    ...
    fn only_missing_default_review_team_path_can_fallback_to_defaults() {
        let matching = NortHingError::NotFound("Config path 'ai.review_teams.default' not found".to_string());
        assert!(is_missing_default_review_team_config_error(&matching));

        let different_path = NortHingError::NotFound("Config path 'ai.review_teams.other' not found".to_string());
        assert!(!is_missing_default_review_team_config_error(&different_path));

        let other_error = NortHingError::config("Config path 'ai.review_teams.default' not found".to_string());
    ...
    fn compatibility_facade_preserves_deep_review_runtime_exports() {
        ...
        assert_eq!(
            policy.member_strategy_overrides.get(REVIEWER_SECURITY_AGENT_TYPE),
