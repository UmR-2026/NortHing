...
impl ModelCapabilityProfile {
    pub fn from_model_id(model_id: Option<&str>) -> Self {
        ...
        let weak_mid_patterns = ["-haiku-", "-mini-", "-small-", "-lite-", "-flash-", "-nano-"];
...
impl ContextProfilePolicy {
    ...
    pub fn for_subagent_context_and_models(
        ...
        let parent_profile = parent_agent_type
            .map(|agent_type| ContextProfile::for_agent_context(agent_type, parent_is_review_subagent))
            .unwrap_or(ContextProfile::Conversation);
        let profile = if child_profile == ContextProfile::LongTask || parent_profile == ContextProfile::LongTask {
    ...
    pub fn effective_loop_threshold(&self, configured: usize) -> usize {
        configured.max(1).min(self.repeated_tool_signature_threshold.max(1))
    }
    ...
    fn apply_weak_model_override(&mut self) {
        ...
        self.consecutive_failed_command_threshold = self.consecutive_failed_command_threshold.min(2);
    }
}
