...
use northhing_product_domains::function_agents::ports::{
    CommitAiAnalysisRequest, FunctionAgentAiPort, FunctionAgentFuture, FunctionAgentGitPort, GitCommitSnapshot,
    StartchatGitSnapshot, StartchatTimeSnapshot, WorkStateAiAnalysisRequest,
};
use northhing_product_domains::function_agents::{
    git_func_agent::AICommitAnalysis, startchat_func_agent::AIGeneratedAnalysis,
};
use northhing_services_integrations::function_agents::FunctionAgentGitService;
...
impl FunctionAgentGitPort for CoreFunctionAgentGitAdapter {
    fn git_commit_snapshot(&self, repo_path: PathBuf) -> FunctionAgentFuture<'_, GitCommitSnapshot> {
        Box::pin(async move { FunctionAgentGitService::git_commit_snapshot(repo_path).await })
    }

    fn startchat_git_snapshot(&self, repo_path: PathBuf) -> FunctionAgentFuture<'_, StartchatGitSnapshot> {
        Box::pin(async move { FunctionAgentGitService::startchat_git_snapshot(repo_path).await })
    }

    fn startchat_time_snapshot(&self, repo_path: PathBuf) -> FunctionAgentFuture<'_, StartchatTimeSnapshot> {
...
impl FunctionAgentAiPort for CoreFunctionAgentAiAdapter {
    fn analyze_commit(&self, request: CommitAiAnalysisRequest) -> FunctionAgentFuture<'_, AICommitAnalysis> {
        ...
        Box::pin(async move {
            ...
            .await?;
            service
                .generate_commit_message_ai(&request.diff_content, &request.project_context, &request.options)
                .await
        })
    }

    fn analyze_work_state(&self, request: WorkStateAiAnalysisRequest) -> FunctionAgentFuture<'_, AIGeneratedAnalysis> {
        let factory = self.factory.clone();
        Box::pin(async move {
            let service =
                crate::function_agents::runtime_services::CoreWorkStateAiAnalysisService::new_with_agent_config(
                    factory,
                    "startchat-func-agent",
                )
                .await?;
            service
                .generate_complete_analysis(&request.git_state, &request.git_diff, &request.language)
                .await
        })
    }
}
...
#[cfg(test)]
mod tests {
    use northhing_product_domains::function_agents::ports::FunctionAgentGitPort;
    use std::fs;
    ...
    async fn git_adapter_builds_commit_snapshot_from_existing_core_git_services() {
        ...
        let snapshot = adapter.git_commit_snapshot(repo.path().to_path_buf()).await.unwrap();
    ...
    async fn git_adapter_builds_startchat_snapshot_without_changing_git_semantics() {
        ...
        let adapter = CoreFunctionAgentGitAdapter::default();
        let snapshot = adapter.startchat_git_snapshot(repo.path().to_path_buf()).await.unwrap();
    ...
    fn init_git_repo(repo: &std::path::Path) {
        ...
        git(repo, &["config", "user.name", "northhing Test"]);
    }
