...
//! Git snapshots are owned by `northhing-services-integrations`.

use std::sync::Arc;

use northhing_product_domains::function_agents::git_func_agent::{
    parse_commit_ai_response, prepare_commit_ai_prompt, AICommitAnalysis, CommitMessageOptions, ProjectContext,
};
use northhing_product_domains::function_agents::startchat_func_agent::{
    build_work_state_analysis_prompt, parse_work_state_analysis_response, AIGeneratedAnalysis, GitWorkState,
};
...
impl CoreCommitAiAnalysisService {
    pub async fn new_with_agent_config(factory: Arc<AIClientFactory>, agent_name: &str) -> AgentResult<Self> {
    ...
    async fn call_ai(&self, prompt: &str) -> AgentResult<String> {
        ...
        let response = self.ai_client.send_message(messages, None).await.map_err(|e| {
...
impl CoreWorkStateAiAnalysisService {
    ...
    fn parse_complete_analysis(&self, response: &str) -> AgentResult<AIGeneratedAnalysis> {
        let parsed_analysis = parse_work_state_analysis_response(response).inspect_err(|error| {
...
#[cfg(test)]
mod tests {
    use northhing_core_types::ReasoningMode;
...
"#,
        ...
        assert_eq!(parsed.title, "refactor(product-domains): add runtime baseline");
