...
impl FrontMatterMarkdown {
    ...
    pub fn load(path: &str) -> Result<(Value, String), String> {
        let content = std::fs::read_to_string(path).map_err(|e| format!("Failed to read markdown file: {}", e))?;
    ...
    pub fn load_str(content: &str) -> Result<(Value, String), String> {
        ...
        let metadata: Value = serde_yaml::from_str(yaml_content).map_err(|e| format!("Failed to parse YAML: {}", e))?;

        let after_front_matter = caps.get(0).ok_or_else(|| "Failed to get captures".to_string())?.end();
