...
impl IngestServerState {
    ...
    pub async fn update_log_path(&self, log_path: PathBuf) {
        ...
        debug!("Debug log path updated to: {:?}", config.log_config.log_path);
    }
}
...
impl From<IngestLogRequest> for DebugLogEntry {
    fn from(req: IngestLogRequest) -> Self {
        DebugLogEntry {
            ...
            id: None,
            // Phase H: HTTP ingest callers don't supply component/mode
            // (those are desktop-side concerns). Default to empty so the
            // round-trip preserves compat — empty strings are skipped by
            // the serializer's `skip_serializing_if`.
...
pub async fn handle_ingest(request: IngestLogRequest, config: &DebugLogConfig) -> Result<IngestResponse> {
    let log_config = if let Some(workspace_path) =
        global_workspace_service().and_then(|service| service.try_get_current_workspace_path())
    {
        let mut cfg = config.clone();
        cfg.log_path = workspace_path.join(".northhing").join("debug.log");
        cfg
    } else {
        config.clone()
    };
