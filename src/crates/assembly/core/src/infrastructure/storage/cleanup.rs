...
impl CleanupService {
    pub fn new(path_manager: PathManager, policy: CleanupPolicy) -> Self {
        Self { path_manager, policy }
    }

    pub async fn cleanup_all(&self) -> NortHingResult<CleanupResult> {
    ...
    async fn cleanup_temp_files(&self) -> NortHingResult<CleanupResult> {
    ...
    async fn cleanup_old_logs(&self) -> NortHingResult<CleanupResult> {
    ...
    async fn cleanup_oversized_cache(&self) -> NortHingResult<CleanupResult> {
    ...
    async fn cleanup_old_files(&self, dir: &Path, retention: Duration) -> NortHingResult<CleanupResult> {
        let mut result = CleanupResult::default();
        ...
        self.cleanup_recursively(
            dir,
            |metadata| metadata.modified().map(|time| time < cutoff_time).unwrap_or(false),
            &mut result,
        )
        .await?;
    ...
    async fn cleanup_by_size(&self, dir: &Path, max_size: u64) -> NortHingResult<CleanupResult> {
        let mut result = CleanupResult::default();
    ...
    ) -> std::pin::Pin<Box<dyn std::future::Future<Output = NortHingResult<()>> + Send + 'a>>
    where
        F: Fn(&std::fs::Metadata) -> bool + Copy + Send + 'a,
    {
        Box::pin(async move {
            ...
            while let Some(entry) = read_dir
                .next_entry()
                .await
                .map_err(|e| NortHingError::service(format!("Failed to read entry: {}", e)))?
            {
                let path = entry.path();
    ...
    fn calculate_dir_size(
        dir: &Path,
    ) -> std::pin::Pin<Box<dyn std::future::Future<Output = NortHingResult<u64>> + Send + '_>> {
        Box::pin(async move {
            let mut total = 0u64;
