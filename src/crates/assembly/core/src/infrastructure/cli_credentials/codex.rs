...
use super::{CliCredentialKind, CliCredentialMode, CredentialResolver, DiscoveredCredential, ResolvedCredential};
...
async fn load_auth_file() -> Result<Option<CodexAuthFile>> {
    ...
    let file: CodexAuthFile =
        serde_json::from_slice(&bytes).with_context(|| format!("parse codex auth.json at {}", path.display()))?;
    Ok(Some(file))
}
...
fn jwt_email(token: &str) -> Option<String> {
    decode_jwt_claims(token)?.get("email")?.as_str().map(|s| s.to_string())
}
...
fn detect_mode(file: &CodexAuthFile) -> Option<CliCredentialMode> {
    ...
    if declared.eq_ignore_ascii_case("apikey") && file.openai_api_key.as_deref().map(|k| !k.is_empty()).unwrap_or(false)
    {
    ...
    if file
        .tokens
        .as_ref()
        .is_some_and(|t| t.access_token.as_deref().map(|s| !s.is_empty()).unwrap_or(false))
    {
        return Some(CliCredentialMode::ChatGpt);
    }
    if file.openai_api_key.as_deref().map(|k| !k.is_empty()).unwrap_or(false) {
        return Some(CliCredentialMode::ApiKey);
    }
    None
}
...
pub async fn discover() -> Result<Option<DiscoveredCredential>> {
    ...
    let (label, account, expires_at, format, base_url, model) = match mode {
        ...
        CliCredentialMode::OauthPersonal => {
            return Err(anyhow!(
                "codex cli never uses OauthPersonal mode (that's a Gemini variant)"
            ));
        }
    };
...
async fn refresh_codex_tokens(refresh_token: &str) -> Result<OAuthTokenResponse> {
    ...
    let parsed: OAuthTokenResponse = resp.json().await.context("parse codex oauth token response")?;
    Ok(parsed)
}
...
async fn ensure_fresh_tokens(file: &mut CodexAuthFile) -> Result<()> {
    ...
    tracing::info!("codex CLI tokens refreshed");
    Ok(())
}
...
impl CredentialResolver for CodexResolver {
    async fn resolve(&self) -> Result<ResolvedCredential> {
        ...
        let mode = detect_mode(&file).ok_or_else(|| anyhow!("codex auth.json present but no usable credentials"))?;
