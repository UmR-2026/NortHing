...
//!
//! Lets northhing reuse already-authenticated Codex CLI / Gemini CLI sessions on
...
pub async fn discover_all() -> Vec<DiscoveredCredential> {
    ...
    match codex::discover().await {
        ...
        Ok(None) => {}
        Err(err) => tracing::debug!("codex credential discovery failed: {err:#}"),
    }
    match gemini::discover().await {
        Ok(Some(item)) => out.push(item),
        Ok(None) => {}
        Err(err) => tracing::debug!("gemini credential discovery failed: {err:#}"),
    }
    out
}
