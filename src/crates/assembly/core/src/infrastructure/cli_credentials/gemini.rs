...
async fn load_oauth_creds() -> Result<Option<(PathBuf, OauthCredsFile)>> {
    ...
    let bytes = tokio::fs::read(&path)
        .await
        .with_context(|| format!("read {}", path.display()))?;
    let parsed: OauthCredsFile = serde_json::from_slice(&bytes).with_context(|| format!("parse {}", path.display()))?;
    Ok(Some((path, parsed)))
}
...
pub async fn discover() -> Result<Option<DiscoveredCredential>> {
    ...
    if env.get("GEMINI_API_KEY").map(|v| !v.is_empty()).unwrap_or(false) {
        let base_url = env
...
async fn refresh_google_oauth(refresh_token: &str) -> Result<GoogleTokenResponse> {
    ...
    if !resp.status().is_success() {
        ...
        return Err(anyhow!("gemini token refresh failed: HTTP {status}: {body}"));
    }
    let parsed: GoogleTokenResponse = resp.json().await.context("parse google oauth token response")?;
    Ok(parsed)
}
...
async fn ensure_fresh_oauth(path: &PathBuf, creds: &mut OauthCredsFile) -> Result<()> {
    ...
    tracing::info!("gemini CLI oauth tokens refreshed");
    Ok(())
}
