...
use crate::infrastructure::cli_credentials::{self, codex::CodexResolver, gemini::GeminiResolver, CredentialResolver};
...
impl AIClientFactory {
    ...
    pub async fn get_client_by_agent(&self, agent_name: &str) -> Result<Arc<AIClient>> {
        let global_config: crate::service::config::GlobalConfig = self.config_service.config(None).await?;
    ...
    async fn get_or_create_client(&self, model_id: &str) -> Result<Arc<AIClient>> {
...

        {
            let cache = match self.client_cache.read() {
                Ok(cache) => cache,
                Err(poisoned) => {
                    warn!("AI client cache read lock poisoned during get_or_create_client, recovering");
                    poisoned.into_inner()
                }
            };
        ...
        let model_config = global_config
            ...
            .iter()
            .find(|m| {
                m.id == normalized_model_id || m.name == normalized_model_id || m.model_name == normalized_model_id
            })
...

        {
            let mut cache = match self.client_cache.write() {
                Ok(cache) => cache,
                Err(poisoned) => {
                    warn!("AI client cache write lock poisoned during get_or_create_client, recovering");
                    poisoned.into_inner()
                }
            };
...
impl AIClientFactory {
    ...
    pub async fn get_global() -> NortHingResult<Arc<AIClientFactory>> {
        let wrapper = GLOBAL_AI_CLIENT_FACTORY.get().ok_or_else(|| {
            NortHingError::service(
                "Global AIClientFactory not initialized. Call initialize_global() first.".to_string(),
            )
        })?;
        ...
        guard
            .as_ref()
            .ok_or_else(|| NortHingError::service("Global AIClientFactory is None".to_string()))
            .map(Arc::clone)
    }
    ...
    pub async fn update_global(new_factory: Arc<AIClientFactory>) -> NortHingResult<()> {
        let wrapper = GLOBAL_AI_CLIENT_FACTORY
            .get()
            .ok_or_else(|| NortHingError::service("Global AIClientFactory not initialized".to_string()))?;
...
pub async fn get_global_ai_client_factory() -> NortHingResult<Arc<AIClientFactory>> {
    AIClientFactory::get_global().await
}

pub async fn initialize_global_ai_client_factory() -> NortHingResult<()> {
...
mod tests {
    ...
    fn resolve_model_reference_supports_id_name_and_model_name() {
        ...
        config.ai.models = vec![build_model("model-123", "Primary Chat", "claude-sonnet-4.5")];
    ...
    fn auto_model_selectors_normalize_to_primary_for_client_lookup() {
        ...
        assert_eq!(AIClientFactory::normalize_model_selector(" default "), "primary");
    ...
    fn resolve_fast_selection_falls_back_to_primary_when_fast_missing() {
        let mut config = GlobalConfig::default();
        config.ai.models = vec![build_model("model-primary", "Primary Chat", "claude-sonnet-4.5")];
