...
use crate::service::snapshot::types::{OperationType, SessionInfo, SnapshotConfig, SnapshotError, SnapshotResult};
...
impl SnapshotService {
    pub fn new(
        ...
        let snapshot_core = Arc::new(RwLock::new(SnapshotCore::new(runtime_context.clone(), snapshot_system)));
    ...
    pub async fn initialize(&mut self) -> SnapshotResult<()> {
        ...
        info!(
            "Snapshot service initialized: git_isolated={} northhing_dir={} duration_ms={}",
            isolation_status,
    ...
    pub async fn get_session_file_diff_stats(
        ...
        self.ensure_initialized().await?;
        let snapshot_core = self.snapshot_core.read().await;
        snapshot_core.get_session_file_diff_stats(session_id, file_path).await
    }
    ...
    pub async fn complete_file_modification(
        ...
        let status = if completed_op.before_snapshot_id.is_none() && completed_op.after_snapshot_id.is_some() {
            "created"
        } else if completed_op.before_snapshot_id.is_some() && completed_op.after_snapshot_id.is_none() {
            "deleted"
        } else {
            "modified"
        };
    ...
    pub async fn rollback_session(&self, session_id: &str) -> SnapshotResult<Vec<PathBuf>> {
        ...
        let released_count = self.file_lock_manager.release_session_locks(session_id).await?;
        if released_count > 0 {
            info!("Released {} file locks: session_id={}", released_count, session_id);
        }

        Ok(restored_files)
    }

    pub async fn rollback_to_turn(&self, session_id: &str, turn_index: usize) -> SnapshotResult<Vec<PathBuf>> {
        self.ensure_initialized().await?;
        info!(
    ...
    pub async fn accept_file(&self, session_id: &str, file_path: &Path) -> SnapshotResult<()> {
        ...
        let mut snapshot_core = self.snapshot_core.write().await;
        snapshot_core.cleanup_file_session(session_id, file_path).await?;
    ...
    pub async fn reject_file(&self, session_id: &str, file_path: &Path) -> SnapshotResult<Vec<PathBuf>> {
        ...
        let mut snapshot_core = self.snapshot_core.write().await;
        let restored_files = snapshot_core.rollback_file_session(session_id, file_path).await?;
    ...
    pub async fn get_turn_files(&self, session_id: &str, turn_index: usize) -> SnapshotResult<Vec<PathBuf>> {
        self.ensure_initialized().await?;
        let snapshot_core = self.snapshot_core.read().await;
        Ok(snapshot_core.get_turn_files(session_id, turn_index))
    }

    pub async fn get_file_diff(&self, session_id: &str, file_path: &Path) -> SnapshotResult<(String, String)> {
        self.ensure_initialized().await?;
        let snapshot_core = self.snapshot_core.read().await;
    ...
    pub async fn get_system_stats(&self) -> SnapshotResult<SystemStats> {
        ...
        Ok(SystemStats {
            ...
            northhing_dir: self.runtime_context.runtime_root.clone(),
        })
    }
    ...
    pub async fn all_modified_files(&self) -> SnapshotResult<Vec<PathBuf>> {
        self.ensure_initialized().await?;
        let snapshot_core = self.snapshot_core.read().await;
        Ok(snapshot_core.all_modified_files())
    }
    ...
    pub async fn release_file_lock(&self, session_id: &str, file_path: &Path) -> SnapshotResult<()> {
        self.ensure_initialized().await?;
        self.file_lock_manager
            .release_lock(&file_path.to_path_buf(), session_id)
            .await
    ...
    pub async fn get_file_lock_status(&self, file_path: &Path) -> SnapshotResult<Option<serde_json::Value>> {
        self.ensure_initialized().await?;
        let lock = self.file_lock_manager.get_lock_status(&file_path.to_path_buf()).await;
    ...
    pub fn northhing_dir(&self) -> &Path {
