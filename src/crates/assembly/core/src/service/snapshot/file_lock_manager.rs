...
impl FileLockManager {
    ...
    pub async fn try_acquire_lock(
        ...
        if let Some(existing_lock) = locks.get(file_path) {
            ...
            debug!(
                "File locked by another session, adding to waiting queue: file_path={} session_id={}",
                file_path.display(),
                session_id
            );
            self.add_to_waiting_queue(file_path, session_id, tool_name).await?;
            return Ok(false);
        }
    ...
    async fn add_to_waiting_queue(&self, file_path: &Path, session_id: &str, tool_name: &str) -> SnapshotResult<()> {
    ...
    pub async fn cleanup_expired_queue_items(&self, max_wait_minutes: u64) -> SnapshotResult<usize> {
        let mut waiting_queue = self.waiting_queue.write().await;
