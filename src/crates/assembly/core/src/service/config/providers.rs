...
impl ConfigProvider for AIConfigProvider {
    ...
    async fn validate_config(&self, config: &serde_json::Value) -> NortHingResult<Vec<String>> {
        ...
        if let Ok(ai_config) = serde_json::from_value::<AIConfig>(config.clone()) {
            ...
            for (index, model) in ai_config.models.iter().enumerate() {
                ...
                if let Some(temperature) = model.temperature {
                    if !(0.0..=2.0).contains(&temperature) {
                        warnings.push(format!("Model '{}' temperature should be between 0 and 2", model.name));
                    }
                }
            }
            ...
            for (func_agent_name, model_id) in &ai_config.func_agent_models {
                if !ai_config.models.iter().any(|m| m.id == *model_id) && model_id != "primary" && model_id != "fast" {
                    return Err(NortHingError::validation(format!(
            ...
            return Err(NortHingError::validation("Invalid AI config format".to_string()));
        }
    ...
    async fn on_config_changed(
        ...
        if let Ok(ai_config) = serde_json::from_value::<AIConfig>(new_config.clone()) {
            info!("AI config changed: {} models configured", ai_config.models.len());
...
impl ConfigProvider for ThemeConfigProvider {
        ...
        if let Ok(theme_config) = serde_json::from_value::<ThemeConfig>(config.clone()) {
            ...
            for color in colors {
                if !color.starts_with("#") && !color.starts_with("rgb") && !color.starts_with("hsl") {
                    return Err(NortHingError::validation(format!("Invalid color format: {}", color)));
                }
            }
        } else {
            return Err(NortHingError::validation("Invalid theme config format".to_string()));
        }
    ...
    async fn migrate_config(&self, _version: &str, config: serde_json::Value) -> NortHingResult<serde_json::Value> {
        Ok(config)
    }
}
...
impl ConfigProvider for ThemesConfigProvider {
    ...
    async fn validate_config(&self, config: &serde_json::Value) -> NortHingResult<Vec<String>> {
        ...
        if let Ok(_themes_config) = serde_json::from_value::<ThemesConfig>(config.clone()) {
            ...
            return Err(NortHingError::validation("Invalid themes config format".to_string()));
        }
    ...
    async fn on_config_changed(
        ...
        if let Ok(themes_config) = serde_json::from_value::<ThemesConfig>(new_config.clone()) {
            info!("Themes config changed: current theme = {}", themes_config.current);
        }
        Ok(())
    }
...
impl ConfigProvider for EditorConfigProvider {
    ...
    async fn validate_config(&self, config: &serde_json::Value) -> NortHingResult<Vec<String>> {
        ...
        if let Ok(editor_config) = serde_json::from_value::<EditorConfig>(config.clone()) {
            ...
            return Err(NortHingError::validation("Invalid editor config format".to_string()));
        }
...
impl ConfigProvider for TerminalConfigProvider {
    ...
    async fn validate_config(&self, config: &serde_json::Value) -> NortHingResult<Vec<String>> {
        ...
        if let Ok(terminal_config) = serde_json::from_value::<TerminalConfig>(config.clone()) {
            ...
            if terminal_config.terminal_panel_position != "right" && terminal_config.terminal_panel_position != "bottom"
            {
                warnings.push("Terminal panel position should be either 'right' or 'bottom'".to_string());
            }
        } else {
            return Err(NortHingError::validation("Invalid terminal config format".to_string()));
        }
...
impl ConfigProvider for WorkspaceConfigProvider {
    ...
    async fn validate_config(&self, config: &serde_json::Value) -> NortHingResult<Vec<String>> {
        ...
        if let Ok(workspace_config) = serde_json::from_value::<WorkspaceConfig>(config.clone()) {
            ...
            return Err(NortHingError::validation("Invalid workspace config format".to_string()));
        }
...
impl ConfigProvider for AppConfigProvider {
    ...
    async fn validate_config(&self, config: &serde_json::Value) -> NortHingResult<Vec<String>> {
        ...
        if let Ok(app_config) = serde_json::from_value::<AppConfig>(config.clone()) {
            ...
            return Err(NortHingError::validation("Invalid app config format".to_string()));
        }
...
impl ConfigProviderRegistry {
    ...
    pub fn default_config(&self) -> GlobalConfig {
        GlobalConfig::default()
    }

    /// Validates the full configuration.
    pub async fn validate_config(&self, config: &GlobalConfig) -> NortHingResult<ConfigValidationResult> {
        ...
        if let Some(provider) = self.get_provider("app") {
            ...
            match provider.validate_config(&app_value).await {
                Ok(provider_warnings) => {
                    warnings.extend(provider_warnings.into_iter().map(|msg| ConfigValidationWarning {
                        path: "app".to_string(),
                        message: msg,
    ...
    fn get_config_section(&self, section: &str, config: &GlobalConfig) -> NortHingResult<serde_json::Value> {
        match section {
            ...
            _ => Err(NortHingError::validation(format!(
