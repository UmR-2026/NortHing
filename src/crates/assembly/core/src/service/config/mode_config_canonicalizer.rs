...
use crate::agentic::agents::{agent_registry, mode_config_profile_member_mode_ids, resolve_mode_config_profile_id};
use crate::agentic::tools::registry::get_all_registered_tools;
use crate::service::config::global::GlobalConfigManager;
use crate::service::config::types::{AgentProfileConfig, AgentProfileView, ParentSubagentOverrideConfig};
...
fn normalize_subagent_overrides(overrides: ParentSubagentOverrideConfig) -> ParentSubagentOverrideConfig {
    overrides
        .into_iter()
...
fn build_agent_profile_view(
    ...
    let (disabled_user_skills, enabled_user_skills) = mode_config
        .map(|config| {
            normalize_skill_override_lists(config.disabled_user_skills.clone(), config.enabled_user_skills.clone())
        })
...
fn canonicalize_agent_profile(
    ...
    default_tools: &[String],
    valid_tools: &HashSet<String>,
) -> NortHingResult<Option<AgentProfileConfig>> {
    ...
    let mut stored: AgentProfileConfig = serde_json::from_value(raw_mode.clone()).map_err(|error| {
...
async fn get_profile_defaults() -> HashMap<String, Vec<String>> {
    ...
    for (mode_id, default_tools) in get_mode_defaults().await {
        defaults.entry(resolve_profile_id(&mode_id)).or_insert(default_tools);
    }
    defaults
}

pub async fn get_agent_profile_configs() -> NortHingResult<HashMap<String, AgentProfileConfig>> {
    let config_service = GlobalConfigManager::service().await?;
    Ok(config_service
        .config(Some("ai.agent_profiles"))
        .await
        .unwrap_or_default())
}

pub async fn get_agent_profile_views() -> NortHingResult<HashMap<String, AgentProfileView>> {
    ...
    for (mode_id, default_tools) in mode_defaults {
        ...
        let view = build_agent_profile_view(&mode_id, default_tools, stored_configs.get(&profile_id), &valid_tools);
...
pub async fn get_agent_profile_view(agent_id: &str) -> NortHingResult<AgentProfileView> {
    let views = get_agent_profile_views().await?;
    views
        .get(agent_id)
        .cloned()
        .ok_or_else(|| NortHingError::config(format!("Agent does not exist: {}", agent_id)))
}

pub async fn persist_agent_profile_from_value(agent_id: &str, config: Value) -> NortHingResult<()> {
    let config_service = GlobalConfigManager::service().await?;
    ...
    let enabled_tools = if let Some(tools) = config.get("enabled_tools") {
        serde_json::from_value::<Vec<String>>(tools.clone()).map_err(|error| {
            NortHingError::config(format!("Invalid enabled_tools for mode '{}': {}", agent_id, error))
        })?
    } else {
        ...
        .unwrap_or(false)
    {
        ...
        current.map(|item| item.enabled_user_skills.clone()).unwrap_or_default()
    };
        ...
        .unwrap_or(false)
    {
        match config.get("subagent_overrides") {
            ...
            Some(value) => serde_json::from_value::<ParentSubagentOverrideConfig>(value.clone()).map_err(|error| {
                NortHingError::config(format!("Invalid subagent_overrides for mode '{}': {}", agent_id, error))
            })?,
        }
    } else {
        current.map(|item| item.subagent_overrides.clone()).unwrap_or_default()
    };
    ...
    config_service.set_config("ai.agent_profiles", stored_configs).await
}

pub async fn reset_agent_profile_to_default(agent_id: &str) -> NortHingResult<()> {
    let config_service = GlobalConfigManager::service().await?;
    let mut stored_configs = get_agent_profile_configs().await?;
    let profile_id = resolve_profile_id(agent_id);
...
pub async fn canonicalize_agent_profile_configs() -> NortHingResult<AgentProfileConfigCanonicalizationReport> {
    let config_service = GlobalConfigManager::service().await?;
    let valid_tools = get_valid_tool_names().await;
    let profile_defaults = get_profile_defaults().await;
    let mut ai_value: Value = config_service.config(Some("ai")).await?;
    let original_ai_value = ai_value.clone();
    let ai_object = ai_value
        .as_object_mut()
        .ok_or_else(|| NortHingError::config("AI config must be a JSON object".to_string()))?;
    ...
    for (profile_id, default_tools) in &profile_defaults {
        ...
        let canonical = canonicalize_agent_profile(profile_id, raw_profile, default_tools, &valid_tools)?;
    ...
    ai_object.insert("agent_profiles".to_string(), Value::Object(rewritten_agent_profiles));
...
pub fn agent_profile_member_mode_ids_for(agent_id: &str) -> Vec<String> {
    ...
    if members.is_empty() {
        ...
        members.iter().map(|mode_id| (*mode_id).to_string()).collect()
    }
}
...
mod tests {
    use super::{
        agent_profile_member_mode_ids_for, canonicalize_agent_profile, normalize_skill_override_lists,
        stored_agent_profile_from_overrides,
    };
    ...
    fn normalize_skill_override_lists_removes_duplicates_and_conflicts() {
        let (disabled, enabled) = normalize_skill_override_lists(
            vec![
                "user::northhing-system::pdf".to_string(),
                "user::northhing-system::pdf".to_string(),
            ],
            vec![
                "user::northhing-system::pdf".to_string(),
                "user::northhing-system::docx".to_string(),
                "user::northhing-system::docx".to_string(),
            ],
        );

        assert_eq!(disabled, vec!["user::northhing-system::pdf".to_string()]);
        assert_eq!(enabled, vec!["user::northhing-system::docx".to_string()]);
    }
    ...
    fn stored_agent_profile_from_overrides_keeps_enabled_user_skills() {
        ...
        let stored = stored_agent_profile_from_overrides(
            ...
            Vec::new(),
            Vec::new(),
            vec!["user::northhing-system::pdf".to_string()],
            Default::default(),
            &[],
            &valid_tools,
        )
    ...
    fn canonicalize_agent_profile_treats_null_as_missing() {
        let canonical = canonicalize_agent_profile("Claw", Some(&Value::Null), &[], &HashSet::new())
    ...
    fn shared_modes_report_shared_profile_members() {
        ...
        assert_eq!(agent_profile_member_mode_ids_for("Cowork"), vec!["Cowork".to_string()]);
    }
}
