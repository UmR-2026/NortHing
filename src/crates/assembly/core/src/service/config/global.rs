...
pub enum ConfigUpdateEvent {
    ...
    AIModelUpdated { model_id: String, model_name: String },
    /// Default AI model updated.
    DefaultAIModelUpdated { model_id: String, model_name: String },
...
impl GlobalConfigManager {
    ...
    pub async fn initialize() -> NortHingResult<()> {
        ...
        #[cfg(feature = "product-full")]
        {
            match super::mode_config_canonicalizer::canonicalize_agent_profile_configs().await {
                Ok(report) => {
                    if !report.removed_profile_configs.is_empty() || !report.updated_profiles.is_empty() {
                        info!(
    ...
    pub async fn service() -> NortHingResult<Arc<ConfigService>> {
        let service_wrapper = GLOBAL_CONFIG_SERVICE
            .get()
            .ok_or_else(|| NortHingError::config("Global config service not initialized".to_string()))?;

        let service_guard = service_wrapper.read().await;
        service_guard
            .as_ref()
            .ok_or_else(|| NortHingError::config("Global config service is None".to_string()))
            .map(Arc::clone)
    }

    /// Updates the global configuration service instance (used for configuration reload).
    pub async fn update_service(new_service: Arc<ConfigService>) -> NortHingResult<()> {
        let service_wrapper = GLOBAL_CONFIG_SERVICE
            .get()
            .ok_or_else(|| NortHingError::config("Global config service not initialized".to_string()))?;
    ...
    pub async fn reload() -> NortHingResult<()> {
        let service = Self::service().await?;
        service.reload().await?;
        #[cfg(feature = "product-full")]
        if let Err(error) = super::mode_config_canonicalizer::canonicalize_agent_profile_configs().await {
            warn!("Mode config canonicalization failed after reload: {}", error);
        }
        Self::broadcast_update(ConfigUpdateEvent::ConfigReloaded).await;
        Ok(())
    }
    ...
    pub async fn update_theme(&self, theme_id: &str) -> NortHingResult<()> {
        let service = Self::service().await?;
...
pub async fn get_global_config_service() -> NortHingResult<Arc<ConfigService>> {
    GlobalConfigManager::service().await
}

/// Convenience helper: initialize the global configuration service.
pub async fn initialize_global_config() -> NortHingResult<()> {
    GlobalConfigManager::initialize().await
}

/// Convenience helper: reload the global configuration.
pub async fn reload_global_config() -> NortHingResult<()> {
