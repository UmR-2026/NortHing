...
pub fn normalize_project_agent_profiles_document(
    ...
    for (profile_id, mut entry) in document {
        ...
        entry.skills.disabled_project_skills = dedupe_preserving_order(entry.skills.disabled_project_skills);
...
pub fn deserialize_project_agent_profiles_document(content: &str) -> NortHingResult<ProjectAgentProfilesDocument> {
    Ok(normalize_project_agent_profiles_document(serde_json::from_str(
        content,
    )?))
}

pub fn serialize_project_agent_profiles_document(document: &ProjectAgentProfilesDocument) -> NortHingResult<Vec<u8>> {
    Ok(serde_json::to_vec_pretty(&normalize_project_agent_profiles_document(
        document.clone(),
    ))?)
}
...
pub async fn load_project_agent_profiles_document_local(
    ...
    let path = path_manager_arc().project_agent_profiles_file(workspace_root);
    match tokio::fs::read_to_string(&path).await {
        Ok(content) => deserialize_project_agent_profiles_document(&content),
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => Ok(ProjectAgentProfilesDocument::new()),
        Err(error) => Err(NortHingError::config(format!(
...
pub fn get_disabled_project_skills(document: &ProjectAgentProfilesDocument, profile_id: &str) -> Vec<String> {
    document
        .get(profile_id)
