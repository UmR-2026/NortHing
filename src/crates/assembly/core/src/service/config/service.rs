...
impl ConfigService {
    ...
    pub async fn with_settings(settings: ConfigManagerSettings) -> NortHingResult<Self> {
    ...
    pub async fn config<T>(&self, path: Option<&str>) -> NortHingResult<T>
    where
    ...
    pub async fn set_config<T>(&self, path: &str, value: T) -> NortHingResult<()>
    where
        T: serde::Serialize,
    {
        {
        ...
        if Self::path_touches_models(path) {
            if let Err(e) = self.reconcile_models("set_config").await {
                warn!("Model reconcile after set_config failed: path={}, error={}", path, e);
            }
        }
    ...
    pub async fn reset_config(&self, path: Option<&str>) -> NortHingResult<()> {
        {
            let mut manager = self.manager.write().await;
    ...
    pub async fn export_config(&self) -> NortHingResult<ConfigExport> {
        let manager = self.manager.read().await;
    ...
    pub async fn import_config(&self, export: ConfigExport) -> NortHingResult<ConfigImportResult> {
        let import_result = {
            let mut manager = self.manager.write().await;
            manager.import_config(serde_json::to_value(export.config)?).await
        };
    ...
    pub async fn statistics(&self) -> ConfigStatistics {
        let manager = self.manager.read().await;
        manager.statistics()
    }

    /// Runs a health check.
    pub async fn health_check(&self) -> NortHingResult<ConfigHealthStatus> {
        let manager = self.manager.read().await;
    ...
    pub async fn reload(&self) -> NortHingResult<()> {
        let settings = ConfigManagerSettings::default();
    ...
    pub async fn create_backup(&self) -> NortHingResult<std::path::PathBuf> {
        let manager = self.manager.read().await;
    ...
    pub async fn reconcile_models(&self, caller: &str) -> NortHingResult<ReconcileModelsReport> {
        let mut config: GlobalConfig = self.config(None).await?;
        ...
        let mut any_ref_to_id: std::collections::HashMap<String, String> = std::collections::HashMap::new();
        ...
        if agent_models_changed {
            let mut manager = self.manager.write().await;
            manager.set("ai.agent_models", &config.ai.agent_models).await?;
            manager
        ...
        if default_models_changed {
            let mut manager = self.manager.write().await;
            manager.set("ai.default_models", &config.ai.default_models).await?;
        }
        ...
        if report.is_noop() {
            tracing::debug!("Reconcile ({caller}): no changes");
        } else {
            info!(
                "Reconcile ({caller}): invalidated={:?}, default_changed={}, agent_changed={}",
                report.invalidated_model_ids, report.default_models_changed, report.agent_models_changed
            );
...
impl northhing_runtime_ports::ConfigReadPort for ConfigService {
    async fn get_config_value(&self, key: &str) -> northhing_runtime_ports::PortResult<Option<serde_json::Value>> {
        self.config::<serde_json::Value>(Some(key))
            .await
            .map(Some)
            .map_err(|error| {
...
impl ReconcileModelsReport {
    pub fn is_noop(&self) -> bool {
        self.invalidated_model_ids.is_empty() && !self.default_models_changed && !self.agent_models_changed
    }
}
