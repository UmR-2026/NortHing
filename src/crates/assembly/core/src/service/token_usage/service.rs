...
impl TokenUsageService {
    ...
    async fn save_model_stats(&self) -> Result<()> {
        ...
        let content = serde_json::to_string_pretty(&*model_stats).context("Failed to serialize model stats")?;
    ...
    async fn persist_record(&self, record: &TokenUsageRecord) -> Result<()> {
        ...
        let mut batch = if path.exists() {
            ...
            serde_json::from_str::<RecordsBatch>(&content).unwrap_or_else(|_| RecordsBatch { records: Vec::new() })
        } else {
            RecordsBatch { records: Vec::new() }
        };
    ...
    pub async fn summary(&self, query: TokenUsageQuery) -> Result<TokenUsageSummary> {
        let records = self.query_records(query).await?;
