...
impl ConfigWatcher {
    ...
    pub fn new(workspace_path: PathBuf, on_config_changed: Arc<dyn Fn(String, String) + Send + Sync>) -> Result<Self> {
        info!("Setting up config file watcher for workspace: {:?}", workspace_path);
        ...
        for path in event.paths {
            if let Some(file_name) = path.file_name().and_then(|n| n.to_str()) {
                ...
                if let Some(lang) = language {
                    info!("Config file changed: {}, restarting {} server", file_name, lang);
...
impl Drop for ConfigWatcher {
    fn drop(&mut self) {
        debug!("ConfigWatcher dropped for workspace: {:?}", self.workspace_path);
    }
}
