...
pub async fn read_message(reader: &mut BufReader<ChildStdout>) -> Result<JsonRpcMessage> {
    ...
    loop {
        ...
        let _bytes_read = tokio::io::AsyncBufReadExt::read_until(reader, b'\n', &mut raw_line).await?;
        line_count += 1;

        let header = match String::from_utf8(raw_line.clone()) {
            Ok(s) => s,
            Err(e) => {
                warn!("[LSP Protocol] Line {} contains non-UTF8 data: {:?}", line_count, e);
...
pub fn create_request(id: u64, method: impl Into<String>, params: Option<serde_json::Value>) -> JsonRpcMessage {
...
pub fn create_notification(method: impl Into<String>, params: Option<serde_json::Value>) -> JsonRpcMessage {
...
pub fn extract_result(response: JsonRpcResponse) -> Result<serde_json::Value> {
    ...
    response.result.ok_or_else(|| anyhow!("Missing result in response"))
}
