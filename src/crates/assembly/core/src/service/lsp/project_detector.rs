...
impl ProjectDetector {
        ...
        if let Ok(mut entries) = fs::read_dir(workspace_path).await {
            while let Ok(Some(entry)) = entries.next_entry().await {
                ...
                if path.is_dir() {
                    ...
                    if matches!(dir_name, "node_modules" | "target" | ".git" | "dist" | "build" | "out") {
                        continue;
                    }

                    if path.join("Cargo.toml").exists() && !info.languages.contains(&"rust".to_string()) {
                        Self::add_language(info, "rust");
                        Self::add_project_type(info, "rust");
                    }
                }
        ...
        if let Ok(entries) = std::fs::read_dir(dir) {
            for entry in entries.flatten() {
                if let Some(file_ext) = entry.path().extension() {
                    if file_ext.to_str().map(|e| e.eq_ignore_ascii_case(ext)).unwrap_or(false) {
                        return true;
                    }
                }
            }
    ...
    async fn detect_by_file_extensions(workspace_path: &Path, info: &mut ProjectInfo) -> Result<()> {
        let mut counts: HashMap<String, usize> = HashMap::new();
        let max_scan_files = 5000;
        let mut scanned = 0;

        Self::scan_directory_iterative(workspace_path, &mut counts, &mut scanned, max_scan_files).await?;
