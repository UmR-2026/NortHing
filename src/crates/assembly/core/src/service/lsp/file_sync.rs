...
#[derive(Debug, Clone)]
struct FileChange {
    // reason: path is reserved for the upcoming change-source diagnostics surface (today's sync uses change_count only)
    path: PathBuf,
    // reason: last_modified is reserved for the upcoming sync-conflict resolution (today's sync is debounce-based)
...
impl LspFileSync {
    ...
    async fn sync_workspace_files(self: &Arc<Self>, workspace: PathBuf, files: Vec<PathBuf>) -> Result<()> {
        let manager = {
            let managers = self.workspace_managers.read().await;
            managers
                .get(&workspace)
        ...
        info!("Syncing {} file(s) to LSP for workspace: {:?}", files.len(), workspace);
