...
pub fn global_lsp_manager() -> anyhow::Result<Arc<RwLock<LspManager>>> {
    GLOBAL_LSP_MANAGER
        .get()
        .cloned()
...
pub async fn open_workspace_with_emitter(
    ...
    let lsp_manager = global_lsp_manager()?;
...
pub async fn get_workspace_manager(workspace_path: PathBuf) -> anyhow::Result<Arc<WorkspaceLspManager>> {
    let workspace_key = workspace_path.to_string_lossy().to_string();
