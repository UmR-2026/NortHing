...
pub struct MCPServerProcess {
    inner: northhing_services_integrations::mcp::server::MCPServerProcess,
}

impl MCPServerProcess {
    pub fn new(id: String, name: String, server_type: MCPServerType) -> Self {
        Self {
            inner: northhing_services_integrations::mcp::server::MCPServerProcess::new(id, name, server_type),
        }
    }
    ...
    pub async fn start_remote(&mut self, config: &MCPServerConfig) -> NortHingResult<()> {
        let data_dir = try_get_path_manager_arc()?.user_data_dir();
        self.inner.start_remote(data_dir, config).await?;
        Ok(())
    }

    pub async fn stop(&mut self) -> NortHingResult<()> {
