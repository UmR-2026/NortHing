...
impl MCPServerRegistry {
    ...
    /// Registers a server.
    pub async fn register(&self, config: &MCPServerConfig) -> crate::util::errors::NortHingResult<()> {
        config.validate()?;

        let process = MCPServerProcess::new(config.id.clone(), config.name.clone(), config.server_type);

        let mut servers = self.servers.write().await;
        servers.insert(config.id.clone(), Arc::new(RwLock::new(process)));

        info!("Registered MCP server: name={} id={}", config.name, config.id);
        Ok(())
    }

    /// Unregisters a server.
    pub async fn unregister(&self, server_id: &str) -> crate::util::errors::NortHingResult<()> {
        let mut servers = self.servers.write().await;
        ...
        if let Some(process) = servers.remove(server_id) {
            ...
            Err(crate::util::errors::NortHingError::NotFound(format!(
    ...
    /// Clears the registry.
    pub async fn clear(&self) -> crate::util::errors::NortHingResult<()> {
        let mut servers = self.servers.write().await;
