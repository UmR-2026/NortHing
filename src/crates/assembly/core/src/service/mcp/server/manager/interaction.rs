use super::*;
use northhing_services_integrations::mcp::server::{detect_mcp_list_changed_kind, MCPListChangedKind};
use std::collections::HashSet;
use std::path::PathBuf;

impl MCPServerManager {
    fn path_to_file_uri(path: &Path) -> Option<String> {
        reqwest::Url::from_directory_path(path).ok().map(|u| u.to_string())
    }
    ...
    async fn handle_server_request(
        ...
        match method.as_str() {
            ...
            "elicitation/create" | "sampling/createMessage" => {
                self.handle_interactive_server_request(server_id, server_name, connection, request_id, method, params)
                    .await;
            }
            _ => {
    ...
    async fn handle_interactive_server_request(
        ...
        match decision {
            ...
            Err(_) => {
                let error =
                    MCPError::internal_error(format!("MCP interaction channel closed before response: {}", method));
                if let Err(e) = connection.send_error(request_id, error).await {
                    warn!(
        ...
        let decision = if approve {
            ...
            MCPInteractionDecision::Reject {
                error: MCPError {
                    ...
                    message: error_message.unwrap_or_else(|| "User rejected MCP interaction request".to_string()),
                    data: error_data,
                },
            }
        };
                    ...
                    Ok(MCPConnectionEvent::Notification { method, .. }) => {
                        match detect_mcp_list_changed_kind(&method) {
                            ...
                            Some(MCPListChangedKind::Prompts) => {
                                ...
                                if let Err(e) = manager
                                    .refresh_prompts_catalog(&server_id_owned, connection_for_refresh.clone())
                                    .await
                                {
                                    warn!(
                            ...
                            Some(MCPListChangedKind::Resources) => {
                                ...
                                if let Err(e) = manager
                                    .refresh_resources_catalog(&server_id_owned, connection_for_refresh.clone())
                                    .await
                                {
                                    warn!(
