...
impl MCPServerManager {
    ...
    pub async fn initialize_all(&self) -> NortHingResult<()> {
        ...
        for config in &configs {
            if config.enabled {
                match self.registry.register(config).await {
                    Ok(_) => {
                        registered_count += 1;
                        debug!("Registered MCP server: name={} id={}", config.name, config.id);
                    }
                    Err(e) => {
                        error!(
        ...
        for config in configs {
            if config.enabled && config.auto_start {
                info!("Auto-starting MCP server: name={} id={}", config.name, config.id);
    ...
    pub async fn initialize_non_destructive(&self) -> NortHingResult<()> {
    ...
    pub async fn ensure_registered(&self, server_id: &str) -> NortHingResult<()> {
    ...
    /// Starts a server.
    pub async fn start_server(&self, server_id: &str) -> NortHingResult<()> {
        self.start_reconnect_monitor_if_needed();
        info!("Starting MCP server: id={}", server_id);

        let config = self.runtime_server_config(server_id).await.inspect_err(|_error| {
            error!("MCP server config not found: id={}", server_id);
        })?;

        if !config.enabled {
            warn!("MCP server is disabled: id={}", server_id);
            return Err(NortHingError::Configuration(format!(
        ...
        let process = self.registry.get_process(server_id).await.ok_or_else(|| {
            ...
            NortHingError::NotFound(format!("MCP server not registered: {}", server_id))
        })?;
        ...
        match config.server_type {
            super::super::MCPServerType::Local => {
                let command = config.command.as_ref().ok_or_else(|| {
                    ...
                    NortHingError::Configuration("Missing command for local MCP server".to_string())
                })?;

                let runtime_manager = RuntimeManager::new()?;
                let resolved = runtime_manager.resolve_command(command).ok_or_else(|| {
                    NortHingError::ProcessError(format!(
                        "MCP server command '{}' not found in system PATH or northhing managed runtimes at {}",
                        command,
            ...
            super::super::MCPServerType::Remote => {
                ...
                if transport != crate::service::mcp::server::MCPServerTransport::StreamableHttp {
                    ...
                    return Err(NortHingError::NotImplemented(format!(
                ...
                let url = config.url.as_ref().ok_or_else(|| {
                    ...
                    NortHingError::Configuration("Missing URL for remote MCP server".to_string())
                })?;
    ...
    /// Stops a server.
    pub async fn stop_server(&self, server_id: &str) -> NortHingResult<()> {
        ...
        let process = self
            .registry
            .get_process(server_id)
            .await
            .ok_or_else(|| NortHingError::NotFound(format!("MCP server not found: {}", server_id)))?;
    ...
    /// Restarts a server.
    pub async fn restart_server(&self, server_id: &str) -> NortHingResult<()> {
        ...
        match config.server_type {
            super::super::MCPServerType::Local => {
                ...
                let command = config
                    .command
                    .as_ref()
                    .ok_or_else(|| NortHingError::Configuration("Missing command".to_string()))?;
    ...
    pub async fn get_server_status(&self, server_id: &str) -> NortHingResult<MCPServerStatus> {
        if !self.registry.contains(server_id).await {
    ...
    pub async fn get_server_status_message(&self, server_id: &str) -> NortHingResult<Option<String>> {
        if !self.registry.contains(server_id).await {
            let _ = self.ensure_registered(server_id).await;
        }
    ...
    /// Adds a server.
    pub async fn add_server(&self, config: MCPServerConfig) -> NortHingResult<()> {
        config.validate()?;
    ...
    pub async fn add_ephemeral_server(&self, config: MCPServerConfig) -> NortHingResult<()> {
        config.validate()?;
    ...
    pub async fn remove_ephemeral_server(&self, server_id: &str) -> NortHingResult<()> {
    ...
    /// Removes a server.
    pub async fn remove_server(&self, server_id: &str) -> NortHingResult<()> {
        ...
        match self.registry.unregister(server_id).await {
            ...
            Err(e) => {
                warn!("Server not running, skipping unregister: id={} error={}", server_id, e);
            }
        }
    ...
    pub async fn update_server_config(&self, config: MCPServerConfig) -> NortHingResult<()> {
        config.validate()?;
        ...
        if matches!(status, MCPServerStatus::Connected | MCPServerStatus::Healthy) {
            info!("Restarting MCP server to apply new configuration: id={}", config.id);
    ...
    pub async fn reauthenticate_remote_server(&self, server_id: &str, authorization_value: &str) -> NortHingResult<()> {
    ...
    pub async fn clear_remote_server_auth(&self, server_id: &str) -> NortHingResult<()> {
        self.clear_remote_oauth_credentials(server_id).await?;
    ...
    pub async fn shutdown(&self) -> NortHingResult<()> {
