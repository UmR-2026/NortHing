use super::*;
use northhing_services_integrations::mcp::server::compute_mcp_backoff_delay;
...
impl MCPServerManager {
    ...
    async fn reconnect_once(&self) -> NortHingResult<()> {
        ...
        for config in configs {
            ...
            if !matches!(status, MCPServerStatus::Reconnecting | MCPServerStatus::Failed) {
                continue;
            }

            self.try_reconnect_server(&config.id, &config.name, status).await;
        }

        Ok(())
    }

    async fn try_reconnect_server(&self, server_id: &str, server_name: &str, status: MCPServerStatus) {
        let now = Instant::now();
