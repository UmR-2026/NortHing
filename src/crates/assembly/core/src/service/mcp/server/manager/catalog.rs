...
impl MCPServerManager {
    pub(super) async fn refresh_resources_catalog(
        ...
        self.catalog_cache.replace_resources(server_id, resources).await;
        Ok(count)
    }
    ...
    pub(super) async fn warm_catalog_caches(&self, server_id: &str, connection: Arc<MCPConnection>) {
        if let Err(e) = self.refresh_resources_catalog(server_id, connection.clone()).await {
            debug!(
    ...
    pub async fn refresh_server_resource_catalog(&self, server_id: &str) -> NortHingResult<usize> {
        let connection = self
    ...
    pub async fn refresh_server_prompt_catalog(&self, server_id: &str) -> NortHingResult<usize> {
        let connection = self
            .get_connection(server_id)
            .await
            .ok_or_else(|| NortHingError::NotFound(format!("MCP server connection not found: {}", server_id)))?;
