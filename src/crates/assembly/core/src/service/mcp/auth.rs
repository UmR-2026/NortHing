...
//!
//! The owner implementation lives in `northhing-services-integrations`. This
...
pub use northhing_services_integrations::mcp::auth::{
    MCPRemoteOAuthSessionSnapshot, MCPRemoteOAuthStatus, PreparedMCPRemoteOAuthAuthorization,
};

fn oauth_data_dir() -> NortHingResult<PathBuf> {
    Ok(try_get_path_manager_arc()?.user_data_dir())
}

pub struct MCPRemoteOAuthCredentialVault {
    inner: northhing_services_integrations::mcp::auth::MCPRemoteOAuthCredentialVault,
}

impl MCPRemoteOAuthCredentialVault {
    pub fn new() -> NortHingResult<Self> {
        Ok(Self {
            inner: northhing_services_integrations::mcp::auth::MCPRemoteOAuthCredentialVault::new(oauth_data_dir()?),
        })
    }
    ...
    pub async fn store(&self, server_id: &str, credentials: &StoredCredentials) -> anyhow::Result<()> {
...
pub fn map_auth_error(error: impl ToString) -> NortHingError {
    NortHingError::MCPError(format!("OAuth error: {}", error.to_string()))
}

pub async fn has_stored_oauth_credentials(server_id: &str) -> NortHingResult<bool> {
    northhing_services_integrations::mcp::auth::has_stored_oauth_credentials(oauth_data_dir()?, server_id)
        .await
        .map_err(map_auth_error)
}

pub async fn clear_stored_oauth_credentials(server_id: &str) -> NortHingResult<()> {
    northhing_services_integrations::mcp::auth::clear_stored_oauth_credentials(oauth_data_dir()?, server_id)
        .await
        .map_err(map_auth_error)
}
...
pub async fn build_authorization_manager(
    server_id: &str,
    server_url: &str,
) -> NortHingResult<(AuthorizationManager, bool)> {
    northhing_services_integrations::mcp::auth::build_authorization_manager(oauth_data_dir()?, server_id, server_url)
        .await
        .map_err(map_auth_error)
}
...
) -> NortHingResult<PreparedMCPRemoteOAuthAuthorization> {
    northhing_services_integrations::mcp::auth::prepare_remote_oauth_authorization(oauth_data_dir()?, config)
        .await
        .map_err(map_auth_error)
}
