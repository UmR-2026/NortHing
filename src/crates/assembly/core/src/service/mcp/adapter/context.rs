...
pub use northhing_services_integrations::mcp::adapter::{
...
impl MCPContextProvider {
    ...
    pub fn with_config(server_manager: Arc<MCPServerManager>, config: ContextEnhancerConfig) -> Self {
        Self {
            server_manager,
    ...
    pub async fn query_resources(
    ...
    ) -> NortHingResult<Vec<(MCPResource, MCPResourceContent)>> {
        let mut all_resources = Vec::new();

        let server_ids = server_ids.unwrap_or_else(|| {
            tokio::task::block_in_place(|| {
                tokio::runtime::Handle::current().block_on(async { self.server_manager.get_all_server_ids().await })
            })
        });
        ...
        for server_id in server_ids {
            ...
            let task = tokio::spawn(async move { Self::query_server_resources(&manager, &server_id, &query).await });
    ...
    async fn query_server_resources(
        ...
        let connection = manager
            ...
            .await
            .ok_or_else(|| NortHingError::NotFound(format!("MCP server connection not found: {}", server_id)))?;
    ...
    pub async fn get_prompt_enhancements(
        ...
        for server_id in server_ids {
            if let Some(connection) = self.server_manager.get_connection(&server_id).await {
                ...
                if prompts.is_empty() {
                    let _ = self.server_manager.refresh_server_prompt_catalog(&server_id).await;
                ...
                for prompt in prompts {
                    if prompt_names.contains(&prompt.name) {
                        if let Ok(content) = connection.get_prompt(&prompt.name, Some(arguments.clone())).await {
