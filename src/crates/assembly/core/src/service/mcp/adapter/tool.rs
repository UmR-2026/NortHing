//! MCP tool adapter
//!
//! Wraps MCP tools as implementations of northhing's `Tool` trait.

use crate::agentic::tools::framework::{
    DynamicMcpToolInfo, DynamicToolInfo, Tool, ToolRenderOptions, ToolResult, ToolUseContext, ValidationResult,
};
use crate::service::mcp::protocol::{MCPTool, MCPToolResult};
use crate::service::mcp::server::MCPConnection;
use crate::util::errors::NortHingResult;
use async_trait::async_trait;
use northhing_services_integrations::mcp::adapter::{
    build_mcp_tool_descriptor, render_mcp_tool_result_for_assistant, MCPDynamicToolProvider, McpDynamicToolDescriptor,
};
use serde_json::Value;
use std::sync::Arc;
use tracing::{debug, error, info, warn};

/// MCP tool wrapper that adapts an MCP tool to northhing's `Tool`.
...
impl MCPToolWrapper {
    ...
    pub fn new(mcp_tool: MCPTool, connection: Arc<MCPConnection>, server_id: String, server_name: String) -> Self {
...
impl Tool for MCPToolWrapper {
    ...
    fn render_result_for_assistant(&self, output: &Value) -> String {
        if let Ok(result) = serde_json::from_value::<MCPToolResult>(output.clone()) {
            return render_mcp_tool_result_for_assistant(&self.mcp_tool.name, &result, Self::MAX_RESULT_TEXT_CHARS);
        }
...
impl MCPToolAdapter {
    ...
    pub async fn load_tools_from_server(
        ...
        info!("Loading tools from MCP server: {} (id={})", server_name, server_id);

        let provider = MCPDynamicToolProvider::new(server_id, server_name);
        let definitions = provider.load_tool_definitions(connection.as_ref()).await.map_err(|e| {
            error!("list_tools call failed: {}", e);
            crate::util::errors::NortHingError::from(e)
        })?;

        info!("Found {} MCP tool(s) from server {}", definitions.len(), server_name);
        ...
        info!("Tool loading complete, adapter now has {} tool(s)", self.tools.len());
        Ok(())
    }
