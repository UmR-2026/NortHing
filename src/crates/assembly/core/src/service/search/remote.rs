...
use crate::service::remote_ssh::workspace_state::{
    lookup_remote_connection, lookup_remote_connection_with_hint, remote_workspace_manager, RemoteWorkspaceEntry,
};
...
use northhing_services_integrations::remote_ssh::workspace_search::{
    RemoteCommandOutput, RemoteWorkspaceSearchProvider,
    RemoteWorkspaceSearchService as ServiceRemoteWorkspaceSearchService, RemoteWorkspaceSearchStdioProtocol,
};
use std::sync::Arc;
...
impl RemoteWorkspaceSearchService {
    ...
    pub fn with_preferred_connection_id(mut self, preferred_connection_id: Option<String>) -> Self {
        self.inner = self.inner.with_preferred_connection_id(preferred_connection_id);
        self
    }
...
impl RemoteWorkspaceSearchProvider for CoreRemoteWorkspaceSearchProvider {
    async fn resolve_workspace_entry(
        ...
        if let Some(entry) = lookup_remote_connection_with_hint(root_path, preferred_connection_id).await {
            return Ok(entry);
        }
    ...
    async fn repo_max_file_size(&self) -> u64 {
        match self.config_service.config::<WorkspaceConfig>(Some("workspace")).await {
...
pub async fn remote_workspace_search_service_for_path(
    ...
    let manager = remote_workspace_manager().ok_or_else(|| "Remote workspace manager is unavailable".to_string())?;
