...
impl RemoteWorkspaceSearchService {
    ...
    pub async fn search_content(&self, _request: ContentSearchRequest) -> Result<ContentSearchResult, String> {
        Err(unsupported())
    }
    ...
    pub async fn resolve_remote_workspace_entry(&self, _repo_root: &str) -> Result<RemoteWorkspaceEntry, String> {
        Err(unsupported())
    }
}
