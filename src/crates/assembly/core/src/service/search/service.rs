...
use northhing_services_integrations::workspace_search as owner;
...
impl WorkspaceSearchService {
    ...
    pub async fn open_repo(&self, repo_root: impl AsRef<Path>) -> NortHingResult<owner::WorkspaceIndexStatus> {
        self.inner
            .open_repo(repo_root)
            .await
    ...
    pub async fn get_index_status(&self, repo_root: impl AsRef<Path>) -> NortHingResult<owner::WorkspaceIndexStatus> {
        self.inner
    ...
    pub async fn build_index(&self, repo_root: impl AsRef<Path>) -> NortHingResult<owner::IndexTaskHandle> {
        self.inner
    ...
    pub async fn rebuild_index(&self, repo_root: impl AsRef<Path>) -> NortHingResult<owner::IndexTaskHandle> {
        self.inner
    ...
    ) -> NortHingResult<owner::ContentSearchResult> {
        self.inner
    ...
    pub async fn glob(&self, request: owner::GlobSearchRequest) -> NortHingResult<owner::GlobSearchResult> {
...
impl owner::WorkspaceSearchRuntimeHooks for CoreWorkspaceSearchRuntimeHooks {
    async fn repo_config(&self) -> owner::WorkspaceSearchRepoConfig {
        let max_file_size = match get_global_config_service().await {
            Ok(config_service) => match config_service.config::<WorkspaceConfig>(Some("workspace")).await {
    ...
    async fn ensure_workspace_ready(&self, repo_root: &Path) -> owner::WorkspaceSearchResult<()> {
        ensure_workspace_gitignore_ignores_northhing(repo_root)
            .await
            .map(|_| ())
...
pub fn global_workspace_search_service() -> Option<Arc<WorkspaceSearchService>> {
...
pub async fn workspace_search_feature_enabled() -> bool {
    match get_global_config_service().await {
        Ok(config_service) => config_service
            .config::<bool>(Some("app.ai_experience.enable_workspace_search"))
            .await
            .unwrap_or(false),
        Err(_) => false,
    }
