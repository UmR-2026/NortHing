...
impl WorkspaceFactory {
    ...
    pub async fn create_default_service() -> NortHingResult<WorkspaceService> {
        WorkspaceService::new().await
    }

    /// Creates a workspace service with a custom config.
    pub async fn create_service_with_config(config: WorkspaceManagerConfig) -> NortHingResult<WorkspaceService> {
