...
impl WorkspaceIdentityWatchService {
    ...
    pub async fn sync_watched_workspaces(&self) -> NortHingResult<()> {
    ...
    async fn create_watcher(&self) -> NortHingResult<()> {
        ...
        let mut watcher = RecommendedWatcher::new(tx, Config::default())
            .map_err(|e| NortHingError::service(format!("Failed to create identity watcher: {}", e)))?;
        ...
        tokio::task::spawn_blocking(move || loop {
            match rx.recv() {
                Ok(Ok(event)) => {
                    let affected_roots = runtime.block_on(Self::extract_affected_roots(&event, &watched_paths));
    ...
    async fn schedule_refresh(
        ...
        let handle = tokio::spawn(async move {
            ...
            match workspace_service.refresh_workspace_identity(&workspace_id).await {
...
// reason: _assert_event_serializable is a compile-time check that the event type stays serializable (so it would be a bug if it lost serde derives)
#[allow(dead_code)]
