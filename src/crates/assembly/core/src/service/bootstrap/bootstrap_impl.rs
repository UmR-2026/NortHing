...
const PERSONA_FILE_NAMES: [&str; 4] = [BOOTSTRAP_FILE_NAME, SOUL_FILE_NAME, USER_FILE_NAME, IDENTITY_FILE_NAME];

fn normalize_line_endings(content: &str) -> String {
    content.replace("\r\n", "\n").replace('\r', "\n")
}

async fn ensure_markdown_placeholder(path: &Path, content: &str) -> NortHingResult<bool> {
    if path.exists() {
        return Ok(false);
    }
    ...
    fs::write(path, normalized_content)
        .await
        .map_err(|e| NortHingError::service(format!("Failed to create {}: {}", path.display(), e)))?;

    Ok(true)
}

fn gitignore_already_ignores_northhing(content: &str) -> bool {
    content.lines().any(|line| {
        let entry = line.trim();
        !entry.starts_with('#') && matches!(entry, ".northhing" | ".northhing/" | "/.northhing" | "/.northhing/")
    })
}

pub(crate) async fn ensure_workspace_gitignore_ignores_northhing(workspace_root: &Path) -> NortHingResult<bool> {
    let gitignore_path = workspace_root.join(".gitignore");
    let northhing_entry = ".northhing/";
    ...
    if gitignore_already_ignores_northhing(&content) {
        return Ok(false);
    }
    ...
    debug!(
        "Added workspace .gitignore entry for .northhing: path={}",
        gitignore_path.display()
    );
...
async fn ensure_workspace_gitignore_ignores_northhing_best_effort(workspace_root: &Path) -> bool {
    match ensure_workspace_gitignore_ignores_northhing(workspace_root).await {
        Ok(updated) => updated,
        Err(e) => {
            warn!(
                "Failed to ensure workspace .gitignore ignores .northhing: workspace={}, error={}",
...
pub(crate) async fn initialize_workspace_persona_files(workspace_root: &Path) -> NortHingResult<()> {
...
#[cfg(feature = "product-full")]
pub(crate) async fn build_workspace_persona_prompt(workspace_root: &Path) -> NortHingResult<Option<String>> {
    ...
    for file_name in PERSONA_FILE_NAMES {
        ...
        match fs::read_to_string(&file_path).await {
            ...
            Err(e) => {
                warn!("Failed to read persona file: path={} error={}", file_path.display(), e);
            }
        }
    }
    ...
    let bootstrap_detected = documents.iter().any(|(file_name, _)| *file_name == BOOTSTRAP_FILE_NAME);
...
mod tests {
    use super::{
        ensure_workspace_gitignore_ignores_northhing, ensure_workspace_persona_files_for_prompt,
        initialize_workspace_persona_files, normalize_line_endings, BOOTSTRAP_FILE_NAME, IDENTITY_FILE_NAME,
    ...
    #[tokio::test]
    async fn ensure_workspace_gitignore_ignores_northhing_skips_when_gitignore_missing() {
        let workspace_root = unique_workspace("northhing-gitignore-missing");
        fs::create_dir_all(&workspace_root)
            .await
            .expect("Failed to create temp workspace");

        let updated = ensure_workspace_gitignore_ignores_northhing(&workspace_root)
            .await
    ...
    #[tokio::test]
    async fn ensure_workspace_gitignore_ignores_northhing_appends_without_clobbering() {
        let workspace_root = unique_workspace("northhing-gitignore-append");
        fs::create_dir_all(&workspace_root)
            .await
            .expect("Failed to create temp workspace");
        ...
        ensure_workspace_gitignore_ignores_northhing(&workspace_root)
            .await
            .expect("Failed to ensure .gitignore");
    ...
    #[tokio::test]
    async fn ensure_workspace_gitignore_ignores_northhing_is_idempotent() {
        let workspace_root = unique_workspace("northhing-gitignore-idempotent");
        fs::create_dir_all(&workspace_root)
            .await
            .expect("Failed to create temp workspace");
        fs::write(workspace_root.join(".gitignore"), "target/\n.northhing/\n")
    ...
    async fn initialize_workspace_persona_files_creates_all_four_files() {
        let workspace_root = unique_workspace("northhing-bootstrap-init");
        ...
        for file_name in [BOOTSTRAP_FILE_NAME, SOUL_FILE_NAME, USER_FILE_NAME, IDENTITY_FILE_NAME] {
            assert!(
    ...
    async fn ensure_workspace_persona_files_for_prompt_preserves_completed_bootstrap() {
        let workspace_root = unique_workspace("northhing-bootstrap-preserve");
