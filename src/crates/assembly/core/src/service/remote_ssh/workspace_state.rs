...
pub use northhing_services_integrations::remote_ssh::{
    local_workspace_stable_storage_id, normalize_remote_workspace_path, remote_root_to_mirror_subpath,
    remote_workspace_stable_id, sanitize_remote_mirror_path_component, sanitize_ssh_connection_id_for_local_dir,
...
pub async fn resolve_workspace_session_identity(
    ...
    let remote_connection_id = remote_connection_id.map(str::trim).filter(|s| !s.is_empty());
...
pub fn remote_workspace_runtime_root(ssh_host: &str, remote_root_norm: &str) -> PathBuf {
    northhing_services_integrations::remote_ssh::remote_workspace_runtime_root(
...
pub fn remote_workspace_session_mirror_dir(ssh_host: &str, remote_root_norm: &str) -> PathBuf {
    northhing_services_integrations::remote_ssh::remote_workspace_session_mirror_dir(
        PathManager::remote_ssh_mirror_root(),
        ssh_host,
        remote_root_norm,
    )
...
pub fn canonicalize_local_workspace_root(path: &Path) -> Result<(PathBuf, String), String> {
    northhing_services_integrations::remote_ssh::canonicalize_local_workspace_root(path)
}
...
pub fn normalize_local_workspace_root_for_stable_id(path: &Path) -> Result<String, String> {
    northhing_services_integrations::remote_ssh::normalize_local_workspace_root_for_stable_id(path)
}
...
pub fn local_workspace_roots_equal(a: &Path, b: &Path) -> bool {
    northhing_services_integrations::remote_ssh::local_workspace_roots_equal(a, b)
}
...
/// This returns a dedicated stub under `~/.northhing/remote_ssh/_unresolved/.../sessions` that is
...
pub fn unresolved_remote_session_storage_dir(connection_id: &str, workspace_path_norm: &str) -> PathBuf {
    northhing_services_integrations::remote_ssh::unresolved_remote_session_storage_dir(
        PathManager::remote_ssh_mirror_root(),
        connection_id,
        workspace_path_norm,
    )
...
impl RemoteWorkspaceStateManager {
    ...
    pub async fn set_active_connection_hint(&self, connection_id: Option<String>) {
        self.registry.set_active_connection_hint(connection_id).await;
    }
    ...
    pub async fn lookup_connection(
        ...
        // Assistant sessions use client-local paths under ~/.northhing/personal_assistant.
        ...
        let is_local_assistant_path = path_manager_arc().is_local_assistant_workspace_path(path);
        if is_local_assistant_path {
            let preferred_connection_id = preferred_connection_id?;
            return self.registry.lookup_by_connection_id(preferred_connection_id).await;
        }

        self.registry.lookup_connection(path, preferred_connection_id).await
    }
    ...
    pub async fn is_remote_path(&self, path: &str) -> bool {
        if path_manager_arc().is_local_assistant_workspace_path(path) {
            return false;
        }
    ...
    pub async fn activate_remote_workspace(&self, connection_id: String, remote_path: String, connection_name: String) {
    ...
    /// Local mirror directory for persisted sessions (`~/.northhing/remote_ssh/.../sessions`).
...
static REMOTE_WORKSPACE_MANAGER: std::sync::OnceLock<Arc<RemoteWorkspaceStateManager>> = std::sync::OnceLock::new();
...
pub fn remote_workspace_manager() -> Option<Arc<RemoteWorkspaceStateManager>> {
...
pub async fn get_effective_session_path(
    ...
    if let Some(identity) =
        resolve_workspace_session_identity(workspace_path, remote_connection_id, remote_ssh_host).await
    {
        if identity.hostname == "_unresolved" {
            if let Some(connection_id) = identity.remote_connection_id.as_deref() {
                return unresolved_remote_session_storage_dir(connection_id, identity.logical_workspace_path());
            }
        }
...
pub async fn is_remote_path(path: &str) -> bool {
    if let Some(manager) = remote_workspace_manager() {
...
pub async fn lookup_remote_connection_with_hint(
    ...
    let manager = remote_workspace_manager()?;
    manager.lookup_connection(path, preferred_connection_id).await
}
...
mod tests {
    use super::{
        normalize_remote_workspace_path, remote_workspace_session_mirror_dir, sanitize_ssh_connection_id_for_local_dir,
    ...
    async fn local_assistant_path_not_remote_without_connection_id() {
        ...
        assert!(
            m.lookup_connection(&assistant_path, Some("conn")).await.is_some(),
    ...
    fn normalize_remote_trims_trailing_slash() {
        assert_eq!(normalize_remote_workspace_path("/home/user/repo/"), "/home/user/repo");
    }

    #[test]
    fn local_stable_id_is_deterministic_and_prefixed() {
        let id1 = super::local_workspace_stable_storage_id("/Users/foo/northhing");
        let id2 = super::local_workspace_stable_storage_id("/Users/foo/northhing");
        assert_eq!(id1, id2);
    ...
    fn local_workspace_session_identity_uses_workspace_root_for_storage() {
        let workspace_root =
            std::env::temp_dir().join(format!("northhing-workspace-identity-{}", uuid::Uuid::new_v4()));
