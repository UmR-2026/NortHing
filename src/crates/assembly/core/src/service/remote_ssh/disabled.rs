...
use crate::service::remote_ssh::types::{
    RemoteDirEntry, RemoteFileEntry, RemoteTreeNode, SSHCommandOptions, SSHCommandResult, SSHConfigEntry,
...
impl RemoteExecProcessManager {
    ...
    pub async fn write_stdin(&self, _request: RemoteWriteStdinRequest) -> RemoteExecResult<RemoteExecCommandResponse> {
        Err(unsupported().into())
    }
...
impl SSHConnectionManager {
    ...
    pub async fn prune_remote_workspaces_without_saved_connections(&self) -> anyhow::Result<Vec<String>> {
        Ok(Vec::new())
    }
    ...
    pub async fn load_stored_password(&self, _connection_id: &str) -> anyhow::Result<Option<String>> {
        Err(unsupported())
    }
    ...
    pub async fn execute_command(&self, _connection_id: &str, _command: &str) -> anyhow::Result<(String, String, i32)> {
        Err(unsupported())
    }
    ...
    pub async fn resolve_sftp_path(&self, _connection_id: &str, _path: &str) -> anyhow::Result<String> {
        Err(unsupported())
    }
    ...
    pub async fn sftp_write(&self, _connection_id: &str, _path: &str, _content: &[u8]) -> anyhow::Result<()> {
        Err(unsupported())
    }
    ...
    pub async fn sftp_rename(&self, _connection_id: &str, _old_path: &str, _new_path: &str) -> anyhow::Result<()> {
        Err(unsupported())
    }
    ...
    pub async fn open_pty(&self, _connection_id: &str, _cols: u32, _rows: u32) -> anyhow::Result<PTYSession> {
        Err(unsupported())
    }
...
impl RemoteFileService {
    pub fn new(_manager: std::sync::Arc<tokio::sync::RwLock<Option<SSHConnectionManager>>>) -> Self {
        Self
    }
    ...
    pub async fn write_file(&self, _connection_id: &str, _path: &str, _content: &[u8]) -> anyhow::Result<()> {
        Err(unsupported())
    }
    ...
    pub async fn read_dir(&self, _connection_id: &str, _path: &str) -> anyhow::Result<Vec<RemoteDirEntry>> {
        Err(unsupported())
    }
    ...
    pub async fn rename(&self, _connection_id: &str, _old_path: &str, _new_path: &str) -> anyhow::Result<()> {
        Err(unsupported())
    }

    pub async fn stat(&self, _connection_id: &str, _path: &str) -> anyhow::Result<Option<RemoteFileEntry>> {
        Err(unsupported())
    }
}
