...
impl I18nService {
    ...
    pub async fn initialize(&self) -> NortHingResult<()> {
        ...
        if let Some(ref config_service) = self.config_service {
            ...
            if let Ok(app_lang) = config_service.config::<String>(Some("app.language")).await {
                resolved = LocaleId::from_str(&app_lang);
            }
            if resolved.is_none() {
                if let Ok(locale) = config_service.config::<LocaleId>(Some("i18n.currentLanguage")).await {
    ...
    async fn load_all_bundles(&self) -> NortHingResult<()> {
    ...
    pub async fn set_locale(&self, locale: LocaleId) -> NortHingResult<()> {
        let old_locale = {
            let mut current = self.current_locale.write().await;
            let old = *current;
            *current = locale;
        ...
        info!("Locale changed: {} -> {}", old_locale.as_str(), locale.as_str());
        Ok(())
    }

    /// Returns all supported locales.
    pub fn supported_locales(&self) -> Vec<LocaleMetadata> {
        LocaleMetadata::all()
    }
    ...
    pub async fn translate_with_locale(&self, locale: &LocaleId, key: &str, args: Option<TranslationArgs>) -> String {
    ...
    /// Formats a message.
    fn format_message(bundle: &ConcurrentBundle, key: &str, args: Option<&TranslationArgs>) -> Option<String> {
        ...
        if !errors.is_empty() {
            warn!("Translation formatting warning for key '{}': {:?}", key, errors);
        }
...
pub async fn sync_global_i18n_service_locale(locale: LocaleId) -> NortHingResult<bool> {
...
pub async fn initialize_global_i18n_service(
    config_service: Option<Arc<ConfigService>>,
) -> NortHingResult<Arc<I18nService>> {
