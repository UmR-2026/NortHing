...
pub fn validate_schedule(schedule: &CronSchedule, created_at_ms: i64) -> NortHingResult<()> {
    let _ = compute_next_run_after_ms(schedule, created_at_ms, created_at_ms - 1)?;
    Ok(())
}

pub fn compute_initial_next_run_at_ms(job: &CronJob, now_ms: i64) -> NortHingResult<Option<i64>> {
    match &job.schedule {
        CronSchedule::At { .. } => {
...
pub fn compute_next_run_after_ms(
    ...
    match schedule {
        ...
        CronSchedule::Every { every_ms, anchor_ms } => {
            compute_every_next_run_ms(*every_ms, anchor_ms.unwrap_or(created_at_ms), after_ms).map(Some)
        }
        CronSchedule::Cron { expr, tz } => compute_cron_next_run_ms(expr, tz.as_deref(), after_ms).map(Some),
    }
}

pub fn parse_at_timestamp_ms(schedule: &CronSchedule) -> NortHingResult<i64> {
    ...
    let parsed = DateTime::parse_from_rfc3339(at)
        .map_err(|error| NortHingError::validation(format!("Invalid ISO-8601 timestamp '{}': {}", at, error)))?;
    Ok(parsed.timestamp_millis())
}

fn compute_every_next_run_ms(every_ms: u64, anchor_ms: i64, after_ms: i64) -> NortHingResult<i64> {
    if every_ms == 0 {
        return Err(NortHingError::validation(
    ...
    i64::try_from(next).map_err(|_| NortHingError::service("Recurring schedule next run timestamp overflowed i64"))
}

fn compute_cron_next_run_ms(expr: &str, tz: Option<&str>, after_ms: i64) -> NortHingResult<i64> {
    let normalized_expr = normalize_cron_expr(expr)?;
    let schedule = Schedule::from_str(&normalized_expr)
        .map_err(|error| NortHingError::validation(format!("Invalid cron expression '{}': {}", expr, error)))?;

    match tz {
        Some(tz_name) => {
            let timezone = parse_timezone(tz_name)?;
            let after = timezone.timestamp_millis_opt(after_ms).single().ok_or_else(|| {
            ...
            schedule
                ...
                .ok_or_else(|| {
                    NortHingError::validation(format!("Cron expression '{}' produced no future run time", expr))
                })
        }
        None => {
            let after = Local.timestamp_millis_opt(after_ms).single().ok_or_else(|| {
                NortHingError::validation(format!("Unable to interpret local timestamp {}", after_ms))
            })?;
...
fn parse_timezone(tz_name: &str) -> NortHingResult<Tz> {
    Tz::from_str(tz_name)
        .map_err(|error| NortHingError::validation(format!("Invalid timezone '{}': {}", tz_name, error)))
}

fn normalize_cron_expr(expr: &str) -> NortHingResult<String> {
    let fields = expr.split_whitespace().collect::<Vec<_>>();
    match fields.len() {
        5 => Ok(format!("0 {}", expr)),
        6 | 7 => Ok(expr.to_string()),
        other => Err(NortHingError::validation(format!(
