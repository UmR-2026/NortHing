...
use super::types::{
    CronJob, CronJobPayload, CronJobState, CronJobTarget, CronJobsFile, CronSchedule, CronWorkspaceRef,
    CRON_JOBS_VERSION,
};
...
impl CronJobStore {
    pub async fn new(path_manager: Arc<PathManager>) -> NortHingResult<Self> {
    ...
    pub async fn load(&self) -> NortHingResult<CronJobsFile> {
        ...
        let content = fs::read_to_string(&jobs_file_path)
            .await
            .map_err(|error| NortHingError::service(format!("Failed to read file: {}", error)))?;
    ...
    pub async fn save_jobs(&self, jobs: Vec<CronJob>) -> NortHingResult<()> {
        let mut jobs = jobs;
    ...
    async fn backup_incompatible_jobs_file(&self, jobs_file_path: &Path) -> NortHingResult<()> {
        let backup_path = incompatible_backup_path(jobs_file_path);
        fs::rename(jobs_file_path, &backup_path).await.map_err(|error| {
...
fn parse_jobs_file_content(content: &str, jobs_file_path: &Path) -> NortHingResult<LoadJobsOutcome> {
    ...
    let version = value.get("version").and_then(|value| value.as_u64()).ok_or_else(|| {
        NortHingError::service(format!(
    ...
    Err(NortHingError::service(format!(
...
mod tests {
    ...
    fn parse_unknown_version_returns_error() {
        ...
        let error = parse_jobs_file_content(content, Path::new("jobs.json")).expect_err("unknown version should fail");

        assert!(error.to_string().contains("Unsupported cron jobs file version"));
    }
}
