...
async fn load_workspace_instruction_files(workspace_root: &Path) -> NortHingResult<Vec<WorkspaceInstructionFile>> {
...
fn render_workspace_instruction_files_section(files: &[WorkspaceInstructionFile]) -> Option<String> {
    if files.is_empty() {
        return None;
    }
...
pub(crate) async fn build_workspace_instruction_files_context(workspace_root: &Path) -> NortHingResult<Option<String>> {
    let instruction_files = load_workspace_instruction_files(workspace_root).await?;
    Ok(render_workspace_instruction_files_section(&instruction_files))
}
