...
impl RemoteFetcher {
    pub fn new(path_manager: &Arc<PathManager>) -> Self {
        let cache_file = path_manager.user_config_dir().join("announcement-remote-cache.json");
        Self {
            cache_file,
    ...
    pub async fn fetch_if_stale(&self) {
        ...
        let request_url = format!("{}?app_version={}&locale={}&platform=desktop", url, version, locale);
        ...
        match reqwest::get(&request_url).await {
            ...
            Ok(resp) => warn!("Remote announcement endpoint returned HTTP {}", resp.status()),
    ...
    async fn load_disk_cache(&self) -> Vec<AnnouncementCard> {
        match fs::read_to_string(&self.cache_file).await {
            Ok(content) => {
                let cards = serde_json::from_str::<Vec<AnnouncementCard>>(&content).unwrap_or_default();
    ...
    async fn remote_url() -> Option<String> {
        ...
        match get_global_config_service().await {
            Ok(svc) => svc.config::<String>(Some("announcements.remote_url")).await.ok(),
            Err(_) => None,
        }
    }
            ...
            .and_then(|svc| {
                tokio::task::block_in_place(|| {
                    tokio::runtime::Handle::current()
                        .block_on(svc.config::<String>(Some("general.language")))
                        .ok()
                })
            })
