...
use super::types::{
    AnnouncementCard, CardSource, CardType, CompletionAction, ModalConfig, ModalPage, ModalSize, PageLayout,
    ToastConfig, TriggerCondition, TriggerRule,
};
...
fn resolve_locale<'a>(category: &str, locale: &'a str) -> &'a str {
    ...
    let has_any = EMBEDDED_ANNOUNCEMENTS.keys().any(|k| k.starts_with(probe.as_str()));
    if has_any {
        locale
    } else {
        FALLBACK_LOCALE
