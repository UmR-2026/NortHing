...
impl AnnouncementScheduler {
    ...
    pub async fn run(&self, locale: &str) -> NortHingResult<Vec<AnnouncementCard>> {
        ...
        eligible.sort_by_key(|b| std::cmp::Reverse(b.priority));

        debug!("Announcement scheduler: {} cards eligible for display", eligible.len());
    ...
    pub async fn mark_seen(&self, id: &str) -> NortHingResult<()> {
        let mut state = self.state.write().await;
    ...
    pub async fn dismiss(&self, id: &str) -> NortHingResult<()> {
        let mut state = self.state.write().await;
    ...
    pub async fn never_show(&self, id: &str) -> NortHingResult<()> {
        let mut state = self.state.write().await;
