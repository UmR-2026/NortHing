use crate::infrastructure::{
    FileInfo, FileOperationOptions, FileReadResult, FileSearchOutcome, FileSearchProgressSink, FileSearchResult,
    FileTreeNode, FileTreeStatistics, FileWriteResult,
};
use crate::util::elapsed_ms_u64;
use crate::util::errors::*;
use northhing_services_core::filesystem::FileSystemService as BaseFileSystemService;
...
fn map_filesystem_error(error: impl std::fmt::Display) -> NortHingError {
...
async fn read_remote_directory_contents(
...
) -> Option<NortHingResult<Vec<FileTreeNode>>> {
    ...
    let manager = crate::service::remote_ssh::workspace_state::remote_workspace_manager()?;
    let file_service = manager.get_file_service().await?;

    Some(match file_service.read_dir(&entry.connection_id, path).await {
        Ok(entries) => Ok(entries
            .into_iter()
            .filter(|entry| entry.name != "." && entry.name != "..")
            .map(|entry| FileTreeNode::new(entry.path.clone(), entry.name.clone(), entry.path, entry.is_dir))
            .collect()),
...
impl FileSystemService {
    ...
    /// Builds a file tree.
    pub async fn build_file_tree(&self, root_path: &str) -> NortHingResult<Vec<FileTreeNode>> {
    ...
    pub async fn build_file_tree_with_remote_hint(
        ...
        root_path: &str,
        preferred_remote_connection_id: Option<&str>,
    ) -> NortHingResult<Vec<FileTreeNode>> {
    ...
    pub async fn scan_directory(&self, root_path: &str) -> NortHingResult<DirectoryScanResult> {
        let start_time = std::time::Instant::now();

        let (files, statistics) = if crate::service::remote_ssh::workspace_state::is_remote_path(root_path).await {
    ...
    pub async fn get_directory_contents(&self, path: &str) -> NortHingResult<Vec<FileTreeNode>> {
    ...
    pub async fn get_directory_contents_with_remote_hint(
        ...
        if let Some(result) = read_remote_directory_contents(path, preferred_remote_connection_id).await {
            return result;
        }
    ...
    ) -> NortHingResult<Vec<FileSearchResult>> {
        self.inner
    ...
    ) -> NortHingResult<FileSearchOutcome> {
    ...
    pub async fn search_file_names_with_progress(
        ...
        self.inner
            .search_file_names_with_progress(root_path, pattern, options, cancel_flag, progress_sink)
            .await
            .map_err(map_filesystem_error)
    }
    ...
    pub async fn search_file_contents_with_progress(
        ...
        self.inner
            .search_file_contents_with_progress(root_path, pattern, options, cancel_flag, progress_sink)
            .await
            .map_err(map_filesystem_error)
    }

    /// Reads a file.
    pub async fn read_file(&self, file_path: &str) -> NortHingResult<FileReadResult> {
        self.inner.read_file(file_path).await.map_err(map_filesystem_error)
    }

    /// Writes a file.
    pub async fn write_file(&self, file_path: &str, content: &str) -> NortHingResult<FileWriteResult> {
        self.inner
    ...
    ) -> NortHingResult<FileWriteResult> {
        self.inner
    ...
    /// Copies a file.
    pub async fn copy_file(&self, from: &str, to: &str) -> NortHingResult<u64> {
        self.inner.copy_file(from, to).await.map_err(map_filesystem_error)
    }

    /// Moves a file.
    pub async fn move_file(&self, from: &str, to: &str) -> NortHingResult<()> {
        self.inner.move_file(from, to).await.map_err(map_filesystem_error)
    }

    /// Deletes a file.
    pub async fn delete_file(&self, file_path: &str) -> NortHingResult<()> {
        self.inner.delete_file(file_path).await.map_err(map_filesystem_error)
    }

    /// Gets file info.
    pub async fn get_file_info(&self, file_path: &str) -> NortHingResult<FileInfo> {
        self.inner.get_file_info(file_path).await.map_err(map_filesystem_error)
    }

    /// Creates a directory.
    pub async fn create_directory(&self, dir_path: &str) -> NortHingResult<()> {
        self.inner
    ...
    /// Deletes a directory.
    pub async fn delete_directory(&self, dir_path: &str, recursive: bool) -> NortHingResult<()> {
        self.inner
    ...
    /// Gets the file size.
    pub async fn get_file_size(&self, file_path: &str) -> NortHingResult<u64> {
        self.inner.get_file_size(file_path).await.map_err(map_filesystem_error)
    }

    /// Reads a text file quickly.
    pub async fn read_text_file(&self, file_path: &str) -> NortHingResult<String> {
        self.inner.read_text_file(file_path).await.map_err(map_filesystem_error)
    }

    /// Writes a text file quickly.
    pub async fn write_text_file(&self, file_path: &str, content: &str) -> NortHingResult<()> {
        self.inner
    ...
    pub async fn list_all_files(&self, root_path: &str) -> NortHingResult<Vec<String>> {
    ...
    pub async fn calculate_directory_size(&self, dir_path: &str) -> NortHingResult<u64> {
        let scan_result = self.scan_directory(dir_path).await?;
        Ok(scan_result.statistics.total_size_bytes)
    }

    /// Finds files by extension.
    pub async fn find_files_by_extension(&self, root_path: &str, extension: &str) -> NortHingResult<Vec<String>> {
    ...
    pub async fn get_directory_stats(&self, dir_path: &str) -> NortHingResult<DirectoryStats> {
        let scan_result = self.scan_directory(dir_path).await?;
    ...
    pub async fn editor_sync_content_sha256_hex(&self, file_path: &str) -> NortHingResult<String> {
        self.inner
