...
use crate::function_agents::port_adapters::{CoreFunctionAgentAiAdapter, CoreFunctionAgentGitAdapter};
use crate::function_agents::{CommitMessage, CommitMessageOptions, WorkStateAnalysis, WorkStateOptions};
...
impl CoreProductDomainRuntime {
    ...
    pub(crate) fn function_agent_ai_adapter(factory: Arc<AIClientFactory>) -> CoreFunctionAgentAiAdapter {
    ...
    pub(crate) async fn generate_function_agent_commit_message(
        ...
        facade.generate_commit_message(repo_path.to_path_buf(), options).await
    }
