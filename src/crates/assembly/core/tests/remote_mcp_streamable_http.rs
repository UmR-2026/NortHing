...
use axum::Router;
use futures::Stream;
use northhing_core::service::mcp::server::MCPConnection;
    ...
    match method {
        "initialize" => {
            ...
            if capabilities.get("elicitation").is_some() {
                state.saw_elicitation_capability.store(true, Ordering::SeqCst);
            }
            ...
            response_headers.insert("Mcp-Session-Id", "test-session".parse().expect("valid header value"));
