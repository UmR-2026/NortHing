...
use agent_client_protocol::{Agent, ByteStreams, Client, ConnectTo, ConnectionTo, Dispatch, Error, Result};
...
pub trait AcpRuntime: Send + Sync + 'static {
    ...
    async fn set_session_mode(&self, _request: SetSessionModeRequest) -> Result<SetSessionModeResponse> {
    ...
    async fn set_session_model(&self, _request: SetSessionModelRequest) -> Result<SetSessionModelResponse> {
        Err(Error::method_not_found().data("session/set_model is not implemented"))
    }
}

/// Typed ACP server backed by an injected northhing runtime.
    ...
    pub async fn serve(self, transport: impl ConnectTo<Agent> + 'static) -> Result<()> {
        ...
        Agent
            ...
            .on_receive_request(
                {
                    ...
                    async move |request: AuthenticateRequest, responder, cx: ConnectionTo<Client>| {
                        let runtime = runtime.clone();
                        cx.spawn(async move { responder.respond_with_result(runtime.authenticate(request).await) })?;
                        Ok(())
                    }
                },
                agent_client_protocol::on_receive_request!(),
            ...
            .on_receive_request(
                {
                    ...
                    async move |request: NewSessionRequest, responder, cx: ConnectionTo<Client>| {
                        ...
                        cx.spawn(async move {
                            responder.respond_with_result(runtime.new_session(request, session_cx).await)
                        })?;
                        Ok(())
                    }
                },
            ...
            .on_receive_request(
                {
                    ...
                    async move |request: LoadSessionRequest, responder, cx: ConnectionTo<Client>| {
                        ...
                        let session_cx = cx.clone();
                        cx.spawn(async move {
                            responder.respond_with_result(runtime.load_session(request, session_cx).await)
                        })?;
                        Ok(())
                    }
                },
            ...
            .on_receive_request(
                {
                    ...
                    async move |request: ListSessionsRequest, responder, cx: ConnectionTo<Client>| {
                        let runtime = runtime.clone();
                        cx.spawn(async move { responder.respond_with_result(runtime.list_sessions(request).await) })?;
                        Ok(())
                    }
                },
                agent_client_protocol::on_receive_request!(),
            ...
            .on_receive_request(
                {
                    ...
                    async move |request: PromptRequest, responder, cx: ConnectionTo<Client>| {
                        let runtime = runtime.clone();
                        cx.spawn(async move { responder.respond_with_result(runtime.prompt(request).await) })?;
                        Ok(())
                    }
                },
                agent_client_protocol::on_receive_request!(),
                    ...
                    async move |notification: CancelNotification, cx: ConnectionTo<Client>| {
                        ...
                        cx.spawn(async move {
                            if let Err(error) = runtime.cancel(notification).await {
                                tracing::error!("Error handling ACP cancel notification: {:?}", error);
                            }
                            Ok(())
                        })?;
                        Ok(())
            ...
            .on_receive_request(
                {
                    ...
                    async move |request: SetSessionModeRequest, responder, cx: ConnectionTo<Client>| {
                        let runtime = runtime.clone();
                        cx.spawn(
                            async move { responder.respond_with_result(runtime.set_session_mode(request).await) },
                        )?;
                        Ok(())
                    }
                },
            ...
            .on_receive_request(
                {
                    let runtime = runtime.clone();
                    async move |request: SetSessionConfigOptionRequest, responder, cx: ConnectionTo<Client>| {
                        let runtime = runtime.clone();
                        cx.spawn(async move {
                            responder.respond_with_result(runtime.set_session_config_option(request).await)
                        })?;
                        Ok(())
                    }
                },
            ...
            .on_receive_request(
                {
                    let runtime = runtime.clone();
                    async move |request: SetSessionModelRequest, responder, cx: ConnectionTo<Client>| {
                        let runtime = runtime.clone();
                        cx.spawn(
                            async move { responder.respond_with_result(runtime.set_session_model(request).await) },
                        )?;
                        Ok(())
                    }
                },
