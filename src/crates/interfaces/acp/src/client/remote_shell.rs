...
const REMOTE_ENV_BOOTSTRAP: &str = r#"__northhing_home="${HOME:-}"
if [ -n "$__northhing_home" ]; then
  if [ -s "$__northhing_home/.nvm/nvm.sh" ]; then
    . "$__northhing_home/.nvm/nvm.sh" >/dev/null 2>&1
  fi
  for __northhing_dir in "$__northhing_home/.local/bin" "$__northhing_home/.cargo/bin" "$__northhing_home/.npm-global/bin"; do
    if [ -d "$__northhing_dir" ]; then
      PATH="$__northhing_dir:$PATH"
    fi
  done
fi
export PATH"#;
...
pub(super) fn remote_user_shell_command(body: &str) -> String {
    format!("bash -lc {}", shell_escape(&format!("{REMOTE_ENV_BOOTSTRAP}\n{body}")))
}

pub(super) fn render_remote_env_assignments(env: &HashMap<String, String>) -> Vec<String> {
    let mut entries = env.iter().filter(|(key, _)| is_shell_env_key(key)).collect::<Vec<_>>();
