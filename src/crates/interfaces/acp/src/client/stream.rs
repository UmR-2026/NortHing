...
use agent_client_protocol::schema::{
    ContentBlock, ContentChunk, SessionConfigOption, SessionNotification, SessionUpdate, ToolCall, ToolCallContent,
    ...
    pub(super) fn apply(&mut self, event: AcpClientStreamEvent) -> Vec<AcpClientStreamEvent> {
        match event {
            AcpClientStreamEvent::AgentText(_) | AcpClientStreamEvent::AgentThought(_) => {
                ...
                if self.last_item_kind.is_none() || self.last_item_kind == Some(AcpStreamItemKind::Tool) {
...
) -> NortHingResult<Vec<AcpClientStreamEvent>> {
    let mut events = Vec::new();
        ...
        .if_notification(async |notification: SessionNotification| {
            match notification.update {
                ...
                SessionUpdate::UsageUpdate(usage_update) => {
                    events.push(AcpClientStreamEvent::ContextUsageUpdated(AcpSessionContextUsage::from(
                        usage_update,
                    )));
                }
                ...
                SessionUpdate::ConfigOptionUpdate(update) => {
                    events.push(AcpClientStreamEvent::ConfigOptionsUpdated(update.config_options));
                }
                _ => {}
            }
            Ok(())
...
impl AcpToolCallTracker {
    ...
    fn update_from_fields(
        ...
        let raw_input = raw_input.or_else(|| previous.as_ref().and_then(|snapshot| snapshot.raw_input.clone()));
...
fn acp_tool_call_events(tool_call: ToolCall, tracker: &mut AcpToolCallTracker) -> Vec<AcpClientStreamEvent> {
    ...
    match tool_call.status {
        ToolCallStatus::Completed => {
            events.push(AcpClientStreamEvent::ToolEvent(ToolEventData::Completed {
                tool_id,
                tool_name,
                result: acp_tool_result_value(tool_call.raw_output, Some(tool_call.content), Some(tool_call.locations)),
...
fn acp_tool_call_update_events(update: ToolCallUpdate, tracker: &mut AcpToolCallTracker) -> Vec<AcpClientStreamEvent> {
    ...
    match update.fields.status {
        Some(ToolCallStatus::Completed) => {
            ...
            events.push(AcpClientStreamEvent::ToolEvent(ToolEventData::Completed {
                tool_id,
                tool_name,
                result: acp_tool_result_value(update.fields.raw_output, update.fields.content, update.fields.locations),
                result_for_assistant: None,
                duration_ms: 0,
                queue_wait_ms: None,
                preflight_ms: None,
        ...
        Some(ToolCallStatus::Failed) => {
            ...
            events.push(AcpClientStreamEvent::ToolEvent(ToolEventData::Failed {
                tool_id,
                tool_name,
                error: acp_tool_error_text(update.fields.raw_output, update.fields.content.unwrap_or_default()),
                duration_ms: None,
                queue_wait_ms: None,
                preflight_ms: None,
                confirmation_wait_ms: None,
...
fn acp_tool_error_text(raw_output: Option<serde_json::Value>, content: Vec<ToolCallContent>) -> String {
    if let Some(raw_output) = raw_output {
        return value_to_display_text(&raw_output);
    }
    if !content.is_empty() {
        return serde_json::to_string_pretty(&content)
            .unwrap_or_else(|_| serde_json::to_string(&content).unwrap_or_else(|_| "ACP tool failed".to_string()));
    }
...
fn protocol_error(error: impl std::fmt::Display) -> NortHingError {
...
mod tests {
    ...
    fn exposes_context_usage_updates() {
        ...
        let notification = SessionNotification::new(
            "session-1",
            SessionUpdate::UsageUpdate(agent_client_protocol::schema::UsageUpdate::new(1_000, 4_000)),
        )
        .to_untyped_message()
        .expect("notification");
        ...
        let events = tokio::runtime::Runtime::new()
            .expect("runtime")
            .block_on(acp_dispatch_to_stream_events_with_tracker(dispatch, &mut tracker))
            .expect("dispatch");
    ...
    fn exposes_config_option_updates() {
        ...
        let notification = SessionNotification::new(
            "session-1",
            SessionUpdate::ConfigOptionUpdate(ConfigOptionUpdate::new(vec![SessionConfigOption::select(
                "model",
                "Model",
                "fast",
    ...
    fn starts_new_round_for_text_after_tool() {
        ...
        assert_eq!(event_kinds(&events), vec!["round", "text", "tool", "round", "text"]);
        assert!(matches!(
            events[0],
    ...
    fn keeps_consecutive_tools_in_one_round_before_text() {
        ...
        assert_eq!(event_kinds(&events), vec!["round", "tool", "tool", "round", "text"]);
    }
    ...
    fn tool_call_tracker_replays_cached_input_before_completed_update() {
        ...
        match &second[0] {
            AcpClientStreamEvent::ToolEvent(ToolEventData::Started { tool_name, params, .. }) => {
