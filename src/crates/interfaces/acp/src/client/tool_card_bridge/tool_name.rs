...
pub(super) fn acp_tool_name(title: &str, raw_input: Option<&serde_json::Value>, kind: Option<&ToolKind>) -> String {
...
fn tool_name_from_raw_input(raw_input: &serde_json::Value) -> Option<String> {
    ...
    for key in ["tool", "toolName", "tool_name", "name", "function", "action"] {
...
fn normalize_tool_name(
    ...
    if let Some(input) = input {
        ...
        if has_any_key(
            input,
            &["glob", "glob_pattern", "globPattern", "file_pattern", "filePattern"],
        ) {
            return "Glob".to_string();
        }
        if has_any_key(input, &["pattern", "search_pattern", "searchPattern", "query"]) {
            if contains_any(&haystack, &["web search", "search web"]) {
                return "WebSearch".to_string();
            }
            return "Grep".to_string();
        }
        if has_any_key(input, &["directory", "dir", "target_directory", "targetDirectory"]) {
            return "LS".to_string();
        }

        let has_file_path = has_any_key(
            input,
            &["file_path", "filePath", "target_file", "targetFile", "filename", "path"],
        );
        if has_file_path {
            if has_any_key(input, &["content", "contents"]) {
                return "Write".to_string();
            }
            if has_any_key(input, &["old_string", "oldString", "new_string", "newString"]) {
    ...
    if contains_any(
        &haystack,
        &["bash", "shell", "terminal", "command", "execute", "exec", "run command"],
    ) {
        return "Bash".to_string();
    }
    ...
    if contains_any(&haystack, &["glob", "find file", "file search", "search files"]) {
        return "Glob".to_string();
    }
...
fn is_native_tool_name(name: &str) -> bool {
    matches!(
        name,
        "Read" | "Write" | "Edit" | "Delete" | "LS" | "Grep" | "Glob" | "Bash" | "TodoWrite" | "WebSearch"
    )
}
