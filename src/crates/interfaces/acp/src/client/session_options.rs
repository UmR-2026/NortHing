use agent_client_protocol::schema::{
    Cost, ModelInfo, SessionConfigKind, SessionConfigOption, SessionConfigOptionCategory, SessionConfigSelectOptions,
    SessionModelState,
};
...
fn select_model_values(option: &SessionConfigOption) -> (Option<String>, Vec<AcpSessionModelOption>) {
...
mod tests {
    ...
    fn includes_context_usage() {
        ...
        assert_eq!(
            context_usage.cost.as_ref().map(|cost| cost.currency.as_str()),
            Some("USD")
        );
    }
    ...
    fn converts_available_command_with_and_without_input() {
        use agent_client_protocol::schema::{AvailableCommand, AvailableCommandInput, UnstructuredCommandInput};
        ...
        let converted = AcpAvailableCommand::from(AvailableCommand::new("compact", "Compact the context"));
