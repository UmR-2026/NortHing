...
use northhing_core::service::session::SessionMetadata;
...
impl AcpSessionPersistence {
    pub(super) fn new(path_manager: Arc<PathManager>) -> NortHingResult<Self> {
        Ok(Self {
    ...
    pub(super) async fn delete_flow_session_record(
        ...
        self.manager
            .delete_session(session_storage_path, northhing_session_id)
            .await
    }
    ...
    pub(super) async fn load_remote_session_id(
        ...
        let Some(metadata) = self
            .manager
            .load_session_metadata(session_storage_path, northhing_session_id)
            .await?
        else {
            return Ok(None);
        };
    ...
    pub(super) async fn update_remote_session_state(
        ...
        self.update_metadata(session_storage_path, northhing_session_id, |metadata| {
            ...
            custom[CUSTOM_METADATA_LAST_RESUME_ERROR_KEY] = last_resume_error.map(Value::String).unwrap_or(Value::Null);
    ...
    async fn update_metadata(
        ...
        session_storage_path: &Path,
        northhing_session_id: &str,
        update: impl FnOnce(&mut SessionMetadata) -> NortHingResult<()>,
    ) -> NortHingResult<()> {
...
fn ensure_object(value: &mut Value) -> NortHingResult<()> {
    if value.is_object() {
        return Ok(());
    }
