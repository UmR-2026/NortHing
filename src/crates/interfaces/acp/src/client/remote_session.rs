...
pub(super) fn preferred_resume_strategies(
    ...
    let has_remote_session_id = remote_session_id.map(str::trim).is_some_and(|value| !value.is_empty());
    ...
    if has_remote_session_id {
        ...
        // notifications are only scoped to the remote session, not a northhing turn.
        if capabilities
...
mod tests {
    ...
    fn prefers_load_when_resume_is_not_supported() {
        assert_eq!(
            preferred_resume_strategies(Some(&AgentCapabilities::new().load_session(true)), Some("s1")),
            vec![AcpRemoteSessionStrategy::Load, AcpRemoteSessionStrategy::New]
        );
    }
