...
impl RemoteAcpCapabilityStore {
    pub(crate) fn new(path: PathBuf) -> Self {
        let snapshots = match std::fs::read_to_string(&path) {
            Ok(content) => match serde_json::from_str::<Vec<RemoteAcpClientRequirementSnapshot>>(&content) {
                Ok(entries) => entries
                    .into_iter()
    ...
    pub(crate) async fn get(&self, connection_id: &str) -> Option<RemoteAcpClientRequirementSnapshot> {
        self.snapshots.read().await.get(connection_id).cloned()
    }

    pub(crate) async fn set(&self, snapshot: RemoteAcpClientRequirementSnapshot) -> NortHingResult<()> {
        let entries = {
    ...
    pub(crate) async fn clear(&self) -> NortHingResult<()> {
        {
            let mut guard = self.snapshots.write().await;
            guard.clear();
        }
    ...
    async fn persist(&self, snapshots: Vec<RemoteAcpClientRequirementSnapshot>) -> NortHingResult<()> {
        ...
        let content = serde_json::to_string_pretty(&snapshots).map_err(|error| {
            NortHingError::serialization(format!(
                "Failed to serialize remote ACP capability snapshots: {}",
                error
            ))
        })?;
        tokio::fs::write(&self.path, content).await.map_err(|error| {
            NortHingError::io(format!("Failed to write remote ACP capability snapshots: {}", error))
        })?;
        Ok(())
    }
}
