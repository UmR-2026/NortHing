...
pub(crate) struct BuiltinAcpClientPreset {
    ...
    /// npm package northhing can install on the user's behalf. `None` means the
    /// agent is user-managed (northhing only provides the integration, the user
...
const BUILTIN_ACP_CLIENT_PRESETS: &[BuiltinAcpClientPreset] = &[
    ...
    // or `curl -fsSL https://omp.sh/install | sh`), which northhing's npm-based
    // installer cannot provide — so install_package is None and northhing only
...
pub(crate) fn builtin_acp_client_preset(client_id: &str) -> Option<&'static BuiltinAcpClientPreset> {
    BUILTIN_ACP_CLIENT_PRESETS.iter().find(|preset| preset.id == client_id)
}
...
mod tests {
    ...
    fn omp_is_a_native_acp_preset() {
        ...
        // User-managed: northhing provides no installer for omp.
