...
use northhing_core::agentic::tools::framework::{
...
impl AcpAgentTool {
    ...
    fn display_name(&self) -> String {
        self.config.name.clone().unwrap_or_else(|| self.client_id.clone())
    }
}
...
impl Tool for AcpAgentTool {
    ...
    fn short_description(&self) -> String {
        format!("Delegate a task to the external ACP agent '{}'.", self.display_name())
    }
    ...
    fn input_schema(&self) -> Value {
        json!({
            ...
            "properties": {
                ...
                "workspace_path": {
                    "type": "string",
                    "description": "Optional absolute workspace path. Defaults to the current northhing workspace."
                },
                "timeout_seconds": {
                    "type": "integer",
                    "minimum": 0,
    ...
    fn render_tool_use_message(&self, input: &Value, _options: &ToolRenderOptions) -> String {
        ...
        format!("Sending ACP prompt to '{}': {}", self.display_name(), prompt_preview)
    }
    ...
    async fn call_impl(&self, input: &Value, context: &ToolUseContext) -> NortHingResult<Vec<ToolResult>> {
        let northhing_session_id = context
            .session_id
            .clone()
            .ok_or_else(|| NortHingError::tool("ACP tool requires an active northhing session".to_string()))?;
        let prompt = input
            .get("prompt")
            .and_then(|value| value.as_str())
            .filter(|value| !value.trim().is_empty())
            .ok_or_else(|| NortHingError::tool("prompt is required".to_string()))?
            .to_string();
        ...
        let timeout_seconds = input.get("timeout_seconds").and_then(|value| value.as_u64());
