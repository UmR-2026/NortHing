...
use agent_client_protocol::schema::{
    CurrentModeUpdate, ListSessionsRequest, ListSessionsResponse, LoadSessionRequest, LoadSessionResponse,
    NewSessionRequest, NewSessionResponse, SessionId, SessionInfo, SessionMode, SessionModeState, SessionUpdate,
    SetSessionModeRequest, SetSessionModeResponse,
};
use agent_client_protocol::{Client, ConnectionTo, Error, Result};
use chrono::{DateTime, Utc};
use northhing_core::agentic::agents::agent_registry;
use northhing_core::agentic::core::SessionConfig;

use super::events::send_update;
use super::model::{build_session_config_options, build_session_model_state, normalize_session_model_id};
use super::{AcpSessionState, NortHingAcpRuntime};
...
impl NortHingAcpRuntime {
    pub(super) async fn create_session(
        ...
        let session = self
            ...
            .coordinator
            .create_session(
                format!("ACP Session - {}", chrono::Local::now().format("%Y-%m-%d %H:%M:%S")),
                "agentic".to_string(),
                SessionConfig {
        ...
        let acp_session = AcpSessionState {
            ...
            northhing_session_id: session.session_id.clone(),
            cwd,
            mode_id: session.agent_type.clone(),
            model_id: normalize_session_model_id(session.config.model_id.as_deref()),
            mcp_server_ids: self.provision_mcp_servers(&session.session_id, mcp_servers).await?,
        };
        self.sessions
            .insert(acp_session.acp_session_id.clone(), acp_session.clone());
        self.connections.insert(acp_session.acp_session_id.clone(), connection);
        ...
        let config_options =
            build_session_config_options(Some(&acp_session.model_id), Some(&acp_session.mode_id)).await?;
        Ok(NewSessionResponse::new(SessionId::new(acp_session.acp_session_id))
            .modes(modes)
            .models(models)
    ...
    pub(super) async fn list_sessions_for_cwd(&self, request: ListSessionsRequest) -> Result<ListSessionsResponse> {
        let cwd = request
            .cwd
            .or_else(|| std::env::current_dir().ok())
            .ok_or_else(|| Error::invalid_params().data("cwd is required"))?;
        let cursor = request.cursor.as_deref().and_then(|value| value.parse::<u128>().ok());
    ...
    pub(super) async fn update_session_mode(&self, request: SetSessionModeRequest) -> Result<SetSessionModeResponse> {
    ...
    pub(super) async fn update_session_mode_inner(&self, session_id: &str, mode_id: &str) -> Result<()> {
        let acp_session = self
            .sessions
            .get(session_id)
            .ok_or_else(|| Error::resource_not_found(Some(session_id.to_string())))?;
        let northhing_session_id = acp_session.northhing_session_id.clone();
        drop(acp_session);
        ...
        self.agentic_system
            .coordinator
            .update_session_agent_type(&northhing_session_id, mode_id)
            .await
            .map_err(Self::internal_error)?;
...
async fn validate_mode_id(mode_id: &str) -> Result<()> {
    let mode_exists = agent_registry()
        .get_modes_info()
        .await
        .into_iter()
