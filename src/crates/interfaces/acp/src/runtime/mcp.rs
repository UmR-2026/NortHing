...
use northhing_core::service::mcp::{
    global_mcp_service, set_global_mcp_service, ConfigLocation, MCPServerConfig, MCPServerManager, MCPServerTransport,
...
async fn mcp_server_manager() -> Result<Arc<MCPServerManager>> {
    if let Some(service) = global_mcp_service() {
    ...
    let service = Arc::new(MCPService::new(config_service).map_err(NortHingAcpRuntime::internal_error)?);
...
fn stdio_server_config(acp_session_id: &str, server: McpServerStdio) -> Result<MCPServerConfig> {
    ...
    Ok(MCPServerConfig {
        ...
        args: server.args,
        env: server.env.into_iter().map(|env| (env.name, env.value)).collect(),
        headers: HashMap::new(),
        url: None,
        auto_start: true,
        enabled: true,
