...
use agent_client_protocol::schema::{
    CancelNotification, ContentChunk, PromptRequest, PromptResponse, RequestPermissionOutcome, SessionUpdate,
    StopReason,
};
...
use northhing_core::agentic::events::EventEnvelope;
use northhing_events::AgenticEvent as CoreEvent;
use serde_json::json;
...
use super::events::{
    permission_request, send_update, tool_event_updates, PERMISSION_ALLOW_ONCE, PERMISSION_REJECT_ONCE,
};
use super::thinking::{InlineThinkRouter, InlineThinkSegment};
use super::NortHingAcpRuntime;
...
impl NortHingAcpRuntime {
    pub(super) async fn run_prompt(&self, request: PromptRequest) -> Result<PromptResponse> {
        ...
        if parsed_prompt.image_contexts.is_empty() {
            self.agentic_system
                .coordinator
                .start_dialog_turn(
                    acp_session.northhing_session_id.clone(),
        ...
        let stop_reason = wait_for_prompt_completion(
            ...
            &acp_session.northhing_session_id,
        )
        .await?;
    ...
    pub(super) async fn cancel_prompt(&self, notification: CancelNotification) -> Result<()> {
        ...
        self.agentic_system
            .coordinator
            .cancel_active_turn_for_session(&acp_session.northhing_session_id, std::time::Duration::from_secs(5))
            .await
            .map_err(Self::internal_error)?;
...
async fn wait_for_prompt_completion(
    ...
    loop {
        ...
        if event.session_id() != Some(northhing_session_id) {
            continue;
        }

        match event {
            CoreEvent::TextChunk { text, .. } => {
                send_inline_think_segments(connection, acp_session_id, inline_think.route_text(text))?;
            }
            ...
            CoreEvent::ToolEvent { tool_event, .. } => {
                ...
                if let northhing_events::ToolEventData::ConfirmationNeeded {
                    tool_id,
                    tool_name,
                    params,
                } = tool_event
                {
                    handle_permission_request(runtime, connection, acp_session_id, &tool_id, &tool_name, &params)
                        .await?;
                }
            }
            ...
            CoreEvent::DialogTurnFailed { error, .. } | CoreEvent::SystemError { error, .. } => {
                ...
                send_update(
                    connection,
                    acp_session_id,
                    SessionUpdate::AgentMessageChunk(ContentChunk::new(format!("Error: {}", error).into())),
                )?;
...
fn send_inline_think_segments(
    ...
    for segment in segments {
        let update = match segment {
            InlineThinkSegment::Text(text) => SessionUpdate::AgentMessageChunk(ContentChunk::new(text.into())),
...
async fn handle_permission_request(
    ...
    match response.outcome {
        RequestPermissionOutcome::Selected(selected) if selected.option_id.to_string() == PERMISSION_ALLOW_ONCE => {
            runtime
                .agentic_system
                .coordinator
                .confirm_tool(tool_id, None)
                .await
                .map_err(NortHingAcpRuntime::internal_error)?;
        }
        RequestPermissionOutcome::Selected(selected) if selected.option_id.to_string() == PERMISSION_REJECT_ONCE => {
            runtime
                .agentic_system
                .coordinator
        ...
        RequestPermissionOutcome::Selected(selected) => {
            let reason = format!("Unknown ACP permission option selected: {}", selected.option_id);
            runtime
                .agentic_system
                .coordinator
