use agent_client_protocol::schema::{
    ModelInfo, SessionConfigOption, SessionConfigOptionCategory, SessionConfigSelectOption, SessionModelState,
...
use northhing_core::service::config::{GlobalConfig, GlobalConfigManager};
...
impl NortHingAcpRuntime {
    ...
    pub(super) async fn update_session_config_option(
        ...
        match config_id.as_str() {
            ...
            _ => {
                return Err(Error::invalid_params().data(format!("unknown session config option: {}", config_id)));
            }
        }
    ...
    async fn set_session_model_id(&self, session_id: &str, model_id: &str) -> Result<()> {
        ...
        self.agentic_system
            .coordinator
            .update_session_model(&northhing_session_id, &normalized_model_id)
            .await
...
pub(super) async fn build_session_model_state(preferred_model_id: Option<&str>) -> Result<SessionModelState> {
...
pub(super) async fn build_session_config_options(
    ...
    let mode_infos = agent_registry().get_modes_info().await.into_iter().collect::<Vec<_>>();
    ...
    let mode_options = mode_infos
        .into_iter()
        .map(|mode| SessionConfigSelectOption::new(mode.id, mode.name).description(mode.description))
        .collect::<Vec<_>>();
...
async fn normalize_model_selection(model_id: &str) -> Result<String> {
    ...
    ai_config
        ...
        .ok_or_else(|| Error::invalid_params().data(format!("unknown or disabled session model: {}", model_id)))
}
...
fn available_model_select_options(ai_config: &AIConfig) -> Vec<SessionConfigSelectOption> {
    ...
    options.push(SessionConfigSelectOption::new(AUTO_MODEL_ID, "Auto").description("Use the mode default model"));
    options.extend(ai_config.models.iter().filter(|model| model.enabled).map(|model| {
...
fn model_display_name(model: &northhing_core::service::config::types::AIModelConfig) -> String {
...
async fn load_ai_config() -> Result<AIConfig> {
    let config_service = GlobalConfigManager::service()
        .await
...
mod tests {
    ...
    use northhing_core::service::config::types::{AIConfig, AIModelConfig};
    ...
    fn current_model_resolves_name_to_model_id() {
        ...
        assert_eq!(current_model_id(&ai_config, Some("Readable Model")), "model-a");
    }
}
