...
use agent_client_protocol::schema::{
    PermissionOption, PermissionOptionKind, RequestPermissionRequest, SessionId, SessionNotification, SessionUpdate,
    ToolCall, ToolCallContent, ToolCallLocation, ToolCallStatus, ToolCallUpdate, ToolCallUpdateFields, ToolKind,
};
...
pub(super) fn send_update(connection: &ConnectionTo<Client>, session_id: &str, update: SessionUpdate) -> Result<()> {
    connection.send_notification(SessionNotification::new(SessionId::new(session_id.to_string()), update))
}
    ...
    RequestPermissionRequest::new(
        ...
        ToolCallUpdate::new(
            ...
            ToolCallUpdateFields::new()
                ...
                .content(vec![text_content(format!("Permission required to run {}.", tool_name))]),
        ),
        vec![
            PermissionOption::new(PERMISSION_ALLOW_ONCE, "Allow once", PermissionOptionKind::AllowOnce),
            PermissionOption::new(PERMISSION_REJECT_ONCE, "Reject once", PermissionOptionKind::RejectOnce),
        ],
    )
}
...
fn tool_call_update(tool_event: &ToolEventData) -> Option<ToolCallUpdate> {
    ...
    let fields = match tool_event {
        ToolEventData::EarlyDetected { tool_name, .. } => ToolCallUpdateFields::new()
            ...
            .kind(tool_kind(tool_name))
            .status(ToolCallStatus::Pending),
        ToolEventData::ParamsPartial { tool_name, params, .. } => {
        ...
        ToolEventData::Queued { position, .. } => ToolCallUpdateFields::new()
            .status(ToolCallStatus::Pending)
            .content(vec![text_content(format!("Queued at position {}.", position))]),
        ...
        ToolEventData::Started { tool_name, params, .. } => ToolCallUpdateFields::new()
            .title(tool_title(tool_name))
            .kind(tool_kind(tool_name))
        ...
        ToolEventData::Progress {
            ...
            .status(ToolCallStatus::InProgress)
            .content(vec![text_content(format!("{} ({:.0}%)", message, percentage))]),
        ToolEventData::Streaming { chunks_received, .. } => ToolCallUpdateFields::new()
            .status(ToolCallStatus::InProgress)
            .content(vec![text_content(format!(
        ...
        ToolEventData::ConfirmationNeeded { tool_name, params, .. } => ToolCallUpdateFields::new()
            .title(format!("Allow {}?", tool_name))
            .status(ToolCallStatus::Pending)
            .locations(tool_locations(params))
            .raw_input(sanitize_tool_input(tool_name, params.clone()))
...
fn tool_kind(tool_name: &str) -> ToolKind {
    ...
    if name.contains("delete") || name.contains("remove") {
        ToolKind::Delete
    } else if name.contains("write") || name.contains("edit") || name.contains("patch") || name.contains("replace") {
        ToolKind::Edit
    } else if name.contains("move") || name.contains("rename") {
        ToolKind::Move
    } else if name.contains("grep") || name.contains("glob") || name.contains("search") || name.contains("find") {
        ToolKind::Search
    } else if name.contains("bash") || name.contains("terminal") || name.contains("command") || name.contains("execute")
    {
        ToolKind::Execute
...
fn is_large_text_payload_tool(tool_name: &str) -> bool {
    matches!(
        tool_name.to_ascii_lowercase().as_str(),
        "write" | "file_write" | "write_file" | "write_notebook" | "edit" | "file_edit" | "search_replace"
    )
}
...
mod tests {
    ...
    fn completed_event_maps_to_completed_update_with_output() {
        ...
        assert_eq!(update.fields.raw_output, Some(serde_json::json!({ "stdout": "ok" })));
    }
    ...
    fn write_started_redacts_large_content_from_raw_input() {
        ...
        let raw_input = update.fields.raw_input.as_ref().expect("raw input should be present");

        assert_eq!(raw_input["file_path"], "src/lib.rs");
        assert_eq!(raw_input["content_bytes"], ACP_LARGE_TEXT_PREVIEW_CHARS + 10);
        assert_eq!(
    ...
    fn write_params_partial_sends_bounded_raw_input() {
        ...
        assert_eq!(raw_input["content_bytes"], ACP_LARGE_TEXT_PREVIEW_CHARS + 25);
        assert_eq!(
            raw_input["content"].as_str().unwrap().len(),
            ACP_LARGE_TEXT_PREVIEW_CHARS
        );
    ...
    fn edit_started_redacts_large_strings_from_raw_input() {
        ...
        assert_eq!(raw_input["file_path"], "src/lib.rs");
        assert_eq!(raw_input["old_string_bytes"], ACP_LARGE_TEXT_PREVIEW_CHARS * 3);
        assert_eq!(raw_input["new_string_bytes"], ACP_LARGE_TEXT_PREVIEW_CHARS * 3);
        assert_eq!(
    ...
    fn edit_completed_redacts_large_strings_from_raw_output() {
        ...
        let raw_output = update.fields.raw_output.as_ref().expect("raw output should be present");

        assert_eq!(raw_output["file_path"], "src/lib.rs");
        assert_eq!(raw_output["old_string_bytes"], ACP_LARGE_TEXT_PREVIEW_CHARS * 3);
        assert_eq!(raw_output["new_string_bytes"], ACP_LARGE_TEXT_PREVIEW_CHARS * 3);
        assert_eq!(
    ...
    fn permission_request_exposes_allow_and_reject_once() {
        ...
        assert_eq!(request.options[0].option_id.to_string(), PERMISSION_ALLOW_ONCE);
        assert_eq!(request.options[0].kind, PermissionOptionKind::AllowOnce);
        assert_eq!(request.options[1].option_id.to_string(), PERMISSION_REJECT_ONCE);
