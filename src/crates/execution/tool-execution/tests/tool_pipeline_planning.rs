use tool_runtime::pipeline::{
    count_tool_states, partition_tool_batches, retry_delay_ms, should_cancel_tool_state, should_retry_tool_attempt,
    summarize_dialog_turn_cancellation, ToolExecutionErrorClass, ToolRetryAttemptFacts, ToolTaskStateKind,
};
...
fn cancellation_policy_preserves_cancellable_and_terminal_state_contract() {
    ...
    assert!(should_cancel_tool_state(ToolTaskStateKind::AwaitingConfirmation));
