...
use tool_runtime::fs::{
    ...
    inspect_local_delete_target, parse_remote_list_entries, write_local_file, DeleteLocalPathRequest,
    EditLocalFileRequest, LocalDeleteTarget, WriteLocalFileMode, WriteLocalFileRequest, WriteLocalFileStatus,
};
use tool_runtime::search::glob_search::{collect_remote_glob_matches, execute_local_glob, LocalGlobRequest};
use tool_runtime::search::grep_search::{
    ...
    render_remote_grep_result_text, OutputMode, RemoteGrepCommandRequest,
};
...
fn make_temp_dir(name: &str) -> PathBuf {
    let unique = SystemTime::now()
        ...
        .as_nanos();
    let dir = std::env::temp_dir().join(format!("northhing-tool-io-{name}-{unique}"));
...
fn write_local_file_reports_created_overwritten_and_identical_retry() {
    ...
    assert_eq!(identical.status, WriteLocalFileStatus::AlreadyExistsSameContent);
    ...
    assert_eq!(fs::read_to_string(&target).expect("file should exist"), "replacement");
...
fn write_local_file_append_mode_appends_and_creates_when_missing() {
    ...
    fs::create_dir_all(existing_target.parent().expect("parent should exist")).expect("parent should be created");
    ...
    assert_eq!(created.status, WriteLocalFileStatus::Created);
    assert_eq!(fs::read_to_string(&new_target).expect("file should exist"), "first");
...
fn edit_local_file_writes_apply_edit_result() {
    ...
    assert_eq!(fs::read_to_string(&target).expect("file should exist"), "alpha\nBETA\n");
...
fn execute_local_glob_keeps_shallowest_matches() {
    ...
    let matches = result.matches.iter().map(|path| normalized(path)).collect::<Vec<_>>();
    assert_eq!(matches.len(), 2);
    assert!(matches.iter().any(|path| path.ends_with("/src/lib.rs")));
    assert!(matches.iter().any(|path| path.ends_with("/tests/mod.rs")));
    assert!(!matches.iter().any(|path| path.ends_with("/src/deep/mod.rs")));
...
fn remote_glob_stdout_is_normalized_and_limited_by_tool_runtime() {
    let matches = collect_remote_glob_matches("C:/repo", "./src/deep/mod.rs\nsrc/lib.rs\nREADME.md\n\n", 2)
        .into_iter()
...
fn bash_shell_owner_preserves_command_wrapping_and_env() {
    ...
    assert_eq!(env.get("northhing_NONINTERACTIVE").map(String::as_str), Some("1"));
...
fn bash_shell_owner_preserves_guard_and_result_rendering() {
    assert_eq!(
        detect_osascript_keystroke_non_ascii(r#"osascript -e 'tell app "System Events" to keystroke "你好"'"#),
        Some("你好".to_string())
    );
    assert_eq!(
    ...
    let truncated = render_output_block_with_limit("stdout", "abcdef", 4).expect("output block should render");
...
fn bash_shell_owner_preserves_background_delivery_texts() {
    ...
    assert!(
        delivery.contains("<background_command status=\"completed\" terminal_session_id=\"term-bg\" exit_code=\"0\">")
    );
    ...
    assert!(error.starts_with("Background Bash command failed before producing a final completion result."));
...
fn remote_read_command_and_parser_preserve_existing_window_markers() {
    let command = build_remote_read_command("C:/repo/a'b.txt", 2, 3, 120, 1_000).expect("command builds");

    assert!(command.starts_with(
        "if [ ! -f 'C:/repo/a'\\''b.txt' ]; then exit 3; fi; awk -v start=2 -v end=4 -v max=120 -v budget=1000"
    ));
    assert!(command.contains("__northhing_TOTAL_LINES__="));
    assert!(command.contains("__northhing_HIT_TOTAL_CHAR_LIMIT__="));
    assert!(command.ends_with("'C:/repo/a'\\''b.txt'"));

    let result = parse_remote_read_output(
        "     2\talpha\n",
        "__northhing_TOTAL_LINES__=5\n__northhing_HIT_TOTAL_CHAR_LIMIT__=1\n",
        0,
        "C:/repo/a'b.txt",
        2,
    )
...
fn remote_delete_command_preserves_existing_recursive_flag_and_escaping() {
    ...
    assert_eq!(build_remote_delete_command("/repo/a'b", true), "rm -rf '/repo/a'\\''b'");
}
