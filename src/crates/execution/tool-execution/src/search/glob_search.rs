...
impl Ord for GlobCandidate {
    fn cmp(&self, other: &Self) -> Ordering {
        self.depth.cmp(&other.depth).then_with(|| self.path.cmp(&other.path))
    }
}
...
pub fn resolve_glob_config(pattern: &str) -> (bool, bool) {
    let is_whitelisted =
        pattern.starts_with(".northhing") || pattern.contains("/.northhing") || pattern.contains("\\.northhing");
...
fn build_rg_args(relative_pattern: &str, apply_gitignore: bool, ignore_hidden_files: bool) -> Vec<String> {
    let mut args = vec![
        "--files".to_string(),
        "--glob".to_string(),
...
pub fn execute_local_glob(request: LocalGlobRequest) -> Result<LocalGlobResult, String> {
    ...
    let search_path_abs = dunce::canonicalize(&request.search_path).map_err(|error| error.to_string())?;
    ...
    if !walk_root.exists() || !walk_root.is_dir() || request.limit == 0 {
        return Ok(LocalGlobResult { matches: Vec::new() });
    }
...
mod tests {
    ...
    fn make_temp_dir(name: &str) -> TempTree {
        let unique = SystemTime::now()
            ...
            .as_nanos();
        let dir = std::env::temp_dir().join(format!("northhing-glob-search-{name}-{unique}"));
    ...
    fn walk_fallback_returns_files_only_and_matches_rg_basename_globs() {
        ...
        fs::write(root.join("src").join("nested").join("lib.rs"), "").expect("file should be written");
        ...
        assert!(wildcard_matches.iter().any(|path| path.ends_with("/src/nested/lib.rs")));
