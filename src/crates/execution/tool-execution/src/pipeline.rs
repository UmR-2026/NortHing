...
pub fn should_retry_tool_attempt(facts: ToolRetryAttemptFacts) -> bool {
    facts.attempts < facts.max_attempts && matches!(facts.error_class, ToolExecutionErrorClass::Retryable)
}
...
pub fn summarize_dialog_turn_cancellation(
    ...
    states
        .into_iter()
        .fold(ToolTurnCancellationSummary::default(), |mut summary, state| {
