...
pub fn detect_osascript_im_app(cmd: &str) -> Option<&'static str> {
    ...
    const IM_APPS: &[&str] = &[
        "WeChat", "微信", "iMessage", "Messages", "Slack", "Lark", "飞书", "Telegram", "DingTalk", "钉钉", "QQ",
        "Discord", "Teams", "Whatsapp", "WhatsApp",
    ];
...
pub fn bash_noninteractive_env() -> HashMap<String, String> {
    ...
    env.insert("northhing_NONINTERACTIVE".to_string(), "1".to_string());
...
pub fn render_output_block_with_limit(tag: &str, output_text: &str, max_chars: usize) -> Option<String> {
...
pub fn render_remote_shell_result(request: RemoteShellResultRenderRequest<'_>) -> String {
    ...
    if let Some(stdout_block) = render_output_block_with_limit("stdout", request.stdout, stdout_budget) {
        result_string.push_str(&stdout_block);
    }
    if let Some(stderr_block) = render_output_block_with_limit("stderr", request.stderr, stderr_budget) {
...
pub fn format_background_command_delivery_text(request: BackgroundCommandDeliveryTextRequest<'_>) -> String {
    let (status, summary) = if request.status.timed_out {
        ...
        ("completed", "Background Bash command completed successfully.")
    } else {
        ("failed", "Background Bash command completed with a non-zero exit code.")
    };
    ...
    let persistence_line = request.output_persist_error.map_or_else(
        || format!("Full output was saved to: {}", request.output_file_reference),
        |error| {
            format!(
...
pub fn format_background_command_error_text(request: BackgroundCommandErrorTextRequest<'_>) -> String {
    let persistence_line = request.output_persist_error.map_or_else(
        || format!("Any captured output was saved to: {}", request.output_file_reference),
        |persist_error| {
            format!(
                "Output persistence encountered an error while writing {}: {}",
