...
pub fn write_local_file(request: WriteLocalFileRequest) -> Result<WriteLocalFileOutcome, String> {
    ...
    if request.mode == WriteLocalFileMode::Write && file_already_exists {
        let existing = fs::read(&request.resolved_path)
            .map_err(|error| format!("Failed to read existing file {}: {}", request.logical_path, error))?;
    ...
    let status = match request.mode {
        ...
        WriteLocalFileMode::Append => {
            let mut file = OpenOptions::new()
                ...
                .map_err(|error| format!("Failed to open file {} for append: {}", request.logical_path, error))?;
            file.write_all(request.content.as_bytes())
                .map_err(|error| format!("Failed to append to file {}: {}", request.logical_path, error))?;
    ...
    let verb = match status {
        ...
        WriteLocalFileStatus::AlreadyExistsSameContent => {
            return Ok(WriteLocalFileOutcome {
                ...
                bytes_written: 0,
                lines_written: 0,
                assistant_message: format!("File {} already exists with identical content.", request.logical_path),
            });
        }
    };
