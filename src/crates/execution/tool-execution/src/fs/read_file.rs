...
const REMOTE_TOTAL_LINES_MARKER: &str = "__northhing_TOTAL_LINES__=";
const REMOTE_HIT_TOTAL_CHAR_LIMIT_MARKER: &str = "__northhing_HIT_TOTAL_CHAR_LIMIT__=";
...
pub fn parse_remote_read_output(
    ...
    let total_lines = total_lines
        .ok_or_else(|| "Failed to read file: remote command did not return line-count markers".to_string())?;
    ...
    let lines_read = if content.is_empty() { 0 } else { content.lines().count() };
...
pub fn read_file(
    ...
    let file = File::open(file_path).map_err(|e| format!("Failed to read file {}: {}", file_path, e))?;
    ...
    for line_result in reader.lines() {
        ...
        let line_content = if line.chars().count() > max_line_chars {
            format!("{} [truncated]", truncate_string_by_chars(&line, max_line_chars))
        } else {
            line
        };
...
mod tests {
    ...
    fn write_temp_file(contents: &str) -> PathBuf {
        ...
        let path = std::env::temp_dir().join(format!(
            "northhing-read-file-test-{process_id}-{timestamp}-{counter}.txt"
        ));
    ...
    fn truncates_when_total_char_budget_is_hit() {
        ...
        let result = read_file(path.to_str().expect("utf-8 path"), 1, 10, 10, 30).expect("read should succeed");
    ...
    fn reads_multiple_lines_when_budget_allows() {
        ...
        let result = read_file(path.to_str().expect("utf-8 path"), 1, 10, 50, 100).expect("read should succeed");
