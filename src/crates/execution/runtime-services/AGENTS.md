...
`northhing-runtime-services` owns typed runtime service assembly. It connects
runtime-facing ports to injected providers without becoming a concrete platform
implementation layer.

## Guardrails

- Depend on `northhing-runtime-ports`; avoid dependencies on `northhing-core`, app
...
```bash
cargo test -p northhing-runtime-services
