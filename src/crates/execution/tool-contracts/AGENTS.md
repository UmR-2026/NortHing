...
`northhing-agent-tools` owns portable tool contracts. It must stay independent of
the product tool runtime.

## Guardrails

- Do not depend on `northhing-core`, concrete service crates,
...
```bash
cargo test -p northhing-agent-tools
cargo test -p northhing-agent-tools --test tool_contracts
