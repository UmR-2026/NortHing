...
`northhing-agent-runtime` owns portable agent runtime decisions and the narrow
port-backed `AgentRuntime` facade that can be built and tested without
`northhing-core`.

## Guardrails

- Do not depend on `northhing-core`, app crates, Tauri, ACP protocol, web UI,
...
- Keep concrete scheduler/session lifecycle execution, session metadata IO,
  ...
  adapter execution in `northhing-core` until a reviewed owner migration proves
