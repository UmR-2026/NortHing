...
pub fn resolve_mode_config_profile_id<'a>(mode_id: &'a str) -> Cow<'a, str> {
    match mode_id.trim() {
        "agentic" | "Plan" | "debug" | "Multitask" => Cow::Borrowed(SHARED_CODING_MODE_CONFIG_PROFILE_ID),
...
pub fn builtin_agent_definition_specs() -> Vec<BuiltinAgentDefinitionSpec> {
    ...
    vec![
        ...
        builtin_agent_spec("Multitask", Mode, "auto", SubagentVisibilityPolicy::default()),
        ...
        builtin_agent_spec("DeepResearch", Mode, "auto", SubagentVisibilityPolicy::default()),
        ...
        builtin_agent_spec("Explore", SubAgent, "primary", SubagentVisibilityPolicy::public()),
        builtin_agent_spec("GeneralPurpose", SubAgent, "fast", SubagentVisibilityPolicy::public()),
        builtin_agent_spec(
            "ResearchSpecialist",
            SubAgent,
            "fast",
...
pub fn default_model_id_for_builtin_agent(agent_type: &str) -> &'static str {
    match agent_type {
        "agentic" | "Cowork" | "ComputerUse" | "Plan" | "debug" | "Claw" | "DeepResearch" | "Team" | "Multitask" => {
            "auto"
        }
...
impl SubagentVisibilityPolicy {
        ...
        S: Into<String>,
    {
        Self {
            ...
            allowed_parent_agent_ids: allowed_parent_agent_ids.into_iter().map(Into::into).collect(),
            denied_parent_agent_ids: HashSet::new(),
            show_in_global_registry: true,
        }
    }
    ...
    where
        ...
        S: Into<String>,
    {
        self.denied_parent_agent_ids = denied_parent_agent_ids.into_iter().map(Into::into).collect();
        self
    }

    pub fn summary(&self) -> SubagentVisibilitySummary {
        let mut allowed_parent_agent_ids: Vec<String> = self.allowed_parent_agent_ids.iter().cloned().collect();
        allowed_parent_agent_ids.sort();

        let mut denied_parent_agent_ids: Vec<String> = self.denied_parent_agent_ids.iter().cloned().collect();
    ...
    pub fn can_access_from_parent(&self, parent_agent_type: Option<&str>) -> bool {
        let normalized_parent = parent_agent_type.map(str::trim).filter(|value| !value.is_empty());
        ...
        match self.exposure {
            ...
            BuiltinSubagentExposure::Restricted | BuiltinSubagentExposure::Hidden => {
                normalized_parent.is_some_and(|parent| self.allowed_parent_agent_ids.contains(parent))
            }
        }
    }
}
...
pub fn resolve_subagent_default_enabled(
    ...
    match source {
        ...
        SubagentSourceKind::Project | SubagentSourceKind::User | SubagentSourceKind::Unspecified => true,
    }
}
...
pub fn resolve_subagent_availability(
    ...
    } else if matches!(source, SubagentSourceKind::Builtin | SubagentSourceKind::User) {
...
const fn default_reason(source: SubagentSourceKind, default_enabled: bool) -> Option<SubagentStateReason> {
    match source {
        ...
        SubagentSourceKind::Project | SubagentSourceKind::User => Some(SubagentStateReason::CustomDefaultEnabled),
