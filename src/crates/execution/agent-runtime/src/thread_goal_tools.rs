...
pub fn parse_create_goal_args(input: Value) -> Result<CreateGoalArgs, ThreadGoalToolError> {
    serde_json::from_value(input)
        .map_err(|error| ThreadGoalToolError::validation(format!("invalid create_goal args: {error}")))
}

pub fn parse_update_goal_args(input: Value) -> Result<UpdateGoalArgs, ThreadGoalToolError> {
    serde_json::from_value(input)
        .map_err(|error| ThreadGoalToolError::validation(format!("invalid update_goal args: {error}")))
}
...
pub fn build_goal_tool_result(
    ...
    let data = serde_json::to_value(response)
        .map_err(|error| ThreadGoalToolError::validation(format!("failed to serialize goal tool result: {error}")))?;
