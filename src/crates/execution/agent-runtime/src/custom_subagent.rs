...
impl CustomSubagentDefinition {
    ...
    pub fn from_front_matter_fields(
        ...
        Ok(Self {
            name: name.ok_or(CustomSubagentDefinitionError::MissingName)?.to_string(),
...
pub struct CustomSubagentDiscoveryRoots {
    ...
    pub northhing_user_agents_dir: Option<PathBuf>,
...
pub fn custom_subagent_possible_dirs(roots: &CustomSubagentDiscoveryRoots) -> Vec<CustomSubagentDirEntry> {
    ...
    if let Some(northhing_agents) = &roots.northhing_user_agents_dir {
        if northhing_agents.exists() && northhing_agents.is_dir() {
            entries.push(CustomSubagentDirEntry {
                path: northhing_agents.clone(),
...
pub fn load_custom_subagent_definitions(roots: &CustomSubagentDiscoveryRoots) -> CustomSubagentLoadReport {
    ...
    for (root_priority, entry) in custom_subagent_possible_dirs(roots).into_iter().enumerate() {
        for md_path in list_custom_subagent_markdown_files(&entry.path) {
            match custom_subagent_read_markdown_file(&md_path, entry.kind) {
                ...
                Err(error) => errors.push(CustomSubagentLoadError { path: md_path, error }),
            }
        }
    }
    ...
    candidates.sort_by(|a, b| {
        a.root_priority
            .cmp(&b.root_priority)
            .then_with(|| a.definition.name.to_lowercase().cmp(&b.definition.name.to_lowercase()))
    ...
    CustomSubagentLoadReport { definitions, errors }
}
...
pub fn custom_subagent_read_markdown_file(
    ...
    let contents = std::fs::read_to_string(path).map_err(|error| format!("Failed to read markdown file: {error}"))?;
...
pub fn custom_subagent_read_markdown_str(
    ...
    let yaml = captures
        ...
        .as_str();
    let metadata: Value = serde_yaml::from_str(yaml).map_err(|error| format!("Failed to parse YAML: {error}"))?;
...
pub fn custom_subagent_save_markdown_parts(
    ...
    let metadata = custom_subagent_markdown_metadata(name, description, tools, readonly, review, model);
    let yaml = serde_yaml::to_string(&metadata).map_err(|error| format!("Failed to serialize YAML: {error}"))?;
    let contents = format!("---\n{}\n---\n\n{}", yaml.trim_end(), prompt.trim_start());

    std::fs::write(path, contents).map_err(|error| format!("Failed to write markdown file: {error}"))
}
...
fn custom_subagent_markdown_metadata(
    ...
    metadata.insert(Value::String("name".into()), Value::String(name.to_string()));
    metadata.insert(
    ...
    if custom_subagent_model_should_save(model) {
        metadata.insert(Value::String("model".into()), Value::String(model.to_string()));
    }
