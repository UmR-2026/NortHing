...
pub fn session_control_agent_type_or_default(agent_type: Option<&SessionControlAgentType>) -> String {
    agent_type
...
pub fn validate_session_control_input(
    ...
    if let Some(workspace) = input.workspace.as_deref() {
        let should_validate_workspace =
            matches!(input.action, SessionControlAction::Create | SessionControlAction::List);
...
pub fn session_control_created_result_message(session_id: &str, workspace: &str, agent_type: &str) -> String {
