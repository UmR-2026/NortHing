...
pub fn resolve_tool_confirmation_plan(request: ToolConfirmationRequestFacts) -> ToolConfirmationPlan {
...
pub fn resolve_confirmation_failure(outcome: ToolConfirmationOutcome) -> Option<ToolConfirmationFailure> {
    match outcome {
...
pub fn resolve_confirmation_wait_result(
    ...
    match result {
        ...
        ToolConfirmationWaitResult::Rejected(reason) => ToolConfirmationOutcome::Rejected { reason },
...
mod tests {
    ...
    fn maps_confirmation_wait_timeout_to_tool_named_outcome() {
        let outcome = resolve_confirmation_wait_result(ToolConfirmationWaitResult::TimedOut, "Bash");
    ...
    fn preserves_rejection_reason_in_confirmation_failure() {
        let outcome = resolve_confirmation_wait_result(ToolConfirmationWaitResult::Rejected("no".to_string()), "Edit");
