...
const SKILL_LISTING_GUIDANCE: &str = "The following skills are available for use with the Skill tool:";
...
<environment_details>
- Local northhing client OS: {} ({}) — applies to Computer use / UI automation on this machine only.
...
impl ToolListingSections {
    pub fn is_empty(&self) -> bool {
        self.skill_listing.is_none() && self.agent_listing.is_none() && self.collapsed_tool_listing.is_none()
    }

    pub fn render_skill_listing_reminder(&self) -> Option<String> {
        self.skill_listing
            .as_deref()
            .map(|skill_listing| Self::render_section(SKILL_LISTING_TITLE, skill_listing, Some(SKILL_LISTING_GUIDANCE)))
    }

    pub fn render_agent_listing_reminder(&self) -> Option<String> {
        self.agent_listing
            .as_deref()
            .map(|agent_listing| Self::render_section(AGENT_LISTING_TITLE, agent_listing, Some(AGENT_LISTING_GUIDANCE)))
    }

    pub fn render_collapsed_tool_listing_reminder(&self) -> Option<String> {
        self.collapsed_tool_listing.as_deref().map(|collapsed_tool_listing| {
            Self::render_section(
