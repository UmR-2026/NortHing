...
impl DeepReviewCapacityQueueDecision {
    pub fn queueable(reason: DeepReviewCapacityQueueReason, retry_after_seconds: Option<u64>) -> Self {
        Self {
            queueable: true,
            reason: Some(reason),
            retry_after_seconds,
...
impl DeepReviewReviewerQueueState {
    pub fn queued_for_capacity(reason: DeepReviewCapacityQueueReason, queue_elapsed_ms: u64) -> Self {
        Self {
...
impl DeepReviewQueueControlTracker {
    pub(crate) fn apply(
        ...
        match action {
            ...
            DeepReviewQueueControlAction::SkipOptional => {
                self.skip_optional_turns.insert(key.parent_dialog_turn_id.clone(), now);
            }
        }
    ...
    pub(crate) fn snapshot(&self, parent_dialog_turn_id: &str, tool_id: &str) -> DeepReviewQueueControlSnapshot {
        let Some(key) = DeepReviewQueueControlKey::new(parent_dialog_turn_id, tool_id) else {
            return DeepReviewQueueControlSnapshot {
                paused: false,
                cancelled: false,
        ...
        let skip_optional = self.skip_optional_turns.contains_key(&key.parent_dialog_turn_id);
...
pub fn classify_deep_review_capacity_error(
    ...
    let retry_after_seconds = retry_after_seconds.or_else(|| extract_retry_after_seconds(&combined));

    if contains_any(&combined, &["user_cancel", "user cancelled", "user canceled"]) {
        return DeepReviewCapacityQueueDecision::fail_fast(DeepReviewCapacityFailFastReason::UserCancellation);
    }
    ...
    if contains_any(
        ...
        return DeepReviewCapacityQueueDecision::fail_fast(DeepReviewCapacityFailFastReason::InvalidReviewerTooling);
    }
    ...
    if contains_any(
        ...
        return DeepReviewCapacityQueueDecision::fail_fast(DeepReviewCapacityFailFastReason::Authentication);
    }
    ...
    if contains_any(
        ...
        return DeepReviewCapacityQueueDecision::fail_fast(DeepReviewCapacityFailFastReason::BillingOrQuota);
    }
    ...
    if contains_any(
        ...
        return DeepReviewCapacityQueueDecision::fail_fast(DeepReviewCapacityFailFastReason::Permission);
    }
    ...
    if contains_any(
        ...
        return DeepReviewCapacityQueueDecision::fail_fast(DeepReviewCapacityFailFastReason::InvalidModel);
    }

    if contains_any(&combined, &["policy", "content_filter", "content filter", "safety"]) {
        return DeepReviewCapacityQueueDecision::fail_fast(DeepReviewCapacityFailFastReason::PolicyViolation);
    }
    ...
    if contains_any(
        ...
        return DeepReviewCapacityQueueDecision::fail_fast(DeepReviewCapacityFailFastReason::Validation);
    }
    ...
    if contains_any(
        &combined,
        &["deterministic", "unsupported", "malformed", "schema", "tool error"],
    ) {
    ...
    DeepReviewCapacityQueueDecision::fail_fast(DeepReviewCapacityFailFastReason::DeterministicProviderError)
}
