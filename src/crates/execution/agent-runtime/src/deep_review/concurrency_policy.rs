...
use super::execution_policy::{
    clamp_u64, clamp_usize, reviewer_agent_type_count, DeepReviewExecutionPolicy, DeepReviewPolicyViolation,
...
impl DeepReviewEffectiveConcurrencyState {
    pub(crate) fn new(configured_max_parallel_instances: usize) -> Self {
        let configured_max_parallel_instances = Self::normalize_configured_max(configured_max_parallel_instances);
        Self {
    ...
    pub(crate) fn set_user_override(&mut self, user_override_parallel_instances: Option<usize>) {
        self.user_override_parallel_instances =
            user_override_parallel_instances.map(|value| value.clamp(1, self.configured_max_parallel_instances));
    }
        ...
        let retry_after_remaining_ms =
            self.retry_after_until
                .and_then(|retry_after_until| match retry_after_until > now {
                    true => Some(u64::try_from(retry_after_until.duration_since(now).as_millis()).unwrap_or(u64::MAX)),
                    false => None,
                });
...
impl DeepReviewConcurrencyPolicy {
    pub fn from_manifest(raw: &Value) -> Self {
        ...
        Self {
            max_parallel_instances: clamp_usize(obj.get("maxParallelInstances"), 1, 16, DEFAULT_MAX_PARALLEL_INSTANCES),
