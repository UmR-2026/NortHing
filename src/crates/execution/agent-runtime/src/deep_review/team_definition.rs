...
use super::constants::{
    ...
    DEFAULT_MAX_SAME_ROLE_INSTANCES, DEFAULT_REVIEWER_FILE_SPLIT_THRESHOLD, REVIEWER_ARCHITECTURE_AGENT_TYPE,
    REVIEWER_BUSINESS_LOGIC_AGENT_TYPE, REVIEWER_FRONTEND_AGENT_TYPE, REVIEWER_PERFORMANCE_AGENT_TYPE,
...
fn review_role(
    ...
    ReviewTeamRoleDefinition {
        ...
        responsibilities: responsibilities.iter().map(|item| item.to_string()).collect(),
...
pub fn default_review_team_definition() -> ReviewTeamDefinition {
    ...
    let mut hidden_agent_ids = vec![DEEP_REVIEW_AGENT_TYPE.to_string(), REVIEW_JUDGE_AGENT_TYPE.to_string()];
    hidden_agent_ids.extend(CORE_REVIEWER_AGENT_TYPES.iter().map(|id| id.to_string()));
    hidden_agent_ids.extend(CONDITIONAL_REVIEWER_AGENT_TYPES.iter().map(|id| id.to_string()));
