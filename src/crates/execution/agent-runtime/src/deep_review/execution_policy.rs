...
use super::constants::{
    CONDITIONAL_REVIEWER_AGENT_TYPES, CORE_REVIEWER_AGENT_TYPES, DEEP_REVIEW_AGENT_TYPE, DEFAULT_MAX_RETRIES_PER_ROLE,
    DEFAULT_MAX_SAME_ROLE_INSTANCES, DEFAULT_REVIEWER_FILE_SPLIT_THRESHOLD, REVIEW_FIXER_AGENT_TYPE,
...
impl DeepReviewExecutionPolicy {
    pub fn from_config_value(raw: Option<&Value>) -> Self {
        ...
        Self {
            ...
            strategy_level: DeepReviewStrategyLevel::from_value(config.get("strategy_level")).unwrap_or_default(),
    ...
    pub fn classify_subagent(&self, subagent_type: &str) -> Result<DeepReviewSubagentRole, DeepReviewPolicyViolation> {
    ...
    pub fn predictive_timeout(
        ...
        let raw = base.saturating_add(file_overhead).saturating_add(line_overhead);
    ...
    pub fn with_run_manifest_execution_policy(&self, raw_manifest: &Value) -> Self {
        ...
        if let Some(strategy_level) = DeepReviewStrategyLevel::from_value(manifest.get("strategyLevel")) {
    ...
    fn apply_strategy_runtime_budget(&mut self) {
        ...
        self.judge_timeout_seconds = strategy_bounded_timeout(self.judge_timeout_seconds, budget.judge_timeout_seconds);
        self.reviewer_file_split_threshold =
            strategy_bounded_split_threshold(self.reviewer_file_split_threshold, budget.reviewer_file_split_threshold);
        self.max_same_role_instances = self.max_same_role_instances.min(budget.max_same_role_instances);
    }
...
fn normalize_member_strategy_overrides(raw: Option<&Value>) -> HashMap<String, DeepReviewStrategyLevel> {
...
fn disallowed_extra_subagent_ids() -> HashSet<&'static str> {
    CORE_REVIEWER_AGENT_TYPES
        ...
        .chain([REVIEW_JUDGE_AGENT_TYPE, DEEP_REVIEW_AGENT_TYPE, REVIEW_FIXER_AGENT_TYPE])
        .collect()
}
...
fn number_as_i64(value: &Value) -> Option<i64> {
    value
        .as_i64()
        .or_else(|| value.as_u64().map(|value| i64::try_from(value).unwrap_or(i64::MAX)))
}
