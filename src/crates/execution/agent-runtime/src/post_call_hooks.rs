...
pub trait SuccessfulToolPostCallHookExecutor<C> {
    fn record_deep_review_shared_context_tool_use(&mut self, tool_name: &str, input: &Value, context: &C);
}

pub fn run_successful_tool_post_call_hooks<C, E>(tool_name: &str, input: &Value, context: &C, executor: &mut E)
where
...
pub fn resolve_deep_review_shared_context_tool_use(
    ...
    if !facts.tool_name.eq_ignore_ascii_case("Read") && !facts.tool_name.eq_ignore_ascii_case("GetFileDiff") {
        return None;
    }
...
fn is_northhing_runtime_uri(path: &str) -> bool {
    path.trim().starts_with("northhing://runtime/")
}
