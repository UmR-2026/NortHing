...
use northhing_runtime_ports::{
    ...
    ThreadGoalToolResponse, GOAL_MODE_METADATA_KEY, MAX_THREAD_GOAL_AUTO_CONTINUATIONS, THREAD_GOAL_METADATA_KEY,
};
use std::fmt;
...
const OBJECTIVE_UPDATED_PROMPT_TEMPLATE: &str = include_str!("thread_goal/templates/objective_updated.md");
...
pub fn should_skip_goal_for_turn(user_input: &str, user_message_metadata: Option<&serde_json::Value>) -> bool {
...
fn should_skip_goal_turn_accounting(user_input: &str, user_message_metadata: Option<&serde_json::Value>) -> bool {
...
pub fn billable_tokens_from_counts(input_tokens: usize, cached_tokens: usize, output_tokens: usize) -> usize {
...
pub fn goal_tool_response(goal: Option<ThreadGoal>, include_completion_report: bool) -> ThreadGoalToolResponse {
...
pub fn build_set_thread_goal_result(
    ...
    let replaced_existing = request.replace_existing || objective.is_some() && request.existing.is_some();
...
impl ThreadGoalRuntime {
    ...
    pub fn continuation_after_turn(
        ...
        let became_budget_limited =
            self.account_turn_tokens(facts.turn_id, facts.turn_tokens, &mut goal, facts.now_epoch_seconds);
...
impl GoalWallClockAccounting {
    ...
    fn mark_accounted(&mut self, accounted_seconds: i64) {
        ...
        self.last_accounted_at = self.last_accounted_at.checked_add(advance).unwrap_or_else(Instant::now);
    }
