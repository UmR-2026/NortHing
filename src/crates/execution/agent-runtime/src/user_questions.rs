...
pub fn validate_ask_user_question_input(input: &AskUserQuestionInput) -> Result<(), String> {
    ...
    for (q_idx, question) in input.questions.iter().enumerate() {
        ...
        if question.header.chars().count() > 20 {
            return Err(format!("Question {} header must be less than 20 characters", q_num));
        }
        ...
        for (opt_idx, opt) in question.options.iter().enumerate() {
            if opt.label.trim().is_empty() {
                return Err(format!("Question {} option {} label is required", q_num, opt_idx + 1));
            }
...
pub fn build_answered_user_question_result(input: &AskUserQuestionInput, answers: Value) -> UserQuestionToolResult {
...
pub fn build_cancelled_user_question_result(input: &AskUserQuestionInput) -> UserQuestionToolResult {
    UserQuestionToolResult {
        data: json!({
