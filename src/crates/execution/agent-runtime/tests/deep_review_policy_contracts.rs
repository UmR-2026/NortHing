use northhing_agent_runtime::deep_review::report::{
    deep_review_cache_from_completed_reviewers, fill_deep_review_packet_metadata, fill_deep_review_reliability_signals,
};
use northhing_agent_runtime::deep_review::task_execution::{
    ...
    deep_review_launch_batch_for_task, deep_review_packet_id_for_cache, ensure_deep_review_retry_coverage,
    local_reviewer_capacity_queue_decision, prompt_with_deep_review_retry_scope,
    provider_capacity_queue_wait_seconds_for_attempt, DeepReviewBlockedReviewerAdmissionQueueStepDecision,
...
use northhing_agent_runtime::deep_review::{
    append_tool_use_context_data, apply_deep_review_queue_control, deep_review_queue_control_snapshot,
    record_deep_review_shared_context_tool_use, ChangeRiskFactors, DeepReviewBudgetTracker,
    DeepReviewCapacityQueueDecision, DeepReviewCapacityQueueReason, DeepReviewConcurrencyPolicy,
    DeepReviewExecutionPolicy, DeepReviewQueueControlAction, DeepReviewQueueControlSnapshot,
    DeepReviewQueueWaitSkipReason, DeepReviewRunManifestGate, DeepReviewStrategyLevel, DeepReviewSubagentRole,
...
fn deep_review_runtime_owner_tracks_budget_queue_and_shared_context() {
    ...
    let snapshot = apply_deep_review_queue_control("turn-runtime-owner", "tool-1", DeepReviewQueueControlAction::Pause);
...
fn deep_review_tool_context_data_injection_stays_provider_neutral() {
    ...
    let context_vars = HashMap::from([
        ...
        ("deep_review_subagent_role".to_string(), "reviewer".to_string()),
        ("deep_review_subagent_type".to_string(), "ReviewSecurity".to_string()),
    ]);
...
fn deep_review_report_owner_enriches_packet_reliability_and_cache_facts() {
    ...
    let cache_update =
        deep_review_cache_from_completed_reviewers(&report, Some(&manifest), None).expect("cache update");
    assert_eq!(cache_update.hit_count, 0);
    assert_eq!(cache_update.miss_count, 1);
    assert!(cache_update.value.pointer("/packets/reviewer:ReviewSecurity").is_some());
}
...
fn deep_review_task_execution_owner_preserves_packet_retry_and_queue_contracts() {
    ...
    assert_eq!(
        deep_review_packet_id_for_cache("ReviewSecurity", Some("Review [packet security-a]"), Some(&manifest)),
    ...
    let retry_scope = ensure_deep_review_retry_coverage(&retry_input, "ReviewSecurity", Some(&manifest))
    ...
    let wait_seconds = provider_capacity_queue_wait_seconds_for_attempt(
        &DeepReviewCapacityQueueDecision::queueable(DeepReviewCapacityQueueReason::ProviderConcurrencyLimit, None),
    ...
    let provider_error_decision = capacity_decision_for_provider_error_facts(DeepReviewProviderCapacityErrorFacts {
    ...
    let provider_step = decide_provider_capacity_queue_step(DeepReviewProviderCapacityQueueStepFacts {
    ...
    assert_eq!(payload["capacity_reason"].as_str(), Some("launch_batch_blocked"));
