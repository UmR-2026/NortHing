use northhing_agent_runtime::events::{session_state_label, turn_outcome_kind, turn_outcome_status_kind, FinishReason};
use northhing_agent_runtime::scheduler::{TurnOutcome, TurnOutcomeStatus};
use northhing_runtime_ports::{DialogSessionStateFact, DialogTurnOutcomeKind};
...
fn session_state_labels_match_existing_event_wire_values() {
    ...
    assert_eq!(session_state_label(DialogSessionStateFact::Processing), "processing");
    assert_eq!(session_state_label(DialogSessionStateFact::Error), "error");
    assert_eq!(session_state_label(DialogSessionStateFact::Missing), "missing");
}
...
fn turn_outcome_kind_matches_existing_reply_policy_contract() {
    ...
    assert_eq!(turn_outcome_kind(&completed), DialogTurnOutcomeKind::Completed);
    assert_eq!(turn_outcome_kind(&cancelled), DialogTurnOutcomeKind::Cancelled);
