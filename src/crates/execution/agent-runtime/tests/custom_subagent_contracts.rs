use northhing_agent_runtime::custom_subagent::{
    custom_subagent_model_or_default, custom_subagent_model_should_save, custom_subagent_read_markdown_file,
    custom_subagent_read_markdown_str, custom_subagent_readonly_or_default, custom_subagent_readonly_should_save,
    custom_subagent_review_or_default, custom_subagent_review_should_save, custom_subagent_save_markdown_file,
    custom_subagent_tools_are_default, custom_subagent_tools_from_front_matter, custom_subagent_tools_to_front_matter,
    CustomSubagentDefinition, CustomSubagentDefinitionError, CustomSubagentKind, DEFAULT_CUSTOM_SUBAGENT_MODEL,
    DEFAULT_CUSTOM_SUBAGENT_READONLY, DEFAULT_CUSTOM_SUBAGENT_REVIEW,
};
...
fn custom_subagent_defaults_match_existing_front_matter_contract() {
    ...
    assert_eq!(custom_subagent_review_or_default(None), DEFAULT_CUSTOM_SUBAGENT_REVIEW);
    assert_eq!(custom_subagent_model_or_default(None), DEFAULT_CUSTOM_SUBAGENT_MODEL);
}
...
fn custom_subagent_default_fields_are_omitted_when_saved() {
    ...
    assert!(!custom_subagent_readonly_should_save(DEFAULT_CUSTOM_SUBAGENT_READONLY));
    assert!(custom_subagent_readonly_should_save(false));
    assert!(!custom_subagent_review_should_save(DEFAULT_CUSTOM_SUBAGENT_REVIEW));
    assert!(custom_subagent_review_should_save(true));
    assert!(!custom_subagent_model_should_save(DEFAULT_CUSTOM_SUBAGENT_MODEL));
...
fn custom_subagent_definition_reports_legacy_missing_field_errors() {
    ...
    assert_eq!(missing_description, CustomSubagentDefinitionError::MissingDescription);
...
fn custom_subagent_markdown_io_preserves_legacy_front_matter_shape() {
    let dir = TestTempDir::new("northhing-agent-runtime-subagent");
    ...
    let loaded =
        custom_subagent_read_markdown_file(&path, CustomSubagentKind::Project).expect("saved definition should load");
    assert_eq!(loaded, definition);
}

#[test]
fn custom_subagent_markdown_parse_errors_match_legacy_prefixes() {
    let missing_front_matter = custom_subagent_read_markdown_str("No front matter", CustomSubagentKind::User)
