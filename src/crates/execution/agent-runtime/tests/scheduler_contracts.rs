use northhing_agent_runtime::scheduler::{
    ...
    resolve_dialog_steering_action, ActiveDialogTurn, ActiveDialogTurnStore, AgentSessionReplyAction,
    BackgroundDeliveryAction, BackgroundDeliveryFacts, BackgroundInjectionKind, DialogReplySuppressionSet,
    DialogRoundInjectionInterrupt, DialogSteeringAction, DialogTurnQueue, DialogTurnQueueError, SessionAbortFlags,
    SessionRoundInjectionBuffer, ThreadGoalDeliveryReminderKind, TurnOutcome, TurnOutcomeQueueAction,
    TurnOutcomeStatus, DEFAULT_MAX_DIALOG_QUEUE_DEPTH,
};
use northhing_runtime_ports::{
    AgentSessionReplyRoute, DialogQueuePriority, DialogSessionStateFact, DialogSteerOutcome, DialogSubmissionPolicy,
    DialogTriggerSource, RoundInjection, RoundInjectionKind, RoundInjectionTarget, ThreadGoal, ThreadGoalStatus,
};
use std::sync::Arc;
...
fn background_delivery_injection_builds_thread_goal_current_turn_message() {
    ...
    assert_eq!(injection.kind, RoundInjectionKind::ThreadGoalObjectiveUpdated);
...
fn thread_goal_resumed_delivery_plan_preserves_follow_up_and_metadata() {
    ...
    assert_eq!(plan.follow_up_original_user_input, Some(plan.display_message.clone()));
    assert!(plan
        .injection_prompt
...
fn thread_goal_objective_updated_delivery_plan_preserves_follow_up_and_metadata() {
    ...
    assert_eq!(plan.follow_up_original_user_input, Some(plan.injection_display.clone()));
    assert!(plan
        .injection_prompt
    ...
    assert_eq!(plan.user_message_metadata["threadGoalObjectiveUpdated"], true);
...
fn dialog_turn_queue_rejects_overflow_and_preserves_current_error_shape() {
    ...
    assert_eq!(error.to_string(), "Message queue full for session s1 (max 1 messages)");
...
fn active_dialog_turn_store_owns_suppression_key_resolution_and_removal() {
    ...
    let removed = store.remove("target-session").expect("active turn should remove");
...
fn agent_session_reply_action_suppresses_cancelled_auto_reply_when_requested() {
    ...
    assert_eq!(action, AgentSessionReplyAction::SkipSuppressedCancelledReply);
}
...
fn dialog_steering_action_buffers_exact_running_turn_with_display_fallback() {
    ...
    assert_eq!(injection.target, RoundInjectionTarget::ExactTurn("turn-1".to_string()));
...
fn dialog_steering_action_rejects_when_target_turn_is_not_running() {
    ...
    assert_eq!(
        ...
        DialogSteeringAction::Reject {
            error: "Dialog turn is no longer running and cannot be steered: session_id=session-1, turn_id=turn-1"
                .to_string(),
        }
    );
}
...
fn turn_outcome_status_reply_and_queue_policy_are_portable() {
    ...
    assert_eq!(completed.queue_action(), TurnOutcomeQueueAction::DispatchNext);
    ...
    assert_eq!(cancelled.queue_action(), TurnOutcomeQueueAction::DispatchNext);
...
fn round_injection_buffer_drains_only_messages_for_the_active_turn() {
    ...
    let interrupt = DialogRoundInjectionInterrupt::new("s1".to_string(), "turn-a".to_string(), buffer.clone());
