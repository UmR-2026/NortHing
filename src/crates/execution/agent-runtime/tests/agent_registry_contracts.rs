use northhing_agent_runtime::agents::{
    ...
    resolve_subagent_availability, resolve_subagent_default_enabled, shared_coding_mode_user_context_policy,
    subagent_source_kind, subagent_source_presentation_rank, BuiltinAgentCategory, BuiltinSubagentExposure,
    SubAgentSource, SubagentOverrideLayers, SubagentOverrideState, SubagentSourceKind, SubagentStateReason,
    SubagentVisibilityPolicy, SHARED_CODING_MODE_CONFIG_PROFILE_ID, SHARED_CODING_MODE_CONFIG_PROFILE_LABEL,
...
fn availability_preserves_builtin_project_and_user_override_layering() {
    ...
    assert_eq!(builtin.state_reason, Some(SubagentStateReason::EnabledByUserOverride));
    ...
    assert_eq!(project.override_state, Some(SubagentOverrideState::Disabled));
    assert_eq!(
    ...
    let custom_default =
        resolve_subagent_availability(SubagentSourceKind::User, true, SubagentOverrideLayers::default());
...
fn subagent_source_contract_preserves_runtime_kind_and_presentation_order() {
    ...
    assert_eq!(subagent_source_presentation_rank(Some(SubAgentSource::Builtin)), 0);
    assert_eq!(subagent_source_presentation_rank(Some(SubAgentSource::Project)), 1);
    assert_eq!(subagent_source_presentation_rank(Some(SubAgentSource::User)), 2);
...
fn builtin_agent_definition_catalog_preserves_order_categories_models_and_visibility() {
    ...
    assert_eq!(default_model_id_for_builtin_agent("ResearchSpecialist"), "fast");
    assert_eq!(default_model_id_for_builtin_agent("ReviewArchitecture"), "fast");
    ...
    assert!(computer_use.visibility_policy.can_access_from_parent(Some("Claw")));
