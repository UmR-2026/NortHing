use northhing_agent_runtime::user_questions::{
    ...
    build_cancelled_user_question_result, validate_ask_user_question_input, AskUserQuestionInput, Question,
    QuestionOption,
};
...
fn ask_user_question_available_flag_matches_acp_transport_contract() {
    ...
    assert!(ask_user_question_available_for_acp_transport(Some(&serde_json::json!(
        false
    ))));
...
fn ask_user_question_answered_and_cancelled_results_keep_wire_shape() {
    ...
    assert_eq!(answered.data["questions"][0]["question"], input.questions[0].question);
    assert_eq!(answered.data["questions"][0]["header"], input.questions[0].header);
    ...
    assert_eq!(cancelled.result_for_assistant, "User input request was cancelled.");
}
