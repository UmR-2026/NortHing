...
use northhing_runtime_ports::{ThreadGoal, ThreadGoalStatus};
...
fn create_goal_args_parser_preserves_snake_case_wire_contract() {
    ...
    assert!(parse_create_goal_args(serde_json::json!({ "token_budget": 4096 }))
...
fn goal_tool_result_preserves_empty_and_complete_wire_shape() {
    ...
    let complete = build_goal_tool_result(Some(goal(ThreadGoalStatus::Complete)), true).expect("serializes");
    ...
    assert_eq!(complete.result_for_assistant, "Thread goal status: complete");
}
