use northhing_agent_runtime::tool_confirmation::{
    ...
    ConfirmationFailureKind, ToolConfirmationOutcome, ToolConfirmationPlan, ToolConfirmationRequestFacts,
...
fn confirmation_failure_mapping_preserves_legacy_reasons_and_errors() {
    assert_eq!(resolve_confirmation_failure(ToolConfirmationOutcome::Confirmed), None);
...
fn confirmation_wait_result_mapping_preserves_legacy_timeout_and_rejection() {
    ...
    assert_eq!(
        resolve_confirmation_wait_result(ToolConfirmationWaitResult::Rejected("unsafe".to_string()), "Edit"),
