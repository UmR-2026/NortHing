use northhing_agent_runtime::prompt::{
    render_prompt_environment_info, PrependedPromptReminders, PromptEnvironmentFacts, ToolListingSections,
...
fn prepended_prompt_reminders_keep_runtime_injection_order() {
    ...
    assert_eq!(
        ...
        vec!["collapsed-tools", "skills", "agents", "runtime-context", "user-context"]
    );
    assert!(PrependedPromptReminders::default().ordered_reminders().is_empty());
}
...
fn prompt_environment_info_preserves_local_and_remote_guidance() {
    ...
    assert!(remote.contains("- Local northhing client OS: linux (unix)"));
