...
fn continuation_outcome_marks_active_goal_blocked_at_limit() {
    ...
    assert_eq!(
        outcome.goal_to_persist.expect("blocked goal should persist").status,
...
fn continuation_outcome_reports_budget_limit_once_when_tokens_cross_budget() {
    ...
    let persisted = outcome.goal_to_persist.expect("budget-limited goal should persist");
...
fn prompt_and_tool_response_contracts_match_thread_goal_wire_shape() {
    ...
    assert_eq!(updated.user_message_metadata["threadGoalObjectiveUpdated"], true);
...
fn thread_goal_event_payload_and_token_usage_filter_preserve_core_delivery_contract() {
    let active = goal(ThreadGoalStatus::Active);
    let payload = thread_goal_event_payload(Some(active.clone())).expect("active goal should serialize for event");
