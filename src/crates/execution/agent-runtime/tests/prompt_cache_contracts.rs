use northhing_agent_runtime::prompt_cache::{
    prompt_cache_scope_key, CachedSystemPrompt, CachedUserContext, PromptCacheLookup, PromptCachePolicy,
    PromptCacheScope, SessionPromptCacheStore, SystemPromptCacheIdentity, UserContextCacheIdentity,
...
fn prompt_cache_lookup_preserves_identity_and_expiry_semantics() {
    ...
    store.set_system_prompt(
        "session-1",
        CachedSystemPrompt::new(SystemPromptCacheIdentity::new("template:agentic_mode"), "system prompt"),
    );
    store.set_user_context(
        "session-1",
        CachedUserContext::new(UserContextCacheIdentity::new("workspace_context"), "user context"),
    );
