use northhing_agent_runtime::scheduled_job::{
    ScheduledJobEnqueueFailureAction, ScheduledJobRunStatus, ScheduledJobRuntimeState, ScheduledJobTriggerAction,
...
fn enqueue_failure_preserves_retry_and_missing_session_disable_semantics() {
    ...
    assert_eq!(retry_state.last_run_status, Some(ScheduledJobRunStatus::Error));
    ...
    let disable_action = missing_session_state.mark_enqueue_failed(200, "session not found".to_string(), 5_000, true);

    assert_eq!(disable_action, ScheduledJobEnqueueFailureAction::DisableMissingSession);
...
fn turn_completion_failure_and_cancel_preserve_legacy_status_fields() {
    ...
    assert_eq!(cancelled.last_run_status, Some(ScheduledJobRunStatus::Cancelled));
...
fn restart_recovery_marks_active_turn_error() {
    ...
    let changed = state.recover_interrupted_turn_after_restart(300, "Application restarted".to_string());
