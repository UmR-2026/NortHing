...
`northhing-agent-stream` owns provider-neutral stream DTOs, tool-call accumulation,
...
- Do not depend on `northhing-core`, app crates, Tauri, concrete services,
  transport adapters, AI adapters, terminal, tool-runtime, or product-domain
  implementations.
- Keep provider-specific SSE or response parsing in `northhing-ai-adapters`; this
...
```bash
cargo test -p northhing-agent-stream
node scripts/check-core-boundaries.mjs
```

When provider fixture parsing changes, run the focused `northhing-ai-adapters`
