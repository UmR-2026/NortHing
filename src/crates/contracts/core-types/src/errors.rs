...
fn is_retryable_category(category: &ErrorCategory) -> bool {
    matches!(
        category,
        ErrorCategory::Network | ErrorCategory::RateLimit | ErrorCategory::Timeout | ErrorCategory::ProviderUnavailable
    )
}
...
fn action_hints_for_category(category: &ErrorCategory) -> Vec<String> {
    let hints: &[&str] = match category {
        ...
        ErrorCategory::Auth | ErrorCategory::Permission => &["open_model_settings", "copy_diagnostics"],
        ...
        ErrorCategory::Network | ErrorCategory::Timeout => &["retry", "switch_model", "copy_diagnostics"],
        ErrorCategory::ContentPolicy | ErrorCategory::InvalidRequest => &["copy_diagnostics"],
        ErrorCategory::ModelError | ErrorCategory::Unknown => &["retry", "switch_model", "copy_diagnostics"],
    };
...
mod tests {
    ...
    fn classifies_quota_and_provider_unavailable_errors() {
        ...
        assert_eq!(
            classify_ai_error_message("DeepSeek API error 402 - Insufficient Balance: You have run out of balance"),
            ErrorCategory::ProviderQuota
        );
        assert_eq!(
