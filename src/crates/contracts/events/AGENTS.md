...
`northhing-events` owns platform-neutral event contracts and the emitter interface.
...
- Use shared DTOs from `northhing-core-types` when event payloads need stable
...
```bash
cargo check -p northhing-events
