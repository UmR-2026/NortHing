...
pub trait EventEmitter: Send + Sync {
    ...
    /// Send LSP events
    async fn emit_lsp(&self, workspace_path: &str, event_data: serde_json::Value) -> anyhow::Result<()> {
        self.emit(
            "lsp-event",
            serde_json::json!({
    ...
    /// Send Profile events
    async fn emit_profile(&self, workspace_path: &str, event_data: serde_json::Value) -> anyhow::Result<()> {
        self.emit(
            "profile-event",
            serde_json::json!({
                "workspace_path": workspace_path,
    ...
    async fn emit_terminal(&self, session_id: &str, output: &str, stream_type: &str) -> anyhow::Result<()> {
        self.emit(
            "terminal-output",
            serde_json::json!({
