...
pub use northhing_core_types::errors::{AiErrorDetail, ErrorCategory};
...
mod tests {
    ...
    fn deep_review_queue_state_event_serializes_stable_contract() {
        ...
        assert_eq!(serialized["queue_state"]["reason"], json!("provider_concurrency_limit"));
        assert_eq!(serialized["queue_state"]["queue_elapsed_ms"], json!(1200));
        assert_eq!(serialized["queue_state"]["effective_parallel_instances"], json!(2));
        assert_eq!(serialized["queue_state"]["run_elapsed_ms"], serde_json::Value::Null);
    }
...
impl AgenticEvent {
    ...
    pub fn default_priority(&self) -> AgenticEventPriority {
        match self {
            Self::SystemError { .. } | Self::DialogTurnFailed { .. } | Self::DialogTurnCancelled { .. } => {
...
impl ToolEventData {
    ...
    pub fn default_priority(&self) -> AgenticEventPriority {
        match self {
            ...
            Self::Started { .. } | Self::Completed { .. } | Self::Failed { .. } | Self::ConfirmationNeeded { .. } => {
