...
use northhing_product_domains::function_agents::{
    git_func_agent::{
        assemble_commit_message, build_changes_summary_from_paths, build_commit_prompt, detect_change_patterns,
        extract_module_name, infer_file_type, parse_commit_analysis_json, parse_commit_analysis_value,
        parse_commit_type_label, prepare_commit_prompt, truncate_diff_for_commit_prompt, ChangePattern, CommitFormat,
        CommitMessageOptions, CommitType, FileChange, FileChangeType, ProjectContext,
    },
    ports::{
    ...
    startchat_func_agent::{
        build_complete_analysis_prompt, combine_git_diffs, limit_quick_actions, normalize_predicted_actions,
        parse_complete_analysis_json, parse_complete_analysis_value, parse_git_status_porcelain,
        parse_predicted_actions_from_values, parse_quick_actions_from_values, time_of_day_for_hour, ActionPriority,
        AheadBehind, GitWorkState, QuickActionType, TimeOfDay, WorkStateOptions,
    },
...
impl FunctionAgentGitPort for FunctionAgentPortStub {
    fn git_commit_snapshot(&self, _repo_path: PathBuf) -> FunctionAgentFuture<'_, GitCommitSnapshot> {
        Box::pin(async {
    ...
    fn startchat_git_snapshot(&self, _repo_path: PathBuf) -> FunctionAgentFuture<'_, StartchatGitSnapshot> {
        Box::pin(async {
    ...
    fn startchat_time_snapshot(&self, _repo_path: PathBuf) -> FunctionAgentFuture<'_, StartchatTimeSnapshot> {
        Box::pin(async {
...
impl FunctionAgentAiPort for FunctionAgentPortStub {
    ...
    ) -> FunctionAgentFuture<'_, northhing_product_domains::function_agents::git_func_agent::AICommitAnalysis> {
        Box::pin(async {
            Ok(
                northhing_product_domains::function_agents::git_func_agent::AICommitAnalysis {
    ...
    ) -> FunctionAgentFuture<'_, northhing_product_domains::function_agents::startchat_func_agent::AIGeneratedAnalysis>
    {
        Box::pin(async {
            Ok(
                northhing_product_domains::function_agents::startchat_func_agent::AIGeneratedAnalysis {
...
fn git_function_agent_summary_helpers_preserve_commit_shape() {
    let changed_files = vec!["src/crates/assembly/core/lib.rs".to_string(), "README.md".to_string()];
    ...
    assert_eq!(summary.file_changes[0].path, "src/crates/assembly/core/lib.rs");
    assert_eq!(summary.file_changes[0].file_type, "rs");
    assert!(summary.affected_modules.contains(&"core".to_string()));
    assert!(summary.change_patterns.contains(&ChangePattern::DocumentationUpdate));
...
fn git_function_agent_commit_prompt_preparation_preserves_truncation_boundary() {
    ...
    assert!(prepared.diff_content.ends_with("\n\n... [content truncated] ..."));
...
fn startchat_complete_analysis_parser_preserves_defaults_and_limits() {
    ...
    assert_eq!(parsed.analysis.predicted_actions[0].priority, ActionPriority::High);
...
fn startchat_git_status_helpers_preserve_porcelain_contract() {
    let (unstaged, staged, files) =
        parse_git_status_porcelain(" M src/lib.rs\nM  Cargo.toml\n?? README.md\nR  old.rs -> new.rs\n");
...
fn function_agent_runtime_facade_generates_commit_message_from_ports() {
    ...
    let message =
        block_on(facade.generate_commit_message(PathBuf::from("repo"), CommitMessageOptions::default())).unwrap();
...
fn function_agent_runtime_facade_preserves_empty_staging_error() {
    ...
    let error =
        block_on(facade.generate_commit_message(PathBuf::from("repo"), CommitMessageOptions::default())).unwrap_err();

    assert_eq!(error.error_type, AgentErrorType::InvalidInput);
    assert_eq!(error.message, "Staging area is empty, please stage files first");
}
...
fn function_agent_runtime_facade_builds_work_state_from_ports_without_surface_logic() {
    ...
    assert_eq!(analysis.current_state.time_info.minutes_since_last_commit, Some(1));
    assert_eq!(analysis.current_state.time_info.time_of_day, TimeOfDay::Afternoon);
...
fn function_agent_runtime_facade_honors_disabled_git_state_boundary_and_preserves_time_info() {
    ...
    assert_eq!(analysis.current_state.time_info.minutes_since_last_commit, Some(1));
    assert_eq!(analysis.current_state.time_info.time_of_day, TimeOfDay::Morning);
    assert!(analysis.predicted_actions.is_empty());
    assert!(analysis.quick_actions.is_empty());
}
...
fn function_agent_json_helpers_parse_ai_payloads_without_core_runtime() {
    ...
    assert_eq!(commit.title, "refactor(product-domains): move parse helpers");
    assert_eq!(commit.body.as_deref(), Some("Keep runtime adapters in core."));
    ...
    .unwrap();
    assert_eq!(work_state.analysis.summary, "Working on product-domain owner closure.");
