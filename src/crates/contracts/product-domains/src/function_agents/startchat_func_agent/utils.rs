...
use crate::function_agents::common::{extract_json_from_ai_response, AgentError, AgentResult, Language};
...
pub fn build_complete_analysis_prompt(
    ...
    template
        ...
        .replace("{git_diff_section}", &build_git_diff_section(git_diff, 8000))
}
...
pub fn parse_predicted_actions_from_values(actions_array: &[serde_json::Value]) -> Vec<PredictedAction> {
    actions_array
        .iter()
        .map(|action_value| PredictedAction {
            description: action_value["description"]
                ...
                .to_string(),
            priority: parse_action_priority_label(action_value["priority"].as_str().unwrap_or("Medium")),
...
pub fn parse_quick_actions_from_values(actions_array: &[serde_json::Value]) -> Vec<QuickAction> {
    actions_array
        ...
        .map(|action_value| QuickAction {
            title: action_value["title"].as_str().unwrap_or("Quick Action").to_string(),
            command: action_value["command"].as_str().unwrap_or("").to_string(),
            icon: action_value["icon"].as_str().unwrap_or("").to_string(),
            action_type: parse_quick_action_type_label(action_value["action_type"].as_str().unwrap_or("Custom")),
        })
        .collect()
}
...
pub fn parse_work_state_analysis_response(response: &str) -> AgentResult<ParsedCompleteAnalysis> {
    let json_str = extract_json_from_ai_response(response)
        .ok_or_else(|| AgentError::internal_error("Failed to extract JSON from analysis response"))?;
...
mod tests {
    ...
    fn work_state_ai_prompt_uses_product_domain_template() {
        ...
        let prompt =
            build_work_state_analysis_prompt(&git_state, "diff --git a/src/lib.rs b/src/lib.rs", &Language::English);

        assert!(prompt.contains("northhing AI assistant"));
...
"#,
        ...
        assert_eq!(analysis.analysis.summary, "Working on product-domain owner closure.");
        ...
        assert_eq!(missing_json.message, "Failed to extract JSON from analysis response");
...
```"#,
        ...
        assert_eq!(invalid_json.message, "Failed to extract JSON from analysis response");
    }
}
