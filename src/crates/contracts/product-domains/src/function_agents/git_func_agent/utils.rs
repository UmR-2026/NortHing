...
pub fn detect_change_patterns(file_changes: &[FileChange]) -> Vec<ChangePattern> {
    ...
    if has_config_changes {
        if file_changes.iter().any(|f| {
            f.path.contains("package.json") || f.path.contains("cargo.toml") || f.path.contains("requirements.txt")
        }) {
    ...
    let total_lines = file_changes.iter().map(|f| f.additions + f.deletions).sum::<u32>();
...
pub fn assemble_commit_message(title: &str, body: &Option<String>, footer: &Option<String>) -> String {
...
pub fn build_commit_prompt(
    ...
    template
        ...
        .replace("{language_desc}", commit_language_description(&options.language))
...
pub fn parse_commit_analysis_value(value: &serde_json::Value) -> Result<AICommitAnalysis, String> {
    Ok(AICommitAnalysis {
        ...
        reasoning: value["reasoning"].as_str().unwrap_or("AI analysis").to_string(),
...
"#,
        ...
        assert_eq!(parsed.title, "refactor(product-domains): move response policy");
        ...
        let missing_title = parse_commit_ai_response(r#"{"type":"refactor","body":"missing title"}"#).unwrap_err();
