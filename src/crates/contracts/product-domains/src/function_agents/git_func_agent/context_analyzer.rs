...
mod tests {
    ...
    fn detects_rust_library_context() {
        ...
        fs::write(root.join("Cargo.toml"), "[lib]\nname = \"demo\"\ntokio = \"1\"\n").unwrap();
        fs::write(root.join("README.md"), "hello docs").unwrap();
        fs::write(root.join("rustfmt.toml"), "").unwrap();

        assert_eq!(ContextAnalyzer::detect_project_type(&root).unwrap(), "rust-library");
        assert_eq!(
            ContextAnalyzer::detect_tech_stack(&root).unwrap(),
            vec!["Rust".to_string(), "Tokio".to_string()]
        );
        assert_eq!(ContextAnalyzer::read_project_docs(&root).as_deref(), Some("hello docs"));
        assert_eq!(
    ...
    fn temp_dir(label: &str) -> std::path::PathBuf {
        ...
        std::env::temp_dir().join(format!(
            "northhing-product-domains-{label}-{}-{nanos}",
            std::process::id()
        ))
    }
}
