...
pub fn resolve_policy_with_request(perms: &MiniAppPermissions, request: &MiniAppPermissionPolicyRequest<'_>) -> Value {
    resolve_policy(
        perms,
        request.app_id,
...
pub fn resolve_policy(
    ...
    if let Some(ref fs) = perms.fs {
        let read = resolve_fs_scopes(fs.read.as_deref().unwrap_or(&[]), app_id, app_data_dir, workspace_dir);
        let write = resolve_fs_scopes(fs.write.as_deref().unwrap_or(&[]), app_id, app_data_dir, workspace_dir);
...
mod tests {
    ...
    fn permission_policy_request_preserves_path_scope_and_granted_paths() {
        ...
        let request =
            MiniAppPermissionPolicyRequest::from_paths("app-1", &app_data_dir, Some(&workspace_dir), &granted_paths);
