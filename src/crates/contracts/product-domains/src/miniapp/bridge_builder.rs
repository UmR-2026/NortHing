//! Bridge script builder — generate window.app Runtime Adapter (northhing Hosted) for iframe.
...
(function() {{
  ...
  // Default to en-US until the host pushes the real locale via 'northhing:event'.
  ...
  window.addEventListener('message', (e) => {{
    if (e.data?.type === 'northhing:event') {{
  ...
  window.parent.postMessage({{ method: 'northhing/request-theme' }}, '*');
  window.parent.postMessage({{ method: 'northhing/request-locale' }}, '*');
}})();
"#,
...
pub fn scroll_boundary_script() -> &'static str {
    r#"<script>(()=>{const s=(e)=>{for(let n=e.target;n;n=n.parentNode){if(!(n instanceof Element))continue;if(n===document.documentElement||n===document.body)continue;const o=window.getComputedStyle(n).overflowY;if(o==='hidden'||o==='visible')continue;if(e.deltaY<0&&n.scrollTop>0)return false;if(e.deltaY>0&&n.scrollTop+n.clientHeight<n.scrollHeight)return false;}return true};window.addEventListener('wheel',e=>{if(!e.defaultPrevented&&s(e))window.parent.postMessage({jsonrpc:'2.0',method:'northhing/sandbox-wheel',params:{deltaX:e.deltaX,deltaY:e.deltaY,deltaZ:e.deltaZ,deltaMode:e.deltaMode}},'*')},{passive:true});})();</script>"#
}
...
pub fn build_miniapp_default_theme_css() -> &'static str {
    r#"<style id="northhing-theme-default">:root{--northhing-bg:#121214;--northhing-bg-secondary:#18181a;--northhing-bg-tertiary:#121214;--northhing-bg-elevated:#18181a;--northhing-text:#e8e8e8;--northhing-text-secondary:#b0b0b0;--northhing-text-muted:#858585;--northhing-accent:#60a5fa;--northhing-accent-hover:#3b82f6;--northhing-success:#34d399;--northhing-warning:#f59e0b;--northhing-error:#ef4444;--northhing-info:#E1AB80;--northhing-border:#2e2e32;--northhing-border-subtle:#27272a;--northhing-element-bg:#27272a;--northhing-element-hover:#3f3f46;--northhing-radius:6px;--northhing-radius-lg:10px;--northhing-font-sans:-apple-system,BlinkMacSystemFont,'PingFang SC','Hiragino Sans GB','Segoe UI','Microsoft YaHei UI','Microsoft YaHei','Helvetica Neue',Helvetica,Arial,sans-serif;--northhing-font-mono:ui-monospace,SFMono-Regular,'SF Mono',Menlo,Monaco,'Cascadia Mono','Cascadia Code',Consolas,'Liberation Mono','Courier New',monospace;--northhing-scrollbar-thumb:rgba(255,255,255,0.12);}</style>"#
}
