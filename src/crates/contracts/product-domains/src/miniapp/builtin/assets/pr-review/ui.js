...
const I18N = {
  'en-US': {
    ...
    publishedItemLocked: 'Published review items cannot be deleted from northhing.',
    owner: 'Owner',
    repo: 'Repository',
    ...
    publishConfirmBody: 'These comments will be posted to the provider. This action cannot be undone from northhing.',
    ...
    statusPartialSync: 'Refreshed what northhing could access. {count} source could not be refreshed.',
    ...
    reviewingBannerBody: 'northhing is reading the diff and preparing editable findings. You can keep browsing the PR or stop this run.',
    ...
    lifecycleUnsupported: 'This provider does not support this action from northhing.',
    ...
    lifecycleGuideHead: 'Refresh the PR so northhing can verify the latest head commit.',
    ...
    lifecycleAutoAuthHint: 'northhing will try to authorize with GitHub CLI before this action.',
    ...
    lifecycleConfirmMergeBody: 'This will merge the selected PR on the provider. northhing will use the expected head SHA shown below.',
  ...
  'zh-CN': {
    title: 'PR 审核????,
    subtitle: '监听仓库、打开 PR、查看变更、组织意见，并在二次确认后发????Review????,
    manageSources: '管理来源',
    sourceSettingsTitle: '来源与队列设????,
    ...
    queueModeMineHint: '需要会????Token，因为代码平台要识别你的身份????,
    directUrl: '打开 PR 链接',
    directPlaceholder: '粘贴 GitHub、GitCode 或内????PR 链接',
    repoRef: '仓库',
    repoRefPlaceholder: 'owner/repo 或仓库链????,
    openPr: '打开 PR',
    openExternal: '在浏览器打开',
    syncQueue: '同步队列',
    syncMine: '同步待我审核',
    ...
    syncMineTitle: '待我审核',
    syncMineDesc: '使用你的平台身份查找需要你审核????PR????,
    startReview: '开????AI 审核',
    refreshNow: '立即刷新',
    autoSync: '自动刷新',
    every: '????,
    ...
    listenDisabled: '已暂????,
    pauseListening: '暂停监听',
    resumeListening: '重新开启监????,
    save: '保存',
    provider: '代码平台',
    token: '会话 Token',
    tokenReady: 'Token 已填????,
    tokenMissing: '未填????Token',
    tokenHelp: '公开 PR 可不????Token 读取。同步待我审核、私有仓库和发布评论需????Token。Token 只保存在当前内存中????,
    authorizeGitHubCli: '使用 GitHub CLI',
    authorizingGitHubCli: '正在读取 GitHub CLI Token...',
    tokenFromGh: '已从 GitHub CLI 读取 Token',
    authUnavailable: '无法读取 GitHub CLI Token。请先运????`gh auth login`，或手动粘贴 Token????,
    ...
    workspaceDiscovered: '已从当前工作区添????{count} 个仓库????,
    workspaceDiscoveredMany: '已从当前工作区添????{count} 个仓库????,
    workspaceNone: '当前工作区没有识别到 Git remote????,
    workspaceRepo: '工作????,
    rediscoverWorkspace: '识别工作区仓????,
    repositoryFirst: '监听仓库',
    repositoryFirstHint: '先添加要关注的仓库，再同步它的审核队列????,
    singlePrFallback: '单独检视一????PR',
    ...
    directOpenBusyGeneric: '当前????PR 操作正在进行，请完成后再单独打开 PR????,
    ...
    busyActionGeneric: '当前????PR 操作正在进行，请完成后再执行此操作????,
    publishedItemLocked: '已发布的 Review 项不能在 northhing 中删除????,
    owner: 'Owner',
    repo: '仓库',
    providerName: '显示名称',
    kind: '类型',
    ...
    publishConfirmTitle: '确认发布选中????Review 内容????,
    publishConfirmBody: '这些评论会提交到代码平台。northhing 无法替你撤回这个操作????,
    overwriteDraftTitle: '替换当前审核草稿????,
    overwriteDraftBody: '新的 AI 审核开始前，会删除 {count} 条未发布审核意见。已发布的意见会作为下一轮审核上下文保留????,
    confirmStartReview: '开始新????AI 审核',
    publishStaleTitle: 'PR head 已变????,
    publishStaleBody: '草稿基于????head 生成。请刷新，或明确确认要基于最????head 继续发布????,
    staleConfirm: '我确????PR head 已变????,
    publishNow: '立即发布',
    cancel: '取消',
    audit: '操作记录',
    statusReady: '就绪',
    statusRefreshing: '正在同步队列...',
    statusPartialSync: '已刷????northhing 可访问的内容。{count} 个来源暂时无法刷新????,
    statusAssignedNeedsToken: '同步待我审核需要当前代码平台的会话 Token????,
    statusNoSubscriptions: '请先添加监听仓库，或粘贴一????PR 链接????,
    ...
    reviewStageBuild: '整理为可编辑????Review ????,
    ...
    reviewingBannerBody: 'northhing 正在读取 diff 并整理可编辑问题。你可以继续浏览 PR，也可以中止本次审核????,
    cancelReview: '中止审核',
    reviewCancelled: '审核已中????,
    statusPublishing: '正在发布 Review...',
    statusSaved: '已保????,
    statusPublished: 'Review 已发????,
    statusPublishFailed: '没有 Review 内容发布成功。请查看操作记录后重试????,
    errorParse: '无法从这个链接识????PR????,
    errorNetwork: '代码平台请求失败',
    newPrTitle: '新的可审????PR',
    newHeadTitle: '已审 PR 有新提交',
    newPrBatchTitle: '审核队列已更????,
    newPrBatchBody: '{count} ????PR 需要关注????,
    newHeadBatchBody: '{count} 个已审核 PR 有新提交????,
    publicRead: '公开读取',
    privateAction: '私有与写入操????,
    draftStatus: '草稿',
    readyStatus: '可审',
    prDraftStatus: '草稿 PR',
    ...
    lifecycleUnsupported: '当前代码平台不支持在 northhing 中执行这个操作????,
    refreshLifecycle: '刷新状????,
    refreshingLifecycle: '正在刷新 PR 生命周期状????..',
    ...
    lifecycleMergeability: '可合并????,
    lifecycleTokenReady: '已授????,
    lifecycleTokenMissing: '需要授????,
    lifecycleHeadReady: 'Head {sha}',
    lifecycleHeadMissing: '未返????Head',
    lifecycleOpen: '打开',
    lifecycleClosed: '已关????,
    lifecycleMerged: '已合????,
    lifecycleMergeable: '可合????,
    lifecycleBlocked: '受阻',
    lifecycleUnknown: '未知',
    lifecycleChecking: '检查中',
    lifecycleChecksPassing: '通过',
    lifecycleChecksPending: '等待????,
    lifecycleChecksFailing: '失败',
    lifecycleChecksMissing: '无检????,
    lifecycleReviewApproved: '已批????,
    lifecycleReviewChanges: '要求修改',
    lifecycleReviewCommented: '仅评????,
    lifecycleReviewMissing: '????Review 信号',
    lifecycleBlockDraft: '草稿 PR 不能合并????,
    lifecycleBlockClosed: '只有打开状态的 PR 可以更新????,
    lifecycleBlockMerged: '这个 PR 已经合并????,
    lifecycleBlockHead: '代码平台未返????head SHA????,
    lifecycleBlockToken: '执行生命周期操作前需要先授权????,
    lifecycleBlockMergeable: '代码平台未确认这????PR 可合并????,
    lifecycleWarnChecks: '检查尚未全部通过????,
    lifecycleWarnReviews: 'Review 状态需要关注????,
    lifecycleGuideToken: '先使????GitHub CLI 授权，或粘贴 Token，再变更 PR 状态????,
    lifecycleGuideDraft: '先将 PR 标记为可审，再执行合并????,
    lifecycleGuideClosed: '打开代码平台页面，重新打开或检查这个已关闭 PR????,
    lifecycleGuideHead: '刷新 PR，让 northhing 确认最????head 提交????,
    ...
    lifecycleAutoAuthHint: 'northhing 会先尝试通过 GitHub CLI 自动授权????,
    ...
    lifecycleConfirmReadyTitle: '将这????PR 标记为可审？',
    lifecycleConfirmDraftTitle: '将这????PR 转为草稿????,
    ...
    lifecycleConfirmMergeBody: '这会在代码平台合并当前选中????PR。northhing 会使用下方显示的预期 head SHA????,
    lifecycleConfirmWarning: '风险提示',
    expectedHeadSha: '预期 Head',
    confirmLifecycle: '确认执行',
    lifecycleActionReady: '可审状????,
    lifecycleActionDraft: '草稿状????,
    lifecycleActionMerge: '合并',
    lifecycleReadyDone: '已标记为可审 PR',
    lifecycleDraftDone: '已转为草????PR',
    lifecycleMergeDone: 'PR 已合????,
    ...
    publishNoDraftItems: '请先生成或添加一????Review 内容，再发布????,
    publishSelectItemFirst: '请至少选中一????Review 草稿后再发布????,
    suggestedFix: '建议处理',
    binary: '二进????,
    large: '过大',
    stale: '已过????,
  ...
  'zh-TW': {
    title: 'PR 審核????,
    subtitle: '監聽倉庫、開????PR、檢視變更、組織意見，並在二次確認後發????Review????,
    manageSources: '管理來源',
    sourceSettingsTitle: '來源與佇列設????,
    sourceSettingsSubtitle: '調整監聽倉庫、佇列範圍、私有授權，或單獨開啟某????PR????,
    queueSettingsTitle: '佇列設定',
    queueSettingsHint: '選擇哪些 PR 進入審核佇列。主介面只保留審核工作流????,
    queueModeAll: '倉庫全部 PR',
    queueModeMine: '待我審核',
    queueModeAllHint: '從已監聽倉庫同步開啟狀態的 PR????,
    queueModeMineHint: '需要工作階????Token，因為程式碼平台要識別你的身分????,
    directUrl: '開啟 PR 連結',
    directPlaceholder: '貼上 GitHub、GitCode 或內????PR 連結',
    repoRef: '倉庫',
    ...
    syncMineTitle: '待我審核',
    syncMineDesc: '使用你的平台身分查找需要你審核????PR????,
    ...
    tokenReady: 'Token 已填????,
    tokenMissing: '未填????Token',
    tokenHelp: '公開 PR 可不????Token 讀取。同步待我審核、私有倉庫和發布評論需????Token。Token 只保存在目前記憶體中????,
    authorizeGitHubCli: '使用 GitHub CLI',
    authorizingGitHubCli: '正在讀????GitHub CLI Token...',
    tokenFromGh: '已從 GitHub CLI 讀????Token',
    authUnavailable: '無法讀????GitHub CLI Token。請先執????`gh auth login`，或手動貼上 Token????,
    authAutoHint: '需要時會從本機 gh 自動讀????GitHub Token，僅保存在目前記憶體????,
    manualToken: '手動 Token',
    repoAddedSyncing: '已新增監聽倉庫，正在同步佇????..',
    advancedProviders: '進階平台設定',
    errorSourceUnavailable: '無法重新整理 {repo}。倉庫????PR 可能無法使用、介面可能不相容，或目前帳號無權存取。請在「管理來源」中檢查倉庫地址；如為私有倉庫，請先完成授權????,
    workspaceDiscovering: '正在從目前工作區識別倉庫...',
    workspaceDiscovered: '已從目前工作區新增 {count} 個倉庫????,
    workspaceDiscoveredMany: '已從目前工作區新增 {count} 個倉庫????,
    workspaceNone: '目前工作區沒有識別????Git remote????,
    workspaceRepo: '工作區',
    rediscoverWorkspace: '識別工作區倉庫',
    repositoryFirst: '監聽倉庫',
    repositoryFirstHint: '先新增要關注的倉庫，再同步它的審核佇列????,
    singlePrFallback: '單獨檢視一????PR',
    singlePrFallbackHint: '當這個倉庫暫時不需要監聽時使用????,
    directOpenBusySync: '正在同步佇列，請稍後再單獨開????PR????,
    directOpenBusyReview: 'AI 審核正在進行，請先完成或中止後再開啟其他 PR????,
    directOpenBusyPublish: 'Review 正在發布，請完成後再開啟其他 PR????,
    directOpenBusyLifecycle: 'PR 狀態操作正在執行，請完成後再開啟其????PR????,
    directOpenBusyGeneric: '目前????PR 操作正在進行，請完成後再單獨開啟 PR????,
    ...
    busyActionGeneric: '目前????PR 操作正在進行，請完成後再執行此操作????,
    publishedItemLocked: '已發布的 Review 項不能在 northhing 中刪除????,
    owner: 'Owner',
    repo: '倉庫',
    providerName: '顯示名稱',
    kind: '類型',
    ...
    noReviews: '程式碼平台沒有回????Review 評論????,
    manualComment: '手寫評論',
    manualCommentPlaceholder: '在這裡????PR 級評論，或貼上你已經發現的問題????,
    addManualComment: '加入 Review',
    expandComment: '展開',
    collapseComment: '收起',
    composer: 'Review 編輯????,
    ...
    decisionRequestChanges: '要求修改',
    selectedOps: '已選',
    publishSelected: '發布選取????,
    publishConfirmTitle: '確認發布選取????Review 內容????,
    publishConfirmBody: '這些評論會提交到程式碼平台。northhing 無法替你撤回這個操作????,
    overwriteDraftTitle: '取代目前審核草稿????,
    overwriteDraftBody: '新的 AI 審核開始前，會刪????{count} 條未發布審核意見。已發布的意見會作為下一輪審核上下文保留????,
    confirmStartReview: '開始新的 AI 審核',
    publishStaleTitle: 'PR head 已變????,
    publishStaleBody: '草稿基於????head 產生。請重新整理，或明確確認要基於最????head 繼續發布????,
    staleConfirm: '我確????PR head 已變????,
    publishNow: '立即發布',
    cancel: '取消',
    audit: '操作記錄',
    statusReady: '就緒',
    statusRefreshing: '正在同步佇列...',
    statusPartialSync: '已重新整????northhing 可存取的內容。{count} 個來源暫時無法重新整理????,
    statusAssignedNeedsToken: '同步待我審核需要目前程式碼平台的工作階????Token????,
    statusNoSubscriptions: '請先新增監聽倉庫，或貼上一????PR 連結????,
    statusNoActiveSubscriptions: '已新增的監聽倉庫都處於暫停狀態，請重新開啟一個倉庫或貼????PR 連結????,
    statusLoading: '正在載入 PR...',
    statusOpeningPr: '正在開啟 PR...',
    statusGenerating: '正在產生審核草稿...',
    reviewProgress: '審核進展',
    reviewStageRead: '讀????PR 中繼資料和變????,
    reviewStageAi: '呼叫 AI 產生審核意見',
    reviewStageBuild: '整理為可編輯????Review ????,
    ...
    reviewingBannerBody: 'northhing 正在讀????diff 並整理可編輯問題。你可以繼續瀏覽 PR，也可以中止本次審核????,
    cancelReview: '中止審核',
    reviewCancelled: '審核已中????,
    statusPublishing: '正在發布 Review...',
    statusSaved: '已儲????,
    statusPublished: 'Review 已發????,
    statusPublishFailed: '沒有 Review 內容發布成功。請查看操作記錄後重試????,
    errorParse: '無法從這個連結識別 PR????,
    errorNetwork: '程式碼平台請求失????,
    newPrTitle: '新的可審????PR',
    newHeadTitle: '已審 PR 有新提交',
    newPrBatchTitle: '審核佇列已更????,
    newPrBatchBody: '{count} ????PR 需要關注????,
    newHeadBatchBody: '{count} 個已審核 PR 有新提交????,
    publicRead: '公開讀????,
    privateAction: '私有與寫入操????,
    draftStatus: '草稿',
    readyStatus: '可審',
    prDraftStatus: '草稿 PR',
    ...
    lifecycleUnsupported: '目前程式碼平台不支援????northhing 中執行此操作????,
    refreshLifecycle: '重新整理狀????,
    refreshingLifecycle: '正在重新整理 PR 生命週期狀????..',
    ...
    lifecycleAutoAuthHint: 'northhing 會先嘗試透過 GitHub CLI 自動授權????,
    ...
    lifecycleConfirmReadyTitle: '將這????PR 標記為可審？',
    lifecycleConfirmDraftTitle: '將這????PR 轉為草稿????,
    lifecycleConfirmMergeTitle: '合併這????PR????,
    lifecycleConfirmReadyBody: '程式碼平台上????PR 狀態會變為可審????,
    lifecycleConfirmDraftBody: '程式碼平台上????PR 狀態會變為草稿，審核者可能不再把它視為待審????,
    lifecycleConfirmMergeBody: '這會在程式碼平台合併目前選中????PR。northhing 會使用下方顯示的預期 head SHA????,
    lifecycleConfirmWarning: '風險提示',
    expectedHeadSha: '預期 Head',
    confirmLifecycle: '確認執行',
    lifecycleActionReady: '可審狀????,
    lifecycleActionDraft: '草稿狀????,
    lifecycleActionMerge: '合併',
    lifecycleReadyDone: '已標記為可審 PR',
    lifecycleDraftDone: '已轉為草????PR',
    lifecycleMergeDone: 'PR 已合????,
    ...
    publishNoDraftItems: '請先產生或新增一????Review 內容，再發布????,
    publishSelectItemFirst: '請至少選取一????Review 草稿後再發布????,
    suggestedFix: '建議處理',
    binary: '二進位',
    large: '過大',
    stale: '已過????,
...
function providerHeaders(profile, jsonBody = false) {
  const headers = {
    ...
    'User-Agent': 'northhing-PR-Review-MiniApp',
  };
  ...
  return `
    <form class="pr-form pr-form--compact" id="subscription-form">
      ...
      <label class="pr-field">
        ...
        <input class="pr-input" name="repo" placeholder="northhing" required />
      </label>
