...
// northhing host accent (cool blue) so the dominant language reads as primary;
...
const I18N = {
  'zh-CN': {
    ...
    weekShort: ['???, '一', '???, '???, '???, '???, '???],
    just: '刚刚',
    noCommitsThisRange: '区间内尚无提???,
    noActivityThisRange: '区间内尚无活???,
    codingStyle: 'CODING STYLE',
    other: '其他',
    last52: (total, days) => `???52 ???· ${total} 提交 · 活跃 ${days} 天`,
    ...
    showXofY: (shown, total) => `显示 ${shown} / ???${total} 次`,
    totalX: (total) => `???${total} 次提交`,
    greetings: { lateNight: '夜深???, morning: '早上???, am: '上午???, noon: '中午???, pm: '下午???, evening: '晚上??? },
    greetWith: (part, name) => name ? `${part}???{name}` : part,
    subtitleEmptyToday: '今天还没开???“一次小提交，就能让今天的画像不再空白???,
    ...
    subtitleSprintRange: (label, n) => `${label} ???${n} 次提交，状态拉满。`,
    subtitleSteadyRange: (label, n) => `${label} ???${n} 次提交，节奏稳健。`,
    subtitleStartRange: (label, n) => `${label} ???${n} 次提交，刚刚铺开。`,
    deltaUp: (n) => `较上一区间 +${n}`,
    deltaDown: (n) => `较上一区间 ${n}`,
    deltaSame: '与上一区间持平',
    deltaUpYesterday: (n) => `较昨???+${n}`,
    deltaDownYesterday: (n) => `较昨???${n}`,
    deltaSameYesterday: '与昨日持???,
    firstToday: 'first commits today',
    streakFoot: [
      { lt: 1, text: '今日打个卡，开启新连胜' },
      { lt: 3, text: '坚持下去，节奏才刚开??? },
      { lt: 7, text: '已经连写了一阵子' },
      { lt: 30, text: '稳定的产出节??? },
      { lt: 100, text: '形成肌肉记忆，状态在??? },
      { lt: 200, text: '难以置信的坚持力' },
      { lt: Infinity, text: '你正在创造历??? },
    ],
    netPos: (n) => `net +${n}`,
    netNeg: (n) => `net ${n}`,
    ...
    errNoWs: '还没有打开工作???,
    errNoWsDesc: '请先???northhing 侧边栏选择一???Git 仓库的工作区???,
    errNotGit: '当前工作区不???Git 仓库或远程工作区',
    ...
    range30d: '???30 ???,
    rangeCustom: '自定???,
    rangeLabel: { '1d': '???1 ???, '7d': '???7 ???, '30d': '???30 ???, custom: '自定义区??? },
  ...
  'zh-TW': {
    ...
    codingStyle: 'CODING STYLE',
    other: '其他',
    last52: (total, days) => `???52 ???· ${total} 提交 · 活躍 ${days} 天`,
    perCellTitle: (date, count) => `${date} · ${count} 提交`,
    hourTitle: (hh, count) => `${hh}:00 “${count} 提交`,
    noCommitsRange: '區間內還沒有提???,
    noCommitsToday: '今天還沒有提交，去寫下第一行代碼吧???,
    showXofY: (shown, total) => `顯示 ${shown} / ???${total} 次`,
    totalX: (total) => `???${total} 次提交`,
    greetings: { lateNight: '夜深???, morning: '早上???, am: '上午???, noon: '中午???, pm: '下午???, evening: '晚上??? },
    greetWith: (part, name) => name ? `${part}???{name}` : part,
    ...
    subtitleSprintRange: (label, n) => `${label} ???${n} 次提交，狀態拉滿。`,
    subtitleSteadyRange: (label, n) => `${label} ???${n} 次提交，節奏穩健。`,
    subtitleStartRange: (label, n) => `${label} ???${n} 次提交，剛剛鋪開。`,
    deltaUp: (n) => `較上一區???+${n}`,
    deltaDown: (n) => `較上一區???${n}`,
    deltaSame: '與上一區間持???,
    deltaUpYesterday: (n) => `較昨???+${n}`,
    deltaDownYesterday: (n) => `較昨???${n}`,
    deltaSameYesterday: '與昨日持???,
    firstToday: 'first commits today',
    streakFoot: [
      { lt: 1, text: '今日打個卡，開啟新連勝' },
      { lt: 3, text: '堅持下去，節奏才剛開??? },
      { lt: 7, text: '已經連寫了一陣子' },
      { lt: 30, text: '穩定的產出節??? },
      { lt: 100, text: '形成肌肉記憶，狀態在??? },
      { lt: 200, text: '難以置信的堅持力' },
      { lt: Infinity, text: '你正在創造歷??? },
    ],
    netPos: (n) => `net +${n}`,
    netNeg: (n) => `net ${n}`,
    allBranches: 'all branches',
    ...
    errNoWs: '還沒有打開工作區',
    errNoWsDesc: '請先???northhing 側邊欄選擇一???Git 倉庫的工作區???,
    ...
    range30d: '???30 ???,
    rangeCustom: '自定???,
    rangeLabel: { '1d': '???1 ???, '7d': '???7 ???, '30d': '???30 ???, custom: '自定義區??? },
    rangeBadge: (label, days) => days != null ? `${label} · ${days} 天` : label,
    customRangeFmt: (s, e) => `${s} →${e}`,
    customRangeInvalid: '請選擇起止日期（??????止）',
  },
  ...
  'en-US': {
    ...
    errNoWsDesc: 'Open a Git repo workspace from the northhing sidebar first.',
