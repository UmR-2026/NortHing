...
impl MiniAppFsHostPathCheck {
    pub fn denied_message(&self) -> String {
        format!("{} not allowed: {}", self.denied_prefix, self.path.display())
    }
}
...
pub fn plan_fs_host_call(
    ...
    match name {
        ...
        "writeFile" => Ok(MiniAppFsHostCallPlan::WriteFile {
            path: require_path(path_param)?,
            data: params.get("data").and_then(|v| v.as_str()).unwrap_or("").to_string(),
        }),
...
pub fn plan_fs_legacy_path_check(name: &str, params: &serde_json::Value) -> Option<MiniAppFsHostPathCheck> {
...
fn require_named_path(params: &serde_json::Value, key: &str) -> Result<PathBuf, MiniAppHostPlanError> {
    params
        .get(key)
        .and_then(|v| v.as_str())
        .map(PathBuf::from)
...
pub fn plan_shell_host_call(
    ...
    let argv: Option<Vec<String>> = params
        .get("args")
    ...
    Ok(MiniAppShellHostCallPlan {
        ...
        cwd: shell_exec_cwd(params.get("cwd").and_then(|v| v.as_str()), workspace_dir, app_data_dir),
...
pub fn command_basename_allowed(allowlist: &[String], basename: &str) -> bool {
    allowlist.is_empty() || allowlist.iter().any(|allowed| allowed.to_lowercase() == basename)
}
...
pub fn host_allowed_by_allowlist(allowlist: &[String], host: &str) -> bool {
    allowlist.is_empty()
        || allowlist
            .iter()
            .any(|allowed| allowed == "*" || host == allowed || host.ends_with(&format!(".{}", allowed)))
}
...
mod tests {
    ...
    fn shell_exec_first_token_prefers_argv_over_shell_command_text() {
        let argv = vec![r"C:\Program Files\Git\cmd\git.exe".to_string(), "status".to_string()];
    ...
    fn miniapp_host_shell_call_plans_preserve_existing_input_and_default_contract() {
        ...
        assert_eq!(plan.argv, Some(vec!["git".to_string(), "status".to_string()]));
