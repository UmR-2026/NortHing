...
impl MiniAppStorageLayout {
    ...
    pub fn draft_source_dir(miniapps_root: impl AsRef<Path>, app_id: &str, draft_id: &str) -> PathBuf {
        Self::draft_dir(miniapps_root, app_id, draft_id).join(SOURCE_DIR)
    }

    pub fn draft_manifest_path(miniapps_root: impl AsRef<Path>, app_id: &str, draft_id: &str) -> PathBuf {
...
pub fn parse_npm_dependencies(package_json: &str) -> Result<Vec<NpmDep>, serde_json::Error> {
    ...
    let Some(deps) = package.get("dependencies").and_then(|deps| deps.as_object()) else {
        return Ok(Vec::new());
    };
...
pub fn build_package_json(app_id: &str, deps: &[NpmDep]) -> serde_json::Value {
    ...
    for dep in deps {
        dependencies.insert(dep.name.clone(), serde_json::Value::String(dep.version.clone()));
    }
...
impl std::error::Error for MiniAppImportBundlePlanError {
    fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
        match self {
            Self::InvalidMeta(error) | Self::MetaSerialization(error) | Self::PackageSerialization(error) => {
                Some(error)
            }
        }
    }
...
pub fn build_import_bundle_plan(
    ...
    let mut meta: MiniAppMeta =
        serde_json::from_str(source_meta_json).map_err(MiniAppImportBundlePlanError::InvalidMeta)?;
    prepare_imported_meta(&mut meta, app_id, now);

    let fallbacks = build_import_fallbacks(app_id);
    let meta_json = serde_json::to_string_pretty(&meta).map_err(MiniAppImportBundlePlanError::MetaSerialization)?;
