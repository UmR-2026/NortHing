...
use crate::miniapp::storage::{build_package_json, ESM_DEPS_JSON, INDEX_HTML, STYLE_CSS, UI_JS, WORKER_JS};
...
pub const BUILTIN_PLACEHOLDER_COMPILED_HTML: &str = "<!DOCTYPE html><html><body>Loading...</body></html>";
...
pub fn builtin_content_hash(app: &BuiltinMiniAppBundle) -> String {
    ...
    hash_builtin_asset(&mut hasher, "esm_dependencies.json", app.esm_dependencies_json);
    format!("sha256:{}", hex_encode(&hasher.finalize()))
}

pub fn build_builtin_install_marker(app: &BuiltinMiniAppBundle, content_hash: &str) -> BuiltinInstallMarker {
...
pub fn resolve_builtin_seed_action(artifacts: BuiltinSeedArtifacts, has_local_override: bool) -> BuiltinSeedAction {
...
pub fn serialize_builtin_install_marker(marker: &BuiltinInstallMarker) -> serde_json::Result<String> {
...
mod tests {
    ...
    #[test]
    #[ignore] // TEMPORARILY SKIPPED - version mismatch
    fn ppt_live_bundle_uses_northhing_host_capabilities() {
        let app = BUILTIN_APPS
            .iter()
            .find(|app| app.id == "builtin-ppt-live")
            .expect("PPT Live should be registered");
        let meta: serde_json::Value = serde_json::from_str(app.meta_json).expect("PPT Live metadata should be valid");
        let bundle: serde_json::Value = serde_json::from_str(include_str!("builtin/assets/ppt-live/bundle.json"))
        ...
        assert_eq!(meta["permissions"]["net"]["allow"].as_array().map(Vec::len), Some(0));
        ...
        assert!(include_str!("builtin/assets/ppt-live/ui.js").contains("installagentAppBackendAdapter"));
        assert!(!meta["permissions"]["ai"]["enabled"].as_bool().unwrap_or(true));
        ...
        assert!(app.ui_js.contains("user::northhing-system::ppt-design"));
