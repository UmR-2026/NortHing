...
pub enum MiniAppCustomizationBaseline {
    Builtin { builtin_id: String, builtin_version: u32 },
    UserCreated,
}
...
pub fn apply_draft_customization_metadata(
    ...
    if matches!(metadata.origin.kind, MiniAppCustomizationOriginKind::Builtin) {
        metadata.local_override = true;
        if let MiniAppCustomizationBaseline::Builtin { builtin_version, .. } = baseline {
...
pub fn declined_builtin_update_needs_local_snapshot(
    ...
    metadata
        ...
        .is_some_and(|record| record.local_app_version.is_some() && record.local_app_updated_at.is_some())
}
...
pub fn is_current_declined_builtin_update(
    ...
    if let (Some(record_version), Some(record_updated_at), Some(snapshot)) =
        (record.local_app_version, record.local_app_updated_at, local_snapshot)
    {
...
pub fn decline_builtin_update_metadata(
    ...
    if let Some(record) = metadata.declined_builtin_updates.iter_mut().find(|record| {
        ...
        metadata.declined_builtin_updates.push(MiniAppDeclinedBuiltinUpdate {
            builtin_version,
            source_hash: source_hash.to_string(),
            declined_at,
            local_app_version,
        ...
        if metadata.declined_builtin_updates.len() > MAX_DECLINED_BUILTIN_UPDATES {
            let remove_count = metadata.declined_builtin_updates.len() - MAX_DECLINED_BUILTIN_UPDATES;
...
pub fn diff_permissions(active: &MiniAppPermissions, draft: &MiniAppPermissions) -> MiniAppPermissionDiff {
...
mod tests {
    ...
    use crate::miniapp::types::{
        AiPermissions, FsPermissions, MiniAppPermissions, NetPermissions, NodePermissions, ShellPermissions,
    };
