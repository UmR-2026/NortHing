...
use crate::miniapp::draft::{build_draft_manifest, build_draft_response, MiniAppDraft, MiniAppDraftManifest};
use crate::miniapp::lifecycle::{
    ...
    build_created_app, clear_worker_restart_required_state, ensure_runtime_state, mark_deps_installed_state,
    prepare_draft_app, prepare_rollback_app, MiniAppCreateInput, MiniAppUpdatePatch,
};
use crate::miniapp::ports::{
    MiniAppCompilePort, MiniAppImportFromPathRequest, MiniAppImportPort, MiniAppPortError, MiniAppPortErrorKind,
    MiniAppPortResult, MiniAppStoragePort,
};
...
impl<'a> MiniAppRuntimeFacade<'a> {
    ...
    pub async fn load_app_ensuring_runtime_state(&self, app_id: String) -> MiniAppPortResult<MiniApp> {
    ...
    pub async fn persist_draft_for_app(
        ...
        self.save_draft_with_manifest(app_id, draft_root, app, manifest).await
    }
    ...
    pub async fn get_draft(
        ...
        let app = self.storage.load_draft_app(app_id.clone(), draft_id.clone()).await?;
    ...
    pub async fn apply_loaded_draft(
        ...
        self.apply_draft_app(current, draft.draft_id, draft.app, compiled_html, baseline, now)
            .await
    }
    ...
    pub async fn apply_draft_app(
        ...
        self.storage
            ...
            .await?;
        self.storage.save(app.clone()).await?;
        self.record_draft_applied(app_id, draft_id, baseline, now).await?;
        Ok(app)
    }
    ...
    pub async fn mark_builtin_update_available(
        ...
        let Some(metadata) = self.storage.load_customization_metadata(app_id.clone()).await? else {
            return Ok(false);
        };
    ...
    pub async fn decline_builtin_update(
        ...
        let metadata =
            decline_builtin_update_metadata(metadata, builtin_version, &source_hash, declined_at, local_snapshot);
        self.storage
    ...
    pub async fn clear_worker_restart_required(&self, app_id: String) -> MiniAppPortResult<MiniApp> {
        let mut app = self.storage.load(app_id).await?;
    ...
    pub async fn rollback(&self, app_id: String, version: u32, now: i64) -> MiniAppPortResult<MiniApp> {
        ...
        self.storage.save_version(app_id, current.version, current).await?;
        self.storage.save(app.clone()).await?;
        Ok(app)
    }
    ...
    pub async fn persist_recompile_result(
        ...
        self.persist_recompile_result_for_app(app, compiled_html, now).await
    }
    ...
    pub async fn persist_sync_from_fs_result_for_app(
        ...
        self.storage.save_version(app_id, previous.version, previous).await?;
        self.storage.save(app.clone()).await?;
        Ok(app)
    }

    pub async fn persist_import_runtime_state(&self, mut app: MiniApp) -> MiniAppPortResult<MiniApp> {
    ...
    pub async fn import_from_path(
        ...
        let meta_content = import_port.read_import_meta_json(request.source_path.clone()).await?;
    ...
    async fn load_draft_manifest(&self, app_id: String, draft_id: String) -> MiniAppPortResult<MiniAppDraftManifest> {
    ...
    async fn save_draft_with_manifest(
        ...
        self.storage
            .save_draft(app_id, manifest.draft_id.clone(), app.clone(), manifest_value)
            .await?;
        Ok(build_draft_response(draft_root, app, manifest))
    }
    ...
    async fn record_draft_applied(
        ...
        let existing = self.storage.load_customization_metadata(app_id.clone()).await?;
...
mod tests {
    ...
    impl MiniAppStoragePort for MemoryStorage {
        ...
        fn save_version(&self, _app_id: String, _version: u32, _app: MiniApp) -> MiniAppPortFuture<'_, ()> {
            Box::pin(async { unreachable!("not needed for import runtime-state facade test") })
        }
        ...
        fn load_draft_app(&self, _app_id: String, _draft_id: String) -> MiniAppPortFuture<'_, MiniApp> {
            Box::pin(async { unreachable!("not needed for import runtime-state facade test") })
        }

        fn load_draft_manifest(&self, _app_id: String, _draft_id: String) -> MiniAppPortFuture<'_, serde_json::Value> {
            Box::pin(async { unreachable!("not needed for import runtime-state facade test") })
        }
    ...
    fn import_runtime_state_facade_applies_state_and_persists_once() {
        ...
        assert_eq!(saved[0].runtime.source_revision, app.runtime.source_revision);
