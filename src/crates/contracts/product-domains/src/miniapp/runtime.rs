...
pub fn detect_runtime_with_probe<P: MiniAppRuntimeProbe + ?Sized>(probe: &P) -> Option<DetectedRuntime> {
    for name in runtime_lookup_order() {
        ...
        if let Some(version) = probe.runtime_version(&path) {
            return Some(DetectedRuntime { kind, path, version });
        }
    }
    None
}
...
fn get_version(executable: &Path) -> Result<String, std::io::Error> {
    let output = create_version_command(executable).arg("--version").output()?;
...
fn find_executable_with_probe<P: MiniAppRuntimeProbe + ?Sized>(probe: &P, name: &str) -> Option<PathBuf> {
...
mod tests {
    ...
    fn detector_uses_common_home_fallbacks_before_version_manager_dirs() {
        ...
        probe.versions.insert(fallback.clone(), "v18.19.0".to_string());
