//! Provider-neutral DeepResearch citation renumbering utilities.
//!
//! This module owns the deterministic `cit_XXX` reference renumbering for
//! finalized research reports. It is a pure string-transformation helper:
//! callers provide the report body and optional citation registry content, then
//! persist the returned report and display map in their own storage boundary.
...
static REGISTRY_STATUS_RE: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"status\s*=\s*([A-Za-z_]+)").unwrap());
static CITATION_INDEX_RE: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"(?m)^#{1,6}\s*(Citation Index|引用索引|引用列表)\s*$").unwrap());
static BRACKETED_GROUP_RE: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"\[(cit_\d+(?:\s*,\s*cit_\d+)*)\]").unwrap());
...
pub fn renumber_research_report(report: &str, registry_content: Option<&str>) -> ResearchCitationRenumberOutput {
    let registry_status = registry_content.map(parse_registry_status).unwrap_or_default();
