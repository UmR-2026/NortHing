...
| # | 层 | 路径 | 职责 | 模块 / 入口 | 层文档 |
|---|---|---|---|---|---|
| 1 | 接口与入口 | `src/apps/*`、`northing-installer`、`tests/e2e`、`src/crates/interfaces` | 产品宿主、命令、UI 入口、协议接口以及跨表面测试 | desktop、CLI、server、installer、E2E、`acp` | 最近本地 `AGENTS.md`；[interfaces](src/crates/interfaces/AGENTS.md) |
...
3. **God-file 防线**：生产环境超过 800 行的 `.rs` 文件会增加 review 压力；超过 1000 行必须拆分或在 `scripts/exception-leases.json` 中登记生效的 exception lease（含 `revisit_after`）。`// allow-god-file` 头注释通道已被机械禁令废除。闸口径 = checker `countLines`，仅换行/重排产生的行数下降不得报告为结构改善（P09）。新模块从底线以下开始。
...
- **UI 线程纪律**：旧规则「非事件循环线程写 Slint 属性会被静默丢弃，所有此类写入必须走 `slint::invoke_from_event_loop`（`error_banners.rs` 的 helper 已封装，直接复用，见 `ad349f9`）」因 Slint 壳已于 2026-08-28 物理删除（commit `707e414`）而不再适用。Dioxus consult-room 壳遵循 Dioxus 0.8 自身的事件循环契约；现行纪律以 Dioxus 文档与 consult-room 的 `ui_dioxus::launch` 路径为准。原 `error_banners.rs` 的 `slint::invoke_from_event_loop` 封装已随 Slint 壳一并删除（`707e414`），无需替代实现——Dioxus 的写入走其自身的响应式运行时。
...
- **v0.1.0 面基线**：发货面仅 Dioxus consult-room 桌面 + `northing-installer`；server / SDLC harness 为冻结-实验面。能力 crates（tools/MCP/search/terminal/git/ssh）是 agent 工具箱，保持激活。见 `docs/tech-debt-cleanup-guide.md` §0。
