...
- **shell_safety stub 立账**：`src/crates/services/services-core/src/shell_safety.rs:223-249` `guard_command_execution` 确认段为 Phase 2 stub（audit 写 "allow-stub"），台账无条目——违反「已知债入账」惯例。缓解事实（须写进条目）：管线层真实确认流在位（tool_confirmation.rs / exec_retry.rs / AND 语义 process_result.rs:231-235 + 默认需确认），故实际风险低但 stub 必须入账。
...
3. **P2-25 立账**（tech-debt-ledger.md P2 段末尾追加）：`shell_safety.rs` 确认闸 Phase 2 stub（log-only "allow-stub"，Phase 3 未接线）。Symptom/Evidence（shell_safety.rs:223-249）/Proposed fix（Phase 3 接 request_user_confirmation）/缓解（管线层确认流真实在位：AND 语义 + 默认需确认，08-22 基线结论已被修正）/Status: active。
...
## 验证（编排者已 BASE 预跑：1/3 绿）

1. `node scripts/check-repo-hygiene.mjs` → exit 0（BASE 实测绿）。
2. 分层表准确性：report 附对照说明（表列 crate 集合 vs `Cargo.toml` members 25 条逐一对账）。
3. `cargo check -p northhing-cli-internal` → exit 0（BASE 已预跑绿；注释改动后复跑）。
