...
BASE = 编排者派发正文给出的 7 位 SHA（brief 复审定版时的 main HEAD = `0b443f2`；其后不得再有非本单允许文件集的 commit，否则 task-gate 起点顺延并以派发正文为准）。
...
- **shell_safety stub 立账**：`src/crates/assembly/core/src/agentic/tools/implementations/shell_safety.rs` 的 `guard_command_execution`（函数本体 `:225-248`）确认段为 Phase 2 stub（`:244` audit 写 "allow-stub"；`:238-239` 注释自曝 Phase 3 未接线），台账无条目——违反「已知债入账」惯例。缓解事实（须写进 P2-25 的 Mitigation 字段）：管线层真实确认流在位（tool_confirmation.rs / exec_retry.rs + AND 语义 process_result.rs:225-234 + 默认需确认），故实际风险低但 stub 必须入账。（编排者 2026-09-20 自审更正：原 brief 误写 services-core 路径，实际在 assembly/core/agentic/tools/implementations/ 下。）
...
2. **验证表修正**（双语同改）：「Shared Rust logic in `core`…」行最小验证列**保持 `cargo check --workspace` 不变**，在同单元格末尾追加一句：**`northhing-core` 的定向测试必须带 `--features product-full`**（模块在 feature 门后，裸跑编译失败 E0433——ZCode 2026-09-09 实测）。即：原文 `cargo check --workspace`, plus the nearest focused `cargo test` when behavior changed → 改为 `cargo check --workspace`, plus the nearest focused `cargo test` when behavior changed（`northhing-core` 定向测试须带 `--features product-full`，裸跑 E0433）。**禁止整列替换**，"when behavior changed" 指引必须保留。
3. **P2-25 立账**（tech-debt-ledger.md P2 段末尾追加，P2-24 之后）：`shell_safety.rs`（`src/crates/assembly/core/src/agentic/tools/implementations/shell_safety.rs`）确认闸 Phase 2 stub（log-only "allow-stub"，Phase 3 未接线）。字段顺序固定为五字段：`- **Symptom**:` / `- **Mitigation**:` / `- **Evidence**:` / `- **Proposed fix**:` / `- **Status**:`。其中 Mitigation 为既有 P2-19~P2-24 没有的新字段，插在 Symptom 之后；Change Protocol 的 "include evidence/proposed fix/status" 是最低要求非穷举，**不改 Change Protocol 段**。Evidence 行内引用精确行号：函数本体 `:225-248`、`"allow-stub"` 审计 `:244`、Phase 3 未接线自曝注释 `:238-239`。Proposed fix：Phase 3 接 `request_user_confirmation`。Mitigation 内容：管线层真实确认流在位（tool_confirmation.rs / exec_retry.rs + AND 语义 process_result.rs:225-234 + 默认需确认），实际风险低但 stub 必须入账。Status: active。
...
- commit 逐文件点名 `git add`（4 文件）；message 前缀 `docs:` + `(W23-1)` 后缀，body 注明 ZCode 审查 A1/D3/U#1/U#2 + 用户拍板 2026-09-09。
