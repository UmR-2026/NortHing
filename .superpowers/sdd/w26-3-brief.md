...
BASE = `18344f8`（续派前 HEAD，承接已合入的 W26-4 18344f8；首派 BASE `cc586ae`）。
...
4. `node scripts/verify-task-gate.mjs verify-attempt --base 18344f8 --tip <本单最后 commit sha> --allowlist .superpowers/sdd/w26-3-allowlist.txt` → exit 0（自建 allowlist 含自身；**窗口内若出现其它 W26 单文件，停手报编排者复跑**）
